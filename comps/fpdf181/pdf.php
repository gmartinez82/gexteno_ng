...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        if ($this->CurOrientation == 'L') {
            //$this->Image(Gral::getPathAbs() . 'comps/fpdf/logo_reportes.jpg', 20, 5, 250);
        } else {
            //$this->Image(Gral::getPathAbs() . 'comps/fpdf/logo_reportes.jpg', 20, 5, 180);
        }
        //Arial bold 15
    ...
    function Footer() {
        ...
        $this->Cell(0, 10, 'Impreso por ' . $this->getUsuario()->getDescripcion(), 0, 0, 'R');
        $this->SetY(-8);
        $this->Cell(0, 10, Gral::getFechaHoraToWeb(Gral::getFechaHoraActual()) . ' hs', 0, 0, 'R');
    }
    ...
    public function setNombreReporte($texto) {
        ...
        if ($this->CurOrientation == 'L') {
            ...
            $this->Cell(200);
            $this->Cell(0, 6, utf8_decode($texto), 'B', 1, 'R', 1);
        } else {
            $this->setXY($x = 4, $y = 14);
            //$this->Ln(4);
            $this->SetFont('Arial', 'B', 16);
    ...
    public function setParSimpleVtaPresupuesto($label, $dato, $x, $y) {
        // Par 
        $this->SetTextColor(0, 0, 0);
        $this->SetFont('Arial', '', 10);
        $this->SetFillColor(255, 255, 255);
        ...
        $length = $this->GetStringWidth(trim($label));
        $this->setXY($x + $length, $y);
        $this->Cell(1, 3, utf8_decode($dato), 0, 1, 'L', 1);
    }
