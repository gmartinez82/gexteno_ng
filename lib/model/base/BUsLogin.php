...
{ 
	...
	const SES_PAGINACION = 'adm_uslogin_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_uslogin_paginas_registros';
	const SES_CRITERIOS = 'adm_uslogin_criterios';
	
	const GEN_CLASE = 'UsLogin';
	const GEN_TABLA = 'us_login';
	...
	/* Constantes de Atributos de BUsLogin */ 
	const GEN_ATTR_ID = 'us_login.id';
	const GEN_ATTR_US_USUARIO_ID = 'us_login.us_usuario_id';
	const GEN_ATTR_SESSION = 'us_login.session';
	const GEN_ATTR_IP = 'us_login.ip';
	const GEN_ATTR_EXITO = 'us_login.exito';
	const GEN_ATTR_LOGIN = 'us_login.login';
	const GEN_ATTR_NAVEGADOR = 'us_login.navegador';
	const GEN_ATTR_OBSERVACION = 'us_login.observacion';
	const GEN_ATTR_ORDEN = 'us_login.orden';
	const GEN_ATTR_ESTADO = 'us_login.estado';
	const GEN_ATTR_CREADO = 'us_login.creado';
	const GEN_ATTR_CREADO_POR = 'us_login.creado_por';
	const GEN_ATTR_MODIFICADO = 'us_login.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'us_login.modificado_por';

	/* Constantes de Atributos Min de BUsLogin */ 
	...
	public static function hidratarObjeto($fila){            
            $o = new UsLogin();
            $o->setId($fila[UsLogin::GEN_ATTR_MIN_ID], false);
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                ".$this->getUsUsuarioId()."
                    ...
                    ) RETURNING currval('us_login_seq')";
...
".self::GEN_ATTR_MIN_US_USUARIO_ID." = ".$this->getUsUsuarioId()."
	...
	public function duplicarUsLogin(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getUsLogins($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = UsLogin::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $uslogin = new UsLogin();
                    $uslogin->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $uslogin->setUsUsuarioId($fila[self::GEN_ATTR_MIN_US_USUARIO_ID]);
			$uslogin->setSession($fila[self::GEN_ATTR_MIN_SESSION]);
			$uslogin->setIp($fila[self::GEN_ATTR_MIN_IP]);
			$uslogin->setExito($fila[self::GEN_ATTR_MIN_EXITO]);
			$uslogin->setLogin($fila[self::GEN_ATTR_MIN_LOGIN]);
			$uslogin->setNavegador($fila[self::GEN_ATTR_MIN_NAVEGADOR]);
			$uslogin->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$uslogin->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$uslogin->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$uslogin->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$uslogin->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$uslogin->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$uslogin->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getUsLoginsHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return UsLogin::getUsLogins($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getUsLogins para listado de Backend */ 	
	static function getUsLoginsDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return UsLogin::getUsLogins($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getUsLoginsCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = UsLogin::getUsLogins($paginador, $criterio);
	...
	/* Método getUsLogins filtrado para select */ 	
	static function getUsLoginsCmbF($paginador = null, $criterio = null){
            $col = UsLogin::getUsLogins($paginador, $criterio);
	...
	/* Método getUsLogins filtrado por una coleccion de objetos foraneos de UsUsuario */ 	
	static function getUsLoginsXUsUsuarios($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(UsLogin::GEN_ATTR_US_USUARIO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(UsLogin::GEN_TABLA);
            $c->addOrden(UsLogin::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'us_login_adm.php';
            $url_gestion = 'us_login_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM us_login'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'us_login';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getUsLogins($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getUsLogins(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'us_login_adm');
            }else{
