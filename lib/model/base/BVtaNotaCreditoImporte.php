...
class BVtaNotaCreditoImporte
{ 
	
	const SES_PAGINACION = 'adm_vtanotacreditoimporte_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_vtanotacreditoimporte_paginas_registros';
	const SES_CRITERIOS = 'adm_vtanotacreditoimporte_criterios';
	
	const GEN_CLASE = 'VtaNotaCreditoImporte';
	const GEN_TABLA = 'vta_nota_credito_importe';
	...
	/* Constantes de Atributos de BVtaNotaCreditoImporte */ 
	const GEN_ATTR_ID = 'vta_nota_credito_importe.id';
	const GEN_ATTR_DESCRIPCION = 'vta_nota_credito_importe.descripcion';
	const GEN_ATTR_VTA_NOTA_CREDITO_ID = 'vta_nota_credito_importe.vta_nota_credito_id';
	const GEN_ATTR_IMPORTE_SUBTOTAL = 'vta_nota_credito_importe.importe_subtotal';
	const GEN_ATTR_IMPORTE_IVA = 'vta_nota_credito_importe.importe_iva';
	const GEN_ATTR_IMPORTE_TRIBUTO = 'vta_nota_credito_importe.importe_tributo';
	const GEN_ATTR_IMPORTE_TOTAL = 'vta_nota_credito_importe.importe_total';
	const GEN_ATTR_IMPORTE_SALDADO = 'vta_nota_credito_importe.importe_saldado';
	const GEN_ATTR_IMPORTE_SALDO = 'vta_nota_credito_importe.importe_saldo';
	const GEN_ATTR_CODIGO = 'vta_nota_credito_importe.codigo';
	const GEN_ATTR_OBSERVACION = 'vta_nota_credito_importe.observacion';
	const GEN_ATTR_ORDEN = 'vta_nota_credito_importe.orden';
	const GEN_ATTR_ESTADO = 'vta_nota_credito_importe.estado';
	const GEN_ATTR_CREADO = 'vta_nota_credito_importe.creado';
	const GEN_ATTR_CREADO_POR = 'vta_nota_credito_importe.creado_por';
	const GEN_ATTR_MODIFICADO = 'vta_nota_credito_importe.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'vta_nota_credito_importe.modificado_por';

	/* Constantes de Atributos Min de BVtaNotaCreditoImporte */ 
	...
	/* Atributos de BVtaNotaCreditoImporte */ 
	private $id;
	private $descripcion;
	...
	/* Getters de BVtaNotaCreditoImporte */ 
	...
	/* Setters de BVtaNotaCreditoImporte */ 	
	...
	public static function hidratarObjeto($fila){            
            $o = new VtaNotaCreditoImporte();
            $o->setId($fila[VtaNotaCreditoImporte::GEN_ATTR_MIN_ID], false);
	...
	/* Control de BVtaNotaCreditoImporte */ 	
	...
	/* Cambia el estado de BVtaNotaCreditoImporte */ 	
	...
	/* Save de BVtaNotaCreditoImporte */ 	
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('vta_nota_credito_importe_seq'), 
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('vta_nota_credito_importe_seq')";
                ...
                $sql = "
				 ...
				 ".VtaNotaCreditoImporte::GEN_TABLA."
				 SET 
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	/* Delete de BVtaNotaCreditoImporte */ 	
	...
	public function duplicarVtaNotaCreditoImporte(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getVtaNotaCreditoImportes($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = VtaNotaCreditoImporte::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $vtanotacreditoimporte = new VtaNotaCreditoImporte();
                    $vtanotacreditoimporte->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $vtanotacreditoimporte->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$vtanotacreditoimporte->setVtaNotaCreditoId($fila[self::GEN_ATTR_MIN_VTA_NOTA_CREDITO_ID]);
			$vtanotacreditoimporte->setImporteSubtotal($fila[self::GEN_ATTR_MIN_IMPORTE_SUBTOTAL]);
			$vtanotacreditoimporte->setImporteIva($fila[self::GEN_ATTR_MIN_IMPORTE_IVA]);
			$vtanotacreditoimporte->setImporteTributo($fila[self::GEN_ATTR_MIN_IMPORTE_TRIBUTO]);
			$vtanotacreditoimporte->setImporteTotal($fila[self::GEN_ATTR_MIN_IMPORTE_TOTAL]);
			$vtanotacreditoimporte->setImporteSaldado($fila[self::GEN_ATTR_MIN_IMPORTE_SALDADO]);
			$vtanotacreditoimporte->setImporteSaldo($fila[self::GEN_ATTR_MIN_IMPORTE_SALDO]);
			$vtanotacreditoimporte->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$vtanotacreditoimporte->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$vtanotacreditoimporte->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$vtanotacreditoimporte->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$vtanotacreditoimporte->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$vtanotacreditoimporte->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$vtanotacreditoimporte->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$vtanotacreditoimporte->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getVtaNotaCreditoImportesHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return VtaNotaCreditoImporte::getVtaNotaCreditoImportes($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getVtaNotaCreditoImportes para listado de Backend */ 	
	static function getVtaNotaCreditoImportesDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return VtaNotaCreditoImporte::getVtaNotaCreditoImportes($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getVtaNotaCreditoImportesCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = VtaNotaCreditoImporte::getVtaNotaCreditoImportes($paginador, $criterio);
	...
	/* Método getVtaNotaCreditoImportes filtrado para select */ 	
	static function getVtaNotaCreditoImportesCmbF($paginador = null, $criterio = null){
            $col = VtaNotaCreditoImporte::getVtaNotaCreditoImportes($paginador, $criterio);
	...
	/* Método getVtaNotaCreditoImportes filtrado por una coleccion de objetos foraneos de VtaNotaCredito */ 	
	static function getVtaNotaCreditoImportesXVtaNotaCreditos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(VtaNotaCreditoImporte::GEN_ATTR_VTA_NOTA_CREDITO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(VtaNotaCreditoImporte::GEN_TABLA);
            $c->addOrden(VtaNotaCreditoImporte::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = VtaNotaCreditoImporte::getVtaNotaCreditoImportes($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'vta_nota_credito_importe_adm.php';
            $url_gestion = 'vta_nota_credito_importe_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM vta_nota_credito_importe'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'vta_nota_credito_importe';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getVtaNotaCreditoImportes($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getVtaNotaCreditoImportes(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'vta_nota_credito_importe_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(VtaNotaCreditoImporte::GEN_TABLA);
                $c->addOrden(VtaNotaCreditoImporte::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $vta_nota_credito_importes = VtaNotaCreditoImporte::getVtaNotaCreditoImportes(null, $c);

                $arr = array();
                foreach($vta_nota_credito_importes as $vta_nota_credito_importe){
                    $descripcion = $vta_nota_credito_importe->getDescripcion();
