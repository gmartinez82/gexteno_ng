...
{ 
	...
	const SES_PAGINACION = 'adm_pdeocenviado_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_pdeocenviado_paginas_registros';
	const SES_CRITERIOS = 'adm_pdeocenviado_criterios';
	
	const GEN_CLASE = 'PdeOcEnviado';
	const GEN_TABLA = 'pde_oc_enviado';
	...
	/* Constantes de Atributos de BPdeOcEnviado */ 
	const GEN_ATTR_ID = 'pde_oc_enviado.id';
	const GEN_ATTR_DESCRIPCION = 'pde_oc_enviado.descripcion';
	const GEN_ATTR_PDE_OC_ID = 'pde_oc_enviado.pde_oc_id';
	const GEN_ATTR_ASUNTO = 'pde_oc_enviado.asunto';
	const GEN_ATTR_DESTINATARIO = 'pde_oc_enviado.destinatario';
	const GEN_ATTR_CUERPO = 'pde_oc_enviado.cuerpo';
	const GEN_ATTR_ADJUNTO = 'pde_oc_enviado.adjunto';
	const GEN_ATTR_CODIGO_ENVIO = 'pde_oc_enviado.codigo_envio';
	const GEN_ATTR_CODIGO = 'pde_oc_enviado.codigo';
	const GEN_ATTR_OBSERVACION = 'pde_oc_enviado.observacion';
	const GEN_ATTR_ORDEN = 'pde_oc_enviado.orden';
	const GEN_ATTR_ESTADO = 'pde_oc_enviado.estado';
	const GEN_ATTR_CREADO = 'pde_oc_enviado.creado';
	const GEN_ATTR_CREADO_POR = 'pde_oc_enviado.creado_por';
	const GEN_ATTR_MODIFICADO = 'pde_oc_enviado.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'pde_oc_enviado.modificado_por';

	/* Constantes de Atributos Min de BPdeOcEnviado */ 
	...
	public static function hidratarObjeto($fila){            
            $o = new PdeOcEnviado();
            $o->setId($fila[PdeOcEnviado::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BPdeOcEnviado */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('pde_oc_enviado_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function duplicarPdeOcEnviado(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPdeOcEnviados($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PdeOcEnviado::setAplicarFiltrosDeAlcance($criterio);
		...
		$pdeocenviados = array();
		while($fila = pg_fetch_array($cons->getResultado())){
                    $pdeocenviado = new PdeOcEnviado();
                    $pdeocenviado->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $pdeocenviado->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$pdeocenviado->setPdeOcId($fila[self::GEN_ATTR_MIN_PDE_OC_ID]);
			$pdeocenviado->setAsunto($fila[self::GEN_ATTR_MIN_ASUNTO]);
			$pdeocenviado->setDestinatario($fila[self::GEN_ATTR_MIN_DESTINATARIO]);
			$pdeocenviado->setCuerpo($fila[self::GEN_ATTR_MIN_CUERPO]);
			$pdeocenviado->setAdjunto($fila[self::GEN_ATTR_MIN_ADJUNTO]);
			$pdeocenviado->setCodigoEnvio($fila[self::GEN_ATTR_MIN_CODIGO_ENVIO]);
			$pdeocenviado->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$pdeocenviado->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$pdeocenviado->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$pdeocenviado->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$pdeocenviado->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$pdeocenviado->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$pdeocenviado->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$pdeocenviado->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPdeOcEnviadosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PdeOcEnviado::getPdeOcEnviados($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPdeOcEnviados para listado de Backend */ 	
	static function getPdeOcEnviadosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PdeOcEnviado::getPdeOcEnviados($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPdeOcEnviadosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PdeOcEnviado::getPdeOcEnviados($paginador, $criterio);
	...
	/* Método getPdeOcEnviados filtrado para select */ 	
	static function getPdeOcEnviadosCmbF($paginador = null, $criterio = null){
            $col = PdeOcEnviado::getPdeOcEnviados($paginador, $criterio);
	...
	/* Método getPdeOcEnviados filtrado por una coleccion de objetos foraneos de PdeOc */ 	
	static function getPdeOcEnviadosXPdeOcs($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdeOcEnviado::GEN_ATTR_PDE_OC_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdeOcEnviado::GEN_TABLA);
            $c->addOrden(PdeOcEnviado::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = PdeOcEnviado::getPdeOcEnviados($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'pde_oc_enviado_adm.php';
            $url_gestion = 'pde_oc_enviado_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM pde_oc_enviado'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'pde_oc_enviado';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPdeOcEnviados($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPdeOcEnviados(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'pde_oc_enviado_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(PdeOcEnviado::GEN_TABLA);
                $c->addOrden(PdeOcEnviado::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $pde_oc_enviados = PdeOcEnviado::getPdeOcEnviados(null, $c);

                $arr = array();
                foreach($pde_oc_enviados as $pde_oc_enviado){
                    $descripcion = $pde_oc_enviado->getDescripcion();
