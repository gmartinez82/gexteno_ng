...
class BVtaReciboItem
{ 
	
	const SES_PAGINACION = 'adm_vtareciboitem_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_vtareciboitem_paginas_registros';
	const SES_CRITERIOS = 'adm_vtareciboitem_criterios';
	
	const GEN_CLASE = 'VtaReciboItem';
	const GEN_TABLA = 'vta_recibo_item';
	...
	/* Constantes de Atributos de BVtaReciboItem */ 
	const GEN_ATTR_ID = 'vta_recibo_item.id';
	const GEN_ATTR_DESCRIPCION = 'vta_recibo_item.descripcion';
	const GEN_ATTR_VTA_RECIBO_ID = 'vta_recibo_item.vta_recibo_id';
	const GEN_ATTR_GRAL_FP_FORMA_PAGO_ID = 'vta_recibo_item.gral_fp_forma_pago_id';
	const GEN_ATTR_GRAL_TIPO_IVA_ID = 'vta_recibo_item.gral_tipo_iva_id';
	const GEN_ATTR_VTA_RECIBO_CONCEPTO_ID = 'vta_recibo_item.vta_recibo_concepto_id';
	const GEN_ATTR_GRAL_MONEDA_ID = 'vta_recibo_item.gral_moneda_id';
	const GEN_ATTR_TIPO_CAMBIO = 'vta_recibo_item.tipo_cambio';
	const GEN_ATTR_TIPO_CAMBIO_REAL = 'vta_recibo_item.tipo_cambio_real';
	const GEN_ATTR_IMPORTE_UNITARIO_REAL = 'vta_recibo_item.importe_unitario_real';
	const GEN_ATTR_IMPORTE_UNITARIO = 'vta_recibo_item.importe_unitario';
	const GEN_ATTR_CANTIDAD = 'vta_recibo_item.cantidad';
	const GEN_ATTR_CONCILIADO = 'vta_recibo_item.conciliado';
	const GEN_ATTR_CODIGO = 'vta_recibo_item.codigo';
	const GEN_ATTR_OBSERVACION = 'vta_recibo_item.observacion';
	const GEN_ATTR_ORDEN = 'vta_recibo_item.orden';
	const GEN_ATTR_ESTADO = 'vta_recibo_item.estado';
	const GEN_ATTR_CREADO = 'vta_recibo_item.creado';
	const GEN_ATTR_CREADO_POR = 'vta_recibo_item.creado_por';
	const GEN_ATTR_MODIFICADO = 'vta_recibo_item.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'vta_recibo_item.modificado_por';

	/* Constantes de Atributos Min de BVtaReciboItem */ 
	...
	const GEN_ATTR_MIN_VTA_RECIBO_CONCEPTO_ID = 'vta_recibo_concepto_id';
	...
	public function getVtaReciboConceptoId(){ if(isset($this->vta_recibo_concepto_id)){ return $this->vta_recibo_concepto_id; }else{ return 'null'; } }
	...
	public function setId($v, $db = true){
            ...
            if($db){
			 ...
			 SELECT 
				...
				, ".self::GEN_ATTR_VTA_RECIBO_CONCEPTO_ID."
			 ...
			 WHERE ". self::GEN_ATTR_ID." = ".$this->getId();
				...
				$this->setVtaReciboConceptoId($fila[self::GEN_ATTR_MIN_VTA_RECIBO_CONCEPTO_ID]);
	...
	public function setVtaReciboConceptoId($v){ $this->vta_recibo_concepto_id = $v; }
	...
	public static function hidratarObjeto($fila){            
            $o = new VtaReciboItem();
            $o->setId($fila[VtaReciboItem::GEN_ATTR_MIN_ID], false);
			...
			$o->setVtaReciboConceptoId($fila[self::GEN_ATTR_MIN_VTA_RECIBO_CONCEPTO_ID]);
	...
	/* Cambia el estado de BVtaReciboItem */ 	
            ...
            if(!isset($this->id)){
                ...
                $sql = "
				 INSERT INTO ".self::GEN_TABLA." (".self::GEN_ATTR_MIN_ID.", ".self::GEN_ATTR_MIN_DESCRIPCION."
						...
						, ".self::GEN_ATTR_MIN_VTA_RECIBO_CONCEPTO_ID."
                ...
                nextval('vta_recibo_item_seq'), 
                '".$this->getDescripcion()."'
						, ".$this->getVtaReciboId()."
						, ".$this->getGralFpFormaPagoId()."
						, ".$this->getGralTipoIvaId()."
						, ".$this->getVtaReciboConceptoId()."
                    ...
                    ) RETURNING currval('vta_recibo_item_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
						...
						, ".self::GEN_ATTR_MIN_VTA_RECIBO_CONCEPTO_ID." = ".$this->getVtaReciboConceptoId()."
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function deleteAll(){
            ...
            // se elimina la coleccion de VtaReciboItemConciliados relacionados
            $c = new Criterio('COLECCION_DELETE_ALL');
            $c->addTabla(VtaReciboItemConciliado::GEN_TABLA);
            $c->setCriterios();		
            foreach($this->getVtaReciboItemConciliados(null, $c) as $o){
                    $o->deleteAll();
            }
	...
	public function duplicarVtaReciboItem(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getVtaReciboItems($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = VtaReciboItem::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $vtareciboitem = new VtaReciboItem();
                    $vtareciboitem->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $vtareciboitem->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$vtareciboitem->setVtaReciboId($fila[self::GEN_ATTR_MIN_VTA_RECIBO_ID]);
			$vtareciboitem->setGralFpFormaPagoId($fila[self::GEN_ATTR_MIN_GRAL_FP_FORMA_PAGO_ID]);
			$vtareciboitem->setGralTipoIvaId($fila[self::GEN_ATTR_MIN_GRAL_TIPO_IVA_ID]);
			$vtareciboitem->setVtaReciboConceptoId($fila[self::GEN_ATTR_MIN_VTA_RECIBO_CONCEPTO_ID]);
			$vtareciboitem->setGralMonedaId($fila[self::GEN_ATTR_MIN_GRAL_MONEDA_ID]);
			$vtareciboitem->setTipoCambio($fila[self::GEN_ATTR_MIN_TIPO_CAMBIO]);
			$vtareciboitem->setTipoCambioReal($fila[self::GEN_ATTR_MIN_TIPO_CAMBIO_REAL]);
			$vtareciboitem->setImporteUnitarioReal($fila[self::GEN_ATTR_MIN_IMPORTE_UNITARIO_REAL]);
			$vtareciboitem->setImporteUnitario($fila[self::GEN_ATTR_MIN_IMPORTE_UNITARIO]);
			$vtareciboitem->setCantidad($fila[self::GEN_ATTR_MIN_CANTIDAD]);
			$vtareciboitem->setConciliado($fila[self::GEN_ATTR_MIN_CONCILIADO]);
			$vtareciboitem->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$vtareciboitem->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$vtareciboitem->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$vtareciboitem->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$vtareciboitem->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$vtareciboitem->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$vtareciboitem->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$vtareciboitem->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getVtaReciboItemsHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return VtaReciboItem::getVtaReciboItems($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getVtaReciboItems para listado de Backend */ 	
	static function getVtaReciboItemsDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return VtaReciboItem::getVtaReciboItems($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getVtaReciboItemsCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = VtaReciboItem::getVtaReciboItems($paginador, $criterio);
	...
	/* Método getVtaReciboItems filtrado para select */ 	
	static function getVtaReciboItemsCmbF($paginador = null, $criterio = null){
            $col = VtaReciboItem::getVtaReciboItems($paginador, $criterio);
	...
	/* Método getVtaReciboItems filtrado por una coleccion de objetos foraneos de VtaRecibo */ 	
	static function getVtaReciboItemsXVtaRecibos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(VtaReciboItem::GEN_ATTR_VTA_RECIBO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(VtaReciboItem::GEN_TABLA);
            $c->addOrden(VtaReciboItem::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = VtaReciboItem::getVtaReciboItems($p, $c);
            foreach($os as $o){
	...
	/* Método getVtaReciboItems filtrado por una coleccion de objetos foraneos de GralFpFormaPago */ 	
	static function getVtaReciboItemsXGralFpFormaPagos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(VtaReciboItem::GEN_ATTR_GRAL_FP_FORMA_PAGO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(VtaReciboItem::GEN_TABLA);
            $c->addOrden(VtaReciboItem::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
	...
	/* Método getVtaReciboItems filtrado por una coleccion de objetos foraneos de GralTipoIva */ 	
	static function getVtaReciboItemsXGralTipoIvas($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(VtaReciboItem::GEN_ATTR_GRAL_TIPO_IVA_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(VtaReciboItem::GEN_TABLA);
            $c->addOrden(VtaReciboItem::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
	...
	/* Método getVtaReciboItems filtrado por una coleccion de objetos foraneos de VtaReciboConcepto */ 	
	static function getVtaReciboItemsXVtaReciboConceptos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(VtaReciboItem::GEN_ATTR_VTA_RECIBO_CONCEPTO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(VtaReciboItem::GEN_TABLA);
            $c->addOrden(VtaReciboItem::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
            ...
            $os = VtaReciboItem::getVtaReciboItems($p, $c);
            foreach($os as $o){
                $os_con_indice[$o->getVtaReciboConceptoId()] = $o;
            }
            return $os_con_indice;
	}	
	

	/* Método getVtaReciboItems filtrado por una coleccion de objetos foraneos de GralMoneda */ 	
	static function getVtaReciboItemsXGralMonedas($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(VtaReciboItem::GEN_ATTR_GRAL_MONEDA_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(VtaReciboItem::GEN_TABLA);
            $c->addOrden(VtaReciboItem::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'vta_recibo_item_adm.php';
            $url_gestion = 'vta_recibo_item_gestion.php';
	...
	/* Metodo getVtaReciboItemConciliados */ 	
	public function getVtaReciboItemConciliados($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            // inicializacion de variables
            if(is_null($paginador)){
                $paginador = new Paginador(VtaReciboItemConciliado::getCantidadTotalDeRegistros(), 1);
            }
            if(is_null($criterio)){
                ...
                //$criterio->addOrden(VtaReciboItemConciliado::GEN_ATTR_ORDEN, 'asc');                            
                $criterio->addOrden('1', 'asc');			
                $criterio->setCriterios();
            }
                
            $criterio->add(VtaReciboItemConciliado::GEN_ATTR_VTA_RECIBO_ITEM_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
                    $criterio->add(VtaReciboItemConciliado::GEN_ATTR_ESTADO, 1, Criterio::IGUAL);
                }else{
                    $criterio->add(VtaReciboItemConciliado::GEN_ATTR_ESTADO, 0, Criterio::IGUAL);			
                }
            }
            ...
            $cons = new Consulta();
            $sql = "SELECT ".$criterio->getDistinct()." ".VtaReciboItemConciliado::GEN_TABLA.".* FROM ".$criterio->getTablas().$criterio->getWhere().$criterio->getOrden();
            ...
            $vtareciboitemconciliados = array();
            while($fila = pg_fetch_array($cons->getResultado())){
                $vtareciboitemconciliado = VtaReciboItemConciliado::hidratarObjeto($fila);			
                $vtareciboitemconciliados[] = $vtareciboitemconciliado;
            }

            return $vtareciboitemconciliados;
	}	
	

	/* Método getVtaReciboItemConciliadosBloque para MasInfo */ 	
	public function getVtaReciboItemConciliadosParaBloqueMasInfo($paginador = null, $criterio = null, $estado = null){
            // inicializacion de variables
            if(is_null($paginador)){
                $paginador = new Paginador(50, 1);
            }

            $os = $this->getVtaReciboItemConciliados($paginador, $criterio, $estado);
            return $os;
		
	}

	/* Método getVtaReciboItemConciliados Habilitados */ 	
	public function getVtaReciboItemConciliadosHabilitados($paginador = null, $criterio = null, $estado = null){

            $os = $this->getVtaReciboItemConciliados($paginador, $criterio, true);
            return $os;
		
	}

	/* Método getVtaReciboItemConciliado */ 	
	public function getVtaReciboItemConciliado($criterio = null){
            $paginador = new Paginador(1, 1);
            $os = $this->getVtaReciboItemConciliados($paginador, $criterio);
            foreach($os as $o){
                return $o;
            }
            return false;
	...
	/* Delete de VtaReciboItemConciliado relacionadas */ 	
	public function deleteVtaReciboItemConciliados(){
            $obs = $this->getVtaReciboItemConciliados();
            foreach($obs as $o){
                $o->delete();
            }
            return true;
	...
	/* Metodos getVtaReciboItemConciliadosCmb() VtaReciboItemConciliado relacionadas */ 	
	public function getVtaReciboItemConciliadosCmb(){
            $c = new Criterio();
            $c->add(VtaReciboItemConciliado::GEN_ATTR_VTA_RECIBO_ITEM_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(VtaReciboItemConciliado::GEN_TABLA);
            $c->setCriterios();

            $os = VtaReciboItemConciliado::getVtaReciboItemConciliadosCmbF(null, $c);
            return $os;
	}
	...
	public function getVtaReciboItemCheques($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            ...
            $criterio->add(VtaReciboItemCheque::GEN_ATTR_VTA_RECIBO_ITEM_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
	...
	public function getVtaReciboItemChequesCmb(){
            $c = new Criterio();
            $c->add(VtaReciboItemCheque::GEN_ATTR_VTA_RECIBO_ITEM_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(VtaReciboItemCheque::GEN_TABLA);
            $c->setCriterios();
	...
	public function getVtaReciboItemRetencions($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            ...
            $criterio->add(VtaReciboItemRetencion::GEN_ATTR_VTA_RECIBO_ITEM_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
	...
	public function getVtaReciboItemRetencionsCmb(){
            $c = new Criterio();
            $c->add(VtaReciboItemRetencion::GEN_ATTR_VTA_RECIBO_ITEM_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(VtaReciboItemRetencion::GEN_TABLA);
            $c->setCriterios();
	...
	/* Getter para obtener VtaRecibos (Coleccion) relacionados a traves de VtaReciboItemRetencion */ 	
	public function getVtaRecibosXVtaReciboItemRetencion($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            $c = new Criterio();
            if($estado){
                $c->add(VtaRecibo::GEN_ATTR_ESTADO, 1, Criterio::IGUAL);
            ...
            $c->add(VtaReciboItemRetencion::GEN_ATTR_VTA_RECIBO_ITEM_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(VtaRecibo::GEN_TABLA);
            $c->addRealJoin(VtaReciboItemRetencion::GEN_TABLA, VtaReciboItemRetencion::GEN_ATTR_VTA_RECIBO_ID, VtaRecibo::GEN_ATTR_ID);
            $c->addOrden(2);
            $c->setCriterios();
	...
	/* Getter para obtener la cantidad de VtaRecibos relacionados a traves de VtaReciboItemRetencion */ 	
	public function getCantidadVtaRecibosXVtaReciboItemRetencion($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            $c = new Criterio();
            $c->addSelect(VtaRecibo::GEN_ATTR_ID);
            if($estado){
                $c->add(VtaRecibo::GEN_ATTR_ESTADO, 1, Criterio::IGUAL);
	...
	/* Getter para obtener VtaRecibo (Objeto) relacionado a traves de VtaReciboItemRetencion */ 	
	public function getVtaReciboXVtaReciboItemRetencion($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            $os = $this->getVtaRecibosXVtaReciboItemRetencion($paginador, $criterio, $estado, $arr_ordens);
            foreach($os as $o){
                return $o;
            }
	...
	public function getFndChqCheques($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            ...
            $criterio->add(FndChqCheque::GEN_ATTR_VTA_RECIBO_ITEM_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
	...
	public function getFndChqChequesCmb(){
            $c = new Criterio();
            $c->add(FndChqCheque::GEN_ATTR_VTA_RECIBO_ITEM_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(FndChqCheque::GEN_TABLA);
            $c->setCriterios();
	...
	/* Metodo que retorna el VtaReciboConcepto (Clave Foranea) */ 	
    public function getVtaReciboConcepto(){
        $o = new VtaReciboConcepto();
        $o->setId($this->getVtaReciboConceptoId());
        return $o;			
    }

	/* Metodo que retorna el VtaReciboConcepto (Clave Foranea) en Array */ 	
    public function getVtaReciboConceptoEnArray(&$arr_os){
        if($this->getVtaReciboConceptoId() != 'null'){
            if(isset($arr_os[$this->getVtaReciboConceptoId()])){
                $o = $arr_os[$this->getVtaReciboConceptoId()];
            }else{
                $o = VtaReciboConcepto::getOxId($this->getVtaReciboConceptoId());
                if($o){
                    $arr_os[$this->getVtaReciboConceptoId()] = $o;
                }
            }
        }else{
            $o = new VtaReciboConcepto();
    ...
    public function getDescripcionVinculante($contexto_solicitante = ''){
        ...
        if($contexto_solicitante != VtaReciboConcepto::GEN_CLASE){
            if($this->getVtaReciboConcepto()->getDescripcion() != ''){
                $descripcion.= Lang::_lang(VtaReciboConcepto::GEN_CLASE, true).':';
                $descripcion.= '<strong>'.$this->getVtaReciboConcepto()->getDescripcion().'</strong>';
                $descripcion.= ' - ';
            }
        }
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM vta_recibo_item'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'vta_recibo_item';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getVtaReciboItems($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getVtaReciboItems(null, $criterio);
            return $obs;
	}
	...
	/* retorna un objeto de acuerdo al 'vta_recibo_concepto_id' */ 	
	static function getOxVtaReciboConceptoId($valor){
            $criterio = new Criterio();
            $criterio->add(self::GEN_ATTR_VTA_RECIBO_CONCEPTO_ID, $valor, Criterio::IGUAL);
            $criterio->addTabla(self::GEN_TABLA);
            $criterio->setCriterios();
	...
	/* retorna una coleccion de objetos de acuerdo al 'vta_recibo_concepto_id' */ 	
	static function getOsxVtaReciboConceptoId($valor){
            $criterio = new Criterio();
            $criterio->add(self::GEN_ATTR_VTA_RECIBO_CONCEPTO_ID, $valor, Criterio::IGUAL);
            $criterio->addTabla(self::GEN_TABLA);
            $criterio->setCriterios();
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'vta_recibo_item_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(VtaReciboItem::GEN_TABLA);
                $c->addOrden(VtaReciboItem::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $vta_recibo_items = VtaReciboItem::getVtaReciboItems(null, $c);
