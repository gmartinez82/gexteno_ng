...
{ 
	...
	const SES_PAGINACION = 'adm_pdiestado_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_pdiestado_paginas_registros';
	const SES_CRITERIOS = 'adm_pdiestado_criterios';
	
	const GEN_CLASE = 'PdiEstado';
	const GEN_TABLA = 'pdi_estado';
	...
	/* Constantes de Atributos de BPdiEstado */ 
	const GEN_ATTR_ID = 'pdi_estado.id';
	const GEN_ATTR_PDI_PEDIDO_ID = 'pdi_estado.pdi_pedido_id';
	const GEN_ATTR_PDI_TIPO_ESTADO_ID = 'pdi_estado.pdi_tipo_estado_id';
	const GEN_ATTR_INICIAL = 'pdi_estado.inicial';
	const GEN_ATTR_ACTUAL = 'pdi_estado.actual';
	const GEN_ATTR_CANTIDAD = 'pdi_estado.cantidad';
	const GEN_ATTR_CANTIDAD_STOCK_REAL = 'pdi_estado.cantidad_stock_real';
	const GEN_ATTR_CANTIDAD_STOCK_COMPROMETIDA = 'pdi_estado.cantidad_stock_comprometida';
	const GEN_ATTR_FECHAHORA = 'pdi_estado.fechahora';
	const GEN_ATTR_VENTA_PERDIDA = 'pdi_estado.venta_perdida';
	const GEN_ATTR_OBSERVACION = 'pdi_estado.observacion';
	const GEN_ATTR_CREADO = 'pdi_estado.creado';
	const GEN_ATTR_CREADO_POR = 'pdi_estado.creado_por';
	const GEN_ATTR_MODIFICADO = 'pdi_estado.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'pdi_estado.modificado_por';

	/* Constantes de Atributos Min de BPdiEstado */ 
	...
	const GEN_ATTR_MIN_CANTIDAD_STOCK_REAL = 'cantidad_stock_real';
	const GEN_ATTR_MIN_CANTIDAD_STOCK_COMPROMETIDA = 'cantidad_stock_comprometida';
	...
	private $actual;
	private $cantidad;
	private $cantidad_stock_real;
	private $cantidad_stock_comprometida;
	private $fechahora;
	...
	public function getCantidadStockReal(){ if(isset($this->cantidad_stock_real)){ return $this->cantidad_stock_real; }else{ return 0; } }
	public function getCantidadStockComprometida(){ if(isset($this->cantidad_stock_comprometida)){ return $this->cantidad_stock_comprometida; }else{ return 0; } }
	...
	public function setId($v, $db = true){
            ...
            if($db){
			 ...
			 SELECT 
				...
				, ".self::GEN_ATTR_CANTIDAD_STOCK_REAL."
				, ".self::GEN_ATTR_CANTIDAD_STOCK_COMPROMETIDA."
			 ...
			 WHERE ". self::GEN_ATTR_ID." = ".$this->getId();
				...
				$this->setCantidadStockReal($fila[self::GEN_ATTR_MIN_CANTIDAD_STOCK_REAL]);
				$this->setCantidadStockComprometida($fila[self::GEN_ATTR_MIN_CANTIDAD_STOCK_COMPROMETIDA]);
	...
	public function setCantidadStockReal($v){ $this->cantidad_stock_real = $v; }
	public function setCantidadStockComprometida($v){ $this->cantidad_stock_comprometida = $v; }
	...
	public static function hidratarObjeto($fila){            
            $o = new PdiEstado();
            $o->setId($fila[PdiEstado::GEN_ATTR_MIN_ID], false);
			...
			$o->setCantidadStockReal($fila[self::GEN_ATTR_MIN_CANTIDAD_STOCK_REAL]);
			$o->setCantidadStockComprometida($fila[self::GEN_ATTR_MIN_CANTIDAD_STOCK_COMPROMETIDA]);
	...
	/* Cambia el estado de BPdiEstado */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                $sql = "
				 INSERT INTO ".self::GEN_TABLA." (".self::GEN_ATTR_MIN_ID.", ".self::GEN_ATTR_MIN_PDI_PEDIDO_ID."
						, ".self::GEN_ATTR_MIN_PDI_TIPO_ESTADO_ID."
						, ".self::GEN_ATTR_MIN_INICIAL."
						, ".self::GEN_ATTR_MIN_ACTUAL."
						, ".self::GEN_ATTR_MIN_CANTIDAD."
						, ".self::GEN_ATTR_MIN_CANTIDAD_STOCK_REAL."
						, ".self::GEN_ATTR_MIN_CANTIDAD_STOCK_COMPROMETIDA."
                ...
                nextval('pdi_estado_seq'), 
                ".$this->getPdiPedidoId()."
						, ".$this->getPdiTipoEstadoId()."
						, ".$this->getInicial()."
						, ".$this->getActual()."
						, '".$this->getCantidad()."'
						, '".$this->getCantidadStockReal()."'
						, '".$this->getCantidadStockComprometida()."'
						, '".$this->getFechahora()."'
                    ...
                    ) RETURNING currval('pdi_estado_seq')";
...
".self::GEN_ATTR_MIN_PDI_PEDIDO_ID." = ".$this->getPdiPedidoId()."
						...
						, ".self::GEN_ATTR_MIN_CANTIDAD_STOCK_REAL." = '".$this->getCantidadStockReal()."'
						, ".self::GEN_ATTR_MIN_CANTIDAD_STOCK_COMPROMETIDA." = '".$this->getCantidadStockComprometida()."'
...
".self::GEN_ATTR_MIN_PDI_PEDIDO_ID." = ".$this->getPdiPedidoId()."
	...
	public function duplicarPdiEstado(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPdiEstados($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PdiEstado::setAplicarFiltrosDeAlcance($criterio);
		...
		$pdiestados = array();
		while($fila = pg_fetch_array($cons->getResultado())){
                    $pdiestado = new PdiEstado();
                    $pdiestado->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $pdiestado->setPdiPedidoId($fila[self::GEN_ATTR_MIN_PDI_PEDIDO_ID]);
			$pdiestado->setPdiTipoEstadoId($fila[self::GEN_ATTR_MIN_PDI_TIPO_ESTADO_ID]);
			$pdiestado->setInicial($fila[self::GEN_ATTR_MIN_INICIAL]);
			$pdiestado->setActual($fila[self::GEN_ATTR_MIN_ACTUAL]);
			$pdiestado->setCantidad($fila[self::GEN_ATTR_MIN_CANTIDAD]);
			$pdiestado->setCantidadStockReal($fila[self::GEN_ATTR_MIN_CANTIDAD_STOCK_REAL]);
			$pdiestado->setCantidadStockComprometida($fila[self::GEN_ATTR_MIN_CANTIDAD_STOCK_COMPROMETIDA]);
			$pdiestado->setFechahora($fila[self::GEN_ATTR_MIN_FECHAHORA]);
			$pdiestado->setVentaPerdida($fila[self::GEN_ATTR_MIN_VENTA_PERDIDA]);
			$pdiestado->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$pdiestado->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$pdiestado->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$pdiestado->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$pdiestado->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPdiEstadosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PdiEstado::getPdiEstados($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPdiEstados para listado de Backend */ 	
	static function getPdiEstadosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PdiEstado::getPdiEstados($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPdiEstadosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PdiEstado::getPdiEstados($paginador, $criterio);
	...
	/* Método getPdiEstados filtrado para select */ 	
	static function getPdiEstadosCmbF($paginador = null, $criterio = null){
            $col = PdiEstado::getPdiEstados($paginador, $criterio);
	...
	/* Método getPdiEstados filtrado por una coleccion de objetos foraneos de PdiPedido */ 	
	static function getPdiEstadosXPdiPedidos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdiEstado::GEN_ATTR_PDI_PEDIDO_ID, $arr_ids, Criterio::IN_ARRAY);
            ...
            $os = PdiEstado::getPdiEstados($p, $c);
            foreach($os as $o){
	...
	/* Método getPdiEstados filtrado por una coleccion de objetos foraneos de PdiTipoEstado */ 	
	static function getPdiEstadosXPdiTipoEstados($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdiEstado::GEN_ATTR_PDI_TIPO_ESTADO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdiEstado::GEN_TABLA);
            $c->addOrden(PdiEstado::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'pdi_estado_adm.php';
            $url_gestion = 'pdi_estado_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM pdi_estado'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'pdi_estado';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPdiEstados($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPdiEstados(null, $criterio);
            return $obs;
	}
	...
	/* retorna un objeto de acuerdo al 'cantidad_stock_real' */ 	
	static function getOxCantidadStockReal($valor){
            $criterio = new Criterio();
            $criterio->add(self::GEN_ATTR_CANTIDAD_STOCK_REAL, $valor, Criterio::IGUAL);
            $criterio->addTabla(self::GEN_TABLA);
            $criterio->setCriterios();
	...
	/* retorna una coleccion de objetos de acuerdo al 'cantidad_stock_real' */ 	
	static function getOsxCantidadStockReal($valor){
            $criterio = new Criterio();
            $criterio->add(self::GEN_ATTR_CANTIDAD_STOCK_REAL, $valor, Criterio::IGUAL);
            $criterio->addTabla(self::GEN_TABLA);
            $criterio->setCriterios();
	...
	/* retorna un objeto de acuerdo al 'cantidad_stock_comprometida' */ 	
	static function getOxCantidadStockComprometida($valor){
            $criterio = new Criterio();
            $criterio->add(self::GEN_ATTR_CANTIDAD_STOCK_COMPROMETIDA, $valor, Criterio::IGUAL);
            $criterio->addTabla(self::GEN_TABLA);
            $criterio->setCriterios();
	...
	/* retorna una coleccion de objetos de acuerdo al 'cantidad_stock_comprometida' */ 	
	static function getOsxCantidadStockComprometida($valor){
            $criterio = new Criterio();
            $criterio->add(self::GEN_ATTR_CANTIDAD_STOCK_COMPROMETIDA, $valor, Criterio::IGUAL);
            $criterio->addTabla(self::GEN_TABLA);
            $criterio->setCriterios();
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'pdi_estado_adm');
            }else{
