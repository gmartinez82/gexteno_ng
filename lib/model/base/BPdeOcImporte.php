...
{ 
	...
	const SES_PAGINACION = 'adm_pdeocimporte_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_pdeocimporte_paginas_registros';
	const SES_CRITERIOS = 'adm_pdeocimporte_criterios';
	
	const GEN_CLASE = 'PdeOcImporte';
	const GEN_TABLA = 'pde_oc_importe';
	...
	/* Constantes de Atributos de BPdeOcImporte */ 
	const GEN_ATTR_ID = 'pde_oc_importe.id';
	const GEN_ATTR_DESCRIPCION = 'pde_oc_importe.descripcion';
	const GEN_ATTR_PDE_OC_ID = 'pde_oc_importe.pde_oc_id';
	const GEN_ATTR_IMPORTE_SUBTOTAL = 'pde_oc_importe.importe_subtotal';
	const GEN_ATTR_IMPORTE_IVA = 'pde_oc_importe.importe_iva';
	const GEN_ATTR_IMPORTE_TRIBUTO = 'pde_oc_importe.importe_tributo';
	const GEN_ATTR_IMPORTE_TOTAL = 'pde_oc_importe.importe_total';
	const GEN_ATTR_IMPORTE_SALDADO = 'pde_oc_importe.importe_saldado';
	const GEN_ATTR_IMPORTE_SALDO = 'pde_oc_importe.importe_saldo';
	const GEN_ATTR_CODIGO = 'pde_oc_importe.codigo';
	const GEN_ATTR_OBSERVACION = 'pde_oc_importe.observacion';
	const GEN_ATTR_ORDEN = 'pde_oc_importe.orden';
	const GEN_ATTR_ESTADO = 'pde_oc_importe.estado';
	const GEN_ATTR_CREADO = 'pde_oc_importe.creado';
	const GEN_ATTR_CREADO_POR = 'pde_oc_importe.creado_por';
	const GEN_ATTR_MODIFICADO = 'pde_oc_importe.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'pde_oc_importe.modificado_por';

	/* Constantes de Atributos Min de BPdeOcImporte */ 
	...
	public static function hidratarObjeto($fila){            
            $o = new PdeOcImporte();
            $o->setId($fila[PdeOcImporte::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BPdeOcImporte */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('pde_oc_importe_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function duplicarPdeOcImporte(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPdeOcImportes($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PdeOcImporte::setAplicarFiltrosDeAlcance($criterio);
		...
		$pdeocimportes = array();
		while($fila = pg_fetch_array($cons->getResultado())){
                    $pdeocimporte = new PdeOcImporte();
                    $pdeocimporte->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $pdeocimporte->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$pdeocimporte->setPdeOcId($fila[self::GEN_ATTR_MIN_PDE_OC_ID]);
			$pdeocimporte->setImporteSubtotal($fila[self::GEN_ATTR_MIN_IMPORTE_SUBTOTAL]);
			$pdeocimporte->setImporteIva($fila[self::GEN_ATTR_MIN_IMPORTE_IVA]);
			$pdeocimporte->setImporteTributo($fila[self::GEN_ATTR_MIN_IMPORTE_TRIBUTO]);
			$pdeocimporte->setImporteTotal($fila[self::GEN_ATTR_MIN_IMPORTE_TOTAL]);
			$pdeocimporte->setImporteSaldado($fila[self::GEN_ATTR_MIN_IMPORTE_SALDADO]);
			$pdeocimporte->setImporteSaldo($fila[self::GEN_ATTR_MIN_IMPORTE_SALDO]);
			$pdeocimporte->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$pdeocimporte->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$pdeocimporte->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$pdeocimporte->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$pdeocimporte->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$pdeocimporte->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$pdeocimporte->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$pdeocimporte->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPdeOcImportesHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PdeOcImporte::getPdeOcImportes($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPdeOcImportes para listado de Backend */ 	
	static function getPdeOcImportesDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PdeOcImporte::getPdeOcImportes($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPdeOcImportesCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PdeOcImporte::getPdeOcImportes($paginador, $criterio);
	...
	/* Método getPdeOcImportes filtrado para select */ 	
	static function getPdeOcImportesCmbF($paginador = null, $criterio = null){
            $col = PdeOcImporte::getPdeOcImportes($paginador, $criterio);
	...
	/* Método getPdeOcImportes filtrado por una coleccion de objetos foraneos de PdeOc */ 	
	static function getPdeOcImportesXPdeOcs($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdeOcImporte::GEN_ATTR_PDE_OC_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdeOcImporte::GEN_TABLA);
            $c->addOrden(PdeOcImporte::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = PdeOcImporte::getPdeOcImportes($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'pde_oc_importe_adm.php';
            $url_gestion = 'pde_oc_importe_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM pde_oc_importe'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'pde_oc_importe';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPdeOcImportes($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPdeOcImportes(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'pde_oc_importe_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(PdeOcImporte::GEN_TABLA);
                $c->addOrden(PdeOcImporte::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $pde_oc_importes = PdeOcImporte::getPdeOcImportes(null, $c);

                $arr = array();
                foreach($pde_oc_importes as $pde_oc_importe){
                    $descripcion = $pde_oc_importe->getDescripcion();
