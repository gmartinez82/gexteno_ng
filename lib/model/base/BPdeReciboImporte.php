...
class BPdeReciboImporte
{ 
	
	const SES_PAGINACION = 'adm_pdereciboimporte_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_pdereciboimporte_paginas_registros';
	const SES_CRITERIOS = 'adm_pdereciboimporte_criterios';
	
	const GEN_CLASE = 'PdeReciboImporte';
	const GEN_TABLA = 'pde_recibo_importe';
	...
	/* Constantes de Atributos de BPdeReciboImporte */ 
	const GEN_ATTR_ID = 'pde_recibo_importe.id';
	const GEN_ATTR_DESCRIPCION = 'pde_recibo_importe.descripcion';
	const GEN_ATTR_PDE_RECIBO_ID = 'pde_recibo_importe.pde_recibo_id';
	const GEN_ATTR_IMPORTE_SUBTOTAL = 'pde_recibo_importe.importe_subtotal';
	const GEN_ATTR_IMPORTE_IVA = 'pde_recibo_importe.importe_iva';
	const GEN_ATTR_IMPORTE_TRIBUTO = 'pde_recibo_importe.importe_tributo';
	const GEN_ATTR_IMPORTE_TOTAL = 'pde_recibo_importe.importe_total';
	const GEN_ATTR_IMPORTE_SALDADO = 'pde_recibo_importe.importe_saldado';
	const GEN_ATTR_IMPORTE_SALDO = 'pde_recibo_importe.importe_saldo';
	const GEN_ATTR_CODIGO = 'pde_recibo_importe.codigo';
	const GEN_ATTR_OBSERVACION = 'pde_recibo_importe.observacion';
	const GEN_ATTR_ORDEN = 'pde_recibo_importe.orden';
	const GEN_ATTR_ESTADO = 'pde_recibo_importe.estado';
	const GEN_ATTR_CREADO = 'pde_recibo_importe.creado';
	const GEN_ATTR_CREADO_POR = 'pde_recibo_importe.creado_por';
	const GEN_ATTR_MODIFICADO = 'pde_recibo_importe.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'pde_recibo_importe.modificado_por';

	/* Constantes de Atributos Min de BPdeReciboImporte */ 
	...
	/* Atributos de BPdeReciboImporte */ 
	private $id;
	private $descripcion;
	...
	public static function hidratarObjeto($fila){            
            $o = new PdeReciboImporte();
            $o->setId($fila[PdeReciboImporte::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BPdeReciboImporte */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('pde_recibo_importe_seq'), 
                '".$this->getDescripcion()."'
						, ".$this->getPdeReciboId()."
                    ...
                    ) RETURNING currval('pde_recibo_importe_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function duplicarPdeReciboImporte(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPdeReciboImportes($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PdeReciboImporte::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $pdereciboimporte = new PdeReciboImporte();
                    $pdereciboimporte->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $pdereciboimporte->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$pdereciboimporte->setPdeReciboId($fila[self::GEN_ATTR_MIN_PDE_RECIBO_ID]);
			$pdereciboimporte->setImporteSubtotal($fila[self::GEN_ATTR_MIN_IMPORTE_SUBTOTAL]);
			$pdereciboimporte->setImporteIva($fila[self::GEN_ATTR_MIN_IMPORTE_IVA]);
			$pdereciboimporte->setImporteTributo($fila[self::GEN_ATTR_MIN_IMPORTE_TRIBUTO]);
			$pdereciboimporte->setImporteTotal($fila[self::GEN_ATTR_MIN_IMPORTE_TOTAL]);
			$pdereciboimporte->setImporteSaldado($fila[self::GEN_ATTR_MIN_IMPORTE_SALDADO]);
			$pdereciboimporte->setImporteSaldo($fila[self::GEN_ATTR_MIN_IMPORTE_SALDO]);
			$pdereciboimporte->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$pdereciboimporte->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$pdereciboimporte->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$pdereciboimporte->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$pdereciboimporte->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$pdereciboimporte->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$pdereciboimporte->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$pdereciboimporte->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPdeReciboImportesHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PdeReciboImporte::getPdeReciboImportes($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPdeReciboImportes para listado de Backend */ 	
	static function getPdeReciboImportesDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PdeReciboImporte::getPdeReciboImportes($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPdeReciboImportesCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PdeReciboImporte::getPdeReciboImportes($paginador, $criterio);
	...
	/* Método getPdeReciboImportes filtrado para select */ 	
	static function getPdeReciboImportesCmbF($paginador = null, $criterio = null){
            $col = PdeReciboImporte::getPdeReciboImportes($paginador, $criterio);
	...
	/* Método getPdeReciboImportes filtrado por una coleccion de objetos foraneos de PdeRecibo */ 	
	static function getPdeReciboImportesXPdeRecibos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdeReciboImporte::GEN_ATTR_PDE_RECIBO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdeReciboImporte::GEN_TABLA);
            $c->addOrden(PdeReciboImporte::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = PdeReciboImporte::getPdeReciboImportes($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'pde_recibo_importe_adm.php';
            $url_gestion = 'pde_recibo_importe_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM pde_recibo_importe'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'pde_recibo_importe';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPdeReciboImportes($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPdeReciboImportes(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'pde_recibo_importe_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(PdeReciboImporte::GEN_TABLA);
                $c->addOrden(PdeReciboImporte::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $pde_recibo_importes = PdeReciboImporte::getPdeReciboImportes(null, $c);

                $arr = array();
                foreach($pde_recibo_importes as $pde_recibo_importe){
                    $descripcion = $pde_recibo_importe->getDescripcion();
