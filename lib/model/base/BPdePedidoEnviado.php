...
class BPdePedidoEnviado
{ 
	
	const SES_PAGINACION = 'adm_pdepedidoenviado_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_pdepedidoenviado_paginas_registros';
	const SES_CRITERIOS = 'adm_pdepedidoenviado_criterios';
	
	const GEN_CLASE = 'PdePedidoEnviado';
	const GEN_TABLA = 'pde_pedido_enviado';
	...
	/* Constantes de Atributos de BPdePedidoEnviado */ 
	const GEN_ATTR_ID = 'pde_pedido_enviado.id';
	const GEN_ATTR_DESCRIPCION = 'pde_pedido_enviado.descripcion';
	const GEN_ATTR_PDE_PEDIDO_ID = 'pde_pedido_enviado.pde_pedido_id';
	const GEN_ATTR_PRV_PROVEEDOR_ID = 'pde_pedido_enviado.prv_proveedor_id';
	const GEN_ATTR_ASUNTO = 'pde_pedido_enviado.asunto';
	const GEN_ATTR_DESTINATARIO = 'pde_pedido_enviado.destinatario';
	const GEN_ATTR_CUERPO = 'pde_pedido_enviado.cuerpo';
	const GEN_ATTR_ADJUNTO = 'pde_pedido_enviado.adjunto';
	const GEN_ATTR_CODIGO_ENVIO = 'pde_pedido_enviado.codigo_envio';
	const GEN_ATTR_CODIGO = 'pde_pedido_enviado.codigo';
	const GEN_ATTR_OBSERVACION = 'pde_pedido_enviado.observacion';
	const GEN_ATTR_ORDEN = 'pde_pedido_enviado.orden';
	const GEN_ATTR_ESTADO = 'pde_pedido_enviado.estado';
	const GEN_ATTR_CREADO = 'pde_pedido_enviado.creado';
	const GEN_ATTR_CREADO_POR = 'pde_pedido_enviado.creado_por';
	const GEN_ATTR_MODIFICADO = 'pde_pedido_enviado.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'pde_pedido_enviado.modificado_por';

	/* Constantes de Atributos Min de BPdePedidoEnviado */ 
	...
	/* Atributos de BPdePedidoEnviado */ 
	private $id;
	private $descripcion;
	...
	public static function hidratarObjeto($fila){            
            $o = new PdePedidoEnviado();
            $o->setId($fila[PdePedidoEnviado::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BPdePedidoEnviado */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('pde_pedido_enviado_seq'), 
                '".$this->getDescripcion()."'
						, ".$this->getPdePedidoId()."
                    ...
                    ) RETURNING currval('pde_pedido_enviado_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function duplicarPdePedidoEnviado(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPdePedidoEnviados($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PdePedidoEnviado::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $pdepedidoenviado = new PdePedidoEnviado();
                    $pdepedidoenviado->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $pdepedidoenviado->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$pdepedidoenviado->setPdePedidoId($fila[self::GEN_ATTR_MIN_PDE_PEDIDO_ID]);
			$pdepedidoenviado->setPrvProveedorId($fila[self::GEN_ATTR_MIN_PRV_PROVEEDOR_ID]);
			$pdepedidoenviado->setAsunto($fila[self::GEN_ATTR_MIN_ASUNTO]);
			$pdepedidoenviado->setDestinatario($fila[self::GEN_ATTR_MIN_DESTINATARIO]);
			$pdepedidoenviado->setCuerpo($fila[self::GEN_ATTR_MIN_CUERPO]);
			$pdepedidoenviado->setAdjunto($fila[self::GEN_ATTR_MIN_ADJUNTO]);
			$pdepedidoenviado->setCodigoEnvio($fila[self::GEN_ATTR_MIN_CODIGO_ENVIO]);
			$pdepedidoenviado->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$pdepedidoenviado->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$pdepedidoenviado->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$pdepedidoenviado->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$pdepedidoenviado->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$pdepedidoenviado->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$pdepedidoenviado->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$pdepedidoenviado->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPdePedidoEnviadosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PdePedidoEnviado::getPdePedidoEnviados($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPdePedidoEnviados para listado de Backend */ 	
	static function getPdePedidoEnviadosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PdePedidoEnviado::getPdePedidoEnviados($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPdePedidoEnviadosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PdePedidoEnviado::getPdePedidoEnviados($paginador, $criterio);
	...
	/* Método getPdePedidoEnviados filtrado para select */ 	
	static function getPdePedidoEnviadosCmbF($paginador = null, $criterio = null){
            $col = PdePedidoEnviado::getPdePedidoEnviados($paginador, $criterio);
	...
	/* Método getPdePedidoEnviados filtrado por una coleccion de objetos foraneos de PdePedido */ 	
	static function getPdePedidoEnviadosXPdePedidos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdePedidoEnviado::GEN_ATTR_PDE_PEDIDO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdePedidoEnviado::GEN_TABLA);
            $c->addOrden(PdePedidoEnviado::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = PdePedidoEnviado::getPdePedidoEnviados($p, $c);
            foreach($os as $o){
	...
	/* Método getPdePedidoEnviados filtrado por una coleccion de objetos foraneos de PrvProveedor */ 	
	static function getPdePedidoEnviadosXPrvProveedors($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdePedidoEnviado::GEN_ATTR_PRV_PROVEEDOR_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdePedidoEnviado::GEN_TABLA);
            $c->addOrden(PdePedidoEnviado::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'pde_pedido_enviado_adm.php';
            $url_gestion = 'pde_pedido_enviado_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM pde_pedido_enviado'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'pde_pedido_enviado';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPdePedidoEnviados($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPdePedidoEnviados(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'pde_pedido_enviado_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(PdePedidoEnviado::GEN_TABLA);
                $c->addOrden(PdePedidoEnviado::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $pde_pedido_enviados = PdePedidoEnviado::getPdePedidoEnviados(null, $c);

                $arr = array();
                foreach($pde_pedido_enviados as $pde_pedido_enviado){
                    $descripcion = $pde_pedido_enviado->getDescripcion();
