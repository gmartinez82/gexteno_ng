...
class BVtaOrdenVentaImporte
{ 
	
	const SES_PAGINACION = 'adm_vtaordenventaimporte_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_vtaordenventaimporte_paginas_registros';
	const SES_CRITERIOS = 'adm_vtaordenventaimporte_criterios';
	
	const GEN_CLASE = 'VtaOrdenVentaImporte';
	const GEN_TABLA = 'vta_orden_venta_importe';
	...
	/* Constantes de Atributos de BVtaOrdenVentaImporte */ 
	const GEN_ATTR_ID = 'vta_orden_venta_importe.id';
	const GEN_ATTR_DESCRIPCION = 'vta_orden_venta_importe.descripcion';
	const GEN_ATTR_VTA_ORDEN_VENTA_ID = 'vta_orden_venta_importe.vta_orden_venta_id';
	const GEN_ATTR_IMPORTE_SUBTOTAL = 'vta_orden_venta_importe.importe_subtotal';
	const GEN_ATTR_IMPORTE_IVA = 'vta_orden_venta_importe.importe_iva';
	const GEN_ATTR_IMPORTE_TRIBUTO = 'vta_orden_venta_importe.importe_tributo';
	const GEN_ATTR_IMPORTE_TOTAL = 'vta_orden_venta_importe.importe_total';
	const GEN_ATTR_IMPORTE_SALDADO = 'vta_orden_venta_importe.importe_saldado';
	const GEN_ATTR_IMPORTE_SALDO = 'vta_orden_venta_importe.importe_saldo';
	const GEN_ATTR_CODIGO = 'vta_orden_venta_importe.codigo';
	const GEN_ATTR_OBSERVACION = 'vta_orden_venta_importe.observacion';
	const GEN_ATTR_ORDEN = 'vta_orden_venta_importe.orden';
	const GEN_ATTR_ESTADO = 'vta_orden_venta_importe.estado';
	const GEN_ATTR_CREADO = 'vta_orden_venta_importe.creado';
	const GEN_ATTR_CREADO_POR = 'vta_orden_venta_importe.creado_por';
	const GEN_ATTR_MODIFICADO = 'vta_orden_venta_importe.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'vta_orden_venta_importe.modificado_por';

	/* Constantes de Atributos Min de BVtaOrdenVentaImporte */ 
	...
	/* Atributos de BVtaOrdenVentaImporte */ 
	private $id;
	private $descripcion;
	...
	/* Getters de BVtaOrdenVentaImporte */ 
	...
	/* Setters de BVtaOrdenVentaImporte */ 	
	...
	public static function hidratarObjeto($fila){            
            $o = new VtaOrdenVentaImporte();
            $o->setId($fila[VtaOrdenVentaImporte::GEN_ATTR_MIN_ID], false);
	...
	/* Control de BVtaOrdenVentaImporte */ 	
	...
	/* Cambia el estado de BVtaOrdenVentaImporte */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('vta_orden_venta_importe_seq'), 
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('vta_orden_venta_importe_seq')";
                ...
                $sql = "
				 ...
				 ".VtaOrdenVentaImporte::GEN_TABLA."
				 SET 
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	/* Delete de BVtaOrdenVentaImporte */ 	
	...
	public function duplicarVtaOrdenVentaImporte(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getVtaOrdenVentaImportes($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = VtaOrdenVentaImporte::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $vtaordenventaimporte = new VtaOrdenVentaImporte();
                    $vtaordenventaimporte->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $vtaordenventaimporte->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$vtaordenventaimporte->setVtaOrdenVentaId($fila[self::GEN_ATTR_MIN_VTA_ORDEN_VENTA_ID]);
			$vtaordenventaimporte->setImporteSubtotal($fila[self::GEN_ATTR_MIN_IMPORTE_SUBTOTAL]);
			$vtaordenventaimporte->setImporteIva($fila[self::GEN_ATTR_MIN_IMPORTE_IVA]);
			$vtaordenventaimporte->setImporteTributo($fila[self::GEN_ATTR_MIN_IMPORTE_TRIBUTO]);
			$vtaordenventaimporte->setImporteTotal($fila[self::GEN_ATTR_MIN_IMPORTE_TOTAL]);
			$vtaordenventaimporte->setImporteSaldado($fila[self::GEN_ATTR_MIN_IMPORTE_SALDADO]);
			$vtaordenventaimporte->setImporteSaldo($fila[self::GEN_ATTR_MIN_IMPORTE_SALDO]);
			$vtaordenventaimporte->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$vtaordenventaimporte->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$vtaordenventaimporte->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$vtaordenventaimporte->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$vtaordenventaimporte->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$vtaordenventaimporte->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$vtaordenventaimporte->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$vtaordenventaimporte->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getVtaOrdenVentaImportesHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return VtaOrdenVentaImporte::getVtaOrdenVentaImportes($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getVtaOrdenVentaImportes para listado de Backend */ 	
	static function getVtaOrdenVentaImportesDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return VtaOrdenVentaImporte::getVtaOrdenVentaImportes($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getVtaOrdenVentaImportesCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = VtaOrdenVentaImporte::getVtaOrdenVentaImportes($paginador, $criterio);
	...
	/* Método getVtaOrdenVentaImportes filtrado para select */ 	
	static function getVtaOrdenVentaImportesCmbF($paginador = null, $criterio = null){
            $col = VtaOrdenVentaImporte::getVtaOrdenVentaImportes($paginador, $criterio);
	...
	/* Método getVtaOrdenVentaImportes filtrado por una coleccion de objetos foraneos de VtaOrdenVenta */ 	
	static function getVtaOrdenVentaImportesXVtaOrdenVentas($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(VtaOrdenVentaImporte::GEN_ATTR_VTA_ORDEN_VENTA_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(VtaOrdenVentaImporte::GEN_TABLA);
            $c->addOrden(VtaOrdenVentaImporte::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = VtaOrdenVentaImporte::getVtaOrdenVentaImportes($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'vta_orden_venta_importe_adm.php';
            $url_gestion = 'vta_orden_venta_importe_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM vta_orden_venta_importe'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'vta_orden_venta_importe';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getVtaOrdenVentaImportes($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getVtaOrdenVentaImportes(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'vta_orden_venta_importe_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(VtaOrdenVentaImporte::GEN_TABLA);
                $c->addOrden(VtaOrdenVentaImporte::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $vta_orden_venta_importes = VtaOrdenVentaImporte::getVtaOrdenVentaImportes(null, $c);

                $arr = array();
                foreach($vta_orden_venta_importes as $vta_orden_venta_importe){
                    $descripcion = $vta_orden_venta_importe->getDescripcion();
