...
class BVtaAjusteDebeImporte
{ 
	
	const SES_PAGINACION = 'adm_vtaajustedebeimporte_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_vtaajustedebeimporte_paginas_registros';
	const SES_CRITERIOS = 'adm_vtaajustedebeimporte_criterios';
	
	const GEN_CLASE = 'VtaAjusteDebeImporte';
	const GEN_TABLA = 'vta_ajuste_debe_importe';
	...
	/* Constantes de Atributos de BVtaAjusteDebeImporte */ 
	const GEN_ATTR_ID = 'vta_ajuste_debe_importe.id';
	const GEN_ATTR_DESCRIPCION = 'vta_ajuste_debe_importe.descripcion';
	const GEN_ATTR_VTA_AJUSTE_DEBE_ID = 'vta_ajuste_debe_importe.vta_ajuste_debe_id';
	const GEN_ATTR_IMPORTE_SUBTOTAL = 'vta_ajuste_debe_importe.importe_subtotal';
	const GEN_ATTR_IMPORTE_IVA = 'vta_ajuste_debe_importe.importe_iva';
	const GEN_ATTR_IMPORTE_TRIBUTO = 'vta_ajuste_debe_importe.importe_tributo';
	const GEN_ATTR_IMPORTE_TOTAL = 'vta_ajuste_debe_importe.importe_total';
	const GEN_ATTR_IMPORTE_SALDADO = 'vta_ajuste_debe_importe.importe_saldado';
	const GEN_ATTR_IMPORTE_SALDO = 'vta_ajuste_debe_importe.importe_saldo';
	const GEN_ATTR_IMPORTE_TOTAL_REAL = 'vta_ajuste_debe_importe.importe_total_real';
	const GEN_ATTR_IMPORTE_COSTO = 'vta_ajuste_debe_importe.importe_costo';
	const GEN_ATTR_IMPORTE_COSTO_REAL = 'vta_ajuste_debe_importe.importe_costo_real';
	const GEN_ATTR_IMPORTE_NC_ANULACION_AFECTADO = 'vta_ajuste_debe_importe.importe_nc_anulacion_afectado';
	const GEN_ATTR_IMPORTE_NC_DESCUENTO_AFECTADO = 'vta_ajuste_debe_importe.importe_nc_descuento_afectado';
	const GEN_ATTR_IMPORTE_RBO_AFECTADO = 'vta_ajuste_debe_importe.importe_rbo_afectado';
	const GEN_ATTR_CODIGO = 'vta_ajuste_debe_importe.codigo';
	const GEN_ATTR_OBSERVACION = 'vta_ajuste_debe_importe.observacion';
	const GEN_ATTR_ORDEN = 'vta_ajuste_debe_importe.orden';
	const GEN_ATTR_ESTADO = 'vta_ajuste_debe_importe.estado';
	const GEN_ATTR_CREADO = 'vta_ajuste_debe_importe.creado';
	const GEN_ATTR_CREADO_POR = 'vta_ajuste_debe_importe.creado_por';
	const GEN_ATTR_MODIFICADO = 'vta_ajuste_debe_importe.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'vta_ajuste_debe_importe.modificado_por';

	/* Constantes de Atributos Min de BVtaAjusteDebeImporte */ 
	...
	/* Atributos de BVtaAjusteDebeImporte */ 
	private $id;
	private $descripcion;
	...
	/* Getters de BVtaAjusteDebeImporte */ 
	...
	/* Setters de BVtaAjusteDebeImporte */ 	
	...
	public static function hidratarObjeto($fila){            
            $o = new VtaAjusteDebeImporte();
            $o->setId($fila[VtaAjusteDebeImporte::GEN_ATTR_MIN_ID], false);
	...
	/* Control de BVtaAjusteDebeImporte */ 	
	...
	/* Cambia el estado de BVtaAjusteDebeImporte */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('vta_ajuste_debe_importe_seq'), 
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('vta_ajuste_debe_importe_seq')";
                ...
                $sql = "
				 ...
				 ".VtaAjusteDebeImporte::GEN_TABLA."
				 SET 
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	/* Delete de BVtaAjusteDebeImporte */ 	
	...
	public function duplicarVtaAjusteDebeImporte(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getVtaAjusteDebeImportes($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = VtaAjusteDebeImporte::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $vtaajustedebeimporte = new VtaAjusteDebeImporte();
                    $vtaajustedebeimporte->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $vtaajustedebeimporte->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$vtaajustedebeimporte->setVtaAjusteDebeId($fila[self::GEN_ATTR_MIN_VTA_AJUSTE_DEBE_ID]);
			$vtaajustedebeimporte->setImporteSubtotal($fila[self::GEN_ATTR_MIN_IMPORTE_SUBTOTAL]);
			$vtaajustedebeimporte->setImporteIva($fila[self::GEN_ATTR_MIN_IMPORTE_IVA]);
			$vtaajustedebeimporte->setImporteTributo($fila[self::GEN_ATTR_MIN_IMPORTE_TRIBUTO]);
			$vtaajustedebeimporte->setImporteTotal($fila[self::GEN_ATTR_MIN_IMPORTE_TOTAL]);
			$vtaajustedebeimporte->setImporteSaldado($fila[self::GEN_ATTR_MIN_IMPORTE_SALDADO]);
			$vtaajustedebeimporte->setImporteSaldo($fila[self::GEN_ATTR_MIN_IMPORTE_SALDO]);
			$vtaajustedebeimporte->setImporteTotalReal($fila[self::GEN_ATTR_MIN_IMPORTE_TOTAL_REAL]);
			$vtaajustedebeimporte->setImporteCosto($fila[self::GEN_ATTR_MIN_IMPORTE_COSTO]);
			$vtaajustedebeimporte->setImporteCostoReal($fila[self::GEN_ATTR_MIN_IMPORTE_COSTO_REAL]);
			$vtaajustedebeimporte->setImporteNcAnulacionAfectado($fila[self::GEN_ATTR_MIN_IMPORTE_NC_ANULACION_AFECTADO]);
			$vtaajustedebeimporte->setImporteNcDescuentoAfectado($fila[self::GEN_ATTR_MIN_IMPORTE_NC_DESCUENTO_AFECTADO]);
			$vtaajustedebeimporte->setImporteRboAfectado($fila[self::GEN_ATTR_MIN_IMPORTE_RBO_AFECTADO]);
			$vtaajustedebeimporte->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$vtaajustedebeimporte->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$vtaajustedebeimporte->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$vtaajustedebeimporte->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$vtaajustedebeimporte->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$vtaajustedebeimporte->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$vtaajustedebeimporte->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$vtaajustedebeimporte->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getVtaAjusteDebeImportesHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return VtaAjusteDebeImporte::getVtaAjusteDebeImportes($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getVtaAjusteDebeImportes para listado de Backend */ 	
	static function getVtaAjusteDebeImportesDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return VtaAjusteDebeImporte::getVtaAjusteDebeImportes($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getVtaAjusteDebeImportesCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = VtaAjusteDebeImporte::getVtaAjusteDebeImportes($paginador, $criterio);
	...
	/* Método getVtaAjusteDebeImportes filtrado para select */ 	
	static function getVtaAjusteDebeImportesCmbF($paginador = null, $criterio = null){
            $col = VtaAjusteDebeImporte::getVtaAjusteDebeImportes($paginador, $criterio);
	...
	/* Método getVtaAjusteDebeImportes filtrado por una coleccion de objetos foraneos de VtaAjusteDebe */ 	
	static function getVtaAjusteDebeImportesXVtaAjusteDebes($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(VtaAjusteDebeImporte::GEN_ATTR_VTA_AJUSTE_DEBE_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(VtaAjusteDebeImporte::GEN_TABLA);
            $c->addOrden(VtaAjusteDebeImporte::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = VtaAjusteDebeImporte::getVtaAjusteDebeImportes($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'vta_ajuste_debe_importe_adm.php';
            $url_gestion = 'vta_ajuste_debe_importe_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM vta_ajuste_debe_importe'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'vta_ajuste_debe_importe';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getVtaAjusteDebeImportes($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getVtaAjusteDebeImportes(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'vta_ajuste_debe_importe_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(VtaAjusteDebeImporte::GEN_TABLA);
                $c->addOrden(VtaAjusteDebeImporte::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $vta_ajuste_debe_importes = VtaAjusteDebeImporte::getVtaAjusteDebeImportes(null, $c);

                $arr = array();
                foreach($vta_ajuste_debe_importes as $vta_ajuste_debe_importe){
                    $descripcion = $vta_ajuste_debe_importe->getDescripcion();
