...
class BPrvConvenioDescuento
{ 
	
	const SES_PAGINACION = 'adm_prvconveniodescuento_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_prvconveniodescuento_paginas_registros';
	const SES_CRITERIOS = 'adm_prvconveniodescuento_criterios';
	
	const GEN_CLASE = 'PrvConvenioDescuento';
	const GEN_TABLA = 'prv_convenio_descuento';
	...
	/* Constantes de Atributos de BPrvConvenioDescuento */ 
	const GEN_ATTR_ID = 'prv_convenio_descuento.id';
	const GEN_ATTR_DESCRIPCION = 'prv_convenio_descuento.descripcion';
	const GEN_ATTR_PRV_PROVEEDOR_ID = 'prv_convenio_descuento.prv_proveedor_id';
	const GEN_ATTR_CODIGO = 'prv_convenio_descuento.codigo';
	const GEN_ATTR_PORCENTAJE_DESCUENTO = 'prv_convenio_descuento.porcentaje_descuento';
	const GEN_ATTR_OBSERVACION = 'prv_convenio_descuento.observacion';
	const GEN_ATTR_ORDEN = 'prv_convenio_descuento.orden';
	const GEN_ATTR_ESTADO = 'prv_convenio_descuento.estado';
	const GEN_ATTR_CREADO = 'prv_convenio_descuento.creado';
	const GEN_ATTR_CREADO_POR = 'prv_convenio_descuento.creado_por';
	const GEN_ATTR_MODIFICADO = 'prv_convenio_descuento.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'prv_convenio_descuento.modificado_por';

	/* Constantes de Atributos Min de BPrvConvenioDescuento */ 
	...
	/* Atributos de BPrvConvenioDescuento */ 
	private $id;
	private $descripcion;
	...
	/* Getters de BPrvConvenioDescuento */ 
	...
	/* Setters de BPrvConvenioDescuento */ 	
	...
	public static function hidratarObjeto($fila){            
            ...
            $o->setId($fila[PrvConvenioDescuento::GEN_ATTR_MIN_ID], false);
	...
	/* Control de BPrvConvenioDescuento */ 	
	...
	/* Cambia el estado de BPrvConvenioDescuento */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('prv_convenio_descuento_seq'), 
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('prv_convenio_descuento_seq')";
                ...
                $sql = "
				 ...
				 ".PrvConvenioDescuento::GEN_TABLA."
				 SET 
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	/* Delete de BPrvConvenioDescuento */ 	
	...
	public function duplicarPrvConvenioDescuento(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPrvConvenioDescuentos($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PrvConvenioDescuento::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $prvconveniodescuento = new PrvConvenioDescuento();
                    $prvconveniodescuento->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $prvconveniodescuento->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$prvconveniodescuento->setPrvProveedorId($fila[self::GEN_ATTR_MIN_PRV_PROVEEDOR_ID]);
			$prvconveniodescuento->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$prvconveniodescuento->setPorcentajeDescuento($fila[self::GEN_ATTR_MIN_PORCENTAJE_DESCUENTO]);
			$prvconveniodescuento->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$prvconveniodescuento->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$prvconveniodescuento->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$prvconveniodescuento->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$prvconveniodescuento->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$prvconveniodescuento->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$prvconveniodescuento->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPrvConvenioDescuentosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PrvConvenioDescuento::getPrvConvenioDescuentos($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPrvConvenioDescuentos para listado de Backend */ 	
	static function getPrvConvenioDescuentosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PrvConvenioDescuento::getPrvConvenioDescuentos($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPrvConvenioDescuentosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PrvConvenioDescuento::getPrvConvenioDescuentos($paginador, $criterio);
	...
	/* Método getPrvConvenioDescuentos filtrado para select */ 	
	static function getPrvConvenioDescuentosCmbF($paginador = null, $criterio = null){
            $col = PrvConvenioDescuento::getPrvConvenioDescuentos($paginador, $criterio);
	...
	/* Método getPrvConvenioDescuentos filtrado por una coleccion de objetos foraneos de PrvProveedor */ 	
	static function getPrvConvenioDescuentosXPrvProveedors($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PrvConvenioDescuento::GEN_ATTR_PRV_PROVEEDOR_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PrvConvenioDescuento::GEN_TABLA);
            $c->addOrden(PrvConvenioDescuento::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = PrvConvenioDescuento::getPrvConvenioDescuentos($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'prv_convenio_descuento_adm.php';
            $url_gestion = 'prv_convenio_descuento_gestion.php';
	...
	public function getPrvImportacions($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            ...
            $criterio->add(PrvImportacion::GEN_ATTR_PRV_CONVENIO_DESCUENTO_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
	...
	public function getPrvImportacionsCmb(){
            $c = new Criterio();
            $c->add(PrvImportacion::GEN_ATTR_PRV_CONVENIO_DESCUENTO_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(PrvImportacion::GEN_TABLA);
            $c->setCriterios();
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM prv_convenio_descuento'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'prv_convenio_descuento';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPrvConvenioDescuentos($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPrvConvenioDescuentos(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'prv_convenio_descuento_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(PrvConvenioDescuento::GEN_TABLA);
                $c->addOrden(PrvConvenioDescuento::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $prv_convenio_descuentos = PrvConvenioDescuento::getPrvConvenioDescuentos(null, $c);
