...
{ 
	...
	const SES_PAGINACION = 'adm_perestado_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_perestado_paginas_registros';
	const SES_CRITERIOS = 'adm_perestado_criterios';
	
	const GEN_CLASE = 'PerEstado';
	const GEN_TABLA = 'per_estado';
	...
	/* Constantes de Atributos de BPerEstado */ 
	const GEN_ATTR_ID = 'per_estado.id';
	const GEN_ATTR_PER_PERSONA_ID = 'per_estado.per_persona_id';
	const GEN_ATTR_PER_TIPO_ESTADO_ID = 'per_estado.per_tipo_estado_id';
	const GEN_ATTR_INICIAL = 'per_estado.inicial';
	const GEN_ATTR_ACTUAL = 'per_estado.actual';
	const GEN_ATTR_CREADO = 'per_estado.creado';
	const GEN_ATTR_CREADO_POR = 'per_estado.creado_por';
	const GEN_ATTR_MODIFICADO = 'per_estado.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'per_estado.modificado_por';

	/* Constantes de Atributos Min de BPerEstado */ 
	...
	public static function hidratarObjeto($fila){            
            $o = new PerEstado();
            $o->setId($fila[PerEstado::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BPerEstado */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                ".$this->getPerPersonaId()."
                    ...
                    ) RETURNING currval('per_estado_seq')";
...
".self::GEN_ATTR_MIN_PER_PERSONA_ID." = ".$this->getPerPersonaId()."
	...
	public function duplicarPerEstado(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPerEstados($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PerEstado::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    ...
                    $perestado->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $perestado->setPerPersonaId($fila[self::GEN_ATTR_MIN_PER_PERSONA_ID]);
			$perestado->setPerTipoEstadoId($fila[self::GEN_ATTR_MIN_PER_TIPO_ESTADO_ID]);
			$perestado->setInicial($fila[self::GEN_ATTR_MIN_INICIAL]);
			$perestado->setActual($fila[self::GEN_ATTR_MIN_ACTUAL]);
			$perestado->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$perestado->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$perestado->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$perestado->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPerEstadosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PerEstado::getPerEstados($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPerEstados para listado de Backend */ 	
	static function getPerEstadosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PerEstado::getPerEstados($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPerEstadosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PerEstado::getPerEstados($paginador, $criterio);
	...
	/* Método getPerEstados filtrado para select */ 	
	static function getPerEstadosCmbF($paginador = null, $criterio = null){
            $col = PerEstado::getPerEstados($paginador, $criterio);
	...
	/* Método getPerEstados filtrado por una coleccion de objetos foraneos de PerPersona */ 	
	static function getPerEstadosXPerPersonas($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PerEstado::GEN_ATTR_PER_PERSONA_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PerEstado::GEN_TABLA);
            $c->addOrden(PerEstado::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = PerEstado::getPerEstados($p, $c);
            foreach($os as $o){
	...
	/* Método getPerEstados filtrado por una coleccion de objetos foraneos de PerTipoEstado */ 	
	static function getPerEstadosXPerTipoEstados($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PerEstado::GEN_ATTR_PER_TIPO_ESTADO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PerEstado::GEN_TABLA);
            $c->addOrden(PerEstado::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'per_estado_adm.php';
            $url_gestion = 'per_estado_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM per_estado'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'per_estado';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPerEstados($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPerEstados(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'per_estado_adm');
            }else{
