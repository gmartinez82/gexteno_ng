...
class BVtaReciboConcepto
{ 
	
	const SES_PAGINACION = 'adm_vtareciboconcepto_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_vtareciboconcepto_paginas_registros';
	const SES_CRITERIOS = 'adm_vtareciboconcepto_criterios';
	
	const GEN_CLASE = 'VtaReciboConcepto';
	const GEN_TABLA = 'vta_recibo_concepto';
	...
	/* Constantes de Atributos de BVtaReciboConcepto */ 
	const GEN_ATTR_ID = 'vta_recibo_concepto.id';
	const GEN_ATTR_DESCRIPCION = 'vta_recibo_concepto.descripcion';
	const GEN_ATTR_CODIGO = 'vta_recibo_concepto.codigo';
	const GEN_ATTR_ES_RETENCION = 'vta_recibo_concepto.es_retencion';
	const GEN_ATTR_ES_RETENCION_IIBB = 'vta_recibo_concepto.es_retencion_iibb';
	const GEN_ATTR_CNTB_CUENTA_ID = 'vta_recibo_concepto.cntb_cuenta_id';
	const GEN_ATTR_OBSERVACION = 'vta_recibo_concepto.observacion';
	const GEN_ATTR_ORDEN = 'vta_recibo_concepto.orden';
	const GEN_ATTR_ESTADO = 'vta_recibo_concepto.estado';
	const GEN_ATTR_CREADO = 'vta_recibo_concepto.creado';
	const GEN_ATTR_CREADO_POR = 'vta_recibo_concepto.creado_por';
	const GEN_ATTR_MODIFICADO = 'vta_recibo_concepto.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'vta_recibo_concepto.modificado_por';

	/* Constantes de Atributos Min de BVtaReciboConcepto */ 
	...
	/* Atributos de BVtaReciboConcepto */ 
	private $id;
	private $descripcion;
	private $codigo;
	...
	public static function hidratarObjeto($fila){            
            ...
            $o->setId($fila[VtaReciboConcepto::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BVtaReciboConcepto */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('vta_recibo_concepto_seq'), 
                '".$this->getDescripcion()."'
						, '".$this->getCodigo()."'
						, ".$this->getEsRetencion()."
                    ...
                    ) RETURNING currval('vta_recibo_concepto_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function duplicarVtaReciboConcepto(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getVtaReciboConceptos($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = VtaReciboConcepto::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $vtareciboconcepto = new VtaReciboConcepto();
                    $vtareciboconcepto->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $vtareciboconcepto->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$vtareciboconcepto->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$vtareciboconcepto->setEsRetencion($fila[self::GEN_ATTR_MIN_ES_RETENCION]);
			$vtareciboconcepto->setEsRetencionIibb($fila[self::GEN_ATTR_MIN_ES_RETENCION_IIBB]);
			$vtareciboconcepto->setCntbCuentaId($fila[self::GEN_ATTR_MIN_CNTB_CUENTA_ID]);
			$vtareciboconcepto->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$vtareciboconcepto->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$vtareciboconcepto->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$vtareciboconcepto->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$vtareciboconcepto->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$vtareciboconcepto->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$vtareciboconcepto->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getVtaReciboConceptosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return VtaReciboConcepto::getVtaReciboConceptos($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getVtaReciboConceptos para listado de Backend */ 	
	static function getVtaReciboConceptosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return VtaReciboConcepto::getVtaReciboConceptos($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getVtaReciboConceptosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = VtaReciboConcepto::getVtaReciboConceptos($paginador, $criterio);
	...
	/* Método getVtaReciboConceptos filtrado para select */ 	
	static function getVtaReciboConceptosCmbF($paginador = null, $criterio = null){
            $col = VtaReciboConcepto::getVtaReciboConceptos($paginador, $criterio);
	...
	/* Método getVtaReciboConceptos filtrado por una coleccion de objetos foraneos de CntbCuenta */ 	
	static function getVtaReciboConceptosXCntbCuentas($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(VtaReciboConcepto::GEN_ATTR_CNTB_CUENTA_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(VtaReciboConcepto::GEN_TABLA);
            $c->addOrden(VtaReciboConcepto::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = VtaReciboConcepto::getVtaReciboConceptos($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'vta_recibo_concepto_adm.php';
            $url_gestion = 'vta_recibo_concepto_gestion.php';
	...
	public function getVtaReciboItems($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            ...
            $criterio->add(VtaReciboItem::GEN_ATTR_VTA_RECIBO_CONCEPTO_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
	...
	public function getVtaReciboItemsCmb(){
            $c = new Criterio();
            $c->add(VtaReciboItem::GEN_ATTR_VTA_RECIBO_CONCEPTO_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(VtaReciboItem::GEN_TABLA);
            $c->setCriterios();
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM vta_recibo_concepto'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'vta_recibo_concepto';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getVtaReciboConceptos($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getVtaReciboConceptos(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'vta_recibo_concepto_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(VtaReciboConcepto::GEN_TABLA);
                $c->addOrden(VtaReciboConcepto::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $vta_recibo_conceptos = VtaReciboConcepto::getVtaReciboConceptos(null, $c);

                $arr = array();
                foreach($vta_recibo_conceptos as $vta_recibo_concepto){
                    $descripcion = $vta_recibo_concepto->getDescripcion();
