...
class BVtaReciboImporte
{ 
	
	const SES_PAGINACION = 'adm_vtareciboimporte_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_vtareciboimporte_paginas_registros';
	const SES_CRITERIOS = 'adm_vtareciboimporte_criterios';
	
	const GEN_CLASE = 'VtaReciboImporte';
	const GEN_TABLA = 'vta_recibo_importe';
	...
	/* Constantes de Atributos de BVtaReciboImporte */ 
	const GEN_ATTR_ID = 'vta_recibo_importe.id';
	const GEN_ATTR_DESCRIPCION = 'vta_recibo_importe.descripcion';
	const GEN_ATTR_VTA_RECIBO_ID = 'vta_recibo_importe.vta_recibo_id';
	const GEN_ATTR_IMPORTE_SUBTOTAL = 'vta_recibo_importe.importe_subtotal';
	const GEN_ATTR_IMPORTE_IVA = 'vta_recibo_importe.importe_iva';
	const GEN_ATTR_IMPORTE_TRIBUTO = 'vta_recibo_importe.importe_tributo';
	const GEN_ATTR_IMPORTE_TOTAL = 'vta_recibo_importe.importe_total';
	const GEN_ATTR_IMPORTE_SALDADO = 'vta_recibo_importe.importe_saldado';
	const GEN_ATTR_IMPORTE_SALDO = 'vta_recibo_importe.importe_saldo';
	const GEN_ATTR_CODIGO = 'vta_recibo_importe.codigo';
	const GEN_ATTR_OBSERVACION = 'vta_recibo_importe.observacion';
	const GEN_ATTR_ORDEN = 'vta_recibo_importe.orden';
	const GEN_ATTR_ESTADO = 'vta_recibo_importe.estado';
	const GEN_ATTR_CREADO = 'vta_recibo_importe.creado';
	const GEN_ATTR_CREADO_POR = 'vta_recibo_importe.creado_por';
	const GEN_ATTR_MODIFICADO = 'vta_recibo_importe.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'vta_recibo_importe.modificado_por';

	/* Constantes de Atributos Min de BVtaReciboImporte */ 
	...
	/* Atributos de BVtaReciboImporte */ 
	private $id;
	private $descripcion;
	...
	public static function hidratarObjeto($fila){            
            $o = new VtaReciboImporte();
            $o->setId($fila[VtaReciboImporte::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BVtaReciboImporte */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('vta_recibo_importe_seq'), 
                '".$this->getDescripcion()."'
						, ".$this->getVtaReciboId()."
                    ...
                    ) RETURNING currval('vta_recibo_importe_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function duplicarVtaReciboImporte(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getVtaReciboImportes($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = VtaReciboImporte::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $vtareciboimporte = new VtaReciboImporte();
                    $vtareciboimporte->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $vtareciboimporte->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$vtareciboimporte->setVtaReciboId($fila[self::GEN_ATTR_MIN_VTA_RECIBO_ID]);
			$vtareciboimporte->setImporteSubtotal($fila[self::GEN_ATTR_MIN_IMPORTE_SUBTOTAL]);
			$vtareciboimporte->setImporteIva($fila[self::GEN_ATTR_MIN_IMPORTE_IVA]);
			$vtareciboimporte->setImporteTributo($fila[self::GEN_ATTR_MIN_IMPORTE_TRIBUTO]);
			$vtareciboimporte->setImporteTotal($fila[self::GEN_ATTR_MIN_IMPORTE_TOTAL]);
			$vtareciboimporte->setImporteSaldado($fila[self::GEN_ATTR_MIN_IMPORTE_SALDADO]);
			$vtareciboimporte->setImporteSaldo($fila[self::GEN_ATTR_MIN_IMPORTE_SALDO]);
			$vtareciboimporte->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$vtareciboimporte->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$vtareciboimporte->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$vtareciboimporte->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$vtareciboimporte->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$vtareciboimporte->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$vtareciboimporte->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$vtareciboimporte->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getVtaReciboImportesHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return VtaReciboImporte::getVtaReciboImportes($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getVtaReciboImportes para listado de Backend */ 	
	static function getVtaReciboImportesDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return VtaReciboImporte::getVtaReciboImportes($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getVtaReciboImportesCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = VtaReciboImporte::getVtaReciboImportes($paginador, $criterio);
	...
	/* Método getVtaReciboImportes filtrado para select */ 	
	static function getVtaReciboImportesCmbF($paginador = null, $criterio = null){
            $col = VtaReciboImporte::getVtaReciboImportes($paginador, $criterio);
	...
	/* Método getVtaReciboImportes filtrado por una coleccion de objetos foraneos de VtaRecibo */ 	
	static function getVtaReciboImportesXVtaRecibos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(VtaReciboImporte::GEN_ATTR_VTA_RECIBO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(VtaReciboImporte::GEN_TABLA);
            $c->addOrden(VtaReciboImporte::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = VtaReciboImporte::getVtaReciboImportes($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'vta_recibo_importe_adm.php';
            $url_gestion = 'vta_recibo_importe_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM vta_recibo_importe'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'vta_recibo_importe';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getVtaReciboImportes($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getVtaReciboImportes(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'vta_recibo_importe_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(VtaReciboImporte::GEN_TABLA);
                $c->addOrden(VtaReciboImporte::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $vta_recibo_importes = VtaReciboImporte::getVtaReciboImportes(null, $c);

                $arr = array();
                foreach($vta_recibo_importes as $vta_recibo_importe){
                    $descripcion = $vta_recibo_importe->getDescripcion();
