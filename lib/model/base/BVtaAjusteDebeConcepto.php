...
class BVtaAjusteDebeConcepto
{ 
	
	const SES_PAGINACION = 'adm_vtaajustedebeconcepto_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_vtaajustedebeconcepto_paginas_registros';
	const SES_CRITERIOS = 'adm_vtaajustedebeconcepto_criterios';
	
	const GEN_CLASE = 'VtaAjusteDebeConcepto';
	const GEN_TABLA = 'vta_ajuste_debe_concepto';
	...
	/* Constantes de Atributos de BVtaAjusteDebeConcepto */ 
	const GEN_ATTR_ID = 'vta_ajuste_debe_concepto.id';
	const GEN_ATTR_DESCRIPCION = 'vta_ajuste_debe_concepto.descripcion';
	const GEN_ATTR_CODIGO = 'vta_ajuste_debe_concepto.codigo';
	const GEN_ATTR_ES_RETENCION = 'vta_ajuste_debe_concepto.es_retencion';
	const GEN_ATTR_ES_RETENCION_IIBB = 'vta_ajuste_debe_concepto.es_retencion_iibb';
	const GEN_ATTR_CNTB_CUENTA_ID = 'vta_ajuste_debe_concepto.cntb_cuenta_id';
	const GEN_ATTR_OBSERVACION = 'vta_ajuste_debe_concepto.observacion';
	const GEN_ATTR_ORDEN = 'vta_ajuste_debe_concepto.orden';
	const GEN_ATTR_ESTADO = 'vta_ajuste_debe_concepto.estado';
	const GEN_ATTR_CREADO = 'vta_ajuste_debe_concepto.creado';
	const GEN_ATTR_CREADO_POR = 'vta_ajuste_debe_concepto.creado_por';
	const GEN_ATTR_MODIFICADO = 'vta_ajuste_debe_concepto.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'vta_ajuste_debe_concepto.modificado_por';

	/* Constantes de Atributos Min de BVtaAjusteDebeConcepto */ 
	...
	/* Atributos de BVtaAjusteDebeConcepto */ 
	private $id;
	private $descripcion;
	private $codigo;
	...
	/* Getters de BVtaAjusteDebeConcepto */ 
	...
	/* Setters de BVtaAjusteDebeConcepto */ 	
	...
	public static function hidratarObjeto($fila){            
            ...
            $o->setId($fila[VtaAjusteDebeConcepto::GEN_ATTR_MIN_ID], false);
	...
	/* Control de BVtaAjusteDebeConcepto */ 	
	...
	/* Cambia el estado de BVtaAjusteDebeConcepto */ 	
	...
	/* Save de BVtaAjusteDebeConcepto */ 	
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('vta_ajuste_debe_concepto_seq'), 
                '".$this->getDescripcion()."'
						, '".$this->getCodigo()."'
						, ".$this->getEsRetencion()."
                    ...
                    ) RETURNING currval('vta_ajuste_debe_concepto_seq')";
                ...
                $sql = "
				 ...
				 ".VtaAjusteDebeConcepto::GEN_TABLA."
				 SET 
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	/* Delete de BVtaAjusteDebeConcepto */ 	
	...
	public function duplicarVtaAjusteDebeConcepto(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getVtaAjusteDebeConceptos($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = VtaAjusteDebeConcepto::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $vtaajustedebeconcepto = new VtaAjusteDebeConcepto();
                    $vtaajustedebeconcepto->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $vtaajustedebeconcepto->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$vtaajustedebeconcepto->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$vtaajustedebeconcepto->setEsRetencion($fila[self::GEN_ATTR_MIN_ES_RETENCION]);
			$vtaajustedebeconcepto->setEsRetencionIibb($fila[self::GEN_ATTR_MIN_ES_RETENCION_IIBB]);
			$vtaajustedebeconcepto->setCntbCuentaId($fila[self::GEN_ATTR_MIN_CNTB_CUENTA_ID]);
			$vtaajustedebeconcepto->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$vtaajustedebeconcepto->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$vtaajustedebeconcepto->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$vtaajustedebeconcepto->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$vtaajustedebeconcepto->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$vtaajustedebeconcepto->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$vtaajustedebeconcepto->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getVtaAjusteDebeConceptosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return VtaAjusteDebeConcepto::getVtaAjusteDebeConceptos($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getVtaAjusteDebeConceptos para listado de Backend */ 	
	static function getVtaAjusteDebeConceptosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return VtaAjusteDebeConcepto::getVtaAjusteDebeConceptos($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getVtaAjusteDebeConceptosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = VtaAjusteDebeConcepto::getVtaAjusteDebeConceptos($paginador, $criterio);
	...
	/* Método getVtaAjusteDebeConceptos filtrado para select */ 	
	static function getVtaAjusteDebeConceptosCmbF($paginador = null, $criterio = null){
            $col = VtaAjusteDebeConcepto::getVtaAjusteDebeConceptos($paginador, $criterio);
	...
	/* Método getVtaAjusteDebeConceptos filtrado por una coleccion de objetos foraneos de CntbCuenta */ 	
	static function getVtaAjusteDebeConceptosXCntbCuentas($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(VtaAjusteDebeConcepto::GEN_ATTR_CNTB_CUENTA_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(VtaAjusteDebeConcepto::GEN_TABLA);
            $c->addOrden(VtaAjusteDebeConcepto::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = VtaAjusteDebeConcepto::getVtaAjusteDebeConceptos($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'vta_ajuste_debe_concepto_adm.php';
            $url_gestion = 'vta_ajuste_debe_concepto_gestion.php';
	...
	public function getVtaAjusteDebeItems($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            ...
            $criterio->add(VtaAjusteDebeItem::GEN_ATTR_VTA_AJUSTE_DEBE_CONCEPTO_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
	...
	public function getVtaAjusteDebeItemsCmb(){
            $c = new Criterio();
            $c->add(VtaAjusteDebeItem::GEN_ATTR_VTA_AJUSTE_DEBE_CONCEPTO_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(VtaAjusteDebeItem::GEN_TABLA);
            $c->setCriterios();
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM vta_ajuste_debe_concepto'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'vta_ajuste_debe_concepto';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getVtaAjusteDebeConceptos($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getVtaAjusteDebeConceptos(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'vta_ajuste_debe_concepto_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(VtaAjusteDebeConcepto::GEN_TABLA);
                $c->addOrden(VtaAjusteDebeConcepto::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $vta_ajuste_debe_conceptos = VtaAjusteDebeConcepto::getVtaAjusteDebeConceptos(null, $c);

                $arr = array();
                foreach($vta_ajuste_debe_conceptos as $vta_ajuste_debe_concepto){
                    $descripcion = $vta_ajuste_debe_concepto->getDescripcion();
