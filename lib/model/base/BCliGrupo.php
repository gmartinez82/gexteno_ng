...
{ 
	...
	const SES_PAGINACION = 'adm_cligrupo_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_cligrupo_paginas_registros';
	const SES_CRITERIOS = 'adm_cligrupo_criterios';
	
	const GEN_CLASE = 'CliGrupo';
	const GEN_TABLA = 'cli_grupo';
	...
	/* Constantes de Atributos de BCliGrupo */ 
	const GEN_ATTR_ID = 'cli_grupo.id';
	const GEN_ATTR_DESCRIPCION = 'cli_grupo.descripcion';
	const GEN_ATTR_DOCUMENTO = 'cli_grupo.documento';
	const GEN_ATTR_CODIGO = 'cli_grupo.codigo';
	const GEN_ATTR_OBSERVACION = 'cli_grupo.observacion';
	const GEN_ATTR_ORDEN = 'cli_grupo.orden';
	const GEN_ATTR_ESTADO = 'cli_grupo.estado';
	const GEN_ATTR_CREADO = 'cli_grupo.creado';
	const GEN_ATTR_CREADO_POR = 'cli_grupo.creado_por';
	const GEN_ATTR_MODIFICADO = 'cli_grupo.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'cli_grupo.modificado_por';

	/* Constantes de Atributos Min de BCliGrupo */ 
	...
	public static function hidratarObjeto($fila){            
            $o = new CliGrupo();
            $o->setId($fila[CliGrupo::GEN_ATTR_MIN_ID], false);
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('cli_grupo_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function duplicarCliGrupo(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getCliGrupos($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = CliGrupo::setAplicarFiltrosDeAlcance($criterio);
		...
		$cligrupos = array();
		while($fila = pg_fetch_array($cons->getResultado())){
                    $cligrupo = new CliGrupo();
                    $cligrupo->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $cligrupo->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$cligrupo->setDocumento($fila[self::GEN_ATTR_MIN_DOCUMENTO]);
			$cligrupo->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$cligrupo->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$cligrupo->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$cligrupo->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$cligrupo->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$cligrupo->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$cligrupo->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$cligrupo->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	/* Método getCliGrupos Habilitados */ 	
	static function getCliGruposHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return CliGrupo::getCliGrupos($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getCliGrupos para listado de Backend */ 	
	static function getCliGruposDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return CliGrupo::getCliGrupos($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getCliGruposCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = CliGrupo::getCliGrupos($paginador, $criterio);
	...
	/* Método getCliGrupos filtrado para select */ 	
	static function getCliGruposCmbF($paginador = null, $criterio = null){
            $col = CliGrupo::getCliGrupos($paginador, $criterio);
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'cli_grupo_adm.php';
            $url_gestion = 'cli_grupo_gestion.php';
	...
	public function getCliClientes($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            ...
            $criterio->add(CliCliente::GEN_ATTR_CLI_GRUPO_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
	...
	public function getCliClientesCmb(){
            $c = new Criterio();
            $c->add(CliCliente::GEN_ATTR_CLI_GRUPO_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(CliCliente::GEN_TABLA);
            $c->setCriterios();
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM cli_grupo'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'cli_grupo';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getCliGrupos($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getCliGrupos(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'cli_grupo_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(CliGrupo::GEN_TABLA);
                $c->addOrden(CliGrupo::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $cli_grupos = CliGrupo::getCliGrupos(null, $c);

                $arr = array();
                foreach($cli_grupos as $cli_grupo){
                    $descripcion = $cli_grupo->getDescripcion();
