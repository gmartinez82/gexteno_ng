...
class BPdeFacturaImporte
{ 
	
	const SES_PAGINACION = 'adm_pdefacturaimporte_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_pdefacturaimporte_paginas_registros';
	const SES_CRITERIOS = 'adm_pdefacturaimporte_criterios';
	
	const GEN_CLASE = 'PdeFacturaImporte';
	const GEN_TABLA = 'pde_factura_importe';
	...
	/* Constantes de Atributos de BPdeFacturaImporte */ 
	const GEN_ATTR_ID = 'pde_factura_importe.id';
	const GEN_ATTR_DESCRIPCION = 'pde_factura_importe.descripcion';
	const GEN_ATTR_PDE_FACTURA_ID = 'pde_factura_importe.pde_factura_id';
	const GEN_ATTR_IMPORTE_SUBTOTAL = 'pde_factura_importe.importe_subtotal';
	const GEN_ATTR_IMPORTE_IVA = 'pde_factura_importe.importe_iva';
	const GEN_ATTR_IMPORTE_TRIBUTO = 'pde_factura_importe.importe_tributo';
	const GEN_ATTR_IMPORTE_TOTAL = 'pde_factura_importe.importe_total';
	const GEN_ATTR_IMPORTE_SALDADO = 'pde_factura_importe.importe_saldado';
	const GEN_ATTR_IMPORTE_SALDO = 'pde_factura_importe.importe_saldo';
	const GEN_ATTR_CODIGO = 'pde_factura_importe.codigo';
	const GEN_ATTR_OBSERVACION = 'pde_factura_importe.observacion';
	const GEN_ATTR_ORDEN = 'pde_factura_importe.orden';
	const GEN_ATTR_ESTADO = 'pde_factura_importe.estado';
	const GEN_ATTR_CREADO = 'pde_factura_importe.creado';
	const GEN_ATTR_CREADO_POR = 'pde_factura_importe.creado_por';
	const GEN_ATTR_MODIFICADO = 'pde_factura_importe.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'pde_factura_importe.modificado_por';

	/* Constantes de Atributos Min de BPdeFacturaImporte */ 
	...
	/* Atributos de BPdeFacturaImporte */ 
	private $id;
	private $descripcion;
	...
	public static function hidratarObjeto($fila){            
            $o = new PdeFacturaImporte();
            $o->setId($fila[PdeFacturaImporte::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BPdeFacturaImporte */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('pde_factura_importe_seq'), 
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('pde_factura_importe_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function duplicarPdeFacturaImporte(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPdeFacturaImportes($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PdeFacturaImporte::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $pdefacturaimporte = new PdeFacturaImporte();
                    $pdefacturaimporte->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $pdefacturaimporte->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$pdefacturaimporte->setPdeFacturaId($fila[self::GEN_ATTR_MIN_PDE_FACTURA_ID]);
			$pdefacturaimporte->setImporteSubtotal($fila[self::GEN_ATTR_MIN_IMPORTE_SUBTOTAL]);
			$pdefacturaimporte->setImporteIva($fila[self::GEN_ATTR_MIN_IMPORTE_IVA]);
			$pdefacturaimporte->setImporteTributo($fila[self::GEN_ATTR_MIN_IMPORTE_TRIBUTO]);
			$pdefacturaimporte->setImporteTotal($fila[self::GEN_ATTR_MIN_IMPORTE_TOTAL]);
			$pdefacturaimporte->setImporteSaldado($fila[self::GEN_ATTR_MIN_IMPORTE_SALDADO]);
			$pdefacturaimporte->setImporteSaldo($fila[self::GEN_ATTR_MIN_IMPORTE_SALDO]);
			$pdefacturaimporte->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$pdefacturaimporte->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$pdefacturaimporte->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$pdefacturaimporte->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$pdefacturaimporte->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$pdefacturaimporte->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$pdefacturaimporte->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$pdefacturaimporte->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPdeFacturaImportesHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PdeFacturaImporte::getPdeFacturaImportes($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPdeFacturaImportes para listado de Backend */ 	
	static function getPdeFacturaImportesDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PdeFacturaImporte::getPdeFacturaImportes($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPdeFacturaImportesCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PdeFacturaImporte::getPdeFacturaImportes($paginador, $criterio);
	...
	/* Método getPdeFacturaImportes filtrado para select */ 	
	static function getPdeFacturaImportesCmbF($paginador = null, $criterio = null){
            $col = PdeFacturaImporte::getPdeFacturaImportes($paginador, $criterio);
	...
	/* Método getPdeFacturaImportes filtrado por una coleccion de objetos foraneos de PdeFactura */ 	
	static function getPdeFacturaImportesXPdeFacturas($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdeFacturaImporte::GEN_ATTR_PDE_FACTURA_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdeFacturaImporte::GEN_TABLA);
            $c->addOrden(PdeFacturaImporte::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = PdeFacturaImporte::getPdeFacturaImportes($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'pde_factura_importe_adm.php';
            $url_gestion = 'pde_factura_importe_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM pde_factura_importe'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'pde_factura_importe';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPdeFacturaImportes($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPdeFacturaImportes(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'pde_factura_importe_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(PdeFacturaImporte::GEN_TABLA);
                $c->addOrden(PdeFacturaImporte::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $pde_factura_importes = PdeFacturaImporte::getPdeFacturaImportes(null, $c);

                $arr = array();
                foreach($pde_factura_importes as $pde_factura_importe){
                    $descripcion = $pde_factura_importe->getDescripcion();
