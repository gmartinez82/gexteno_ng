...
class BInsInsumoEnviado
{ 
	
	const SES_PAGINACION = 'adm_insinsumoenviado_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_insinsumoenviado_paginas_registros';
	const SES_CRITERIOS = 'adm_insinsumoenviado_criterios';
	
	const GEN_CLASE = 'InsInsumoEnviado';
	const GEN_TABLA = 'ins_insumo_enviado';
	...
	/* Constantes de Atributos de BInsInsumoEnviado */ 
	const GEN_ATTR_ID = 'ins_insumo_enviado.id';
	const GEN_ATTR_DESCRIPCION = 'ins_insumo_enviado.descripcion';
	const GEN_ATTR_INS_INSUMO_ID = 'ins_insumo_enviado.ins_insumo_id';
	const GEN_ATTR_ASUNTO = 'ins_insumo_enviado.asunto';
	const GEN_ATTR_DESTINATARIO = 'ins_insumo_enviado.destinatario';
	const GEN_ATTR_CUERPO = 'ins_insumo_enviado.cuerpo';
	const GEN_ATTR_ADJUNTO = 'ins_insumo_enviado.adjunto';
	const GEN_ATTR_CODIGO_ENVIO = 'ins_insumo_enviado.codigo_envio';
	const GEN_ATTR_CODIGO = 'ins_insumo_enviado.codigo';
	const GEN_ATTR_OBSERVACION = 'ins_insumo_enviado.observacion';
	const GEN_ATTR_ORDEN = 'ins_insumo_enviado.orden';
	const GEN_ATTR_ESTADO = 'ins_insumo_enviado.estado';
	const GEN_ATTR_CREADO = 'ins_insumo_enviado.creado';
	const GEN_ATTR_CREADO_POR = 'ins_insumo_enviado.creado_por';
	const GEN_ATTR_MODIFICADO = 'ins_insumo_enviado.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'ins_insumo_enviado.modificado_por';

	/* Constantes de Atributos Min de BInsInsumoEnviado */ 
	...
	/* Atributos de BInsInsumoEnviado */ 
	private $id;
	private $descripcion;
	...
	public static function hidratarObjeto($fila){            
            $o = new InsInsumoEnviado();
            $o->setId($fila[InsInsumoEnviado::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BInsInsumoEnviado */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('ins_insumo_enviado_seq'), 
                '".$this->getDescripcion()."'
						, ".$this->getInsInsumoId()."
						, '".$this->getAsunto()."'
                    ...
                    ) RETURNING currval('ins_insumo_enviado_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function duplicarInsInsumoEnviado(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getInsInsumoEnviados($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = InsInsumoEnviado::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $insinsumoenviado = new InsInsumoEnviado();
                    $insinsumoenviado->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $insinsumoenviado->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$insinsumoenviado->setInsInsumoId($fila[self::GEN_ATTR_MIN_INS_INSUMO_ID]);
			$insinsumoenviado->setAsunto($fila[self::GEN_ATTR_MIN_ASUNTO]);
			$insinsumoenviado->setDestinatario($fila[self::GEN_ATTR_MIN_DESTINATARIO]);
			$insinsumoenviado->setCuerpo($fila[self::GEN_ATTR_MIN_CUERPO]);
			$insinsumoenviado->setAdjunto($fila[self::GEN_ATTR_MIN_ADJUNTO]);
			$insinsumoenviado->setCodigoEnvio($fila[self::GEN_ATTR_MIN_CODIGO_ENVIO]);
			$insinsumoenviado->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$insinsumoenviado->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$insinsumoenviado->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$insinsumoenviado->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$insinsumoenviado->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$insinsumoenviado->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$insinsumoenviado->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$insinsumoenviado->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getInsInsumoEnviadosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return InsInsumoEnviado::getInsInsumoEnviados($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getInsInsumoEnviados para listado de Backend */ 	
	static function getInsInsumoEnviadosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return InsInsumoEnviado::getInsInsumoEnviados($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getInsInsumoEnviadosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = InsInsumoEnviado::getInsInsumoEnviados($paginador, $criterio);
	...
	/* Método getInsInsumoEnviados filtrado para select */ 	
	static function getInsInsumoEnviadosCmbF($paginador = null, $criterio = null){
            $col = InsInsumoEnviado::getInsInsumoEnviados($paginador, $criterio);
	...
	/* Método getInsInsumoEnviados filtrado por una coleccion de objetos foraneos de InsInsumo */ 	
	static function getInsInsumoEnviadosXInsInsumos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(InsInsumoEnviado::GEN_ATTR_INS_INSUMO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(InsInsumoEnviado::GEN_TABLA);
            $c->addOrden(InsInsumoEnviado::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = InsInsumoEnviado::getInsInsumoEnviados($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'ins_insumo_enviado_adm.php';
            $url_gestion = 'ins_insumo_enviado_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM ins_insumo_enviado'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'ins_insumo_enviado';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getInsInsumoEnviados($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getInsInsumoEnviados(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'ins_insumo_enviado_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(InsInsumoEnviado::GEN_TABLA);
                $c->addOrden(InsInsumoEnviado::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $ins_insumo_enviados = InsInsumoEnviado::getInsInsumoEnviados(null, $c);

                $arr = array();
                foreach($ins_insumo_enviados as $ins_insumo_enviado){
                    $descripcion = $ins_insumo_enviado->getDescripcion();
