...
class BPdeOrdenPagoImporte
{ 
	
	const SES_PAGINACION = 'adm_pdeordenpagoimporte_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_pdeordenpagoimporte_paginas_registros';
	const SES_CRITERIOS = 'adm_pdeordenpagoimporte_criterios';
	
	const GEN_CLASE = 'PdeOrdenPagoImporte';
	const GEN_TABLA = 'pde_orden_pago_importe';
	...
	/* Constantes de Atributos de BPdeOrdenPagoImporte */ 
	const GEN_ATTR_ID = 'pde_orden_pago_importe.id';
	const GEN_ATTR_DESCRIPCION = 'pde_orden_pago_importe.descripcion';
	const GEN_ATTR_PDE_ORDEN_PAGO_ID = 'pde_orden_pago_importe.pde_orden_pago_id';
	const GEN_ATTR_IMPORTE_SUBTOTAL = 'pde_orden_pago_importe.importe_subtotal';
	const GEN_ATTR_IMPORTE_IVA = 'pde_orden_pago_importe.importe_iva';
	const GEN_ATTR_IMPORTE_TRIBUTO = 'pde_orden_pago_importe.importe_tributo';
	const GEN_ATTR_IMPORTE_TOTAL = 'pde_orden_pago_importe.importe_total';
	const GEN_ATTR_IMPORTE_SALDADO = 'pde_orden_pago_importe.importe_saldado';
	const GEN_ATTR_IMPORTE_SALDO = 'pde_orden_pago_importe.importe_saldo';
	const GEN_ATTR_CODIGO = 'pde_orden_pago_importe.codigo';
	const GEN_ATTR_OBSERVACION = 'pde_orden_pago_importe.observacion';
	const GEN_ATTR_ORDEN = 'pde_orden_pago_importe.orden';
	const GEN_ATTR_ESTADO = 'pde_orden_pago_importe.estado';
	const GEN_ATTR_CREADO = 'pde_orden_pago_importe.creado';
	const GEN_ATTR_CREADO_POR = 'pde_orden_pago_importe.creado_por';
	const GEN_ATTR_MODIFICADO = 'pde_orden_pago_importe.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'pde_orden_pago_importe.modificado_por';

	/* Constantes de Atributos Min de BPdeOrdenPagoImporte */ 
	...
	/* Atributos de BPdeOrdenPagoImporte */ 
	private $id;
	private $descripcion;
	...
	/* Getters de BPdeOrdenPagoImporte */ 
	...
	/* Setters de BPdeOrdenPagoImporte */ 	
	...
	public static function hidratarObjeto($fila){            
            $o = new PdeOrdenPagoImporte();
            $o->setId($fila[PdeOrdenPagoImporte::GEN_ATTR_MIN_ID], false);
	...
	/* Control de BPdeOrdenPagoImporte */ 	
	...
	/* Cambia el estado de BPdeOrdenPagoImporte */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('pde_orden_pago_importe_seq'), 
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('pde_orden_pago_importe_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	/* Delete de BPdeOrdenPagoImporte */ 	
	...
	public function duplicarPdeOrdenPagoImporte(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPdeOrdenPagoImportes($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PdeOrdenPagoImporte::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $pdeordenpagoimporte = new PdeOrdenPagoImporte();
                    $pdeordenpagoimporte->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $pdeordenpagoimporte->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$pdeordenpagoimporte->setPdeOrdenPagoId($fila[self::GEN_ATTR_MIN_PDE_ORDEN_PAGO_ID]);
			$pdeordenpagoimporte->setImporteSubtotal($fila[self::GEN_ATTR_MIN_IMPORTE_SUBTOTAL]);
			$pdeordenpagoimporte->setImporteIva($fila[self::GEN_ATTR_MIN_IMPORTE_IVA]);
			$pdeordenpagoimporte->setImporteTributo($fila[self::GEN_ATTR_MIN_IMPORTE_TRIBUTO]);
			$pdeordenpagoimporte->setImporteTotal($fila[self::GEN_ATTR_MIN_IMPORTE_TOTAL]);
			$pdeordenpagoimporte->setImporteSaldado($fila[self::GEN_ATTR_MIN_IMPORTE_SALDADO]);
			$pdeordenpagoimporte->setImporteSaldo($fila[self::GEN_ATTR_MIN_IMPORTE_SALDO]);
			$pdeordenpagoimporte->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$pdeordenpagoimporte->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$pdeordenpagoimporte->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$pdeordenpagoimporte->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$pdeordenpagoimporte->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$pdeordenpagoimporte->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$pdeordenpagoimporte->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$pdeordenpagoimporte->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPdeOrdenPagoImportesHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PdeOrdenPagoImporte::getPdeOrdenPagoImportes($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPdeOrdenPagoImportes para listado de Backend */ 	
	static function getPdeOrdenPagoImportesDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PdeOrdenPagoImporte::getPdeOrdenPagoImportes($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPdeOrdenPagoImportesCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PdeOrdenPagoImporte::getPdeOrdenPagoImportes($paginador, $criterio);
	...
	/* Método getPdeOrdenPagoImportes filtrado para select */ 	
	static function getPdeOrdenPagoImportesCmbF($paginador = null, $criterio = null){
            $col = PdeOrdenPagoImporte::getPdeOrdenPagoImportes($paginador, $criterio);
	...
	/* Método getPdeOrdenPagoImportes filtrado por una coleccion de objetos foraneos de PdeOrdenPago */ 	
	static function getPdeOrdenPagoImportesXPdeOrdenPagos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdeOrdenPagoImporte::GEN_ATTR_PDE_ORDEN_PAGO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdeOrdenPagoImporte::GEN_TABLA);
            $c->addOrden(PdeOrdenPagoImporte::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = PdeOrdenPagoImporte::getPdeOrdenPagoImportes($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'pde_orden_pago_importe_adm.php';
            $url_gestion = 'pde_orden_pago_importe_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM pde_orden_pago_importe'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'pde_orden_pago_importe';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPdeOrdenPagoImportes($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPdeOrdenPagoImportes(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'pde_orden_pago_importe_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(PdeOrdenPagoImporte::GEN_TABLA);
                $c->addOrden(PdeOrdenPagoImporte::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $pde_orden_pago_importes = PdeOrdenPagoImporte::getPdeOrdenPagoImportes(null, $c);

                $arr = array();
                foreach($pde_orden_pago_importes as $pde_orden_pago_importe){
                    $descripcion = $pde_orden_pago_importe->getDescripcion();
