...
{ 
	...
	const SES_PAGINACION = 'adm_pdeocestado_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_pdeocestado_paginas_registros';
	const SES_CRITERIOS = 'adm_pdeocestado_criterios';
	
	const GEN_CLASE = 'PdeOcEstado';
	const GEN_TABLA = 'pde_oc_estado';
	...
	/* Constantes de Atributos de BPdeOcEstado */ 
	const GEN_ATTR_ID = 'pde_oc_estado.id';
	const GEN_ATTR_PDE_OC_ID = 'pde_oc_estado.pde_oc_id';
	const GEN_ATTR_PDE_OC_TIPO_ESTADO_ID = 'pde_oc_estado.pde_oc_tipo_estado_id';
	const GEN_ATTR_INICIAL = 'pde_oc_estado.inicial';
	const GEN_ATTR_ACTUAL = 'pde_oc_estado.actual';
	const GEN_ATTR_OBSERVACION = 'pde_oc_estado.observacion';
	const GEN_ATTR_CREADO = 'pde_oc_estado.creado';
	const GEN_ATTR_CREADO_POR = 'pde_oc_estado.creado_por';
	const GEN_ATTR_MODIFICADO = 'pde_oc_estado.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'pde_oc_estado.modificado_por';

	/* Constantes de Atributos Min de BPdeOcEstado */ 
	...
	public static function hidratarObjeto($fila){            
            $o = new PdeOcEstado();
            $o->setId($fila[PdeOcEstado::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BPdeOcEstado */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                $sql = "
				 INSERT INTO ".self::GEN_TABLA." (".self::GEN_ATTR_MIN_ID.", ".self::GEN_ATTR_MIN_PDE_OC_ID."
                ...
                ".$this->getPdeOcId()."
                    ...
                    ) RETURNING currval('pde_oc_estado_seq')";
...
".self::GEN_ATTR_MIN_PDE_OC_ID." = ".$this->getPdeOcId()."
	...
	public function duplicarPdeOcEstado(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPdeOcEstados($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PdeOcEstado::setAplicarFiltrosDeAlcance($criterio);
		...
		$pdeocestados = array();
		while($fila = pg_fetch_array($cons->getResultado())){
                    $pdeocestado = new PdeOcEstado();
                    $pdeocestado->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $pdeocestado->setPdeOcId($fila[self::GEN_ATTR_MIN_PDE_OC_ID]);
			$pdeocestado->setPdeOcTipoEstadoId($fila[self::GEN_ATTR_MIN_PDE_OC_TIPO_ESTADO_ID]);
			$pdeocestado->setInicial($fila[self::GEN_ATTR_MIN_INICIAL]);
			$pdeocestado->setActual($fila[self::GEN_ATTR_MIN_ACTUAL]);
			$pdeocestado->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$pdeocestado->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$pdeocestado->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$pdeocestado->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$pdeocestado->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPdeOcEstadosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PdeOcEstado::getPdeOcEstados($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPdeOcEstados para listado de Backend */ 	
	static function getPdeOcEstadosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PdeOcEstado::getPdeOcEstados($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPdeOcEstadosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PdeOcEstado::getPdeOcEstados($paginador, $criterio);
	...
	/* Método getPdeOcEstados filtrado para select */ 	
	static function getPdeOcEstadosCmbF($paginador = null, $criterio = null){
            $col = PdeOcEstado::getPdeOcEstados($paginador, $criterio);
	...
	/* Método getPdeOcEstados filtrado por una coleccion de objetos foraneos de PdeOc */ 	
	static function getPdeOcEstadosXPdeOcs($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdeOcEstado::GEN_ATTR_PDE_OC_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdeOcEstado::GEN_TABLA);
            $c->addOrden(PdeOcEstado::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = PdeOcEstado::getPdeOcEstados($p, $c);
            foreach($os as $o){
	...
	/* Método getPdeOcEstados filtrado por una coleccion de objetos foraneos de PdeOcTipoEstado */ 	
	static function getPdeOcEstadosXPdeOcTipoEstados($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdeOcEstado::GEN_ATTR_PDE_OC_TIPO_ESTADO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdeOcEstado::GEN_TABLA);
            $c->addOrden(PdeOcEstado::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'pde_oc_estado_adm.php';
            $url_gestion = 'pde_oc_estado_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM pde_oc_estado'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'pde_oc_estado';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPdeOcEstados($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPdeOcEstados(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'pde_oc_estado_adm');
            }else{
