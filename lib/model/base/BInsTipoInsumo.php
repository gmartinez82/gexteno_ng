...
class BInsTipoInsumo
{ 
	
	const SES_PAGINACION = 'adm_instipoinsumo_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_instipoinsumo_paginas_registros';
	const SES_CRITERIOS = 'adm_instipoinsumo_criterios';
	
	const GEN_CLASE = 'InsTipoInsumo';
	const GEN_TABLA = 'ins_tipo_insumo';
	...
	/* Constantes de Atributos de BInsTipoInsumo */ 
	const GEN_ATTR_ID = 'ins_tipo_insumo.id';
	const GEN_ATTR_DESCRIPCION = 'ins_tipo_insumo.descripcion';
	const GEN_ATTR_CODIGO = 'ins_tipo_insumo.codigo';
	const GEN_ATTR_CNTB_CUENTA_COMPRA = 'ins_tipo_insumo.cntb_cuenta_compra';
	const GEN_ATTR_CNTB_CUENTA_VENTA = 'ins_tipo_insumo.cntb_cuenta_venta';
	const GEN_ATTR_OBSERVACION = 'ins_tipo_insumo.observacion';
	const GEN_ATTR_ORDEN = 'ins_tipo_insumo.orden';
	const GEN_ATTR_ESTADO = 'ins_tipo_insumo.estado';
	const GEN_ATTR_CREADO = 'ins_tipo_insumo.creado';
	const GEN_ATTR_CREADO_POR = 'ins_tipo_insumo.creado_por';
	const GEN_ATTR_MODIFICADO = 'ins_tipo_insumo.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'ins_tipo_insumo.modificado_por';

	/* Constantes de Atributos Min de BInsTipoInsumo */ 
	...
	public static function hidratarObjeto($fila){            
            $o = new InsTipoInsumo();
            $o->setId($fila[InsTipoInsumo::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BInsTipoInsumo */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('ins_tipo_insumo_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function duplicarInsTipoInsumo(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getInsTipoInsumos($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = InsTipoInsumo::setAplicarFiltrosDeAlcance($criterio);
		...
		$instipoinsumos = array();
		while($fila = pg_fetch_array($cons->getResultado())){
                    $instipoinsumo = new InsTipoInsumo();
                    $instipoinsumo->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $instipoinsumo->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$instipoinsumo->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$instipoinsumo->setCntbCuentaCompra($fila[self::GEN_ATTR_MIN_CNTB_CUENTA_COMPRA]);
			$instipoinsumo->setCntbCuentaVenta($fila[self::GEN_ATTR_MIN_CNTB_CUENTA_VENTA]);
			$instipoinsumo->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$instipoinsumo->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$instipoinsumo->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$instipoinsumo->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$instipoinsumo->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$instipoinsumo->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$instipoinsumo->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
				

                    $instipoinsumos[] = $instipoinsumo;
		}
		
		return $instipoinsumos;
	}	
	

	/* Método getInsTipoInsumos Habilitados */ 	
	static function getInsTipoInsumosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return InsTipoInsumo::getInsTipoInsumos($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getInsTipoInsumos para listado de Backend */ 	
	static function getInsTipoInsumosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return InsTipoInsumo::getInsTipoInsumos($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getInsTipoInsumosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = InsTipoInsumo::getInsTipoInsumos($paginador, $criterio);
	...
	/* Método getInsTipoInsumos filtrado para select */ 	
	static function getInsTipoInsumosCmbF($paginador = null, $criterio = null){
            $col = InsTipoInsumo::getInsTipoInsumos($paginador, $criterio);
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'ins_tipo_insumo_adm.php';
            $url_gestion = 'ins_tipo_insumo_gestion.php';
	...
	public function getInsInsumos($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            ...
            $criterio->add(InsInsumo::GEN_ATTR_INS_TIPO_INSUMO_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
	...
	public function getInsInsumosCmb(){
            $c = new Criterio();
            $c->add(InsInsumo::GEN_ATTR_INS_TIPO_INSUMO_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(InsInsumo::GEN_TABLA);
            $c->setCriterios();
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM ins_tipo_insumo'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'ins_tipo_insumo';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getInsTipoInsumos($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getInsTipoInsumos(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'ins_tipo_insumo_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(InsTipoInsumo::GEN_TABLA);
                $c->addOrden(InsTipoInsumo::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $ins_tipo_insumos = InsTipoInsumo::getInsTipoInsumos(null, $c);

                $arr = array();
                foreach($ins_tipo_insumos as $ins_tipo_insumo){
                    $descripcion = $ins_tipo_insumo->getDescripcion();
