...
{ 
	...
	const SES_PAGINACION = 'adm_usnavegacion_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_usnavegacion_paginas_registros';
	const SES_CRITERIOS = 'adm_usnavegacion_criterios';
	
	const GEN_CLASE = 'UsNavegacion';
	const GEN_TABLA = 'us_navegacion';
	...
	/* Constantes de Atributos de BUsNavegacion */ 
	const GEN_ATTR_ID = 'us_navegacion.id';
	const GEN_ATTR_US_USUARIO_ID = 'us_navegacion.us_usuario_id';
	const GEN_ATTR_SESSION = 'us_navegacion.session';
	const GEN_ATTR_IP = 'us_navegacion.ip';
	const GEN_ATTR_NAVEGADOR = 'us_navegacion.navegador';
	const GEN_ATTR_PAGINA = 'us_navegacion.pagina';
	const GEN_ATTR_URL = 'us_navegacion.url';
	const GEN_ATTR_URL_REFERER = 'us_navegacion.url_referer';
	const GEN_ATTR_OBSERVACION = 'us_navegacion.observacion';
	const GEN_ATTR_ORDEN = 'us_navegacion.orden';
	const GEN_ATTR_ESTADO = 'us_navegacion.estado';
	const GEN_ATTR_CREADO = 'us_navegacion.creado';
	const GEN_ATTR_CREADO_POR = 'us_navegacion.creado_por';
	const GEN_ATTR_MODIFICADO = 'us_navegacion.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'us_navegacion.modificado_por';

	/* Constantes de Atributos Min de BUsNavegacion */ 
	...
	public static function hidratarObjeto($fila){            
            $o = new UsNavegacion();
            $o->setId($fila[UsNavegacion::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BUsNavegacion */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                $sql = "
				 INSERT INTO ".self::GEN_TABLA." (".self::GEN_ATTR_MIN_ID.", ".self::GEN_ATTR_MIN_US_USUARIO_ID."
                ...
                ".$this->getUsUsuarioId()."
                    ...
                    ) RETURNING currval('us_navegacion_seq')";
...
".self::GEN_ATTR_MIN_US_USUARIO_ID." = ".$this->getUsUsuarioId()."
	...
	public function duplicarUsNavegacion(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getUsNavegacions($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = UsNavegacion::setAplicarFiltrosDeAlcance($criterio);
		...
		$usnavegacions = array();
		while($fila = pg_fetch_array($cons->getResultado())){
                    $usnavegacion = new UsNavegacion();
                    $usnavegacion->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $usnavegacion->setUsUsuarioId($fila[self::GEN_ATTR_MIN_US_USUARIO_ID]);
			$usnavegacion->setSession($fila[self::GEN_ATTR_MIN_SESSION]);
			$usnavegacion->setIp($fila[self::GEN_ATTR_MIN_IP]);
			$usnavegacion->setNavegador($fila[self::GEN_ATTR_MIN_NAVEGADOR]);
			$usnavegacion->setPagina($fila[self::GEN_ATTR_MIN_PAGINA]);
			$usnavegacion->setUrl($fila[self::GEN_ATTR_MIN_URL]);
			$usnavegacion->setUrlReferer($fila[self::GEN_ATTR_MIN_URL_REFERER]);
			$usnavegacion->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$usnavegacion->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$usnavegacion->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$usnavegacion->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$usnavegacion->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$usnavegacion->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$usnavegacion->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getUsNavegacionsHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return UsNavegacion::getUsNavegacions($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getUsNavegacions para listado de Backend */ 	
	static function getUsNavegacionsDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return UsNavegacion::getUsNavegacions($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getUsNavegacionsCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = UsNavegacion::getUsNavegacions($paginador, $criterio);
	...
	/* Método getUsNavegacions filtrado para select */ 	
	static function getUsNavegacionsCmbF($paginador = null, $criterio = null){
            $col = UsNavegacion::getUsNavegacions($paginador, $criterio);
	...
	/* Método getUsNavegacions filtrado por una coleccion de objetos foraneos de UsUsuario */ 	
	static function getUsNavegacionsXUsUsuarios($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(UsNavegacion::GEN_ATTR_US_USUARIO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(UsNavegacion::GEN_TABLA);
            $c->addOrden(UsNavegacion::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = UsNavegacion::getUsNavegacions($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'us_navegacion_adm.php';
            $url_gestion = 'us_navegacion_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM us_navegacion'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'us_navegacion';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getUsNavegacions($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getUsNavegacions(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'us_navegacion_adm');
            }else{
