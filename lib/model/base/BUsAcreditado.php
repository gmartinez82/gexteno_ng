...
{ 
	...
	const SES_PAGINACION = 'adm_usacreditado_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_usacreditado_paginas_registros';
	const SES_CRITERIOS = 'adm_usacreditado_criterios';
	
	const GEN_CLASE = 'UsAcreditado';
	const GEN_TABLA = 'us_acreditado';
	...
	/* Constantes de Atributos de BUsAcreditado */ 
	const GEN_ATTR_ID = 'us_acreditado.id';
	const GEN_ATTR_US_CREDENCIAL_ID = 'us_acreditado.us_credencial_id';
	const GEN_ATTR_US_USUARIO_ID = 'us_acreditado.us_usuario_id';
	const GEN_ATTR_CREADO = 'us_acreditado.creado';
	const GEN_ATTR_CREADO_POR = 'us_acreditado.creado_por';

	/* Constantes de Atributos Min de BUsAcreditado */ 
	...
	public static function hidratarObjeto($fila){            
            $o = new UsAcreditado();
            $o->setId($fila[UsAcreditado::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BUsAcreditado */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('us_acreditado_seq'), 
                ".$this->getUsCredencialId()."
						, ".$this->getUsUsuarioId()."
						, '".$this->getCreado()."'
						, ".$this->getCreadoPor()."
                    ) RETURNING currval('us_acreditado_seq')";
...
".self::GEN_ATTR_MIN_US_CREDENCIAL_ID." = ".$this->getUsCredencialId()."
	...
	public function duplicarUsAcreditado(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getUsAcreditados($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = UsAcreditado::setAplicarFiltrosDeAlcance($criterio);
		...
		$usacreditados = array();
		while($fila = pg_fetch_array($cons->getResultado())){
                    $usacreditado = new UsAcreditado();
                    $usacreditado->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $usacreditado->setUsCredencialId($fila[self::GEN_ATTR_MIN_US_CREDENCIAL_ID]);
			$usacreditado->setUsUsuarioId($fila[self::GEN_ATTR_MIN_US_USUARIO_ID]);
			$usacreditado->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$usacreditado->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
	...
	static function getUsAcreditadosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return UsAcreditado::getUsAcreditados($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getUsAcreditados para listado de Backend */ 	
	static function getUsAcreditadosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return UsAcreditado::getUsAcreditados($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getUsAcreditadosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = UsAcreditado::getUsAcreditados($paginador, $criterio);
	...
	/* Método getUsAcreditados filtrado para select */ 	
	static function getUsAcreditadosCmbF($paginador = null, $criterio = null){
            $col = UsAcreditado::getUsAcreditados($paginador, $criterio);
	...
	/* Método getUsAcreditados filtrado por una coleccion de objetos foraneos de UsCredencial */ 	
	static function getUsAcreditadosXUsCredencials($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(UsAcreditado::GEN_ATTR_US_CREDENCIAL_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(UsAcreditado::GEN_TABLA);
            $c->addOrden(UsAcreditado::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = UsAcreditado::getUsAcreditados($p, $c);
            foreach($os as $o){
	...
	/* Método getUsAcreditados filtrado por una coleccion de objetos foraneos de UsUsuario */ 	
	static function getUsAcreditadosXUsUsuarios($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(UsAcreditado::GEN_ATTR_US_USUARIO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(UsAcreditado::GEN_TABLA);
            $c->addOrden(UsAcreditado::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'us_acreditado_adm.php';
            $url_gestion = 'us_acreditado_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM us_acreditado'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'us_acreditado';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getUsAcreditados($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getUsAcreditados(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'us_acreditado_adm');
            }else{
