...
class BPdeNotaDebitoImporte
{ 
	
	const SES_PAGINACION = 'adm_pdenotadebitoimporte_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_pdenotadebitoimporte_paginas_registros';
	const SES_CRITERIOS = 'adm_pdenotadebitoimporte_criterios';
	
	const GEN_CLASE = 'PdeNotaDebitoImporte';
	const GEN_TABLA = 'pde_nota_debito_importe';
	...
	/* Constantes de Atributos de BPdeNotaDebitoImporte */ 
	const GEN_ATTR_ID = 'pde_nota_debito_importe.id';
	const GEN_ATTR_DESCRIPCION = 'pde_nota_debito_importe.descripcion';
	const GEN_ATTR_PDE_NOTA_DEBITO_ID = 'pde_nota_debito_importe.pde_nota_debito_id';
	const GEN_ATTR_IMPORTE_SUBTOTAL = 'pde_nota_debito_importe.importe_subtotal';
	const GEN_ATTR_IMPORTE_IVA = 'pde_nota_debito_importe.importe_iva';
	const GEN_ATTR_IMPORTE_TRIBUTO = 'pde_nota_debito_importe.importe_tributo';
	const GEN_ATTR_IMPORTE_TOTAL = 'pde_nota_debito_importe.importe_total';
	const GEN_ATTR_IMPORTE_SALDADO = 'pde_nota_debito_importe.importe_saldado';
	const GEN_ATTR_IMPORTE_SALDO = 'pde_nota_debito_importe.importe_saldo';
	const GEN_ATTR_CODIGO = 'pde_nota_debito_importe.codigo';
	const GEN_ATTR_OBSERVACION = 'pde_nota_debito_importe.observacion';
	const GEN_ATTR_ORDEN = 'pde_nota_debito_importe.orden';
	const GEN_ATTR_ESTADO = 'pde_nota_debito_importe.estado';
	const GEN_ATTR_CREADO = 'pde_nota_debito_importe.creado';
	const GEN_ATTR_CREADO_POR = 'pde_nota_debito_importe.creado_por';
	const GEN_ATTR_MODIFICADO = 'pde_nota_debito_importe.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'pde_nota_debito_importe.modificado_por';

	/* Constantes de Atributos Min de BPdeNotaDebitoImporte */ 
	...
	/* Atributos de BPdeNotaDebitoImporte */ 
	private $id;
	private $descripcion;
	...
	/* Getters de BPdeNotaDebitoImporte */ 
	...
	/* Setters de BPdeNotaDebitoImporte */ 	
	...
	public static function hidratarObjeto($fila){            
            $o = new PdeNotaDebitoImporte();
            $o->setId($fila[PdeNotaDebitoImporte::GEN_ATTR_MIN_ID], false);
	...
	/* Control de BPdeNotaDebitoImporte */ 	
	...
	/* Cambia el estado de BPdeNotaDebitoImporte */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('pde_nota_debito_importe_seq'), 
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('pde_nota_debito_importe_seq')";
                ...
                $sql = "
				 ...
				 ".PdeNotaDebitoImporte::GEN_TABLA."
				 SET 
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	/* Delete de BPdeNotaDebitoImporte */ 	
	...
	public function duplicarPdeNotaDebitoImporte(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPdeNotaDebitoImportes($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PdeNotaDebitoImporte::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $pdenotadebitoimporte = new PdeNotaDebitoImporte();
                    $pdenotadebitoimporte->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $pdenotadebitoimporte->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$pdenotadebitoimporte->setPdeNotaDebitoId($fila[self::GEN_ATTR_MIN_PDE_NOTA_DEBITO_ID]);
			$pdenotadebitoimporte->setImporteSubtotal($fila[self::GEN_ATTR_MIN_IMPORTE_SUBTOTAL]);
			$pdenotadebitoimporte->setImporteIva($fila[self::GEN_ATTR_MIN_IMPORTE_IVA]);
			$pdenotadebitoimporte->setImporteTributo($fila[self::GEN_ATTR_MIN_IMPORTE_TRIBUTO]);
			$pdenotadebitoimporte->setImporteTotal($fila[self::GEN_ATTR_MIN_IMPORTE_TOTAL]);
			$pdenotadebitoimporte->setImporteSaldado($fila[self::GEN_ATTR_MIN_IMPORTE_SALDADO]);
			$pdenotadebitoimporte->setImporteSaldo($fila[self::GEN_ATTR_MIN_IMPORTE_SALDO]);
			$pdenotadebitoimporte->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$pdenotadebitoimporte->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$pdenotadebitoimporte->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$pdenotadebitoimporte->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$pdenotadebitoimporte->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$pdenotadebitoimporte->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$pdenotadebitoimporte->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$pdenotadebitoimporte->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPdeNotaDebitoImportesHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PdeNotaDebitoImporte::getPdeNotaDebitoImportes($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPdeNotaDebitoImportes para listado de Backend */ 	
	static function getPdeNotaDebitoImportesDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PdeNotaDebitoImporte::getPdeNotaDebitoImportes($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPdeNotaDebitoImportesCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PdeNotaDebitoImporte::getPdeNotaDebitoImportes($paginador, $criterio);
	...
	/* Método getPdeNotaDebitoImportes filtrado para select */ 	
	static function getPdeNotaDebitoImportesCmbF($paginador = null, $criterio = null){
            $col = PdeNotaDebitoImporte::getPdeNotaDebitoImportes($paginador, $criterio);
	...
	/* Método getPdeNotaDebitoImportes filtrado por una coleccion de objetos foraneos de PdeNotaDebito */ 	
	static function getPdeNotaDebitoImportesXPdeNotaDebitos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdeNotaDebitoImporte::GEN_ATTR_PDE_NOTA_DEBITO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdeNotaDebitoImporte::GEN_TABLA);
            $c->addOrden(PdeNotaDebitoImporte::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = PdeNotaDebitoImporte::getPdeNotaDebitoImportes($p, $c);
            foreach($os as $o){
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'pde_nota_debito_importe_adm.php';
            $url_gestion = 'pde_nota_debito_importe_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM pde_nota_debito_importe'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'pde_nota_debito_importe';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPdeNotaDebitoImportes($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPdeNotaDebitoImportes(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'pde_nota_debito_importe_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(PdeNotaDebitoImporte::GEN_TABLA);
                $c->addOrden(PdeNotaDebitoImporte::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $pde_nota_debito_importes = PdeNotaDebitoImporte::getPdeNotaDebitoImportes(null, $c);

                $arr = array();
                foreach($pde_nota_debito_importes as $pde_nota_debito_importe){
                    $descripcion = $pde_nota_debito_importe->getDescripcion();
