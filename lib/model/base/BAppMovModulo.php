...
{ 
	...
	const SES_PAGINACION = 'adm_appmovmodulo_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_appmovmodulo_paginas_registros';
	const SES_CRITERIOS = 'adm_appmovmodulo_criterios';
	
	const GEN_CLASE = 'AppMovModulo';
	const GEN_TABLA = 'app_mov_modulo';
	...
	/* Constantes de Atributos de BAppMovModulo */ 
	const GEN_ATTR_ID = 'app_mov_modulo.id';
	const GEN_ATTR_DESCRIPCION = 'app_mov_modulo.descripcion';
	const GEN_ATTR_CODIGO = 'app_mov_modulo.codigo';
	const GEN_ATTR_OBSERVACION = 'app_mov_modulo.observacion';
	const GEN_ATTR_ORDEN = 'app_mov_modulo.orden';
	const GEN_ATTR_ESTADO = 'app_mov_modulo.estado';
	const GEN_ATTR_CREADO = 'app_mov_modulo.creado';
	const GEN_ATTR_CREADO_POR = 'app_mov_modulo.creado_por';
	const GEN_ATTR_MODIFICADO = 'app_mov_modulo.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'app_mov_modulo.modificado_por';

	/* Constantes de Atributos Min de BAppMovModulo */ 
	...
	public static function hidratarObjeto($fila){            
            $o = new AppMovModulo();
            $o->setId($fila[AppMovModulo::GEN_ATTR_MIN_ID], false);
	...
	/* Cambia el estado de BAppMovModulo */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                '".$this->getDescripcion()."'
                    ...
                    ) RETURNING currval('app_mov_modulo_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	public function deleteAll(){
            ...
            // se elimina la coleccion de AppMovVersions relacionados
            $c = new Criterio('COLECCION_DELETE_ALL');
            $c->addTabla(AppMovVersion::GEN_TABLA);
            $c->setCriterios();		
            foreach($this->getAppMovVersions(null, $c) as $o){
                    $o->deleteAll();
            }
	...
	public function duplicarAppMovModulo(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getAppMovModulos($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = AppMovModulo::setAplicarFiltrosDeAlcance($criterio);
		...
		$appmovmodulos = array();
		while($fila = pg_fetch_array($cons->getResultado())){
                    $appmovmodulo = new AppMovModulo();
                    $appmovmodulo->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $appmovmodulo->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$appmovmodulo->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$appmovmodulo->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$appmovmodulo->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$appmovmodulo->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$appmovmodulo->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$appmovmodulo->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$appmovmodulo->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$appmovmodulo->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
				

                    $appmovmodulos[] = $appmovmodulo;
		}
		
		return $appmovmodulos;
	}	
	

	/* Método getAppMovModulos Habilitados */ 	
	static function getAppMovModulosHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return AppMovModulo::getAppMovModulos($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getAppMovModulos para listado de Backend */ 	
	static function getAppMovModulosDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return AppMovModulo::getAppMovModulos($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getAppMovModulosCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = AppMovModulo::getAppMovModulos($paginador, $criterio);
	...
	/* Método getAppMovModulos filtrado para select */ 	
	static function getAppMovModulosCmbF($paginador = null, $criterio = null){
            $col = AppMovModulo::getAppMovModulos($paginador, $criterio);
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'app_mov_modulo_adm.php';
            $url_gestion = 'app_mov_modulo_gestion.php';
	...
	/* Metodo getAppMovVersions */ 	
	public function getAppMovVersions($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            // inicializacion de variables
            if(is_null($paginador)){
                $paginador = new Paginador(AppMovVersion::getCantidadTotalDeRegistros(), 1);
            }
            if(is_null($criterio)){
                ...
                //$criterio->addOrden(AppMovVersion::GEN_ATTR_ORDEN, 'asc');                            
                $criterio->addOrden('1', 'asc');			
                $criterio->setCriterios();
            }
                
            $criterio->add(AppMovVersion::GEN_ATTR_APP_MOV_MODULO_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
                    $criterio->add(AppMovVersion::GEN_ATTR_ESTADO, 1, Criterio::IGUAL);
                }else{
                    $criterio->add(AppMovVersion::GEN_ATTR_ESTADO, 0, Criterio::IGUAL);			
                }
            }
            ...
            $cons = new Consulta();
            $sql = "SELECT ".$criterio->getDistinct()." ".AppMovVersion::GEN_TABLA.".* FROM ".$criterio->getTablas().$criterio->getWhere().$criterio->getOrden();
            ...
            $appmovversions = array();
            while($fila = pg_fetch_array($cons->getResultado())){
                $appmovversion = AppMovVersion::hidratarObjeto($fila);			
                $appmovversions[] = $appmovversion;
            }

            return $appmovversions;
	}	
	

	/* Método getAppMovVersionsBloque para MasInfo */ 	
	public function getAppMovVersionsParaBloqueMasInfo($paginador = null, $criterio = null, $estado = null){
            // inicializacion de variables
            if(is_null($paginador)){
                $paginador = new Paginador(50, 1);
            }

            $os = $this->getAppMovVersions($paginador, $criterio, $estado);
            return $os;
		
	}

	/* Método getAppMovVersions Habilitados */ 	
	public function getAppMovVersionsHabilitados($paginador = null, $criterio = null, $estado = null){

            $os = $this->getAppMovVersions($paginador, $criterio, true);
            return $os;
		
	}

	/* Método getAppMovVersion */ 	
	public function getAppMovVersion($criterio = null){
            $paginador = new Paginador(1, 1);
            $os = $this->getAppMovVersions($paginador, $criterio);
            foreach($os as $o){
                return $o;
            }
            return false;
	...
	/* Delete de AppMovVersion relacionadas */ 	
	public function deleteAppMovVersions(){
            $obs = $this->getAppMovVersions();
            foreach($obs as $o){
                $o->delete();
            }
            return true;
	...
	/* Metodos getAppMovVersionsCmb() AppMovVersion relacionadas */ 	
	public function getAppMovVersionsCmb(){
            $c = new Criterio();
            $c->add(AppMovVersion::GEN_ATTR_APP_MOV_MODULO_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(AppMovVersion::GEN_TABLA);
            $c->setCriterios();

            $os = AppMovVersion::getAppMovVersionsCmbF(null, $c);
            return $os;
	}
	...
	public function getAppMovInstalacions($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            ...
            $criterio->add(AppMovInstalacion::GEN_ATTR_APP_MOV_MODULO_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
	...
	public function getAppMovInstalacionsCmb(){
            $c = new Criterio();
            $c->add(AppMovInstalacion::GEN_ATTR_APP_MOV_MODULO_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(AppMovInstalacion::GEN_TABLA);
            $c->setCriterios();
	...
	public function getAppMovActividads($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            ...
            $criterio->add(AppMovActividad::GEN_ATTR_APP_MOV_MODULO_ID, $this->getId(), Criterio::IGUAL);
            if(!is_null($estado)){
                if($estado){
	...
	public function getAppMovActividadsCmb(){
            $c = new Criterio();
            $c->add(AppMovActividad::GEN_ATTR_APP_MOV_MODULO_ID, $this->getId(), Criterio::IGUAL);
            $c->addTabla(AppMovActividad::GEN_TABLA);
            $c->setCriterios();
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM app_mov_modulo'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'app_mov_modulo';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getAppMovModulos($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getAppMovModulos(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'app_mov_modulo_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                ...
                $c->addOrden(AppMovModulo::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $app_mov_modulos = AppMovModulo::getAppMovModulos(null, $c);

                $arr = array();
                foreach($app_mov_modulos as $app_mov_modulo){
                    $descripcion = $app_mov_modulo->getDescripcion();
