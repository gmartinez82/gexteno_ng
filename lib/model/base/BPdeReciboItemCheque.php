...
class BPdeReciboItemCheque
{ 
	
	const SES_PAGINACION = 'adm_pdereciboitemcheque_paginas';
	const SES_PAGINACION_REGISTROS = 'adm_pdereciboitemcheque_paginas_registros';
	const SES_CRITERIOS = 'adm_pdereciboitemcheque_criterios';
	
	const GEN_CLASE = 'PdeReciboItemCheque';
	const GEN_TABLA = 'pde_recibo_item_cheque';
	...
	/* Constantes de Atributos de BPdeReciboItemCheque */ 
	const GEN_ATTR_ID = 'pde_recibo_item_cheque.id';
	const GEN_ATTR_DESCRIPCION = 'pde_recibo_item_cheque.descripcion';
	const GEN_ATTR_PDE_RECIBO_ID = 'pde_recibo_item_cheque.pde_recibo_id';
	const GEN_ATTR_PDE_RECIBO_ITEM_ID = 'pde_recibo_item_cheque.pde_recibo_item_id';
	const GEN_ATTR_GRAL_BANCO_ID = 'pde_recibo_item_cheque.gral_banco_id';
	const GEN_ATTR_NUMERO_CHEQUE = 'pde_recibo_item_cheque.numero_cheque';
	const GEN_ATTR_FECHA_EMISION = 'pde_recibo_item_cheque.fecha_emision';
	const GEN_ATTR_FECHA_COBRO = 'pde_recibo_item_cheque.fecha_cobro';
	const GEN_ATTR_FIRMANTE = 'pde_recibo_item_cheque.firmante';
	const GEN_ATTR_ENTREGADOR = 'pde_recibo_item_cheque.entregador';
	const GEN_ATTR_CODIGO = 'pde_recibo_item_cheque.codigo';
	const GEN_ATTR_OBSERVACION = 'pde_recibo_item_cheque.observacion';
	const GEN_ATTR_ORDEN = 'pde_recibo_item_cheque.orden';
	const GEN_ATTR_ESTADO = 'pde_recibo_item_cheque.estado';
	const GEN_ATTR_CREADO = 'pde_recibo_item_cheque.creado';
	const GEN_ATTR_CREADO_POR = 'pde_recibo_item_cheque.creado_por';
	const GEN_ATTR_MODIFICADO = 'pde_recibo_item_cheque.modificado';
	const GEN_ATTR_MODIFICADO_POR = 'pde_recibo_item_cheque.modificado_por';

	/* Constantes de Atributos Min de BPdeReciboItemCheque */ 
	...
	/* Atributos de BPdeReciboItemCheque */ 
	private $id;
	private $descripcion;
	...
	/* Getters de BPdeReciboItemCheque */ 
	...
	/* Setters de BPdeReciboItemCheque */ 	
	...
	public static function hidratarObjeto($fila){            
            $o = new PdeReciboItemCheque();
            $o->setId($fila[PdeReciboItemCheque::GEN_ATTR_MIN_ID], false);
	...
	/* Control de BPdeReciboItemCheque */ 	
	...
	/* Cambia el estado de BPdeReciboItemCheque */ 	
	...
	public function save($array_datos_auditoria = false){
            ...
            if(!isset($this->id)){
                ...
                nextval('pde_recibo_item_cheque_seq'), 
                '".$this->getDescripcion()."'
						, ".$this->getPdeReciboId()."
                    ...
                    ) RETURNING currval('pde_recibo_item_cheque_seq')";
...
".self::GEN_ATTR_MIN_DESCRIPCION." = '".$this->getDescripcion()."'
	...
	/* Delete de BPdeReciboItemCheque */ 	
	...
	public function duplicarPdeReciboItemCheque(){
            // duplicar el elemento
            
	}

	/* Metodo que retorna la coleccion principal de la clase */ 	
	static function getPdeReciboItemCheques($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
		...
		if(is_null($criterio)){
                    ...
                    $criterio = PdeReciboItemCheque::setAplicarFiltrosDeAlcance($criterio);
		...
		while($fila = pg_fetch_array($cons->getResultado())){
                    $pdereciboitemcheque = new PdeReciboItemCheque();
                    $pdereciboitemcheque->setId($fila[self::GEN_ATTR_MIN_ID], false);


                    $pdereciboitemcheque->setDescripcion($fila[self::GEN_ATTR_MIN_DESCRIPCION]);
			$pdereciboitemcheque->setPdeReciboId($fila[self::GEN_ATTR_MIN_PDE_RECIBO_ID]);
			$pdereciboitemcheque->setPdeReciboItemId($fila[self::GEN_ATTR_MIN_PDE_RECIBO_ITEM_ID]);
			$pdereciboitemcheque->setGralBancoId($fila[self::GEN_ATTR_MIN_GRAL_BANCO_ID]);
			$pdereciboitemcheque->setNumeroCheque($fila[self::GEN_ATTR_MIN_NUMERO_CHEQUE]);
			$pdereciboitemcheque->setFechaEmision($fila[self::GEN_ATTR_MIN_FECHA_EMISION]);
			$pdereciboitemcheque->setFechaCobro($fila[self::GEN_ATTR_MIN_FECHA_COBRO]);
			$pdereciboitemcheque->setFirmante($fila[self::GEN_ATTR_MIN_FIRMANTE]);
			$pdereciboitemcheque->setEntregador($fila[self::GEN_ATTR_MIN_ENTREGADOR]);
			$pdereciboitemcheque->setCodigo($fila[self::GEN_ATTR_MIN_CODIGO]);
			$pdereciboitemcheque->setObservacion($fila[self::GEN_ATTR_MIN_OBSERVACION]);
			$pdereciboitemcheque->setOrden($fila[self::GEN_ATTR_MIN_ORDEN]);
			$pdereciboitemcheque->setEstado($fila[self::GEN_ATTR_MIN_ESTADO]);
			$pdereciboitemcheque->setCreado($fila[self::GEN_ATTR_MIN_CREADO]);
			$pdereciboitemcheque->setCreadoPor($fila[self::GEN_ATTR_MIN_CREADO_POR]);
			$pdereciboitemcheque->setModificado($fila[self::GEN_ATTR_MIN_MODIFICADO]);
			$pdereciboitemcheque->setModificadoPor($fila[self::GEN_ATTR_MIN_MODIFICADO_POR]);
	...
	static function getPdeReciboItemChequesHabilitados($paginador = null, $criterio = null, $arr_ordens = false){
            return PdeReciboItemCheque::getPdeReciboItemCheques($paginador, $criterio, true, $arr_ordens);
	}

	/* Método getPdeReciboItemCheques para listado de Backend */ 	
	static function getPdeReciboItemChequesDesdeBackend($paginador = null, $criterio = null, $estado = null, $arr_ordens = false){
            return PdeReciboItemCheque::getPdeReciboItemCheques($paginador, $criterio, $estado, $arr_ordens);
	}

	/* Método coleccion principal de la clase para select */ 	
	static function getPdeReciboItemChequesCmb($estado = true){
            $paginador = new Paginador(self::getCantidadTotalDeRegistros(), 1);
            ...
            $col = PdeReciboItemCheque::getPdeReciboItemCheques($paginador, $criterio);
	...
	/* Método getPdeReciboItemCheques filtrado para select */ 	
	static function getPdeReciboItemChequesCmbF($paginador = null, $criterio = null){
            $col = PdeReciboItemCheque::getPdeReciboItemCheques($paginador, $criterio);
	...
	/* Método getPdeReciboItemCheques filtrado por una coleccion de objetos foraneos de PdeRecibo */ 	
	static function getPdeReciboItemChequesXPdeRecibos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdeReciboItemCheque::GEN_ATTR_PDE_RECIBO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdeReciboItemCheque::GEN_TABLA);
            $c->addOrden(PdeReciboItemCheque::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();

            $p = null;

            $os = PdeReciboItemCheque::getPdeReciboItemCheques($p, $c);
            foreach($os as $o){
	...
	/* Método getPdeReciboItemCheques filtrado por una coleccion de objetos foraneos de PdeReciboItem */ 	
	static function getPdeReciboItemChequesXPdeReciboItems($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdeReciboItemCheque::GEN_ATTR_PDE_RECIBO_ITEM_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdeReciboItemCheque::GEN_TABLA);
            $c->addOrden(PdeReciboItemCheque::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
	...
	/* Método getPdeReciboItemCheques filtrado por una coleccion de objetos foraneos de GralBanco */ 	
	static function getPdeReciboItemChequesXGralBancos($os_foraneas){
            $os_con_indice = array();
            $arr_ids = array();
            ...
            $c= new Criterio();
            $c->add(PdeReciboItemCheque::GEN_ATTR_GRAL_BANCO_ID, $arr_ids, Criterio::IN_ARRAY);
            $c->addTabla(PdeReciboItemCheque::GEN_TABLA);
            $c->addOrden(PdeReciboItemCheque::GEN_ATTR_ID, Criterio::_ASC);
            $c->setCriterios();
	...
	static function getInfoBtnVolver($indice = false){
            $url = 'pde_recibo_item_cheque_adm.php';
            $url_gestion = 'pde_recibo_item_cheque_gestion.php';
    ...
    static function getCantidadTotalDeRegistros(){
            ...
            //$cons->setSQL('SELECT count(id) cantidad FROM pde_recibo_item_cheque'); // modificado por reltuples
            $cons->setSQL("SELECT reltuples AS cantidad FROM pg_class WHERE relname = 'pde_recibo_item_cheque';");
	...
	static function getOxId($valor){
            ...
            $obs = self::getPdeReciboItemCheques($paginador, $criterio);
            foreach($obs as $o){
                    return $o;
            }
            return false;
	...
	static function getOsxId($valor){
            ...
            $obs = self::getPdeReciboItemCheques(null, $criterio);
            return $obs;
	}
	...
	*/
	...
	static function getFiltrosArrGral($archivo = ''){
            if(trim($archivo) == ''){
                $arr = array('clase' => self::GEN_CLASE, 'tabla' => self::GEN_TABLA, 'archivo' => 'pde_recibo_item_cheque_adm');
            }else{
	...
	public function getModificadoDiferenciaDiasDescripcion(){
            ...
            static function getArrDescripcionsUnificado(){
                $c = new Criterio();
                $c->addTabla(PdeReciboItemCheque::GEN_TABLA);
                $c->addOrden(PdeReciboItemCheque::GEN_ATTR_DESCRIPCION, Criterio::_ASC);
                $c->setCriterios();

                $pde_recibo_item_cheques = PdeReciboItemCheque::getPdeReciboItemCheques(null, $c);

                $arr = array();
                foreach($pde_recibo_item_cheques as $pde_recibo_item_cheque){
                    $descripcion = $pde_recibo_item_cheque->getDescripcion();
