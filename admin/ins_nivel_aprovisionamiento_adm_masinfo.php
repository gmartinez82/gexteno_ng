...
$ins_nivel_aprovisionamiento_id = Gral::getVars(2, 'id');
$ins_nivel_aprovisionamiento = InsNivelAprovisionamiento::getOxId($ins_nivel_aprovisionamiento_id);
?>
<div class="masinfo-datos">

	<div class="par ">
            <div class="label"><?php Lang::_lang('Observaciones') ?></div>
            <div class="dato"><?php Gral::_echo(nl2br($ins_nivel_aprovisionamiento->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'INS_NIVEL_APROVISIONAMIENTO_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($ins_nivel_aprovisionamiento->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($ins_nivel_aprovisionamiento->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'INS_NIVEL_APROVISIONAMIENTO_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($ins_nivel_aprovisionamiento->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($ins_nivel_aprovisionamiento->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('INS_NIVEL_APROVISIONAMIENTO_MASINFO_INS_INSUMO')){ ?>
	...
	<div id="tab_ins_insumo" class="bloque-relacion ins_insumo">
            ...
            <h4><?php Lang::_lang('InsInsumo') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('InsNivelAprovisionamiento') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($ins_nivel_aprovisionamiento->getInsInsumosParaBloqueMasInfo() as $ins_insumo){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('INS_NIVEL_APROVISIONAMIENTO_MASINFO_INS_INSUMO_BOTONERA')){ ?>
                    <div class="botonera">
                        ...
                        <?php if($ins_insumo){ ?>
                            <div class="boton">
                                ...
                                <?php Lang::_lang('Ver InsInsumos de ') ?> <strong><?php echo($ins_nivel_aprovisionamiento->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
