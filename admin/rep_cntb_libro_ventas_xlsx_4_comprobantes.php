...
foreach ($gral_tipo_ivas_gravados as $gral_tipo_iva) {
    $cols[$columna++] = array('ancho' => 20, 'titulo' => 'BI ' . $gral_tipo_iva->getDescripcion(), 'indice' => $gral_tipo_iva->getCodigo());
    $cols[$columna++] = array('ancho' => 20, 'titulo' => 'Imp ' . $gral_tipo_iva->getDescripcion(), 'indice' => $gral_tipo_iva->getCodigo());
}

foreach ($vta_tributos as $vta_tributo) {
    $cols[$columna++] = array('ancho' => 20, 'titulo' => 'BI ' . $vta_tributo->getDescripcion(), 'indice' => $vta_tributo->getCodigo());
    $cols[$columna++] = array('ancho' => 20, 'titulo' => 'Imp ' . $vta_tributo->getDescripcion(), 'indice' => $vta_tributo->getCodigo());
}

$cols[$columna++] = array('ancho' => 20, 'titulo' => 'Total', 'indice' => 'total');
...
foreach ($vta_comprobantes as $vta_comprobante) {
    ...
    $tipo_comprobante = $vta_comprobante->getTipoComprobante();
    $fecha_emision = Gral::getFechaToWeb($vta_comprobante->getFechaEmision());
    $numero_comprobante = $vta_comprobante->getNumeroComprobanteCompleto();
    $cae = $vta_comprobante->getCae();
    //$razon_social = $vta_comprobante->getRazonSocial();
    $razon_social = $vta_comprobante->getPersonaDescripcion();
    //$cuit = $vta_comprobante->getCuit();
    $cuit = $vta_comprobante->getPersonaDocumento();
    ...
    foreach ($gral_tipo_ivas_gravados as $gral_tipo_iva) {
        $importe_iva_base_imponible = $vta_comprobante->getImporteIvaBaseImponible($gral_tipo_iva->getCodigo()) * $multiplicador;
        $importe_iva_importe = $vta_comprobante->getImporteIvaImporte($gral_tipo_iva->getCodigo()) * $multiplicador;
    ...
    foreach ($vta_tributos as $vta_tributo) {
        $importe_tributo_base_imponible = $vta_comprobante->getImporteTributoBaseImponible($vta_tributo->getCodigo()) * $multiplicador;
        $importe_tributo_importe = $vta_comprobante->getImporteTributoImporte($vta_tributo->getCodigo()) * $multiplicador;

        $importe_total_comprobante += $importe_tributo_importe;
        $arr_importe_tributo_total["BASE_IMPONIBLE_" . $vta_tributo->getCodigo()] += $importe_tributo_base_imponible;
        $arr_importe_tributo_total["IMPORTE_" . $vta_tributo->getCodigo()] += $importe_tributo_importe;
...
foreach ($vta_tributos as $vta_tributo) {
    $xls->getActiveSheet()->getCell($columna . $linea)->setValueExplicit($arr_importe_tributo_total["BASE_IMPONIBLE_" . $vta_tributo->getCodigo()], PHPExcel_Cell_DataType::TYPE_NUMERIC);
    $xls->getActiveSheet()->getStyle($columna . $linea)->getNumberFormat()->setFormatCode("$ #,##0.00");
    $xls->getActiveSheet()->getStyle($columna . $linea)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
    $columna++;
    
    $xls->getActiveSheet()->getCell($columna . $linea)->setValueExplicit($arr_importe_tributo_total["IMPORTE_" . $vta_tributo->getCodigo()], PHPExcel_Cell_DataType::TYPE_NUMERIC);
    $xls->getActiveSheet()->getStyle($columna . $linea)->getNumberFormat()->setFormatCode("$ #,##0.00");
    $xls->getActiveSheet()->getStyle($columna . $linea)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
    $columna++;    
}
