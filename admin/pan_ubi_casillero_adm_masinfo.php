...
$pan_ubi_casillero_id = Gral::getVars(2, 'id');
$pan_ubi_casillero = PanUbiCasillero::getOxId($pan_ubi_casillero_id);
?>
<div class="masinfo-datos">

	<div class="par ">
            <div class="label"><?php Lang::_lang('Observaciones') ?></div>
            <div class="dato"><?php Gral::_echo(nl2br($pan_ubi_casillero->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PAN_UBI_CASILLERO_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($pan_ubi_casillero->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($pan_ubi_casillero->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PAN_UBI_CASILLERO_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($pan_ubi_casillero->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($pan_ubi_casillero->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('PAN_UBI_CASILLERO_MASINFO_INS_INSUMO_PAN_PANOL')){ ?>
	...
	<div id="tab_ins_insumo_pan_panol" class="bloque-relacion ins_insumo_pan_panol">
            ...
            <h4><?php Lang::_lang('InsInsumoPanPanol') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('PanUbiCasillero') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($pan_ubi_casillero->getInsInsumoPanPanolsParaBloqueMasInfo() as $ins_insumo_pan_panol){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('PAN_UBI_CASILLERO_MASINFO_INS_INSUMO_PAN_PANOL_BOTONERA')){ ?>
                    <div class="botonera">
                        ...
                        <?php if($ins_insumo_pan_panol){ ?>
                            <div class="boton">
                                ...
                                <?php Lang::_lang('Ver InsInsumoPanPanols de ') ?> <strong><?php echo($pan_ubi_casillero->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
