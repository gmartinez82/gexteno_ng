...
$cntb_tipo_movimiento_id = Gral::getVars(2, 'id');
$cntb_tipo_movimiento = CntbTipoMovimiento::getOxId($cntb_tipo_movimiento_id);
?>
<div class="masinfo-datos">

	<div class="par ">
            <div class="label"><?php Lang::_lang('Observaciones') ?></div>
            <div class="dato"><?php Gral::_echo(nl2br($cntb_tipo_movimiento->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'CNTB_TIPO_MOVIMIENTO_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($cntb_tipo_movimiento->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($cntb_tipo_movimiento->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'CNTB_TIPO_MOVIMIENTO_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($cntb_tipo_movimiento->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($cntb_tipo_movimiento->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('CNTB_TIPO_MOVIMIENTO_MASINFO_CNTB_DIARIO_ASIENTO')){ ?>
	...
	<div id="tab_cntb_diario_asiento" class="bloque-relacion cntb_diario_asiento">
            ...
            <h4><?php Lang::_lang('CntbDiarioAsiento') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('CntbTipoMovimiento') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($cntb_tipo_movimiento->getCntbDiarioAsientosParaBloqueMasInfo() as $cntb_diario_asiento){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('CNTB_TIPO_MOVIMIENTO_MASINFO_CNTB_DIARIO_ASIENTO_BOTONERA')){ ?>
                    <div class="botonera">
                        ...
                        <?php if($cntb_diario_asiento){ ?>
                            <div class="boton">
                                ...
                                <?php Lang::_lang('Ver CntbDiarioAsientos de ') ?> <strong><?php echo($cntb_tipo_movimiento->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
