...
if(Gral::esPost()){
	...
	$ins_insumo_ins_etiqueta->setInsInsumoId(Gral::getVars(1, "dbsug_ins_insumo_id", null));
	$ins_insumo_ins_etiqueta->setInsEtiquetaId(Gral::getVars(1, "cmb_ins_etiqueta_id", null));
	$ins_insumo_ins_etiqueta->setCreado(Gral::getFechaHoraToDb(Gral::getVars(1, "txt_creado")));
	$ins_insumo_ins_etiqueta->setCreadoPor(Gral::getVars(1, "_creado_por", null));
	switch($accion){
		case 'guardar':
			...
			if(!$error->getExisteError()){
				...
				header('Location: ?cs=1&id='.$ins_insumo_ins_etiqueta->getId());
			}
		break;
		case 'guardarnvo':
			$error = $ins_insumo_ins_etiqueta->control();
	...
	if(trim($hdn_id) != ''){ 
		$ins_insumo_ins_etiqueta->setId($hdn_id);
	}else{
...
<div id='cuerpo' >
	...
	<div class='adm_cuerpo_titulo'><?php Lang::_lang('InsInsumoInsEtiquetas') ?> </div>
      ...
      <table width='100%' border='0' align='center'>
        <tr>
          <td align='right' class='adm_carga_datos_botones'>
            <input name='btn_listado' type='button' id='btn_listado' value='<?php Lang::_lang(InsInsumoInsEtiqueta::getInfoBtnVolver('label')) ?>' onclick='location.href="<?php Gral::_echo(InsInsumoInsEtiqueta::getInfoBtnVolver('url')) ?>"' />
...
<?php if(UsCredencial::getEsAcreditado('INS_INSUMO_INS_ETIQUETA_ALTA')){ ?>
        ...
        <table width='100%' border='0' cellspacing='10' class='adm_carga_datos' align='center' id='alta_ins_insumo_ins_etiqueta'>
            ...
            <tr>
                <td id="label_dbsug_ins_insumo_id" class="adm_carga_datos_titulos" width="" help="<?php echo 'help_'.$db_nombre_pagina.'_ins_insumo_id' ?>" >
                    ...
                    <?php if (UsCredencial::getEsAcreditado('GEN_ATRIBUTO_CONFIG')) { ?>
                        <div class='gen-atributos-config db_context' archivo='<?php Gral::_echo(Gral::getPath('path_http')) ?>admin/ajax/atributos/atributos_db_context_acciones.php?modulo=ins_insumo_ins_etiqueta_alta&atributo=ins_insumo_id' modulo_metodo_init='setInit()'>
                ...
                <td id="dato_dbsug_ins_insumo_id" class="adm_carga_datos_datos" width="">
                  <?php $error_input_css = ($error->getErrorXIndice('ins_insumo_id')->getMensaje()) ? 'error-mensaje-dbsug' : ''; ?>
                    <?php echo Html::html_get_dbsuggest(1, 'dbsug_ins_insumo', 'ajax/modulos/ins_insumo/ins_insumo_dbsuggest.php', false, true, '', 'Ingrese ...', $ins_insumo_ins_etiqueta->getInsInsumoId(), $ins_insumo_ins_etiqueta->getInsInsumo()->getDescripcion(), 40, false, $error_input_css) ?>
            ...
            <tr>
                <td id="label_cmb_ins_etiqueta_id" class="adm_carga_datos_titulos" width="" help="<?php echo 'help_'.$db_nombre_pagina.'_ins_etiqueta_id' ?>" >
                    ...
                    <?php if (UsCredencial::getEsAcreditado('GEN_ATRIBUTO_CONFIG')) { ?>
                        <div class='gen-atributos-config db_context' archivo='<?php Gral::_echo(Gral::getPath('path_http')) ?>admin/ajax/atributos/atributos_db_context_acciones.php?modulo=ins_insumo_ins_etiqueta_alta&atributo=ins_etiqueta_id' modulo_metodo_init='setInit()'>
                            <img src='imgs/btn_config.png' width='20' align='absmiddle' />
                        </div>
                    <?php } ?>
                ...
                <div class="div_botonera_cmb_alta_editar">
                   <img class="img_btn_editar" elemento_id="cmb_ins_etiqueta_id" clase_id="ins_etiqueta" prefijo='' src='imgs/btn_modi.gif' border='0' width='20' align='absmiddle' id="img_btn_editar_cmb_ins_etiqueta_id" <?php echo ($ins_insumo_ins_etiqueta->getInsEtiquetaId() == 'null') ? "style='display:none;'" : '' ?> />
		...
		<?php Html::html_dib_select(1, 'cmb_ins_etiqueta_id', Gral::getCmbFiltro(InsEtiqueta::getInsEtiquetasCmb(), '...'), $ins_insumo_ins_etiqueta->getInsEtiquetaId(), 'textbox '.$error_input_css) ?>
...
<?php } ?>
    ...
    <table width='100%' border='0' align='center'>
        <tr>
          <td align='right'  class='adm_carga_datos_botones'><input name='hdn_id' type='hidden' id='hdn_id' size='1' value='<?php Gral::_echoTxt($ins_insumo_ins_etiqueta->getId()) ?>'/>
    ...
    <?php if(trim($ins_insumo_ins_etiqueta->getId()) != ''){ ?>
