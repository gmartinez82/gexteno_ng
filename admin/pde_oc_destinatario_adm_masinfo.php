...
$pde_oc_destinatario_id = Gral::getVars(2, 'id');
$pde_oc_destinatario = PdeOcDestinatario::getOxId($pde_oc_destinatario_id);
?>
<div class="masinfo-datos">

	<div class="par ">
            <div class="label"><?php Lang::_lang('Observaciones') ?></div>
            <div class="dato"><?php Gral::_echo(nl2br($pde_oc_destinatario->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PDE_OC_DESTINATARIO_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($pde_oc_destinatario->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($pde_oc_destinatario->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PDE_OC_DESTINATARIO_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($pde_oc_destinatario->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($pde_oc_destinatario->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('PDE_OC_DESTINATARIO_MASINFO_PDE_OC_ENVIO_EMAIL')){ ?>
	...
	<div id="tab_pde_oc_envio_email" class="bloque-relacion pde_oc_envio_email">
            ...
            <h4><?php Lang::_lang('PdeOcEnvioEmail') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('PdeOcDestinatario') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($pde_oc_destinatario->getPdeOcEnvioEmailsParaBloqueMasInfo() as $pde_oc_envio_email){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('PDE_OC_DESTINATARIO_MASINFO_PDE_OC_ENVIO_EMAIL_BOTONERA')){ ?>
                    <div class="botonera">
                        ...
                        <?php if($pde_oc_envio_email){ ?>
                            <div class="boton">
                                ...
                                <?php Lang::_lang('Ver PdeOcEnvioEmails de ') ?> <strong><?php echo($pde_oc_destinatario->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
