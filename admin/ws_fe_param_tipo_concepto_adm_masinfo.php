...
$ws_fe_param_tipo_concepto_id = Gral::getVars(2, 'id');
$ws_fe_param_tipo_concepto = WsFeParamTipoConcepto::getOxId($ws_fe_param_tipo_concepto_id);
?>
<div class="masinfo-datos">

	<div class="par ">
            <div class="label"><?php Lang::_lang('Observaciones') ?></div>
            <div class="dato"><?php Gral::_echo(nl2br($ws_fe_param_tipo_concepto->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'WS_FE_PARAM_TIPO_CONCEPTO_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($ws_fe_param_tipo_concepto->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($ws_fe_param_tipo_concepto->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'WS_FE_PARAM_TIPO_CONCEPTO_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($ws_fe_param_tipo_concepto->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($ws_fe_param_tipo_concepto->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('WS_FE_PARAM_TIPO_CONCEPTO_MASINFO_WS_FE_OPE_SOLICITUD')){ ?>
	...
	<div id="tab_ws_fe_ope_solicitud" class="bloque-relacion ws_fe_ope_solicitud">
            ...
            <h4><?php Lang::_lang('WsFeOpeSolicitud') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('WsFeParamTipoConcepto') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($ws_fe_param_tipo_concepto->getWsFeOpeSolicitudsParaBloqueMasInfo() as $ws_fe_ope_solicitud){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('WS_FE_PARAM_TIPO_CONCEPTO_MASINFO_WS_FE_OPE_SOLICITUD_BOTONERA')){ ?>
                    <div class="botonera">
                        ...
                        <?php if($ws_fe_ope_solicitud){ ?>
                            <div class="boton">
                                ...
                                <?php Lang::_lang('Ver WsFeOpeSolicituds de ') ?> <strong><?php echo($ws_fe_param_tipo_concepto->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
