...
$pan_ubi_celda_id = Gral::getVars(2, 'id');
$pan_ubi_celda = PanUbiCelda::getOxId($pan_ubi_celda_id);
?>
<div class="masinfo-datos">

	<div class="par ">
            <div class="label"><?php Lang::_lang('Observaciones') ?></div>
            <div class="dato"><?php Gral::_echo(nl2br($pan_ubi_celda->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PAN_UBI_CELDA_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($pan_ubi_celda->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($pan_ubi_celda->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PAN_UBI_CELDA_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($pan_ubi_celda->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($pan_ubi_celda->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('PAN_UBI_CELDA_MASINFO_INS_INSUMO_PAN_PANOL')){ ?>
	...
	<div id="tab_ins_insumo_pan_panol" class="bloque-relacion ins_insumo_pan_panol">
            ...
            <h4><?php Lang::_lang('InsInsumoPanPanol') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('PanUbiCelda') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($pan_ubi_celda->getInsInsumoPanPanolsParaBloqueMasInfo() as $ins_insumo_pan_panol){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('PAN_UBI_CELDA_MASINFO_INS_INSUMO_PAN_PANOL_BOTONERA')){ ?>
                    <div class="botonera">
                            
                        <?php if($ins_insumo_pan_panol){ ?>
                            <div class="boton">
                                <a href="_init.php?arr_gral=<?php echo InsInsumoPanPanol::getFiltrosArrGral() ?>&arr=<?php echo $ins_insumo_pan_panol->getFiltrosArrXCampo('PanUbiCelda', 'pan_ubi_celda_id') ?>" >
                                <?php Lang::_lang('Ver InsInsumoPanPanols de ') ?> <strong><?php echo($pan_ubi_celda->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
