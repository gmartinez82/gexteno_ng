...
$fnd_caja_egreso_id = Gral::getVars(2, 'id');
...
<div class="masinfo-datos">
	...
	<div class="par ">
            ...
            <div class="dato"><?php Gral::_echo(nl2br($fnd_caja_egreso->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'FND_CAJA_EGRESO_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($fnd_caja_egreso->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($fnd_caja_egreso->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'FND_CAJA_EGRESO_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($fnd_caja_egreso->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($fnd_caja_egreso->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('FND_CAJA_EGRESO_MASINFO_FND_CHQ_CHEQUE')){ ?>
	...
	<div id="tab_fnd_chq_cheque" class="bloque-relacion fnd_chq_cheque">
            ...
            <h4><?php Lang::_lang('FndChqCheque') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('FndCajaEgreso') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($fnd_caja_egreso->getFndChqChequesParaBloqueMasInfo() as $fnd_chq_cheque){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('FND_CAJA_EGRESO_MASINFO_FND_CHQ_CHEQUE_BOTONERA')){ ?>
                    <div class="botonera">
                            
                        <?php if($fnd_chq_cheque){ ?>
                            <div class="boton">
                                <a href="_init.php?arr_gral=<?php echo FndChqCheque::getFiltrosArrGral() ?>&arr=<?php echo $fnd_chq_cheque->getFiltrosArrXCampo('FndCajaEgreso', 'fnd_caja_egreso_id') ?>" >
                                <?php Lang::_lang('Ver FndChqCheques de ') ?> <strong><?php echo($fnd_caja_egreso->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
