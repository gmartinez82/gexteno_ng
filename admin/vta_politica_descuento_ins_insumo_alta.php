...
if(Gral::esPost()){
	...
	$vta_politica_descuento_ins_insumo->setVtaPoliticaDescuentoId(Gral::getVars(1, "cmb_vta_politica_descuento_id", null));
	$vta_politica_descuento_ins_insumo->setInsInsumoId(Gral::getVars(1, "cmb_ins_insumo_id", null));
	$vta_politica_descuento_ins_insumo->setCreado(Gral::getFechaHoraToDb(Gral::getVars(1, "txt_creado")));
	$vta_politica_descuento_ins_insumo->setCreadoPor(Gral::getVars(1, "_creado_por", null));
	switch($accion){
		case 'guardar':
			...
			if(!$error->getExisteError()){
				...
				header('Location: ?cs=1&id='.$vta_politica_descuento_ins_insumo->getId());
			}
		break;
		case 'guardarnvo':
			$error = $vta_politica_descuento_ins_insumo->control();
	...
	if(trim($hdn_id) != ''){ 
		$vta_politica_descuento_ins_insumo->setId($hdn_id);
	}else{
      ...
      <table width='100%' border='0' align='center'>
        <tr>
          <td align='right' class='adm_carga_datos_botones'>
            <input name='btn_listado' type='button' id='btn_listado' value='<?php Lang::_lang(VtaPoliticaDescuentoInsInsumo::getInfoBtnVolver('label')) ?>' onclick='location.href="<?php Gral::_echo(VtaPoliticaDescuentoInsInsumo::getInfoBtnVolver('url')) ?>"' />
...
<?php if(UsCredencial::getEsAcreditado('VTA_POLITICA_DESCUENTO_INS_INSUMO_ALTA')){ ?>
        ...
        <table width='100%' border='0' cellspacing='10' class='adm_carga_datos' align='center' id='alta_vta_politica_descuento_ins_insumo'>
            ...
            <tr>
                <td id="label_cmb_vta_politica_descuento_id" class="adm_carga_datos_titulos" width="" help="<?php echo 'help_'.$db_nombre_pagina.'_vta_politica_descuento_id' ?>" >
                    ...
                    <?php if (UsCredencial::getEsAcreditado('GEN_ATRIBUTO_CONFIG')) { ?>
                        <div class='gen-atributos-config db_context' archivo='<?php Gral::_echo(Gral::getPath('path_http')) ?>admin/ajax/atributos/atributos_db_context_acciones.php?modulo=vta_politica_descuento_ins_insumo_alta&atributo=vta_politica_descuento_id' modulo_metodo_init='setInit()'>
		...
		<?php Html::html_dib_select(1, 'cmb_vta_politica_descuento_id', Gral::getCmbFiltro(VtaPoliticaDescuento::getVtaPoliticaDescuentosCmb(), '...'), $vta_politica_descuento_ins_insumo->getVtaPoliticaDescuentoId(), 'textbox '.$error_input_css) ?>
            ...
            <tr>
                <td id="label_cmb_ins_insumo_id" class="adm_carga_datos_titulos" width="" help="<?php echo 'help_'.$db_nombre_pagina.'_ins_insumo_id' ?>" >
                    ...
                    <?php if (UsCredencial::getEsAcreditado('GEN_ATRIBUTO_CONFIG')) { ?>
                        <div class='gen-atributos-config db_context' archivo='<?php Gral::_echo(Gral::getPath('path_http')) ?>admin/ajax/atributos/atributos_db_context_acciones.php?modulo=vta_politica_descuento_ins_insumo_alta&atributo=ins_insumo_id' modulo_metodo_init='setInit()'>
                            <img src='imgs/btn_config.png' width='20' align='absmiddle' />
                        </div>
                    <?php } ?>
                ...
                <div class="div_botonera_cmb_alta_editar">
                   <img class="img_btn_editar" elemento_id="cmb_ins_insumo_id" clase_id="ins_insumo" prefijo='' src='imgs/btn_modi.gif' border='0' width='20' align='absmiddle' id="img_btn_editar_cmb_ins_insumo_id" <?php echo ($vta_politica_descuento_ins_insumo->getInsInsumoId() == 'null') ? "style='display:none;'" : '' ?> />
		...
		<?php Html::html_dib_select(1, 'cmb_ins_insumo_id', Gral::getCmbFiltro(InsInsumo::getInsInsumosCmb(), '...'), $vta_politica_descuento_ins_insumo->getInsInsumoId(), 'textbox '.$error_input_css) ?>
...
<?php } ?>
    ...
    <table width='100%' border='0' align='center'>
        <tr>
          <td align='right'  class='adm_carga_datos_botones'><input name='hdn_id' type='hidden' id='hdn_id' size='1' value='<?php Gral::_echoTxt($vta_politica_descuento_ins_insumo->getId()) ?>'/>
    ...
    <?php if(trim($vta_politica_descuento_ins_insumo->getId()) != ''){ ?>
