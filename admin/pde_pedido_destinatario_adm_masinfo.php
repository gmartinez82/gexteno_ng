...
$pde_pedido_destinatario_id = Gral::getVars(2, 'id');
$pde_pedido_destinatario = PdePedidoDestinatario::getOxId($pde_pedido_destinatario_id);
?>
<div class="masinfo-datos">

	<div class="par ">
            <div class="label"><?php Lang::_lang('Observaciones') ?></div>
            <div class="dato"><?php Gral::_echo(nl2br($pde_pedido_destinatario->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PDE_PEDIDO_DESTINATARIO_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($pde_pedido_destinatario->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($pde_pedido_destinatario->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PDE_PEDIDO_DESTINATARIO_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($pde_pedido_destinatario->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($pde_pedido_destinatario->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('PDE_PEDIDO_DESTINATARIO_MASINFO_PDE_PEDIDO_ENVIO_EMAIL')){ ?>
	...
	<div id="tab_pde_pedido_envio_email" class="bloque-relacion pde_pedido_envio_email">
            ...
            <h4><?php Lang::_lang('PdePedidoEnvioEmail') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('PdePedidoDestinatario') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($pde_pedido_destinatario->getPdePedidoEnvioEmailsParaBloqueMasInfo() as $pde_pedido_envio_email){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('PDE_PEDIDO_DESTINATARIO_MASINFO_PDE_PEDIDO_ENVIO_EMAIL_BOTONERA')){ ?>
                    <div class="botonera">
                        ...
                        <?php if($pde_pedido_envio_email){ ?>
                            <div class="boton">
                                ...
                                <?php Lang::_lang('Ver PdePedidoEnvioEmails de ') ?> <strong><?php echo($pde_pedido_destinatario->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
