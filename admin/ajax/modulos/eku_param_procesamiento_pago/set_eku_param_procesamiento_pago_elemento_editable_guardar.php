...
if(UsCredencial::getEsAcreditado('EKU_PARAM_PROCESAMIENTO_PAGO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamProcesamientoPago::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_procesamiento_pago = EkuParamProcesamientoPago::getOxId($identificador);
    if($eku_param_procesamiento_pago){
        if($eku_param_procesamiento_pago->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_procesamiento_pago->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_procesamiento_pago->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_procesamiento_pago->saveDesdeListado($campo, $valor);
            }
