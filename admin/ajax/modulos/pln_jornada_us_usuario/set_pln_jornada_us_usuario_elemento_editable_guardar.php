...
if(UsCredencial::getEsAcreditado('PLN_JORNADA_US_USUARIO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PlnJornadaUsUsuario::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($pln_jornada_us_usuario){
        if($pln_jornada_us_usuario->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$pln_jornada_us_usuario->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $pln_jornada_us_usuario->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $pln_jornada_us_usuario->saveDesdeListado($campo, $valor);
            }
