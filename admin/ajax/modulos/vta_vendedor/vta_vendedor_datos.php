...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/vta_vendedor.php' ?>

<?php if(count($vta_vendedors) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_vta_vendedor'>
  ...
  <tr>
	...
	<td width='140' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=vta_tipo_vendedor_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('VtaTipoVendedor') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='vta_tipo_vendedor_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_vta_vendedor_uno_<?php Gral::_echo($vta_vendedor->getId()) ?>" class="uno" identificador="<?php Gral::_echo($vta_vendedor->getId()) ?>">
  	<?php include "vta_vendedor_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($vta_vendedor->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($vta_vendedor->getId()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($vta_vendedor->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($vta_vendedor->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($vta_vendedor->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $vta_vendedor->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($vta_vendedor->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='7' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $vta_vendedor->getId()){ 
			include "vta_vendedor_adm_masinfo.php";
		}
		?>
	</div>
