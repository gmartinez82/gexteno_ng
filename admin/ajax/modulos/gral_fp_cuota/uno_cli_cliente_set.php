...
if(in_array($cli_cliente_relacionado->getId(), $o_ids)){
    ...
    $array = array(
        array('campo' => 'gral_fp_cuota_id', 'valor' => $o_padre->getId()),
...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('GRAL_FP_CUOTA_ALTA_RELACION_CLI_CLIENTE_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_cli_cliente_id_<?php echo $cli_cliente_relacionado->getId() ?>' name='chk_cli_cliente[<?php echo $cli_cliente_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $cli_cliente_relacionado->getId() ?>' relacion_id='<?php echo $cli_cliente_gral_fp_cuota_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('GRAL_FP_CUOTA_ALTA_RELACION_CLI_CLIENTE_ACCIONES_EDITAR')){ ?>	
        <?php if($cli_cliente_gral_fp_cuota_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/cli_cliente_gral_fp_cuota/cli_cliente_gral_fp_cuota_alta.php?id=<?php Gral::_echo($cli_cliente_gral_fp_cuota_id) ?>" contenedor="div_modal" ancho="750" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($cli_cliente_relacionado->getId()) ?>, 'cli_cliente', 'gral_fp_cuota', <?php Gral::_echo($o_padre->getId()) ?>, 'GralFpCuota', 'CliClienteGralFpCuota')" title="Editar CliClienteGralFpCuota"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
