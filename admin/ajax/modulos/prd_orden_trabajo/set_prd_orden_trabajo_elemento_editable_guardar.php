...
if(UsCredencial::getEsAcreditado('PRD_ORDEN_TRABAJO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdOrdenTrabajo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_orden_trabajo = PrdOrdenTrabajo::getOxId($identificador);
    if($prd_orden_trabajo){
        if($prd_orden_trabajo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_orden_trabajo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_orden_trabajo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_orden_trabajo->saveDesdeListado($campo, $valor);
            }
