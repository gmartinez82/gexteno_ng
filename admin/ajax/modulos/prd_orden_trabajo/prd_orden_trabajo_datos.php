...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/prd_orden_trabajo.php' ?>

<?php if(count($prd_orden_trabajos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_prd_orden_trabajo'>
    <thead>
    <tr>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=prd_tipo_origen_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('PrdTipoOrigen') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='prd_tipo_origen_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=prd_prioridad_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('PrdPrioridad') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='prd_prioridad_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  ?>
    ...
    <tr id="tr_prd_orden_trabajo_uno_<?php Gral::_echo($prd_orden_trabajo->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($prd_orden_trabajo->getId()) ?>" hash="<?php Gral::_echo($prd_orden_trabajo->getHash()) ?>" >
  	<?php include "prd_orden_trabajo_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($prd_orden_trabajo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($prd_orden_trabajo->getId()) ?>" hash="<?php Gral::_echo($prd_orden_trabajo->getHash()) ?>" >
