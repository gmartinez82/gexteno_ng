...
$arr_comprobantes_vinculados_por_conciliacion = $vta_recibo->getVtaComprobantesVinculadosPorConciliacion();
    ...
    <table class="listado" id="listado_adm_vta_recibo_gestion" border="0" align="center">
        ...
        foreach ($arr_comprobantes_vinculados_por_conciliacion['INTERMEDIO'] as $i => $arr_comprobante_vinculado_por_conciliacion) {
            ...
            if (get_class($vta_comprobante) == 'VtaFactura') {
                $vta_comprobante_vta_nota_creditos_vinculadas = $vta_comprobante->getVtaFacturaVtaNotaCreditos(null, null, true);
            }
            if (get_class($vta_comprobante) == 'VtaNotaDebito') {
                $vta_comprobante_vta_nota_creditos_vinculadas = $vta_comprobante->getVtaNotaDebitoVtaNotaCreditos(null, null, true);
            }
            ?>
