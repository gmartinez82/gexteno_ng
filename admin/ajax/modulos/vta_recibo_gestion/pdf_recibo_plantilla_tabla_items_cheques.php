...
<?php if (is_array($arr_vta_recibo_item_cheque) && count($arr_vta_recibo_item_cheque) > 0) { ?>
    <br />
    ...
    <table class="listado" id="listado_adm_vta_recibo_gestion" border="0" align="center">
        ...
        <tbody>
            ...
            <?php
            $cont = 0;
            foreach ($arr_vta_recibo_item_cheque as $i => $vta_recibo_item_cheque) {
                $cont++;
                ...
                <tr id="tr_vta_presupuesto_gestion_uno_<?php echo $vta_recibo_item->getId() ?>" class="uno" identificador="<?php echo $vta_recibo_item->getId() ?>">
                    ...
                    <td class="adm_tbl_lineas <?php echo $css_bg ?>" align="left">
                        <label class="importe-total">
                            <?php Gral::_echo(substr($vta_recibo_item_cheque->getEntregador(), 0, 15)) ?>
                        </label>
                    </td>

                    <td class="adm_tbl_lineas <?php echo $css_bg ?>" align="left">
                        <label class="importe-total">
                            <?php Gral::_echo(substr($vta_recibo_item_cheque->getFirmante(), 0, 15)) ?>
                        </label>
                    </td>

                    <td class="adm_tbl_lineas <?php echo $css_bg ?>" align="right">
                        <label class="importe-total">
                            <?php Gral::_echoImp($vta_recibo_item_cheque->getVtaReciboItem()->getImporteUnitario()) ?>
                        </label>
                    </td>
