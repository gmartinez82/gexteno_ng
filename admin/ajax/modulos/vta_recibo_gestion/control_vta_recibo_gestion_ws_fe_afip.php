...
$arr_vta_recibo_cheque_infos = Gral::getSes($var_sesion_modulo);
...
if (is_null($cmb_vta_recibo_concepto_ids)) {
    $arr["error_general"] .= Lang::_lang("Seleccionar un concepto", true) . '<br />';
    $arr["error"] = true;
} else {
    foreach ($cmb_vta_recibo_concepto_ids as $key => $cmb_vta_recibo_concepto_id) {
        if ($cmb_vta_recibo_concepto_id == '') {
            $arr["cmb_vta_recibo_concepto_id_" . $key] = Lang::_lang("Seleccionar un concepto", true);
            $arr["error"] = true;
        } else {
            ...
            if ($vta_recibo_concepto->getEsRetencion()) {
                ...
                if (is_null($arr_vta_recibo_retencion_infos[$key])) {
                    $arr["cmb_vta_recibo_concepto_id_" . $key] = Lang::_lang("Indique los datos de la Retencion", true);
                    $arr["error"] = true;
                }
            }
        }
...
if (is_null($txt_descripcions)) {
    $arr["error_general"] .= Lang::_lang("Agregue una descripcion del Item", true) . '<br />';
    $arr["error"] = true;
} else {
...
if (is_null($txt_referencias)) {
    $arr["error_general"] .= Lang::_lang("Agregue una referencia del Item", true) . '<br />';
    $arr["error"] = true;
} else {
...
if (is_null($cmb_gral_fp_forma_pago_ids)) {
    $arr["error_general"] .= Lang::_lang("Seleccionar forma de pago", true) . '<br />';
    $arr["error"] = true;
} else {
    foreach ($cmb_gral_fp_forma_pago_ids as $key => $cmb_gral_fp_forma_pago_id) {
        if ($cmb_gral_fp_forma_pago_id == '') {
            ...
            /* }else{
              ...
              if($gral_fp_forma_pago && $gral_fp_forma_pago->getRequiereInfoExtendida()){
