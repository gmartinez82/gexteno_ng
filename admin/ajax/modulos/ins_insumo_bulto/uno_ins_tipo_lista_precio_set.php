...
if(in_array($ins_tipo_lista_precio_relacionado->getId(), $o_ids)){
    ...
    $array = array(
        array('campo' => 'ins_insumo_bulto_id', 'valor' => $o_padre->getId()),
...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('INS_INSUMO_BULTO_ALTA_RELACION_INS_TIPO_LISTA_PRECIO_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_ins_tipo_lista_precio_id_<?php echo $ins_tipo_lista_precio_relacionado->getId() ?>' name='chk_ins_tipo_lista_precio[<?php echo $ins_tipo_lista_precio_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $ins_tipo_lista_precio_relacionado->getId() ?>' relacion_id='<?php echo $ins_insumo_bulto_ins_tipo_lista_precio_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('INS_INSUMO_BULTO_ALTA_RELACION_INS_TIPO_LISTA_PRECIO_ACCIONES_EDITAR')){ ?>	
        <?php if($ins_insumo_bulto_ins_tipo_lista_precio_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/ins_insumo_bulto_ins_tipo_lista_precio/ins_insumo_bulto_ins_tipo_lista_precio_alta.php?id=<?php Gral::_echo($ins_insumo_bulto_ins_tipo_lista_precio_id) ?>" contenedor="div_modal" ancho="950" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($ins_tipo_lista_precio_relacionado->getId()) ?>, 'ins_tipo_lista_precio', 'ins_insumo_bulto', <?php Gral::_echo($o_padre->getId()) ?>, 'InsInsumoBulto', 'InsInsumoBultoInsTipoListaPrecio')" title="Editar InsInsumoBultoInsTipoListaPrecio"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
