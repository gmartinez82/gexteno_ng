...
if(UsCredencial::getEsAcreditado('EKU_DE_G001_G_CAM_GEN_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeG001GCamGen::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_g001_g_cam_gen){
        if($eku_de_g001_g_cam_gen->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_g001_g_cam_gen->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_g001_g_cam_gen->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_g001_g_cam_gen->saveDesdeListado($campo, $valor);
            }
