...
if(UsCredencial::getEsAcreditado('US_USUARIO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = UsUsuario::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $us_usuario = UsUsuario::getOxId($identificador);
    if($us_usuario){
        if($us_usuario->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$us_usuario->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $us_usuario->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $us_usuario->saveDesdeListado($campo, $valor);
            }
