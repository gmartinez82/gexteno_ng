...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('US_USUARIO_ALTA_RELACION_GRAL_CENTRO_COSTO_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_gral_centro_costo_id_<?php echo $gral_centro_costo_relacionado->getId() ?>' name='chk_gral_centro_costo[<?php echo $gral_centro_costo_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $gral_centro_costo_relacionado->getId() ?>' relacion_id='<?php echo $gral_centro_costo_us_usuario_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('US_USUARIO_ALTA_RELACION_GRAL_CENTRO_COSTO_ACCIONES_EDITAR')){ ?>	
        <?php if($gral_centro_costo_us_usuario_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/gral_centro_costo_us_usuario/gral_centro_costo_us_usuario_alta.php?id=<?php Gral::_echo($gral_centro_costo_us_usuario_id) ?>" contenedor="div_modal" ancho="950" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($gral_centro_costo_relacionado->getId()) ?>, 'gral_centro_costo', 'us_usuario', <?php Gral::_echo($o_padre->getId()) ?>, 'UsUsuario', 'GralCentroCostoUsUsuario')" title="Editar GralCentroCostoUsUsuario"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
