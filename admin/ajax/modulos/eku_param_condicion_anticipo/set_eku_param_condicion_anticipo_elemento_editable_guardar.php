...
if(UsCredencial::getEsAcreditado('EKU_PARAM_CONDICION_ANTICIPO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamCondicionAnticipo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_condicion_anticipo = EkuParamCondicionAnticipo::getOxId($identificador);
    if($eku_param_condicion_anticipo){
        if($eku_param_condicion_anticipo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_condicion_anticipo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_condicion_anticipo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_condicion_anticipo->saveDesdeListado($campo, $valor);
            }
