...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/prv_tipo.php' ?>

<?php if(count($prv_tipos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_prv_tipo'>
  ...
  foreach($prv_tipos as $prv_tipo){ 
  	$estado = ($prv_tipo->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

  <tr id="tr_prv_tipo_uno_<?php Gral::_echo($prv_tipo->getId()) ?>" class="uno" identificador="<?php Gral::_echo($prv_tipo->getId()) ?>">
  	<?php include "prv_tipo_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($prv_tipo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($prv_tipo->getId()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($prv_tipo->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($prv_tipo->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($prv_tipo->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $prv_tipo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($prv_tipo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='5' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $prv_tipo->getId()){ 
			include "prv_tipo_adm_masinfo.php";
		}
		?>
	</div>
