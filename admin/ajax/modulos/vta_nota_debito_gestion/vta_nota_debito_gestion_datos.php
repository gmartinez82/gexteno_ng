...
<?php include Gral::getPathAbs() . 'admin/parciales/buscadores/activos/vta_nota_debito_gestion.php' ?>

<?php if (count($vta_nota_debitos) > 0) { ?>

    <table border='0' align='center' class='listado' id='listado_adm_vta_nota_debito_gestion'>
        ...
        foreach ($vta_nota_debitos as $vta_nota_debito) {
            ...
            <tr id="tr_vta_nota_debito_gestion_uno_<?php Gral::_echo($vta_nota_debito->getId()) ?>" class="uno" identificador="<?php Gral::_echo($vta_nota_debito->getId()) ?>">
                <?php include "vta_nota_debito_gestion_uno.php" ?>
            </tr>
            ...
            <tr id='tr_eliminar_<?php Gral::_echo($vta_nota_debito->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($vta_nota_debito->getId()) ?>" >
                <td colspan='14' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($vta_nota_debito->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
                        <br />
            ...
            <tr id='tr_mi_<?php Gral::_echo($vta_nota_debito->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
                <td colspan='14' align='center' class='adm_tbl_lineas'>
                    ...
                    <div class="masinfo">
                        <?php
                        if (trim($id) == $vta_nota_debito->getId()) {
                            include "vta_nota_debito_gestion_adm_masinfo.php";
                        }
                        ?>
                    </div>
