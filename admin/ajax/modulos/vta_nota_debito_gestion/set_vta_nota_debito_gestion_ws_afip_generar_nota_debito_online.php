...
if (is_null($vta_nota_credito_ids) && is_null($vta_factura_ids)) {
...
if (is_null($cmb_vta_nota_debito_concepto_ids)) {
    $arr["error_general"] .= Lang::_lang(" Debe seleccionar un concepto", true);
    $arr["error"] = true;
} else {
    foreach ($cmb_vta_nota_debito_concepto_ids as $key => $cmb_vta_nota_debito_concepto_id) {
        if ($cmb_vta_nota_debito_concepto_id == '') {
            $arr["cmb_vta_nota_debito_concepto_id_" . $key] = Lang::_lang("Debe seleccionar un tipo de concepto", true);
            $arr["error"] = true;
        }
    }
}
...
if (!$arr["error"]) {
    $vta_nota_debito = VtaNotaDebito::setInicializarVtaNotaDebitoItem($cmb_vta_preventista_id, $gral_actividad_id, $gral_escenario_id, $gral_fp_forma_pago_id, $txt_fecha_vencimiento, $gral_empresa_id, $vta_punto_venta_id, $txt_nro_timbrado, $cli_cliente_id, $txt_cantidads, $txt_descripcions, $txt_importe_unitarios, $cmb_gral_tipo_iva_ids, $cmb_vta_nota_debito_concepto_ids, $cmb_aplica_percepcion_iibbs, $txa_nota_publica, $txa_observacion, $chk_tributo_omitir_minimo, $vta_nota_credito_ids, $vta_factura_ids);
}
