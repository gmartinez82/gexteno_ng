...
if(UsCredencial::getEsAcreditado('GRAL_MEDIO_PAGO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralMedioPago::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $gral_medio_pago = GralMedioPago::getOxId($identificador);
    if($gral_medio_pago){
        if($gral_medio_pago->getHash() == $hash){
            
            // -----------------------------------------------------------------
            // Se ejecutan controles
            // -----------------------------------------------------------------
            eval("\$gral_medio_pago->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_medio_pago->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_medio_pago->saveDesdeListado($campo, $valor);
            }
