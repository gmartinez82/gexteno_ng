...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_AFECTACION_TRIBUTARIA_GRAL_TIPO_IVA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoAfectacionTributariaGralTipoIva::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_tipo_afectacion_tributaria_gral_tipo_iva){
        if($eku_param_tipo_afectacion_tributaria_gral_tipo_iva->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_afectacion_tributaria_gral_tipo_iva->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_afectacion_tributaria_gral_tipo_iva->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_afectacion_tributaria_gral_tipo_iva->saveDesdeListado($campo, $valor);
            }
