...
if(UsCredencial::getEsAcreditado('PER_PERSONA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerPersona::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_persona = PerPersona::getOxId($identificador);
    if($per_persona){
        if($per_persona->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_persona->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_persona->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_persona->saveDesdeListado($campo, $valor);
            }
