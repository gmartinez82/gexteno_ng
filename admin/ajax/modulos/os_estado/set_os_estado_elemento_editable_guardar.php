...
if(UsCredencial::getEsAcreditado('OS_ESTADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = OsEstado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $os_estado = OsEstado::getOxId($identificador);
    if($os_estado){
        if($os_estado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$os_estado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $os_estado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $os_estado->saveDesdeListado($campo, $valor);
            }
