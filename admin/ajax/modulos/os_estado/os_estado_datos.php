...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/os_estado.php' ?>

<?php if(count($os_estados) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_os_estado'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_os_estado_uno_<?php Gral::_echo($os_estado->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($os_estado->getId()) ?>" hash="<?php Gral::_echo($os_estado->getHash()) ?>" >
  	<?php include "os_estado_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($os_estado->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($os_estado->getId()) ?>" hash="<?php Gral::_echo($os_estado->getHash()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($os_estado->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($os_estado->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($os_estado->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $os_estado->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($os_estado->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='7' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $os_estado->getId()){ 
			include "os_estado_adm_masinfo.php";
		}
		?>
	</div>
