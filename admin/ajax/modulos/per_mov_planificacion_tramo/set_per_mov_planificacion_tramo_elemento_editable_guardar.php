...
if(UsCredencial::getEsAcreditado('PER_MOV_PLANIFICACION_TRAMO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerMovPlanificacionTramo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_mov_planificacion_tramo = PerMovPlanificacionTramo::getOxId($identificador);
    if($per_mov_planificacion_tramo){
        if($per_mov_planificacion_tramo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_mov_planificacion_tramo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_mov_planificacion_tramo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_mov_planificacion_tramo->saveDesdeListado($campo, $valor);
            }
