...
if(UsCredencial::getEsAcreditado('EKU_PARAM_GEO_CIUDAD_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamGeoCiudad::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_geo_ciudad = EkuParamGeoCiudad::getOxId($identificador);
    if($eku_param_geo_ciudad){
        if($eku_param_geo_ciudad->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_geo_ciudad->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_geo_ciudad->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_geo_ciudad->saveDesdeListado($campo, $valor);
            }
