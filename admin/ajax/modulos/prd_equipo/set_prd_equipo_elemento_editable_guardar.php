...
if(UsCredencial::getEsAcreditado('PRD_EQUIPO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdEquipo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_equipo = PrdEquipo::getOxId($identificador);
    if($prd_equipo){
        if($prd_equipo->getHash() == $hash){
            
            // -----------------------------------------------------------------
            // Se ejecutan controles
            // -----------------------------------------------------------------
            eval("\$prd_equipo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_equipo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_equipo->saveDesdeListado($campo, $valor);
            }
