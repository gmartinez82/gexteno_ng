...
if(UsCredencial::getEsAcreditado('EKU_DE_E300_G_DTIP_D_E_G_CAM_A_E_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE300GDtipDEGCamAE::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_e300_g_dtip_d_e_g_cam_a_e = EkuDeE300GDtipDEGCamAE::getOxId($identificador);
    if($eku_de_e300_g_dtip_d_e_g_cam_a_e){
        if($eku_de_e300_g_dtip_d_e_g_cam_a_e->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e300_g_dtip_d_e_g_cam_a_e->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e300_g_dtip_d_e_g_cam_a_e->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e300_g_dtip_d_e_g_cam_a_e->saveDesdeListado($campo, $valor);
            }
