...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/ins_insumo_ins_marca.php' ?>

<?php if(count($ins_insumo_ins_marcas) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_ins_insumo_ins_marca'>
  ...
  <tr id="tr_ins_insumo_ins_marca_uno_<?php Gral::_echo($ins_insumo_ins_marca->getId()) ?>" class="uno" identificador="<?php Gral::_echo($ins_insumo_ins_marca->getId()) ?>">
  	<?php include "ins_insumo_ins_marca_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($ins_insumo_ins_marca->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($ins_insumo_ins_marca->getId()) ?>" >
	<td colspan='6' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($ins_insumo_ins_marca->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($ins_insumo_ins_marca->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $ins_insumo_ins_marca->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($ins_insumo_ins_marca->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $ins_insumo_ins_marca->getId()){ 
			include "ins_insumo_ins_marca_adm_masinfo.php";
		}
		?>
	</div>
