...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/gral_ruta_geo_localidad.php' ?>

<?php if(count($gral_ruta_geo_localidads) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_gral_ruta_geo_localidad'>
  ...
  <tr id="tr_gral_ruta_geo_localidad_uno_<?php Gral::_echo($gral_ruta_geo_localidad->getId()) ?>" class="uno" identificador="<?php Gral::_echo($gral_ruta_geo_localidad->getId()) ?>">
  	<?php include "gral_ruta_geo_localidad_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($gral_ruta_geo_localidad->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($gral_ruta_geo_localidad->getId()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'>
            <div id='div_eliminar_<?php Gral::_echo($gral_ruta_geo_localidad->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
                <br />
                <div>
                      <input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($gral_ruta_geo_localidad->getId()) ?>,0)'/>
                </div>
            </div>		
        </td>
	<td align='center'></td>
...
if(trim($id) == $gral_ruta_geo_localidad->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($gral_ruta_geo_localidad->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='7' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $gral_ruta_geo_localidad->getId()){ 
			include "gral_ruta_geo_localidad_adm_masinfo.php";
		}
		?>
	</div>
