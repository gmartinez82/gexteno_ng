...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_CONDICION_OPERACION_GRAL_FP_FORMA_PAGO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoCondicionOperacionGralFpFormaPago::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_tipo_condicion_operacion_gral_fp_forma_pago){
        if($eku_param_tipo_condicion_operacion_gral_fp_forma_pago->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_condicion_operacion_gral_fp_forma_pago->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_condicion_operacion_gral_fp_forma_pago->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_condicion_operacion_gral_fp_forma_pago->saveDesdeListado($campo, $valor);
            }
