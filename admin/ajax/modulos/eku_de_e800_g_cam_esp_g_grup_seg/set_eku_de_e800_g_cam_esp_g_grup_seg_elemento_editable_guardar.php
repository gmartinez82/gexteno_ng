...
if(UsCredencial::getEsAcreditado('EKU_DE_E800_G_CAM_ESP_G_GRUP_SEG_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE800GCamEspGGrupSeg::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_e800_g_cam_esp_g_grup_seg = EkuDeE800GCamEspGGrupSeg::getOxId($identificador);
    if($eku_de_e800_g_cam_esp_g_grup_seg){
        if($eku_de_e800_g_cam_esp_g_grup_seg->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e800_g_cam_esp_g_grup_seg->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e800_g_cam_esp_g_grup_seg->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e800_g_cam_esp_g_grup_seg->saveDesdeListado($campo, $valor);
            }
