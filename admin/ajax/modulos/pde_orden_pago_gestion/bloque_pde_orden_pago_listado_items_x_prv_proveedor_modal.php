...
if (count($txt_descripcions) > 0) {
    ?>
    <table border='0' align='center' class='listado_pde_orden_pago_items' id='listado_pde_orden_pago_items_confirmacion' multiseleccion = "<?php echo $control_presupuesto ?>">
        <tr>
        ...
        foreach ($txt_descripcions as $key => $txt_descripcion) {
            ...
            $pde_orden_pago_concepto_id = $cmb_pde_orden_pago_concepto_ids[$key];
            $pde_orden_pago_concepto = PdeOrdenPagoConcepto::getOxId($pde_orden_pago_concepto_id);
            ...
            ?>
            <tr id="tr_pde_orden_pago_uno_<?php Gral::_echo($key) ?>" class="uno">
                <?php //include "bloque_modal_pde_orden_pago_listado_uno.php"  ?>

                <td align="center">
                    <div class="pde_orden_pago_concepto" id="cmb_pde_orden_pago_concepto_id[<?php echo $key ?>]" name="cmb_pde_orden_pago_concepto_id[<?php echo $key ?>]">
                        <?php Gral::_echo($pde_orden_pago_concepto->getDescripcion()) ?>
                    </div>
                    <div class="label-error" id="cmb_pde_orden_pago_concepto_id_<?php echo $key ?>_error"></div>
                </td>
