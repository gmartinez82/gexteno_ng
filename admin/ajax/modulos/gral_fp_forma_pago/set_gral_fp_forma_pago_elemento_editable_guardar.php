...
if(UsCredencial::getEsAcreditado('GRAL_FP_FORMA_PAGO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralFpFormaPago::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($gral_fp_forma_pago){
        if($gral_fp_forma_pago->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_fp_forma_pago->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_fp_forma_pago->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_fp_forma_pago->saveDesdeListado($campo, $valor);
            }
