...
function setInitFndCaja() {
    ...
    setClickDbsugCajaReabrir();

    // cierre de caja
    setChangeCajaCerrarBilletes();
    ...
    // reasignacion de caja
    setChangeReasignacionCaja();
    ...
    $.ajax({
        ...
        beforeSend: function (objeto) {
            ...
            $('.div_modal').dialog({
                ...
                close: function () {
                    refreshAdmUno('fnd_caja_gestion', fnd_caja_id);
                },
                hide: 'fade',
            });
        },
...
function setChangeCajaCerrarBilletes() {
    $(".div_modal .caja.cierre #txt_saldo_final").unbind();
    $(".div_modal .caja.cierre #txt_saldo_final").keyup(function () {
        setTimeout(function () {
            setRefreshTotalImporteTendicion();
        }, 1000);
    });

    $(".div_modal .caja.cierre .txt-gral-billete").unbind();
    $(".div_modal .caja.cierre .txt-gral-billete").keyup(function () {
        setRefreshTotalImporteTendicion();
    });
}

function setRefreshTotalImporteTendicion() {

    var fnd_caja_id = $(".datos.caja.cierre").attr('fnd_caja_id');
    var form = $("#form_caja_cerrar");

    $.ajax({
        type: 'POST',
        url: "ajax/modulos/fnd_caja_gestion/refresh_bloque_cerrar_datos_rendicion.php",
        data: form.serialize() + '&fnd_caja_id=' + fnd_caja_id,
        dataType: "html",
        beforeSend: function (objeto) {
            $(".bloque-datos-rendicion").html(img_loading);
        },
        success: function (data) {
            $(".bloque-datos-rendicion").html(data);
...
function setClickCajaCerrarCerar() {
    ...
    $(".div_modal .caja.cierre .boton.cerrar").click(function () {
        ...
        if (confirm("¿Desea cerrar la caja?")) {
            setCerrarCaja(elem, 0);
        }
    });
    
    $(".div_modal .caja.cierre .boton.cerrar-con-ajuste-egreso").unbind();
    $(".div_modal .caja.cierre .boton.cerrar-con-ajuste-egreso").click(function () {
        var elem = $(this);
        if (confirm("¿Desea cerrar la caja con ajuste de egreso?")) {
            setCerrarCaja(elem, 1);
        }
    });
    
    $(".div_modal .caja.cierre .boton.cerrar-con-ajuste-ingreso").unbind();
    $(".div_modal .caja.cierre .boton.cerrar-con-ajuste-ingreso").click(function () {
        var elem = $(this);
        if (confirm("¿Desea cerrar la caja con ajuste de ingreso?")) {
            setCerrarCaja(elem, 2);
        }
    });
}

function setCerrarCaja(elem, tipo_ajuste) {
    ...
    $.ajax({
        ...
        data: form.serialize() + '&fnd_caja_id=' + fnd_caja_id + '&tipo_ajuste=' + tipo_ajuste,
        dataType: "json",
        beforeSend: function (objeto)
        {
            $(".div_modal .botonera").hide();
...
/*
 Reabrir Caja
 */
function setClickDbsugCajaReabrir() {
    $('.db_context_content .uno.reabrir').unbind();
    $('.db_context_content .uno.reabrir').click(function (e) {
        var caja_id = $(this).parents('.datos').attr('caja_id');
        verModalReabrirCaja(caja_id);
    });
}
function verModalReabrirCaja(caja_id) {
    $.ajax({
        type: 'GET',
        url: "ajax/modulos/fnd_caja_gestion/modal_caja_reabrir.php",
        data: 'caja_id=' + caja_id,
        dataType: "html",
        beforeSend: function (objeto) {
            $('.div_modal').html(img_loading);
...
function setClickCajaReabrirReabrir() {
    $(".div_modal .caja.cierre .boton.reabrir").unbind();
    $(".div_modal .caja.cierre .boton.reabrir").click(function () {
        var elem = $(this);
        var caja_id = $("#hdn_caja_id").val();

        if (confirm("¿Desea reabrir la caja?")) {
            setReabrirCaja(elem, caja_id);
        }

    });
}
function setReabrirCaja(elem, caja_id){
    var form = $("#form_caja").serialize();
    $.ajax({
        type: 'POST',
        url: "ajax/modulos/fnd_caja_gestion/set_caja_reabrir.php",
        data: form + '&caja_id=' + caja_id,
        dataType: "json",
        beforeSend: function (objeto)
        {
...
function setChangeReasignacionCaja() {
    $('.div_modal .ficha-caja .tabla-modal .fnd_caja_id .cmb_fnd_caja_id').unbind();
    $('.div_modal .ficha-caja .tabla-modal .fnd_caja_id .cmb_fnd_caja_id').change(function (e) {
        var elem = $(this);
        var identificador = elem.parents('.uno').attr('identificador');
        var tabla = elem.parents('.uno').attr('tabla');
        var clase = elem.parents('.uno').attr('clase');
        var fnd_caja_id = elem.val();

        setReasignacionCaja(elem, identificador, tabla, clase, fnd_caja_id);
    });
}
function setReasignacionCaja(elem, identificador, tabla, clase, fnd_caja_id) {
    $.ajax({
        type: 'POST',
        url: 'ajax/modulos/fnd_caja_gestion/set_fnd_caja_gestion_reasignar_caja.php',
