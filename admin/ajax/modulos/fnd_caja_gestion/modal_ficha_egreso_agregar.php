...
if (Gral::esPost()) {
    ...
    $fnd_caja_egreso_id = Gral::getVars(1, 'hdn_fnd_caja_egreso_id', 0);
    $fnd_caja = FndCaja::getOxId($hdn_id);

    $fnd_caja_importe = FndCajaEgreso::getOxId($fnd_caja_egreso_id);

    // se reciben variable por post
    $txt_descripcion = Gral::getVars(1, 'txt_descripcion', '');
    $cmb_fnd_caja_tipo_egreso_id = Gral::getVars(1, 'cmb_fnd_caja_tipo_egreso_id', 0);
    $cmb_gral_fp_forma_pago_id = Gral::getVars(1, 'cmb_gral_fp_forma_pago_id', 0);
    $txt_importe = Gral::getVars(1, 'txt_importe', 0);
    $txt_codigo_referencia = Gral::getVars(1, 'txt_codigo_referencia', '');
    ...
    if ($cmb_fnd_caja_tipo_egreso_id == 0) {
        $estado = false;
        $cmb_fnd_caja_tipo_egreso_id_error = 'Debe seleccionar un tipo de egreso';
    }
    ...
    if ($estado) {
        $fnd_caja->setRegistrarEgreso($fnd_caja_egreso_id, $txt_descripcion, $cmb_fnd_caja_tipo_egreso_id, $cmb_gral_fp_forma_pago_id, $txt_importe, $txt_codigo_referencia, $txa_observacion, $var_sesion_modulo);
        $hdn_error = 0;
    }
} else {
    ...
    foreach ($_SESSION as $key => $value) {
        //if (strpos($key, 'fnd_caja_egreso_cheque_info') === 0)
        //    unset($_SESSION[$key]);
    }
    ...
    $fnd_caja_egreso_id = Gral::getVars(2, 'fnd_caja_egreso_id', 0);
    $fnd_caja_egreso = FndCajaEgreso::getOxId($fnd_caja_egreso_id);
    if ($fnd_caja_egreso) {
        $txt_descripcion = $fnd_caja_egreso->getDescripcion();
        $cmb_fnd_caja_tipo_egreso_id = $fnd_caja_egreso->getFndCajaTipoEgresoId();
        $cmb_gral_fp_forma_pago_id = $fnd_caja_egreso->getGralFpFormaPagoId();
        $txt_importe = $fnd_caja_egreso->getImporte();
        $txt_codigo_referencia = $fnd_caja_egreso->getCodigoReferencia();
        $txa_observacion = $fnd_caja_egreso->getObservacion();
    }
}
?>
<body>
    <form id='formu-egreso' name='formu-egreso' method='post' var_sesion_random='<?php echo $var_sesion_random; ?>' action='ajax/modulos/fnd_caja_gestion/modal_ficha_egreso_agregar.php' >
        ...
        <table border='0' cellspacing='10' class='adm_carga_datos' align='center' id='alta_modal_fnd_caja_pan_panol'>
            ...
            <tr>
                ...
                <td width='300' class='adm_carga_datos_datos' id="dato_fnd_caja_egreso_codigo_referencia">
            ...
            <tr>
                <td width='150' class='adm_carga_datos_titulos'>
                    <?php Lang::_lang('Tipo de Egreso') ?>
                </td>
                <td width='300' class='adm_carga_datos_datos' id="dato_fnd_caja_egreso_tipo_egreso">
                    <?php $error_input_css = ($cmb_fnd_caja_tipo_egreso_id_error) ? 'error-mensaje-input' : ''; ?>
                    <?php Html::html_dib_select(1, 'cmb_fnd_caja_tipo_egreso_id', Gral::getCmbFiltro(FndCajaTipoEgreso::getFndCajaTipoEgresosCmb(), '...'), $cmb_fnd_caja_tipo_egreso_id, 'textbox ' . $error_input_css) ?>
                    <div class="error-mensaje-input-texto"><?php Gral::_echo($cmb_fnd_caja_tipo_egreso_id_error) ?></div>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td width='300' class='adm_carga_datos_datos' id="dato_fnd_caja_egreso_forma_pago">
            ...
            <tr>
                ...
                <td width='300' class='adm_carga_datos_datos' id="dato_fnd_caja_egreso_importe">
            ...
            <tr>
                ...
                <td width='300' class='adm_carga_datos_datos' id="dato_fnd_caja_egreso_observacion">
        ...
        <table border='0' align='center'>
            <tr>
                <td width='550' class='adm_carga_datos_botones'>
                    ...
                    <input name='hdn_fnd_caja_egreso_id' type='hidden' id='hdn_id' size='1' value='<?php echo $fnd_caja_egreso_id ?>'/>
