...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_REGIMEN_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoRegimen::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_regimen = EkuParamTipoRegimen::getOxId($identificador);
    if($eku_param_tipo_regimen){
        if($eku_param_tipo_regimen->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_regimen->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_regimen->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_regimen->saveDesdeListado($campo, $valor);
            }
