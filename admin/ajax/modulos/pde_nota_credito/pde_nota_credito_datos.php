...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/pde_nota_credito.php' ?>

<?php if(count($pde_nota_creditos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_pde_nota_credito'>
  ...
  <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=pde_tipo_origen_nota_credito_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('PdeTipoOrigenNotaCredito') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='pde_tipo_origen_nota_credito_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_pde_nota_credito_uno_<?php Gral::_echo($pde_nota_credito->getId()) ?>" class="uno" identificador="<?php Gral::_echo($pde_nota_credito->getId()) ?>">
  	<?php include "pde_nota_credito_uno.php" ?>
  </tr>
  ...
  <tr id='tr_eliminar_<?php Gral::_echo($pde_nota_credito->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($pde_nota_credito->getId()) ?>" >
	<td colspan='25' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($pde_nota_credito->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
...
?>
  <tr id='tr_mi_<?php Gral::_echo($pde_nota_credito->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='25' align='center' class='adm_tbl_lineas'>
	
	
	<div class="masinfo">
		<?php 
		if(trim($id) == $pde_nota_credito->getId()){ 
			include "pde_nota_credito_adm_masinfo.php";
		}
		?>
	</div>
  ...
  <tr>
	<td colspan='25' align='center'><?php include Gral::getPathAbs().'admin/parciales/paginador_adm.php';?></td>
	</tr>
</table>
