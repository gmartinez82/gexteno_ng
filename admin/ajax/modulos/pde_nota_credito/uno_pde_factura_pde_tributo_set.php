...
if(in_array($pde_factura_pde_tributo_relacionado->getId(), $o_ids)){
    $sel = "checked = 'checked'";
    $css_sel = 'sel';

    $array = array(
        array('campo' => 'pde_nota_credito_id', 'valor' => $o_padre->getId()),
        array('campo' => 'pde_factura_pde_tributo_id', 'valor' => $pde_factura_pde_tributo_relacionado->getId())
    );
...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('PDE_NOTA_CREDITO_ALTA_RELACION_PDE_FACTURA_PDE_TRIBUTO_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_pde_factura_pde_tributo_id_<?php echo $pde_factura_pde_tributo_relacionado->getId() ?>' name='chk_pde_factura_pde_tributo[<?php echo $pde_factura_pde_tributo_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $pde_factura_pde_tributo_relacionado->getId() ?>' relacion_id='<?php echo $pde_nota_credito_pde_factura_pde_tributo_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('PDE_NOTA_CREDITO_ALTA_RELACION_PDE_FACTURA_PDE_TRIBUTO_ACCIONES_EDITAR')){ ?>	
        <?php if($pde_nota_credito_pde_factura_pde_tributo_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/pde_nota_credito_pde_factura_pde_tributo/pde_nota_credito_pde_factura_pde_tributo_alta.php?id=<?php Gral::_echo($pde_nota_credito_pde_factura_pde_tributo_id) ?>" contenedor="div_modal" ancho="750" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($pde_factura_pde_tributo_relacionado->getId()) ?>, 'pde_factura_pde_tributo', 'pde_nota_credito', <?php Gral::_echo($o_padre->getId()) ?>, 'PdeNotaCredito', 'PdeNotaCreditoPdeFacturaPdeTributo')" title="Editar PdeNotaCreditoPdeFacturaPdeTributo"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>

        <?php if(UsCredencial::getEsAcreditado('PDE_FACTURA_PDE_TRIBUTO_ALTA')){ ?>	
        <a class='boton' href='pde_factura_pde_tributo_alta.php?id=<?php echo $pde_factura_pde_tributo_relacionado->getId() ?>' title="<?php Lang::_lang('Ver/Editar') ?>: '<?php Gral::_echo($pde_factura_pde_tributo_relacionado->getDescripcion()) ?>' "><?php Lang::_lang('Ver') ?></a>
        <?php } ?>
    </label>
    	
    <label class='descripcion'><?php Gral::_echo($pde_factura_pde_tributo_relacionado->getDescripcionParaRelacion()) ?></label>
        	
    <?php if(trim($pde_factura_pde_tributo_relacionado->getObservacion()) != ''){ ?>
    <img class="gen-help-icon" src="imgs/btn_ayuda_verde.png" alt="help" title="<?php echo $pde_factura_pde_tributo_relacionado->getObservacion() ?>" width="20" />
    <?php } ?>
