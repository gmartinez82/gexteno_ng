...
<?php //include Gral::getPathAbs() . 'admin/parciales/buscadores/activos/vta_comision.php' ?>

<?php if (count($vta_comisions) > 0) { ?>

    <table border='0' align='center' class='listado' id='listado_adm_vta_comision_gestion'>
        ...
        foreach ($vta_comisions as $vta_comision) {
            ...
            <tr id="tr_vta_comision_gestion_uno_<?php Gral::_echo($vta_comision->getId()) ?>" class="uno" identificador="<?php Gral::_echo($vta_comision->getId()) ?>">
                <?php include "vta_comision_gestion_uno.php" ?>
            </tr>


            <tr id='tr_eliminar_<?php Gral::_echo($vta_comision->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($vta_comision->getId()) ?>" >
                <td colspan='11' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($vta_comision->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
                        <br />
                        <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
                            <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($vta_comision->getId()) ?>, 0)'/>
                        </div>
                    </div>		</td>
                <td align='center'></td>
            </tr>
            ...
            if (trim($id) == $vta_comision->getId()) {
                $mi_display = '';
                $mi_hash = "location.hash = 'mi_" . $id . "';";
            }
            ?>
            <tr id='tr_mi_<?php Gral::_echo($vta_comision->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
                <td colspan='11' align='center' class='adm_tbl_lineas'>
                    ...
                    <div class="masinfo">
                        <?php
                        if (trim($id) == $vta_comision->getId()) {
                            include "vta_comision_adm_masinfo.php";
                        }
                        ?>
                    </div>
