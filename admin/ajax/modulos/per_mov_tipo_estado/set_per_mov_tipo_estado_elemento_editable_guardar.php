...
if(UsCredencial::getEsAcreditado('PER_MOV_TIPO_ESTADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerMovTipoEstado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_mov_tipo_estado = PerMovTipoEstado::getOxId($identificador);
    if($per_mov_tipo_estado){
        if($per_mov_tipo_estado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_mov_tipo_estado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_mov_tipo_estado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_mov_tipo_estado->saveDesdeListado($campo, $valor);
            }
