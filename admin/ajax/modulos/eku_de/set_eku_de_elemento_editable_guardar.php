...
if(UsCredencial::getEsAcreditado('EKU_DE_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDe::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de = EkuDe::getOxId($identificador);
    if($eku_de){
        if($eku_de->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de->saveDesdeListado($campo, $valor);
            }
