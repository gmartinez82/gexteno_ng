...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/cli_tipo_cliente.php' ?>

<?php if(count($cli_tipo_clientes) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_cli_tipo_cliente'>
    <thead>
    <tr>
    ...
    <tbody>
  <?php 
  foreach($cli_tipo_clientes as $cli_tipo_cliente){ 
  	$estado = ($cli_tipo_cliente->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

    <tr id="tr_cli_tipo_cliente_uno_<?php Gral::_echo($cli_tipo_cliente->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($cli_tipo_cliente->getId()) ?>" hash="<?php Gral::_echo($cli_tipo_cliente->getHash()) ?>" >
  	<?php include "cli_tipo_cliente_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($cli_tipo_cliente->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($cli_tipo_cliente->getId()) ?>" hash="<?php Gral::_echo($cli_tipo_cliente->getHash()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($cli_tipo_cliente->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($cli_tipo_cliente->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($cli_tipo_cliente->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $cli_tipo_cliente->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($cli_tipo_cliente->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='5' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $cli_tipo_cliente->getId()){ 
			include "cli_tipo_cliente_adm_masinfo.php";
		}
		?>
	</div>
