...
if(UsCredencial::getEsAcreditado('CLI_TIPO_CLIENTE_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = CliTipoCliente::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $cli_tipo_cliente = CliTipoCliente::getOxId($identificador);
    if($cli_tipo_cliente){
        if($cli_tipo_cliente->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$cli_tipo_cliente->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $cli_tipo_cliente->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $cli_tipo_cliente->saveDesdeListado($campo, $valor);
            }
