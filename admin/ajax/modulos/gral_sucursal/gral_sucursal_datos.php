...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/gral_sucursal.php' ?>

<?php if(count($gral_sucursals) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_gral_sucursal'>
  ...
  <tr>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=gral_sucursal_tipo_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('GralSucursalTipo') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='gral_sucursal_tipo_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_gral_sucursal_uno_<?php Gral::_echo($gral_sucursal->getId()) ?>" class="uno" identificador="<?php Gral::_echo($gral_sucursal->getId()) ?>">
  	<?php include "gral_sucursal_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($gral_sucursal->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($gral_sucursal->getId()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($gral_sucursal->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($gral_sucursal->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($gral_sucursal->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $gral_sucursal->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($gral_sucursal->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='7' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $gral_sucursal->getId()){ 
			include "gral_sucursal_adm_masinfo.php";
		}
		?>
	</div>
