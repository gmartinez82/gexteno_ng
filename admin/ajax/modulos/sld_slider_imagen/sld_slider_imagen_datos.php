...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/sld_slider_imagen.php' ?>

<?php if(count($sld_slider_imagens) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_sld_slider_imagen'>
  ...
  <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=sld_slider_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('SldSlider') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='sld_slider_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
	...
	<td width='200' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=sld_tipo_imagen_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('SldTipoImagen') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='sld_tipo_imagen_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_sld_slider_imagen_uno_<?php Gral::_echo($sld_slider_imagen->getId()) ?>" class="uno" identificador="<?php Gral::_echo($sld_slider_imagen->getId()) ?>">
  	<?php include "sld_slider_imagen_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($sld_slider_imagen->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($sld_slider_imagen->getId()) ?>" >
	<td colspan='12' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($sld_slider_imagen->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($sld_slider_imagen->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $sld_slider_imagen->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($sld_slider_imagen->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $sld_slider_imagen->getId()){ 
			include "sld_slider_imagen_adm_masinfo.php";
		}
		?>
	</div>
