...
if(UsCredencial::getEsAcreditado('PLN_JORNADA_TRAMO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PlnJornadaTramo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $pln_jornada_tramo = PlnJornadaTramo::getOxId($identificador);
    if($pln_jornada_tramo){
        if($pln_jornada_tramo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$pln_jornada_tramo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $pln_jornada_tramo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $pln_jornada_tramo->saveDesdeListado($campo, $valor);
            }
