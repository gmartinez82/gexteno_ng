...
<td align='left' class='adm_tbl_lineas <?php echo $estado ?> <?php echo $publicado ?>'>
    ...
    <div class="vta_numero_comprobante" title="<?php Gral::_echo($vta_comprobante->getCodigo()) ?>">	
        ...
        <?php if ($vta_comprobante->getCodigoOpCliente() != '') { ?>
            <label class="codigo-op-cliente" title="OP del Cliente">
                <?php Gral::_echo($vta_comprobante->getCodigoOpCliente()) ?>
            </label>    
        <?php } ?>    
    </div>
</td>
    ...
    <?php if (is_array($arr_comprobantes_vinculados_por_conciliacion['INTERMEDIO'])) { ?>
        <div class="comprobantes-vinculados">
            <?php foreach ($arr_comprobantes_vinculados_por_conciliacion['INTERMEDIO'] as $arr_comprobante_vinculado_extremo) { ?>
                <div class="comprobante-vinculado" title="Emitido el <?php Gral::_echo(Gral::getFechaToWeb($arr_comprobante_vinculado_extremo->getVtaComprobanteDebe()->getFechaEmision())) ?>">
                    <?php Gral::_echo($arr_comprobante_vinculado_extremo->getVtaComprobanteDebe()->getTipoComprobanteSiglas()) ?> -
                    <?php Gral::_echo($arr_comprobante_vinculado_extremo->getVtaComprobanteDebe()->getNumeroComprobanteCompleto()) ?>
                    <br />
    ...
    <ul class="adm_botones_acciones">
        ...
        <?php if (UsCredencial::getEsAcreditado('VTA_COMPROBANTE_GESTION_ACCION_COMPROBANTE')) { ?>
            <li class='adm_botones_accion vta-comprobante-pdf'>
                <?php if (get_class($vta_comprobante) == 'VtaRecibo') { ?>
                    <a href="ajax/modulos/vta_recibo_gestion/pdf_recibo.php?vta_recibo_id=<?php echo $vta_comprobante->getId() ?>" target="_blank">
                        <img src='imgs/btn_pdf.png' width='16' border='0' title="Ver Recibo" />
                    </a>
                <?php } ?>
                <?php if (get_class($vta_comprobante) == 'VtaNotaCredito') { ?>
                    <a href="ajax/modulos/vta_nota_credito_gestion/pdf_nota_credito.php?vta_nota_credito_id=<?php echo $vta_comprobante->getId() ?>" target="_blank">
                        <img src='imgs/btn_pdf.png' width='16' border='0' title="Ver Nota de Credito" />
                    </a>
                <?php } ?>
            </li>
        <?php } ?>   
