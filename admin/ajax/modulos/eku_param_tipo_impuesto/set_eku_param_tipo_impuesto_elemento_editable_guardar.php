...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_IMPUESTO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoImpuesto::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_impuesto = EkuParamTipoImpuesto::getOxId($identificador);
    if($eku_param_tipo_impuesto){
        if($eku_param_tipo_impuesto->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_impuesto->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_impuesto->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_impuesto->saveDesdeListado($campo, $valor);
            }
