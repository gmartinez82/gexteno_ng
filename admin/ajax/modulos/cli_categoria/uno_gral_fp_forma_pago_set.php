...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('CLI_CATEGORIA_ALTA_RELACION_GRAL_FP_FORMA_PAGO_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_gral_fp_forma_pago_id_<?php echo $gral_fp_forma_pago_relacionado->getId() ?>' name='chk_gral_fp_forma_pago[<?php echo $gral_fp_forma_pago_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $gral_fp_forma_pago_relacionado->getId() ?>' relacion_id='<?php echo $cli_categoria_gral_fp_forma_pago_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('CLI_CATEGORIA_ALTA_RELACION_GRAL_FP_FORMA_PAGO_ACCIONES_EDITAR')){ ?>	
        <?php if($cli_categoria_gral_fp_forma_pago_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/cli_categoria_gral_fp_forma_pago/cli_categoria_gral_fp_forma_pago_alta.php?id=<?php Gral::_echo($cli_categoria_gral_fp_forma_pago_id) ?>" contenedor="div_modal" ancho="750" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($gral_fp_forma_pago_relacionado->getId()) ?>, 'gral_fp_forma_pago', 'cli_categoria', <?php Gral::_echo($o_padre->getId()) ?>, 'CliCategoria', 'CliCategoriaGralFpFormaPago')" title="Editar CliCategoriaGralFpFormaPago"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
