...
if (count($txt_descripcions) > 0) {
    ?>
    <table border='0' align='center' class='listado_pde_nota_debito_items' id='listado_pde_nota_debito_items_confirmacion' multiseleccion = "<?php echo $control_presupuesto ?>">
        <tr>
        ...
        foreach ($txt_descripcions as $key => $txt_descripcion) {
            ...
            $pde_nota_debito_concepto_id = $cmb_pde_nota_debito_concepto_ids[$key];
            ...
            ?>
            <tr id="tr_pde_nota_debito_uno_<?php Gral::_echo($key) ?>" class="uno">
                <?php //include "bloque_modal_pde_nota_debito_listado_uno.php"   ?>

                <td align="center">
                    <div class="pde_nota_debito_concepto" id="cmb_pde_nota_debito_concepto_id[<?php echo $key ?>]" name="cmb_pde_nota_debito_concepto_id[<?php echo $key ?>]">
                        <?php Gral::_echo($pde_nota_debito_concepto->getDescripcion()) ?>
                    </div>
                    <div class="label-error" id="cmb_pde_nota_debito_concepto_id_<?php echo $key ?>_error"></div>
                </td>
