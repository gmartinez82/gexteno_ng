...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/ins_insumo_costo.php' ?>

<?php if(count($ins_insumo_costos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_ins_insumo_costo'>
  ...
  <tr>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=costo&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('Costo') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='costo'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
	...
	<td width='150' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=prv_importacion_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('PrvImportacion') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='prv_importacion_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_ins_insumo_costo_uno_<?php Gral::_echo($ins_insumo_costo->getId()) ?>" class="uno" identificador="<?php Gral::_echo($ins_insumo_costo->getId()) ?>">
  	<?php include "ins_insumo_costo_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($ins_insumo_costo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($ins_insumo_costo->getId()) ?>" >
	<td colspan='12' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($ins_insumo_costo->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($ins_insumo_costo->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $ins_insumo_costo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($ins_insumo_costo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $ins_insumo_costo->getId()){ 
			include "ins_insumo_costo_adm_masinfo.php";
		}
		?>
	</div>
  ...
  <tr>
	...
	<td align='center' class="adm_tbl_pie" >&nbsp;</td>
	<td align='center' class="adm_tbl_pie" >&nbsp;</td>
	<td align='center' class="adm_tbl_pie" ><?php Gral::_echoImp(InsInsumoCosto::getCostoAdmTotal()) ?></td>
	<td align='center' class="adm_tbl_pie" >&nbsp;</td>
	<td align='center' class="adm_tbl_pie" >&nbsp;</td>
	<td align='center' class="adm_tbl_pie" >&nbsp;</td>
	<td align='center' class="adm_tbl_pie" >&nbsp;</td>
