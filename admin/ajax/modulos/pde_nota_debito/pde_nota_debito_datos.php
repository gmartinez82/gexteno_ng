...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/pde_nota_debito.php' ?>

<?php if(count($pde_nota_debitos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_pde_nota_debito'>
  ...
  <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=pde_tipo_origen_nota_debito_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('PdeTipoOrigenNotaDebito') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='pde_tipo_origen_nota_debito_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=pde_centro_pedido_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('PdeCentroPedido') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='pde_centro_pedido_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=prv_descuento_financiero_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('PrvDescuentoFinanciero') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='prv_descuento_financiero_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_pde_nota_debito_uno_<?php Gral::_echo($pde_nota_debito->getId()) ?>" class="uno" identificador="<?php Gral::_echo($pde_nota_debito->getId()) ?>">
  	<?php include "pde_nota_debito_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($pde_nota_debito->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($pde_nota_debito->getId()) ?>" >
	<td colspan='26' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($pde_nota_debito->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($pde_nota_debito->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
?>
  <tr id='tr_mi_<?php Gral::_echo($pde_nota_debito->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='26' align='center' class='adm_tbl_lineas'>
	
	
	<div class="masinfo">
		<?php 
		if(trim($id) == $pde_nota_debito->getId()){ 
			include "pde_nota_debito_adm_masinfo.php";
		}
		?>
	</div>
  ...
  <tr>
	<td colspan='26' align='center'><?php include Gral::getPathAbs().'admin/parciales/paginador_adm.php';?></td>
	</tr>
</table>
