...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_OPERACION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoOperacion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_operacion = EkuParamTipoOperacion::getOxId($identificador);
    if($eku_param_tipo_operacion){
        if($eku_param_tipo_operacion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_operacion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_operacion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_operacion->saveDesdeListado($campo, $valor);
            }
