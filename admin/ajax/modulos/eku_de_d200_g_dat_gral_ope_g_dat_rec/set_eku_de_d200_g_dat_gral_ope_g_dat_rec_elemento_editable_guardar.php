...
if(UsCredencial::getEsAcreditado('EKU_DE_D200_G_DAT_GRAL_OPE_G_DAT_REC_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeD200GDatGralOpeGDatRec::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_d200_g_dat_gral_ope_g_dat_rec){
        if($eku_de_d200_g_dat_gral_ope_g_dat_rec->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_d200_g_dat_gral_ope_g_dat_rec->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_d200_g_dat_gral_ope_g_dat_rec->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_d200_g_dat_gral_ope_g_dat_rec->saveDesdeListado($campo, $valor);
            }
