...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('US_GRUPO_ALTA_RELACION_NOV_NOVEDAD_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_nov_novedad_id_<?php echo $nov_novedad_relacionado->getId() ?>' name='chk_nov_novedad[<?php echo $nov_novedad_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $nov_novedad_relacionado->getId() ?>' relacion_id='<?php echo $nov_novedad_us_grupo_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('US_GRUPO_ALTA_RELACION_NOV_NOVEDAD_ACCIONES_EDITAR')){ ?>	
        <?php if($nov_novedad_us_grupo_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/nov_novedad_us_grupo/nov_novedad_us_grupo_alta.php?id=<?php Gral::_echo($nov_novedad_us_grupo_id) ?>" contenedor="div_modal" ancho="750" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($nov_novedad_relacionado->getId()) ?>, 'nov_novedad', 'us_grupo', <?php Gral::_echo($o_padre->getId()) ?>, 'UsGrupo', 'NovNovedadUsGrupo')" title="Editar NovNovedadUsGrupo"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>

        <?php if(UsCredencial::getEsAcreditado('NOV_NOVEDAD_ALTA')){ ?>	
        <a class='boton' href='nov_novedad_alta.php?id=<?php echo $nov_novedad_relacionado->getId() ?>' title="<?php Lang::_lang('Ver/Editar') ?>: '<?php Gral::_echo($nov_novedad_relacionado->getDescripcion()) ?>' "><?php Lang::_lang('Ver') ?></a>
        <?php } ?>
    </label>
