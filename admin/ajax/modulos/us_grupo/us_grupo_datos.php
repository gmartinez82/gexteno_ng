...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/us_grupo.php' ?>

<?php if(count($us_grupos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_us_grupo'>
  ...
  <tr id="tr_us_grupo_uno_<?php Gral::_echo($us_grupo->getId()) ?>" class="uno" identificador="<?php Gral::_echo($us_grupo->getId()) ?>">
  	<?php include "us_grupo_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($us_grupo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($us_grupo->getId()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($us_grupo->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($us_grupo->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $us_grupo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($us_grupo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $us_grupo->getId()){ 
			include "us_grupo_adm_masinfo.php";
		}
		?>
	</div>
