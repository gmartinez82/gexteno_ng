...
if(UsCredencial::getEsAcreditado('EKU_DE_OPE_TIPO_ESTADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeOpeTipoEstado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_ope_tipo_estado = EkuDeOpeTipoEstado::getOxId($identificador);
    if($eku_de_ope_tipo_estado){
        if($eku_de_ope_tipo_estado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_ope_tipo_estado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_ope_tipo_estado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_ope_tipo_estado->saveDesdeListado($campo, $valor);
            }
