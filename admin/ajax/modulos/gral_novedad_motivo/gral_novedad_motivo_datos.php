...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/gral_novedad_motivo.php' ?>

<?php if(count($gral_novedad_motivos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_gral_novedad_motivo'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_gral_novedad_motivo_uno_<?php Gral::_echo($gral_novedad_motivo->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($gral_novedad_motivo->getId()) ?>" hash="<?php Gral::_echo($gral_novedad_motivo->getHash()) ?>" >
  	<?php include "gral_novedad_motivo_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($gral_novedad_motivo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($gral_novedad_motivo->getId()) ?>" hash="<?php Gral::_echo($gral_novedad_motivo->getHash()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($gral_novedad_motivo->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($gral_novedad_motivo->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($gral_novedad_motivo->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $gral_novedad_motivo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($gral_novedad_motivo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='5' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $gral_novedad_motivo->getId()){ 
			include "gral_novedad_motivo_adm_masinfo.php";
		}
		?>
	</div>
