...
if(UsCredencial::getEsAcreditado('GRAL_NOVEDAD_MOTIVO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralNovedadMotivo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $gral_novedad_motivo = GralNovedadMotivo::getOxId($identificador);
    if($gral_novedad_motivo){
        if($gral_novedad_motivo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_novedad_motivo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_novedad_motivo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_novedad_motivo->saveDesdeListado($campo, $valor);
            }
