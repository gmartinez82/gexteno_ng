...
if(UsCredencial::getEsAcreditado('OS_TIPO_RESOLUCION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = OsTipoResolucion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $os_tipo_resolucion = OsTipoResolucion::getOxId($identificador);
    if($os_tipo_resolucion){
        if($os_tipo_resolucion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$os_tipo_resolucion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $os_tipo_resolucion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $os_tipo_resolucion->saveDesdeListado($campo, $valor);
            }
