...
if(UsCredencial::getEsAcreditado('EKU_PARAM_CONDICION_TIPO_CAMBIO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamCondicionTipoCambio::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_condicion_tipo_cambio){
        if($eku_param_condicion_tipo_cambio->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_condicion_tipo_cambio->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_condicion_tipo_cambio->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_condicion_tipo_cambio->saveDesdeListado($campo, $valor);
            }
