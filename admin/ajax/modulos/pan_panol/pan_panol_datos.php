...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/pan_panol.php' ?>

<?php if(count($pan_panols) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_pan_panol'>
  ...
  <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=pan_tipo_panol_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('PanTipoPanol') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='pan_tipo_panol_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_pan_panol_uno_<?php Gral::_echo($pan_panol->getId()) ?>" class="uno" identificador="<?php Gral::_echo($pan_panol->getId()) ?>">
  	<?php include "pan_panol_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($pan_panol->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($pan_panol->getId()) ?>" >
	<td colspan='9' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($pan_panol->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($pan_panol->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($pan_panol->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $pan_panol->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($pan_panol->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='9' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $pan_panol->getId()){ 
			include "pan_panol_adm_masinfo.php";
		}
		?>
	</div>
