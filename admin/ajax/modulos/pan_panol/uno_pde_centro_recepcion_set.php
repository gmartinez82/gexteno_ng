...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('PAN_PANOL_ALTA_RELACION_PDE_CENTRO_RECEPCION_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_pde_centro_recepcion_id_<?php echo $pde_centro_recepcion_relacionado->getId() ?>' name='chk_pde_centro_recepcion[<?php echo $pde_centro_recepcion_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $pde_centro_recepcion_relacionado->getId() ?>' relacion_id='<?php echo $pde_centro_recepcion_pan_panol_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('PAN_PANOL_ALTA_RELACION_PDE_CENTRO_RECEPCION_ACCIONES_EDITAR')){ ?>	
        <?php if($pde_centro_recepcion_pan_panol_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/pde_centro_recepcion_pan_panol/pde_centro_recepcion_pan_panol_alta.php?id=<?php Gral::_echo($pde_centro_recepcion_pan_panol_id) ?>" contenedor="div_modal" ancho="950" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($pde_centro_recepcion_relacionado->getId()) ?>, 'pde_centro_recepcion', 'pan_panol', <?php Gral::_echo($o_padre->getId()) ?>, 'PanPanol', 'PdeCentroRecepcionPanPanol')" title="Editar PdeCentroRecepcionPanPanol"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
