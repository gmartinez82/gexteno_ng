...
if(UsCredencial::getEsAcreditado('PER_MOV_TIPO_MOVIMIENTO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerMovTipoMovimiento::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_mov_tipo_movimiento = PerMovTipoMovimiento::getOxId($identificador);
    if($per_mov_tipo_movimiento){
        if($per_mov_tipo_movimiento->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_mov_tipo_movimiento->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_mov_tipo_movimiento->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_mov_tipo_movimiento->saveDesdeListado($campo, $valor);
            }
