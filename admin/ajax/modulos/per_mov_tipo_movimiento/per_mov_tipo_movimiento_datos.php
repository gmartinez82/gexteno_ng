...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/per_mov_tipo_movimiento.php' ?>

<?php if(count($per_mov_tipo_movimientos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_per_mov_tipo_movimiento'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_per_mov_tipo_movimiento_uno_<?php Gral::_echo($per_mov_tipo_movimiento->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($per_mov_tipo_movimiento->getId()) ?>" hash="<?php Gral::_echo($per_mov_tipo_movimiento->getHash()) ?>" >
  	<?php include "per_mov_tipo_movimiento_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($per_mov_tipo_movimiento->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($per_mov_tipo_movimiento->getId()) ?>" hash="<?php Gral::_echo($per_mov_tipo_movimiento->getHash()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($per_mov_tipo_movimiento->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($per_mov_tipo_movimiento->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($per_mov_tipo_movimiento->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $per_mov_tipo_movimiento->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($per_mov_tipo_movimiento->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='5' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $per_mov_tipo_movimiento->getId()){ 
			include "per_mov_tipo_movimiento_adm_masinfo.php";
		}
		?>
	</div>
