...
if($buscar != '...'){
    ...
    $c->add('per_mov_tipo_movimiento.id', $buscar, Criterio::LIKE, false, Criterio::_AND);

	$c->add('per_mov_tipo_movimiento.descripcion', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('per_mov_tipo_movimiento.codigo', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('per_mov_tipo_movimiento.observacion', $buscar, Criterio::LIKE, false, Criterio::_OR);
    $c->addFinSubconsulta();
}

$c->addTabla('per_mov_tipo_movimiento');
$c->addOrden('per_mov_tipo_movimiento.orden', 'asc');
$c->setCriterios();

$p = new Paginador(10, $pag);

$os = PerMovTipoMovimiento::getPerMovTipoMovimientos($p, $c);
