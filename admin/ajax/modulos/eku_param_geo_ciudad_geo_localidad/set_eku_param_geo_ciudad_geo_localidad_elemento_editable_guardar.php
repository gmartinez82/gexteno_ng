...
if(UsCredencial::getEsAcreditado('EKU_PARAM_GEO_CIUDAD_GEO_LOCALIDAD_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamGeoCiudadGeoLocalidad::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_geo_ciudad_geo_localidad = EkuParamGeoCiudadGeoLocalidad::getOxId($identificador);
    if($eku_param_geo_ciudad_geo_localidad){
        if($eku_param_geo_ciudad_geo_localidad->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_geo_ciudad_geo_localidad->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_geo_ciudad_geo_localidad->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_geo_ciudad_geo_localidad->saveDesdeListado($campo, $valor);
            }
