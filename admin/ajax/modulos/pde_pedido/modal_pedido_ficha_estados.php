<table border="0" class="tabla-modal">
  <tr>
    ...
    <td class="adm_tbl_encabezado" width="350" align='center'><?php Lang::_lang('Comentarios') ?></td>
  ...
  <tr>
    ...
    <td class="adm_tbl_lineas <?php echo $strong ?>" align="center">
        <?php Gral::_echo(Gral::getFechaHoraToWeb($pde_estado->getCreado())) ?> hs.
    </td>
    <td class="adm_tbl_lineas <?php echo $strong ?>" align="left">
        &nbsp;&nbsp;
        <img src='imgs/pde_estado/<?php Gral::_echo($pde_estado->getPdeTipoEstado()->getCodigo()) ?>.png' width="10" align='absmiddle' />
        &nbsp;
        <?php Gral::_echo($pde_estado->getPdeTipoEstado()->getDescripcion()) ?>
    </td>
    <td class="adm_tbl_lineas <?php echo $strong ?>">
        <span title="<?php Gral::_echo($pde_estado->getObservacion()) ?>">
            <?php Gral::_echo(substr($pde_estado->getObservacion(), 0, 150)) ?> ...
        </span>
    </td>
    <td class="adm_tbl_lineas <?php echo $strong ?>" align="center">
        <?php if($pde_estado->getCreadoPor() != 'null'){ ?>
        <img src='imgs/icn_usuario.gif' width="15" align='absmiddle' alt="usuario" title="<?php Gral::_echo($pde_estado->getCreadoPorDescripcion()) ?>" /> <?php Gral::_echo($pde_estado->getCreadoPorDescripcion()) ?>
        <?php } ?>
    </td>
  </tr>
  <?php } ?>
