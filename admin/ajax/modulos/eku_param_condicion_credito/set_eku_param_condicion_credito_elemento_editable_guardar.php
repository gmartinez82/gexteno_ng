...
if(UsCredencial::getEsAcreditado('EKU_PARAM_CONDICION_CREDITO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamCondicionCredito::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_condicion_credito = EkuParamCondicionCredito::getOxId($identificador);
    if($eku_param_condicion_credito){
        if($eku_param_condicion_credito->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_condicion_credito->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_condicion_credito->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_condicion_credito->saveDesdeListado($campo, $valor);
            }
