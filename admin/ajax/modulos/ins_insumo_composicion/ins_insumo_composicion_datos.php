...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/ins_insumo_composicion.php' ?>

<?php if(count($ins_insumo_composicions) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_ins_insumo_composicion'>
  ...
  <tr>
	...
	<td width='160' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=ins_insumo_composicion&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('InsInsumoComposicion') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='ins_insumo_composicion'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_ins_insumo_composicion_uno_<?php Gral::_echo($ins_insumo_composicion->getId()) ?>" class="uno" identificador="<?php Gral::_echo($ins_insumo_composicion->getId()) ?>">
  	<?php include "ins_insumo_composicion_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($ins_insumo_composicion->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($ins_insumo_composicion->getId()) ?>" >
	<td colspan='8' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($ins_insumo_composicion->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($ins_insumo_composicion->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($ins_insumo_composicion->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $ins_insumo_composicion->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($ins_insumo_composicion->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='8' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $ins_insumo_composicion->getId()){ 
			include "ins_insumo_composicion_adm_masinfo.php";
		}
		?>
	</div>
