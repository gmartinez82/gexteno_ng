...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/pde_nota_credito_pde_factura_pde_oc.php' ?>

<?php if(count($pde_nota_credito_pde_factura_pde_ocs) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_pde_nota_credito_pde_factura_pde_oc'>
  ...
  <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=pde_factura_pde_oc_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('PdeFacturaPdeOc') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='pde_factura_pde_oc_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  foreach($pde_nota_credito_pde_factura_pde_ocs as $pde_nota_credito_pde_factura_pde_oc){ 
  	$estado = ($pde_nota_credito_pde_factura_pde_oc->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

  <tr id="tr_pde_nota_credito_pde_factura_pde_oc_uno_<?php Gral::_echo($pde_nota_credito_pde_factura_pde_oc->getId()) ?>" class="uno" identificador="<?php Gral::_echo($pde_nota_credito_pde_factura_pde_oc->getId()) ?>">
  	<?php include "pde_nota_credito_pde_factura_pde_oc_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($pde_nota_credito_pde_factura_pde_oc->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($pde_nota_credito_pde_factura_pde_oc->getId()) ?>" >
	<td colspan='11' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($pde_nota_credito_pde_factura_pde_oc->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($pde_nota_credito_pde_factura_pde_oc->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $pde_nota_credito_pde_factura_pde_oc->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($pde_nota_credito_pde_factura_pde_oc->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $pde_nota_credito_pde_factura_pde_oc->getId()){ 
			include "pde_nota_credito_pde_factura_pde_oc_adm_masinfo.php";
		}
		?>
	</div>
