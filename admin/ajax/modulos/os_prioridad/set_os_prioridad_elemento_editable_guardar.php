...
if(UsCredencial::getEsAcreditado('OS_PRIORIDAD_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = OsPrioridad::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $os_prioridad = OsPrioridad::getOxId($identificador);
    if($os_prioridad){
        if($os_prioridad->getHash() == $hash){
            
            // -----------------------------------------------------------------
            // Se ejecutan controles
            // -----------------------------------------------------------------
            eval("\$os_prioridad->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $os_prioridad->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $os_prioridad->saveDesdeListado($campo, $valor);
            }
