...
if($buscar != '...'){
    ...
    $c->add('os_prioridad.id', $buscar, Criterio::LIKE, false, Criterio::_AND);

	$c->add('os_prioridad.descripcion', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('os_prioridad.codigo', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('os_prioridad.observacion', $buscar, Criterio::LIKE, false, Criterio::_OR);
    $c->addFinSubconsulta();
}

$c->addTabla('os_prioridad');
$c->addOrden('os_prioridad.orden', 'asc');
$c->setCriterios();

$p = new Paginador(10, $pag);

$os = OsPrioridad::getOsPrioridads($p, $c);
