...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/per_persona_archivo.php' ?>

<?php if(count($per_persona_archivos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_per_persona_archivo'>
    <thead>
    <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=per_persona_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('PerPersona') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='per_persona_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
    ...
    <tbody>
  <?php 
  foreach($per_persona_archivos as $per_persona_archivo){ 
  	$estado = ($per_persona_archivo->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

    <tr id="tr_per_persona_archivo_uno_<?php Gral::_echo($per_persona_archivo->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($per_persona_archivo->getId()) ?>" hash="<?php Gral::_echo($per_persona_archivo->getHash()) ?>" >
  	<?php include "per_persona_archivo_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($per_persona_archivo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($per_persona_archivo->getId()) ?>" hash="<?php Gral::_echo($per_persona_archivo->getHash()) ?>" >
	<td colspan='9' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($per_persona_archivo->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($per_persona_archivo->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($per_persona_archivo->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $per_persona_archivo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($per_persona_archivo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='9' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $per_persona_archivo->getId()){ 
			include "per_persona_archivo_adm_masinfo.php";
		}
		?>
	</div>
