...
if(UsCredencial::getEsAcreditado('PER_PERSONA_ARCHIVO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerPersonaArchivo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_persona_archivo = PerPersonaArchivo::getOxId($identificador);
    if($per_persona_archivo){
        if($per_persona_archivo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_persona_archivo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_persona_archivo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_persona_archivo->saveDesdeListado($campo, $valor);
            }
