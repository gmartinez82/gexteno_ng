...
if($buscar != '...'){
    ...
    $c->add('per_persona_archivo.id', $buscar, Criterio::LIKE, false, Criterio::_AND);

	$c->add('per_persona_archivo.descripcion', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('per_persona_archivo.codigo', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('per_persona_archivo.observacion', $buscar, Criterio::LIKE, false, Criterio::_OR);
    $c->addFinSubconsulta();
}

$c->addTabla('per_persona_archivo');
$c->addOrden('per_persona_archivo.orden', 'asc');
$c->setCriterios();

$p = new Paginador(10, $pag);

$os = PerPersonaArchivo::getPerPersonaArchivos($p, $c);
