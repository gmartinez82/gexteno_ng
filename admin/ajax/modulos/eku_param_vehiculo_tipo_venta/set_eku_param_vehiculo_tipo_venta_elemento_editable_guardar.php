...
if(UsCredencial::getEsAcreditado('EKU_PARAM_VEHICULO_TIPO_VENTA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamVehiculoTipoVenta::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_vehiculo_tipo_venta){
        if($eku_param_vehiculo_tipo_venta->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_vehiculo_tipo_venta->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_vehiculo_tipo_venta->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_vehiculo_tipo_venta->saveDesdeListado($campo, $valor);
            }
