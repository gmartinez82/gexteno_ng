...
if(UsCredencial::getEsAcreditado('VTA_VENDEDOR_VALORACION_TIPO_ITEM_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = VtaVendedorValoracionTipoItem::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $vta_vendedor_valoracion_tipo_item = VtaVendedorValoracionTipoItem::getOxId($identificador);
    if($vta_vendedor_valoracion_tipo_item){
        if($vta_vendedor_valoracion_tipo_item->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$vta_vendedor_valoracion_tipo_item->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $vta_vendedor_valoracion_tipo_item->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $vta_vendedor_valoracion_tipo_item->saveDesdeListado($campo, $valor);
            }
