...
if(UsCredencial::getEsAcreditado('GRAL_DIA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralDia::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $gral_dia = GralDia::getOxId($identificador);
    if($gral_dia){
        if($gral_dia->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_dia->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_dia->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_dia->saveDesdeListado($campo, $valor);
            }
