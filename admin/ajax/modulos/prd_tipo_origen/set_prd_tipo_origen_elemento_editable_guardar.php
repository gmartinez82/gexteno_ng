...
if(UsCredencial::getEsAcreditado('PRD_TIPO_ORIGEN_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdTipoOrigen::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_tipo_origen = PrdTipoOrigen::getOxId($identificador);
    if($prd_tipo_origen){
        if($prd_tipo_origen->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_tipo_origen->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_tipo_origen->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_tipo_origen->saveDesdeListado($campo, $valor);
            }
