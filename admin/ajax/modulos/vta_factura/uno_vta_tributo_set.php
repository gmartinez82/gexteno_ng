...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('VTA_FACTURA_ALTA_RELACION_VTA_TRIBUTO_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_vta_tributo_id_<?php echo $vta_tributo_relacionado->getId() ?>' name='chk_vta_tributo[<?php echo $vta_tributo_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $vta_tributo_relacionado->getId() ?>' relacion_id='<?php echo $vta_factura_vta_tributo_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('VTA_FACTURA_ALTA_RELACION_VTA_TRIBUTO_ACCIONES_EDITAR')){ ?>	
        <?php if($vta_factura_vta_tributo_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/vta_factura_vta_tributo/vta_factura_vta_tributo_alta.php?id=<?php Gral::_echo($vta_factura_vta_tributo_id) ?>" contenedor="div_modal" ancho="750" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($vta_tributo_relacionado->getId()) ?>, 'vta_tributo', 'vta_factura', <?php Gral::_echo($o_padre->getId()) ?>, 'VtaFactura', 'VtaFacturaVtaTributo')" title="Editar VtaFacturaVtaTributo"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
