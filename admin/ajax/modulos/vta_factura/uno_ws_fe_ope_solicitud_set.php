...
if(in_array($ws_fe_ope_solicitud_relacionado->getId(), $o_ids)){
    ...
    $array = array(
        array('campo' => 'vta_factura_id', 'valor' => $o_padre->getId()),
...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('VTA_FACTURA_ALTA_RELACION_WS_FE_OPE_SOLICITUD_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_ws_fe_ope_solicitud_id_<?php echo $ws_fe_ope_solicitud_relacionado->getId() ?>' name='chk_ws_fe_ope_solicitud[<?php echo $ws_fe_ope_solicitud_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $ws_fe_ope_solicitud_relacionado->getId() ?>' relacion_id='<?php echo $vta_factura_ws_fe_ope_solicitud_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('VTA_FACTURA_ALTA_RELACION_WS_FE_OPE_SOLICITUD_ACCIONES_EDITAR')){ ?>	
        <?php if($vta_factura_ws_fe_ope_solicitud_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/vta_factura_ws_fe_ope_solicitud/vta_factura_ws_fe_ope_solicitud_alta.php?id=<?php Gral::_echo($vta_factura_ws_fe_ope_solicitud_id) ?>" contenedor="div_modal" ancho="750" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($ws_fe_ope_solicitud_relacionado->getId()) ?>, 'ws_fe_ope_solicitud', 'vta_factura', <?php Gral::_echo($o_padre->getId()) ?>, 'VtaFactura', 'VtaFacturaWsFeOpeSolicitud')" title="Editar VtaFacturaWsFeOpeSolicitud"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
