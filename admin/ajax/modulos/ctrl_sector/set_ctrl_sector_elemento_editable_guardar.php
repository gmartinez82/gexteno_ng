...
if(UsCredencial::getEsAcreditado('CTRL_SECTOR_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = CtrlSector::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $ctrl_sector = CtrlSector::getOxId($identificador);
    if($ctrl_sector){
        if($ctrl_sector->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$ctrl_sector->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $ctrl_sector->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $ctrl_sector->saveDesdeListado($campo, $valor);
            }
