...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/ctrl_sector.php' ?>

<?php if(count($ctrl_sectors) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_ctrl_sector'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_ctrl_sector_uno_<?php Gral::_echo($ctrl_sector->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($ctrl_sector->getId()) ?>" hash="<?php Gral::_echo($ctrl_sector->getHash()) ?>" >
  	<?php include "ctrl_sector_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($ctrl_sector->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($ctrl_sector->getId()) ?>" hash="<?php Gral::_echo($ctrl_sector->getHash()) ?>" >
	<td colspan='6' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($ctrl_sector->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($ctrl_sector->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($ctrl_sector->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $ctrl_sector->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($ctrl_sector->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='6' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $ctrl_sector->getId()){ 
			include "ctrl_sector_adm_masinfo.php";
		}
		?>
	</div>
