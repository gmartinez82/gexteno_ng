...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/veh_marca_imagen.php' ?>

<?php if(count($veh_marca_imagens) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_veh_marca_imagen'>
  ...
  <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=veh_marca_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('VehMarca') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='veh_marca_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_veh_marca_imagen_uno_<?php Gral::_echo($veh_marca_imagen->getId()) ?>" class="uno" identificador="<?php Gral::_echo($veh_marca_imagen->getId()) ?>">
  	<?php include "veh_marca_imagen_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($veh_marca_imagen->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($veh_marca_imagen->getId()) ?>" >
	<td colspan='11' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($veh_marca_imagen->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($veh_marca_imagen->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $veh_marca_imagen->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($veh_marca_imagen->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $veh_marca_imagen->getId()){ 
			include "veh_marca_imagen_adm_masinfo.php";
		}
		?>
	</div>
