...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/vta_recibo_imagen.php' ?>

<?php if(count($vta_recibo_imagens) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_vta_recibo_imagen'>
  ...
  <tr id="tr_vta_recibo_imagen_uno_<?php Gral::_echo($vta_recibo_imagen->getId()) ?>" class="uno" identificador="<?php Gral::_echo($vta_recibo_imagen->getId()) ?>">
  	<?php include "vta_recibo_imagen_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($vta_recibo_imagen->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($vta_recibo_imagen->getId()) ?>" >
	<td colspan='11' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($vta_recibo_imagen->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($vta_recibo_imagen->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $vta_recibo_imagen->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($vta_recibo_imagen->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $vta_recibo_imagen->getId()){ 
			include "vta_recibo_imagen_adm_masinfo.php";
		}
		?>
	</div>
