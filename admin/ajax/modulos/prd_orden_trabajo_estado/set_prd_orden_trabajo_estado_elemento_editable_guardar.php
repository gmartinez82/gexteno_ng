...
if(UsCredencial::getEsAcreditado('PRD_ORDEN_TRABAJO_ESTADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdOrdenTrabajoEstado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_orden_trabajo_estado = PrdOrdenTrabajoEstado::getOxId($identificador);
    if($prd_orden_trabajo_estado){
        if($prd_orden_trabajo_estado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_orden_trabajo_estado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_orden_trabajo_estado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_orden_trabajo_estado->saveDesdeListado($campo, $valor);
            }
