...
if(UsCredencial::getEsAcreditado('EKU_DE_E940_G_DTIP_D_E_G_TRANSP_G_CAM_ENT_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE940GDtipDEGTranspGCamEnt::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_e940_g_dtip_d_e_g_transp_g_cam_ent = EkuDeE940GDtipDEGTranspGCamEnt::getOxId($identificador);
    if($eku_de_e940_g_dtip_d_e_g_transp_g_cam_ent){
        if($eku_de_e940_g_dtip_d_e_g_transp_g_cam_ent->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e940_g_dtip_d_e_g_transp_g_cam_ent->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e940_g_dtip_d_e_g_transp_g_cam_ent->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e940_g_dtip_d_e_g_transp_g_cam_ent->saveDesdeListado($campo, $valor);
            }
