...
if(UsCredencial::getEsAcreditado('PRD_LINEA_PRODUCCION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdLineaProduccion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_linea_produccion = PrdLineaProduccion::getOxId($identificador);
    if($prd_linea_produccion){
        if($prd_linea_produccion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_linea_produccion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_linea_produccion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_linea_produccion->saveDesdeListado($campo, $valor);
            }
