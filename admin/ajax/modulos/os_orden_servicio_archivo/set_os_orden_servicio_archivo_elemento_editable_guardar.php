...
if(UsCredencial::getEsAcreditado('OS_ORDEN_SERVICIO_ARCHIVO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = OsOrdenServicioArchivo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $os_orden_servicio_archivo = OsOrdenServicioArchivo::getOxId($identificador);
    if($os_orden_servicio_archivo){
        if($os_orden_servicio_archivo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$os_orden_servicio_archivo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $os_orden_servicio_archivo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $os_orden_servicio_archivo->saveDesdeListado($campo, $valor);
            }
