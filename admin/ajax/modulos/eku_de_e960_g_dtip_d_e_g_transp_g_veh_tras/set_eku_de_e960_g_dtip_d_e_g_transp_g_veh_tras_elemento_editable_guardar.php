...
if(UsCredencial::getEsAcreditado('EKU_DE_E960_G_DTIP_D_E_G_TRANSP_G_VEH_TRAS_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE960GDtipDEGTranspGVehTras::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e960_g_dtip_d_e_g_transp_g_veh_tras){
        if($eku_de_e960_g_dtip_d_e_g_transp_g_veh_tras->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e960_g_dtip_d_e_g_transp_g_veh_tras->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e960_g_dtip_d_e_g_transp_g_veh_tras->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e960_g_dtip_d_e_g_transp_g_veh_tras->saveDesdeListado($campo, $valor);
            }
