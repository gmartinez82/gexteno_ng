...
if(UsCredencial::getEsAcreditado('EKU_DE_E640_G_DTIP_D_E_G_CAM_COND_G_PAG_CRED_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE640GDtipDEGCamCondGPagCred::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e640_g_dtip_d_e_g_cam_cond_g_pag_cred){
        if($eku_de_e640_g_dtip_d_e_g_cam_cond_g_pag_cred->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e640_g_dtip_d_e_g_cam_cond_g_pag_cred->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e640_g_dtip_d_e_g_cam_cond_g_pag_cred->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e640_g_dtip_d_e_g_cam_cond_g_pag_cred->saveDesdeListado($campo, $valor);
            }
