...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/alt_control.php' ?>

<?php if(count($alt_controls) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_alt_control'>
  ...
  <tr id="tr_alt_control_uno_<?php Gral::_echo($alt_control->getId()) ?>" class="uno" identificador="<?php Gral::_echo($alt_control->getId()) ?>">
  	<?php include "alt_control_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($alt_control->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($alt_control->getId()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($alt_control->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($alt_control->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $alt_control->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($alt_control->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $alt_control->getId()){ 
			include "alt_control_adm_masinfo.php";
		}
		?>
	</div>
