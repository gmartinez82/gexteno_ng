...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_CONTRIBUYENTE_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoContribuyente::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_contribuyente = EkuParamTipoContribuyente::getOxId($identificador);
    if($eku_param_tipo_contribuyente){
        if($eku_param_tipo_contribuyente->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_contribuyente->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_contribuyente->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_contribuyente->saveDesdeListado($campo, $valor);
            }
