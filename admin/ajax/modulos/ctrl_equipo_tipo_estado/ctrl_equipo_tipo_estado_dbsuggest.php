...
if($buscar != '...'){
    ...
    $c->add('ctrl_equipo_tipo_estado.id', $buscar, Criterio::LIKE, false, Criterio::_AND);

	$c->add('ctrl_equipo_tipo_estado.descripcion', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('ctrl_equipo_tipo_estado.codigo', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('ctrl_equipo_tipo_estado.observacion', $buscar, Criterio::LIKE, false, Criterio::_OR);
    $c->addFinSubconsulta();
}

$c->addTabla('ctrl_equipo_tipo_estado');
$c->addOrden('ctrl_equipo_tipo_estado.orden', 'asc');
$c->setCriterios();

$p = new Paginador(10, $pag);

$os = CtrlEquipoTipoEstado::getCtrlEquipoTipoEstados($p, $c);
