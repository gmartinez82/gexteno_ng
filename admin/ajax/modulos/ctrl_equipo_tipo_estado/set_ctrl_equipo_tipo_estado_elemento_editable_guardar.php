...
if(UsCredencial::getEsAcreditado('CTRL_EQUIPO_TIPO_ESTADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = CtrlEquipoTipoEstado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $ctrl_equipo_tipo_estado = CtrlEquipoTipoEstado::getOxId($identificador);
    if($ctrl_equipo_tipo_estado){
        if($ctrl_equipo_tipo_estado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$ctrl_equipo_tipo_estado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $ctrl_equipo_tipo_estado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $ctrl_equipo_tipo_estado->saveDesdeListado($campo, $valor);
            }
