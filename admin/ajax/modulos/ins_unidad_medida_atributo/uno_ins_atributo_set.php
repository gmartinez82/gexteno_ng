...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('INS_UNIDAD_MEDIDA_ATRIBUTO_ALTA_RELACION_INS_ATRIBUTO_ACCIONES_SELECCION')){ ?>	
    ...
    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('INS_UNIDAD_MEDIDA_ATRIBUTO_ALTA_RELACION_INS_ATRIBUTO_ACCIONES_EDITAR')){ ?>	
        <?php if($ins_insumo_ins_atributo_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/ins_insumo_ins_atributo/ins_insumo_ins_atributo_alta.php?id=<?php Gral::_echo($ins_insumo_ins_atributo_id) ?>" contenedor="div_modal" ancho="750" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($ins_atributo_relacionado->getId()) ?>, 'ins_atributo', 'ins_unidad_medida_atributo', <?php Gral::_echo($o_padre->getId()) ?>, 'InsUnidadMedidaAtributo', 'InsInsumoInsAtributo')" title="Editar InsInsumoInsAtributo"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
