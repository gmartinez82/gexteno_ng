...
if(UsCredencial::getEsAcreditado('OS_ORDEN_SERVICIO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = OsOrdenServicio::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $os_orden_servicio = OsOrdenServicio::getOxId($identificador);
    if($os_orden_servicio){
        if($os_orden_servicio->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$os_orden_servicio->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $os_orden_servicio->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $os_orden_servicio->saveDesdeListado($campo, $valor);
            }
