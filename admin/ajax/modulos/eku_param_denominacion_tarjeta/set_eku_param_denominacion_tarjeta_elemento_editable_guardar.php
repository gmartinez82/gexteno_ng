...
if(UsCredencial::getEsAcreditado('EKU_PARAM_DENOMINACION_TARJETA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamDenominacionTarjeta::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_denominacion_tarjeta = EkuParamDenominacionTarjeta::getOxId($identificador);
    if($eku_param_denominacion_tarjeta){
        if($eku_param_denominacion_tarjeta->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_denominacion_tarjeta->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_denominacion_tarjeta->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_denominacion_tarjeta->saveDesdeListado($campo, $valor);
            }
