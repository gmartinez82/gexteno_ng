...
if(UsCredencial::getEsAcreditado('OS_ORDEN_SERVICIO_IMAGEN_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = OsOrdenServicioImagen::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $os_orden_servicio_imagen = OsOrdenServicioImagen::getOxId($identificador);
    if($os_orden_servicio_imagen){
        if($os_orden_servicio_imagen->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$os_orden_servicio_imagen->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $os_orden_servicio_imagen->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $os_orden_servicio_imagen->saveDesdeListado($campo, $valor);
            }
