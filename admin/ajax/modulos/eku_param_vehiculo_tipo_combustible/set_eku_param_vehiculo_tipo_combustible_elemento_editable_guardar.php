...
if(UsCredencial::getEsAcreditado('EKU_PARAM_VEHICULO_TIPO_COMBUSTIBLE_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamVehiculoTipoCombustible::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_vehiculo_tipo_combustible){
        if($eku_param_vehiculo_tipo_combustible->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_vehiculo_tipo_combustible->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_vehiculo_tipo_combustible->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_vehiculo_tipo_combustible->saveDesdeListado($campo, $valor);
            }
