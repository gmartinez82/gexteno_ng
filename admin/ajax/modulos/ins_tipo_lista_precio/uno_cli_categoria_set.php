...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('INS_TIPO_LISTA_PRECIO_ALTA_RELACION_CLI_CATEGORIA_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_cli_categoria_id_<?php echo $cli_categoria_relacionado->getId() ?>' name='chk_cli_categoria[<?php echo $cli_categoria_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $cli_categoria_relacionado->getId() ?>' relacion_id='<?php echo $cli_categoria_ins_tipo_lista_precio_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('INS_TIPO_LISTA_PRECIO_ALTA_RELACION_CLI_CATEGORIA_ACCIONES_EDITAR')){ ?>	
        <?php if($cli_categoria_ins_tipo_lista_precio_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/cli_categoria_ins_tipo_lista_precio/cli_categoria_ins_tipo_lista_precio_alta.php?id=<?php Gral::_echo($cli_categoria_ins_tipo_lista_precio_id) ?>" contenedor="div_modal" ancho="950" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($cli_categoria_relacionado->getId()) ?>, 'cli_categoria', 'ins_tipo_lista_precio', <?php Gral::_echo($o_padre->getId()) ?>, 'InsTipoListaPrecio', 'CliCategoriaInsTipoListaPrecio')" title="Editar CliCategoriaInsTipoListaPrecio"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
