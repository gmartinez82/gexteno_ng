...
if(UsCredencial::getEsAcreditado('INS_UNIDAD_MEDIDA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = InsUnidadMedida::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($ins_unidad_medida){
        if($ins_unidad_medida->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$ins_unidad_medida->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $ins_unidad_medida->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $ins_unidad_medida->saveDesdeListado($campo, $valor);
            }
