...
if(UsCredencial::getEsAcreditado('PER_PERSONA_US_USUARIO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerPersonaUsUsuario::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_persona_us_usuario = PerPersonaUsUsuario::getOxId($identificador);
    if($per_persona_us_usuario){
        if($per_persona_us_usuario->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_persona_us_usuario->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_persona_us_usuario->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_persona_us_usuario->saveDesdeListado($campo, $valor);
            }
