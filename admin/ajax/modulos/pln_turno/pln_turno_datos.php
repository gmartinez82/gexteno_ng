...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/pln_turno.php' ?>

<?php if(count($pln_turnos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_pln_turno'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_pln_turno_uno_<?php Gral::_echo($pln_turno->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($pln_turno->getId()) ?>" hash="<?php Gral::_echo($pln_turno->getHash()) ?>" >
  	<?php include "pln_turno_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($pln_turno->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($pln_turno->getId()) ?>" hash="<?php Gral::_echo($pln_turno->getHash()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($pln_turno->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($pln_turno->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($pln_turno->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $pln_turno->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($pln_turno->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='5' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $pln_turno->getId()){ 
			include "pln_turno_adm_masinfo.php";
		}
		?>
	</div>
