...
if(UsCredencial::getEsAcreditado('PLN_TURNO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PlnTurno::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $pln_turno = PlnTurno::getOxId($identificador);
    if($pln_turno){
        if($pln_turno->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$pln_turno->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $pln_turno->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $pln_turno->saveDesdeListado($campo, $valor);
            }
