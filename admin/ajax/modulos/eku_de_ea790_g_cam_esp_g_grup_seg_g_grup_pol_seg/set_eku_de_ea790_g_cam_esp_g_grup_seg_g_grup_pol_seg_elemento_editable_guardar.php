...
if(UsCredencial::getEsAcreditado('EKU_DE_EA790_G_CAM_ESP_G_GRUP_SEG_G_GRUP_POL_SEG_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeEa790GCamEspGGrupSegGGrupPolSeg::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_ea790_g_cam_esp_g_grup_seg_g_grup_pol_seg){
        if($eku_de_ea790_g_cam_esp_g_grup_seg_g_grup_pol_seg->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_ea790_g_cam_esp_g_grup_seg_g_grup_pol_seg->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_ea790_g_cam_esp_g_grup_seg_g_grup_pol_seg->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_ea790_g_cam_esp_g_grup_seg_g_grup_pol_seg->saveDesdeListado($campo, $valor);
            }
