...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/gral_billete.php' ?>

<?php if(count($gral_billetes) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_gral_billete'>
  ...
  <tr>
	...
	<td width='200' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=gral_moneda_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('GralMoneda') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='gral_moneda_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_gral_billete_uno_<?php Gral::_echo($gral_billete->getId()) ?>" class="uno" identificador="<?php Gral::_echo($gral_billete->getId()) ?>">
  	<?php include "gral_billete_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($gral_billete->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($gral_billete->getId()) ?>" >
	<td colspan='8' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($gral_billete->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($gral_billete->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $gral_billete->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($gral_billete->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $gral_billete->getId()){ 
			include "gral_billete_adm_masinfo.php";
		}
		?>
	</div>
