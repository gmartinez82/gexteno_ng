...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_NATURALEZA_RECEPTOR_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoNaturalezaReceptor::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_tipo_naturaleza_receptor){
        if($eku_param_tipo_naturaleza_receptor->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_naturaleza_receptor->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_naturaleza_receptor->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_naturaleza_receptor->saveDesdeListado($campo, $valor);
            }
