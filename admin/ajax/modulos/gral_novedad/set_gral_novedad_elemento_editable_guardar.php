...
if(UsCredencial::getEsAcreditado('GRAL_NOVEDAD_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralNovedad::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $gral_novedad = GralNovedad::getOxId($identificador);
    if($gral_novedad){
        if($gral_novedad->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_novedad->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_novedad->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_novedad->saveDesdeListado($campo, $valor);
            }
