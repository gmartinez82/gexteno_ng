...
if(UsCredencial::getEsAcreditado('PER_PERSONA_DEDO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerPersonaDedo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_persona_dedo = PerPersonaDedo::getOxId($identificador);
    if($per_persona_dedo){
        if($per_persona_dedo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_persona_dedo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_persona_dedo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_persona_dedo->saveDesdeListado($campo, $valor);
            }
