...
if($buscar != '...'){
    ...
    $c->add('per_persona_dedo.id', $buscar, Criterio::LIKE, false, Criterio::_AND);

	$c->add('per_persona_dedo.descripcion', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('per_persona_dedo.codigo', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('per_persona_dedo.observacion', $buscar, Criterio::LIKE, false, Criterio::_OR);
    $c->addFinSubconsulta();
}

$c->addTabla('per_persona_dedo');
$c->addOrden('per_persona_dedo.orden', 'asc');
$c->setCriterios();

$p = new Paginador(10, $pag);

$os = PerPersonaDedo::getPerPersonaDedos($p, $c);
