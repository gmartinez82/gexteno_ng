...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/eku_de_vta_factura.php' ?>

<?php if(count($eku_de_vta_facturas) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_eku_de_vta_factura'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_eku_de_vta_factura_uno_<?php Gral::_echo($eku_de_vta_factura->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($eku_de_vta_factura->getId()) ?>" hash="<?php Gral::_echo($eku_de_vta_factura->getHash()) ?>" >
  	<?php include "eku_de_vta_factura_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($eku_de_vta_factura->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($eku_de_vta_factura->getId()) ?>" hash="<?php Gral::_echo($eku_de_vta_factura->getHash()) ?>" >
	<td colspan='9' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($eku_de_vta_factura->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($eku_de_vta_factura->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($eku_de_vta_factura->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $eku_de_vta_factura->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($eku_de_vta_factura->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='9' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $eku_de_vta_factura->getId()){ 
			include "eku_de_vta_factura_adm_masinfo.php";
		}
		?>
	</div>
