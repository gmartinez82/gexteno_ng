...
if(UsCredencial::getEsAcreditado('EKU_DE_VTA_FACTURA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeVtaFactura::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_vta_factura = EkuDeVtaFactura::getOxId($identificador);
    if($eku_de_vta_factura){
        if($eku_de_vta_factura->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_vta_factura->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_vta_factura->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_vta_factura->saveDesdeListado($campo, $valor);
            }
