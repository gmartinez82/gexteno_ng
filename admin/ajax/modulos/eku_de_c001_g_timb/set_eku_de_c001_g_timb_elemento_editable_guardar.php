...
if(UsCredencial::getEsAcreditado('EKU_DE_C001_G_TIMB_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeC001GTimb::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_c001_g_timb = EkuDeC001GTimb::getOxId($identificador);
    if($eku_de_c001_g_timb){
        if($eku_de_c001_g_timb->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_c001_g_timb->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_c001_g_timb->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_c001_g_timb->saveDesdeListado($campo, $valor);
            }
