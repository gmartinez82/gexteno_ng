...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/per_persona_gral_sucursal.php' ?>

<?php if(count($per_persona_gral_sucursals) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_per_persona_gral_sucursal'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_per_persona_gral_sucursal_uno_<?php Gral::_echo($per_persona_gral_sucursal->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($per_persona_gral_sucursal->getId()) ?>" hash="<?php Gral::_echo($per_persona_gral_sucursal->getHash()) ?>" >
  	<?php include "per_persona_gral_sucursal_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($per_persona_gral_sucursal->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($per_persona_gral_sucursal->getId()) ?>" hash="<?php Gral::_echo($per_persona_gral_sucursal->getHash()) ?>" >
	<td colspan='6' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($per_persona_gral_sucursal->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($per_persona_gral_sucursal->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($per_persona_gral_sucursal->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $per_persona_gral_sucursal->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($per_persona_gral_sucursal->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='6' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $per_persona_gral_sucursal->getId()){ 
			include "per_persona_gral_sucursal_adm_masinfo.php";
		}
		?>
	</div>
