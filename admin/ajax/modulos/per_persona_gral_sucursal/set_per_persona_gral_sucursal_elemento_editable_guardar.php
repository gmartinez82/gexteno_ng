...
if(UsCredencial::getEsAcreditado('PER_PERSONA_GRAL_SUCURSAL_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerPersonaGralSucursal::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($per_persona_gral_sucursal){
        if($per_persona_gral_sucursal->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_persona_gral_sucursal->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_persona_gral_sucursal->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_persona_gral_sucursal->saveDesdeListado($campo, $valor);
            }
