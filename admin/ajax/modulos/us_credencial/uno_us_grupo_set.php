...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('US_CREDENCIAL_ALTA_RELACION_US_GRUPO_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_us_grupo_id_<?php echo $us_grupo_relacionado->getId() ?>' name='chk_us_grupo[<?php echo $us_grupo_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $us_grupo_relacionado->getId() ?>' relacion_id='<?php echo $us_agrupador_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('US_CREDENCIAL_ALTA_RELACION_US_GRUPO_ACCIONES_EDITAR')){ ?>	
        <?php if($us_agrupador_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/us_agrupador/us_agrupador_alta.php?id=<?php Gral::_echo($us_agrupador_id) ?>" contenedor="div_modal" ancho="750" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($us_grupo_relacionado->getId()) ?>, 'us_grupo', 'us_credencial', <?php Gral::_echo($o_padre->getId()) ?>, 'UsCredencial', 'UsAgrupador')" title="Editar UsAgrupador"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
