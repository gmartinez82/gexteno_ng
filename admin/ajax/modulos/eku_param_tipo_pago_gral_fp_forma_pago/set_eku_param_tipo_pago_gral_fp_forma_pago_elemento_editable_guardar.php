...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_PAGO_GRAL_FP_FORMA_PAGO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoPagoGralFpFormaPago::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_pago_gral_fp_forma_pago = EkuParamTipoPagoGralFpFormaPago::getOxId($identificador);
    if($eku_param_tipo_pago_gral_fp_forma_pago){
        if($eku_param_tipo_pago_gral_fp_forma_pago->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_pago_gral_fp_forma_pago->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_pago_gral_fp_forma_pago->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_pago_gral_fp_forma_pago->saveDesdeListado($campo, $valor);
            }
