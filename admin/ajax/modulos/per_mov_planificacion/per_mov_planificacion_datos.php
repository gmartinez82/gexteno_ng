...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/per_mov_planificacion.php' ?>

<?php if(count($per_mov_planificacions) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_per_mov_planificacion'>
    <thead>
    <tr>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=jornada_editada&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('Jornada Editada') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='jornada_editada'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=gral_dia_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('Dia') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='gral_dia_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  ?>
    ...
    <tr id="tr_per_mov_planificacion_uno_<?php Gral::_echo($per_mov_planificacion->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($per_mov_planificacion->getId()) ?>" hash="<?php Gral::_echo($per_mov_planificacion->getHash()) ?>" >
  	<?php include "per_mov_planificacion_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($per_mov_planificacion->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($per_mov_planificacion->getId()) ?>" hash="<?php Gral::_echo($per_mov_planificacion->getHash()) ?>" >
	<td colspan='11' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($per_mov_planificacion->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($per_mov_planificacion->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($per_mov_planificacion->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $per_mov_planificacion->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($per_mov_planificacion->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='11' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $per_mov_planificacion->getId()){ 
			include "per_mov_planificacion_adm_masinfo.php";
		}
		?>
	</div>
