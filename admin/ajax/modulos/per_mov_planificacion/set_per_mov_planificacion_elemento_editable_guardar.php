...
if(UsCredencial::getEsAcreditado('PER_MOV_PLANIFICACION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerMovPlanificacion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_mov_planificacion = PerMovPlanificacion::getOxId($identificador);
    if($per_mov_planificacion){
        if($per_mov_planificacion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_mov_planificacion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_mov_planificacion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_mov_planificacion->saveDesdeListado($campo, $valor);
            }
