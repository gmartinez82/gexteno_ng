...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/pde_factura_archivo.php' ?>

<?php if(count($pde_factura_archivos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_pde_factura_archivo'>
  ...
  <?php 
  foreach($pde_factura_archivos as $pde_factura_archivo){ 
  	$estado = ($pde_factura_archivo->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

  <tr id="tr_pde_factura_archivo_uno_<?php Gral::_echo($pde_factura_archivo->getId()) ?>" class="uno" identificador="<?php Gral::_echo($pde_factura_archivo->getId()) ?>">
  	<?php include "pde_factura_archivo_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($pde_factura_archivo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($pde_factura_archivo->getId()) ?>" >
	<td colspan='9' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($pde_factura_archivo->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($pde_factura_archivo->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $pde_factura_archivo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($pde_factura_archivo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $pde_factura_archivo->getId()){ 
			include "pde_factura_archivo_adm_masinfo.php";
		}
		?>
	</div>
