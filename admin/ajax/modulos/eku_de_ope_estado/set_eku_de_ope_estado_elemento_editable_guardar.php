...
if(UsCredencial::getEsAcreditado('EKU_DE_OPE_ESTADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeOpeEstado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_ope_estado = EkuDeOpeEstado::getOxId($identificador);
    if($eku_de_ope_estado){
        if($eku_de_ope_estado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_ope_estado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_ope_estado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_ope_estado->saveDesdeListado($campo, $valor);
            }
