...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_PRESENCIA_GRAL_ESCENARIO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoPresenciaGralEscenario::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_tipo_presencia_gral_escenario){
        if($eku_param_tipo_presencia_gral_escenario->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_presencia_gral_escenario->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_presencia_gral_escenario->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_presencia_gral_escenario->saveDesdeListado($campo, $valor);
            }
