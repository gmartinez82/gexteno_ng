...
if(UsCredencial::getEsAcreditado('EKU_DE_E900_G_DTIP_D_E_G_TRANSP_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE900GDtipDEGTransp::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e900_g_dtip_d_e_g_transp){
        if($eku_de_e900_g_dtip_d_e_g_transp->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e900_g_dtip_d_e_g_transp->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e900_g_dtip_d_e_g_transp->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e900_g_dtip_d_e_g_transp->saveDesdeListado($campo, $valor);
            }
