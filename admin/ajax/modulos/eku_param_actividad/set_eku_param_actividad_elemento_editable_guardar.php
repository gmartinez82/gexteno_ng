...
if(UsCredencial::getEsAcreditado('EKU_PARAM_ACTIVIDAD_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamActividad::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_actividad = EkuParamActividad::getOxId($identificador);
    if($eku_param_actividad){
        if($eku_param_actividad->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_actividad->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_actividad->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_actividad->saveDesdeListado($campo, $valor);
            }
