...
if(UsCredencial::getEsAcreditado('CLI_CLIENTE_INFO_SIFEN_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = CliClienteInfoSifen::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $cli_cliente_info_sifen = CliClienteInfoSifen::getOxId($identificador);
    if($cli_cliente_info_sifen){
        if($cli_cliente_info_sifen->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$cli_cliente_info_sifen->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $cli_cliente_info_sifen->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $cli_cliente_info_sifen->saveDesdeListado($campo, $valor);
            }
