...
if(UsCredencial::getEsAcreditado('INS_INSUMO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = InsInsumo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $ins_insumo = InsInsumo::getOxId($identificador);
    if($ins_insumo){
        if($ins_insumo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$ins_insumo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $ins_insumo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $ins_insumo->saveDesdeListado($campo, $valor);
            }
