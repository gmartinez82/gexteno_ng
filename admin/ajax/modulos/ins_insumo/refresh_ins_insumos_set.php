...
$o_ids = eval('return $o_padre->getInsInsumoVinculado();');


// elementos seleccionados
$c = new Criterio();
$c->add('ins_insumo_vinculado.ins_insumo_id', $padre_id, Criterio::IGUAL);
$c->addTabla('ins_insumo');
$c->addRealJoin('ins_insumo_vinculado', 'ins_insumo_vinculado.ins_insumo_vinculado', 'ins_insumo.id', 'LEFT JOIN');
$c->addOrden('2');
$c->setCriterios();
...
foreach($ins_insumos as $ins_insumo){
	...
	$coma = '';
	if($cont > 1) $coma = ',';
	$ids_seleccionados.= $coma.$ins_insumo->getId();
	?>
	<div id='uno_ins_insumo_<?php echo $ins_insumo->getId() ?>' >
	<?php
...
?>
	<div class='comentario' ><?php Lang::_lang('No existen') ?> <?php Lang::_lang('InsInsumos') ?> <?php Lang::_lang('Seleccionados') ?></div>
<?php
}
...
?>
	<div class='comentario' ><?php Lang::_lang('No existen otros') ?> <?php Lang::_lang('InsInsumos') ?> <?php Lang::_lang('con el criterio elegido') ?></div>
<?php
}
