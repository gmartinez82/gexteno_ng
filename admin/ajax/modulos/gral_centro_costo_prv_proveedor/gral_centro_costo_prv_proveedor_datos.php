...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/gral_centro_costo_prv_proveedor.php' ?>

<?php if(count($gral_centro_costo_prv_proveedors) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_gral_centro_costo_prv_proveedor'>
  ...
  <?php 
  foreach($gral_centro_costo_prv_proveedors as $gral_centro_costo_prv_proveedor){ 
  	$estado = ($gral_centro_costo_prv_proveedor->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

  <tr id="tr_gral_centro_costo_prv_proveedor_uno_<?php Gral::_echo($gral_centro_costo_prv_proveedor->getId()) ?>" class="uno" identificador="<?php Gral::_echo($gral_centro_costo_prv_proveedor->getId()) ?>">
  	<?php include "gral_centro_costo_prv_proveedor_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($gral_centro_costo_prv_proveedor->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($gral_centro_costo_prv_proveedor->getId()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($gral_centro_costo_prv_proveedor->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($gral_centro_costo_prv_proveedor->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($gral_centro_costo_prv_proveedor->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $gral_centro_costo_prv_proveedor->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($gral_centro_costo_prv_proveedor->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='7' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $gral_centro_costo_prv_proveedor->getId()){ 
			include "gral_centro_costo_prv_proveedor_adm_masinfo.php";
		}
		?>
	</div>
