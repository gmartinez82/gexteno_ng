...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/prv_proveedor.php' ?>

<?php if(count($prv_proveedors) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_prv_proveedor'>
  ...
  <tr>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=convenio_multilateral&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('Conv Multilateral') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='convenio_multilateral'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  foreach($prv_proveedors as $prv_proveedor){ 
  	$estado = ($prv_proveedor->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

  <tr id="tr_prv_proveedor_uno_<?php Gral::_echo($prv_proveedor->getId()) ?>" class="uno" identificador="<?php Gral::_echo($prv_proveedor->getId()) ?>">
  	<?php include "prv_proveedor_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($prv_proveedor->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($prv_proveedor->getId()) ?>" >
	<td colspan='8' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($prv_proveedor->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($prv_proveedor->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($prv_proveedor->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $prv_proveedor->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($prv_proveedor->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='8' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $prv_proveedor->getId()){ 
			include "prv_proveedor_adm_masinfo.php";
		}
		?>
	</div>
