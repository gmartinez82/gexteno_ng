...
<div class='list <?php echo $css_sel ?>'>
    ...
    <label class='link'>
        ...
        <div class="trigger wopenModal boton" archivo="ajax/modulos/pde_pedido_prv_proveedor/pde_pedido_prv_proveedor_alta.php?id=<?php Gral::_echo($pde_pedido_prv_proveedor_id) ?>" contenedor="div_modal" ancho="950" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($pde_pedido_relacionado->getId()) ?>, 'pde_pedido', 'prv_proveedor', <?php Gral::_echo($o_padre->getId()) ?>, 'PrvProveedor', 'PdePedidoPrvProveedor')" title="Editar PdePedidoPrvProveedor"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>

        <?php if(UsCredencial::getEsAcreditado('PDE_PEDIDO_ALTA')){ ?>	
        <a class='boton' href='pde_pedido_alta.php?id=<?php echo $pde_pedido_relacionado->getId() ?>&hash=<?php echo $pde_pedido_relacionado->getHash() ?>' title="<?php Lang::_lang('Ver/Editar') ?>: '<?php Gral::_echo($pde_pedido_relacionado->getDescripcion()) ?>' "><?php Lang::_lang('Ver') ?></a>
        <?php } ?>
    </label>
