...
if(UsCredencial::getEsAcreditado('GRAL_SUCURSAL_VALORACION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralSucursalValoracion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $gral_sucursal_valoracion = GralSucursalValoracion::getOxId($identificador);
    if($gral_sucursal_valoracion){
        if($gral_sucursal_valoracion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_sucursal_valoracion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_sucursal_valoracion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_sucursal_valoracion->saveDesdeListado($campo, $valor);
            }
