...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/us_memo_tipo.php' ?>

<?php if(count($us_memo_tipos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_us_memo_tipo'>
  ...
  <tr id="tr_us_memo_tipo_uno_<?php Gral::_echo($us_memo_tipo->getId()) ?>" class="uno" identificador="<?php Gral::_echo($us_memo_tipo->getId()) ?>">
  	<?php include "us_memo_tipo_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($us_memo_tipo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($us_memo_tipo->getId()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($us_memo_tipo->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($us_memo_tipo->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $us_memo_tipo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($us_memo_tipo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $us_memo_tipo->getId()){ 
			include "us_memo_tipo_adm_masinfo.php";
		}
		?>
	</div>
