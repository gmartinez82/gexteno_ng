...
if(UsCredencial::getEsAcreditado('VTA_PUNTO_VENTA_ACTUAL_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = VtaPuntoVentaActual::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($vta_punto_venta_actual){
        if($vta_punto_venta_actual->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$vta_punto_venta_actual->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $vta_punto_venta_actual->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $vta_punto_venta_actual->saveDesdeListado($campo, $valor);
            }
