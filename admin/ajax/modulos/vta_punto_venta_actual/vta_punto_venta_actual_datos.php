...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/vta_punto_venta_actual.php' ?>

<?php if(count($vta_punto_venta_actuals) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_vta_punto_venta_actual'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_vta_punto_venta_actual_uno_<?php Gral::_echo($vta_punto_venta_actual->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($vta_punto_venta_actual->getId()) ?>" hash="<?php Gral::_echo($vta_punto_venta_actual->getHash()) ?>" >
  	<?php include "vta_punto_venta_actual_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($vta_punto_venta_actual->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($vta_punto_venta_actual->getId()) ?>" hash="<?php Gral::_echo($vta_punto_venta_actual->getHash()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($vta_punto_venta_actual->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($vta_punto_venta_actual->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($vta_punto_venta_actual->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $vta_punto_venta_actual->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($vta_punto_venta_actual->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='7' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $vta_punto_venta_actual->getId()){ 
			include "vta_punto_venta_actual_adm_masinfo.php";
		}
		?>
	</div>
