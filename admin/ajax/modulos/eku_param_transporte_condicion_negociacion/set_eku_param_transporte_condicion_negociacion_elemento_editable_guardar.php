...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TRANSPORTE_CONDICION_NEGOCIACION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTransporteCondicionNegociacion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_transporte_condicion_negociacion){
        if($eku_param_transporte_condicion_negociacion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_transporte_condicion_negociacion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_transporte_condicion_negociacion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_transporte_condicion_negociacion->saveDesdeListado($campo, $valor);
            }
