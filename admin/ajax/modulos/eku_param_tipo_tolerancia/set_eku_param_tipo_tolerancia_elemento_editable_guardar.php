...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_TOLERANCIA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoTolerancia::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_tolerancia = EkuParamTipoTolerancia::getOxId($identificador);
    if($eku_param_tipo_tolerancia){
        if($eku_param_tipo_tolerancia->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_tolerancia->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_tolerancia->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_tolerancia->saveDesdeListado($campo, $valor);
            }
