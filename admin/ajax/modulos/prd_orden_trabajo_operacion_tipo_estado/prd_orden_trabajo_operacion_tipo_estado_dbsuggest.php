...
if($buscar != '...'){
    ...
    $c->add('prd_orden_trabajo_operacion_tipo_estado.id', $buscar, Criterio::LIKE, false, Criterio::_AND);

	$c->add('prd_orden_trabajo_operacion_tipo_estado.descripcion', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('prd_orden_trabajo_operacion_tipo_estado.codigo', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('prd_orden_trabajo_operacion_tipo_estado.observacion', $buscar, Criterio::LIKE, false, Criterio::_OR);
    $c->addFinSubconsulta();
}

$c->addTabla('prd_orden_trabajo_operacion_tipo_estado');
$c->addOrden('prd_orden_trabajo_operacion_tipo_estado.orden', 'asc');
$c->setCriterios();

$p = new Paginador(10, $pag);

$os = PrdOrdenTrabajoOperacionTipoEstado::getPrdOrdenTrabajoOperacionTipoEstados($p, $c);
