...
if(UsCredencial::getEsAcreditado('PRD_ORDEN_TRABAJO_OPERACION_TIPO_ESTADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdOrdenTrabajoOperacionTipoEstado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($prd_orden_trabajo_operacion_tipo_estado){
        if($prd_orden_trabajo_operacion_tipo_estado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_orden_trabajo_operacion_tipo_estado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_orden_trabajo_operacion_tipo_estado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_orden_trabajo_operacion_tipo_estado->saveDesdeListado($campo, $valor);
            }
