...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_AFECTACION_TRIBUTARIA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoAfectacionTributaria::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_afectacion_tributaria = EkuParamTipoAfectacionTributaria::getOxId($identificador);
    if($eku_param_tipo_afectacion_tributaria){
        if($eku_param_tipo_afectacion_tributaria->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_afectacion_tributaria->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_afectacion_tributaria->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_afectacion_tributaria->saveDesdeListado($campo, $valor);
            }
