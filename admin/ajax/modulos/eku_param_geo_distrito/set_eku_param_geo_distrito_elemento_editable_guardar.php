...
if(UsCredencial::getEsAcreditado('EKU_PARAM_GEO_DISTRITO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamGeoDistrito::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_geo_distrito = EkuParamGeoDistrito::getOxId($identificador);
    if($eku_param_geo_distrito){
        if($eku_param_geo_distrito->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_geo_distrito->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_geo_distrito->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_geo_distrito->saveDesdeListado($campo, $valor);
            }
