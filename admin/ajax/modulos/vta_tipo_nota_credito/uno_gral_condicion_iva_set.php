...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('VTA_TIPO_NOTA_CREDITO_ALTA_RELACION_GRAL_CONDICION_IVA_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_gral_condicion_iva_id_<?php echo $gral_condicion_iva_relacionado->getId() ?>' name='chk_gral_condicion_iva[<?php echo $gral_condicion_iva_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $gral_condicion_iva_relacionado->getId() ?>' relacion_id='<?php echo $gral_condicion_iva_vta_tipo_nota_credito_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('VTA_TIPO_NOTA_CREDITO_ALTA_RELACION_GRAL_CONDICION_IVA_ACCIONES_EDITAR')){ ?>	
        <?php if($gral_condicion_iva_vta_tipo_nota_credito_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/gral_condicion_iva_vta_tipo_nota_credito/gral_condicion_iva_vta_tipo_nota_credito_alta.php?id=<?php Gral::_echo($gral_condicion_iva_vta_tipo_nota_credito_id) ?>" contenedor="div_modal" ancho="750" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($gral_condicion_iva_relacionado->getId()) ?>, 'gral_condicion_iva', 'vta_tipo_nota_credito', <?php Gral::_echo($o_padre->getId()) ?>, 'VtaTipoNotaCredito', 'GralCondicionIvaVtaTipoNotaCredito')" title="Editar GralCondicionIvaVtaTipoNotaCredito"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
