...
if(UsCredencial::getEsAcreditado('OS_RESOLUCION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = OsResolucion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $os_resolucion = OsResolucion::getOxId($identificador);
    if($os_resolucion){
        if($os_resolucion->getHash() == $hash){
            
            // -----------------------------------------------------------------
            // Se ejecutan controles
            // -----------------------------------------------------------------
            eval("\$os_resolucion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $os_resolucion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $os_resolucion->saveDesdeListado($campo, $valor);
            }
