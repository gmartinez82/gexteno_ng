...
if(UsCredencial::getEsAcreditado('EKU_DE_E750_G_DTIP_D_E_G_CAM_ITEM_G_RAS_MERC_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE750GDtipDEGCamItemGRasMerc::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e750_g_dtip_d_e_g_cam_item_g_ras_merc){
        if($eku_de_e750_g_dtip_d_e_g_cam_item_g_ras_merc->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e750_g_dtip_d_e_g_cam_item_g_ras_merc->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e750_g_dtip_d_e_g_cam_item_g_ras_merc->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e750_g_dtip_d_e_g_cam_item_g_ras_merc->saveDesdeListado($campo, $valor);
            }
