...
if($buscar != '...'){
    ...
    $c->add('os_tipo.id', $buscar, Criterio::LIKE, false, Criterio::_AND);

	$c->add('os_tipo.descripcion', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('os_tipo.codigo', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('os_tipo.observacion', $buscar, Criterio::LIKE, false, Criterio::_OR);
    $c->addFinSubconsulta();
}

$c->addTabla('os_tipo');
$c->addOrden('os_tipo.orden', 'asc');
$c->setCriterios();

$p = new Paginador(10, $pag);

$os = OsTipo::getOsTipos($p, $c);
