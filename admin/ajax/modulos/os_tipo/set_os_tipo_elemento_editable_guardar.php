...
if(UsCredencial::getEsAcreditado('OS_TIPO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = OsTipo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $os_tipo = OsTipo::getOxId($identificador);
    if($os_tipo){
        if($os_tipo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$os_tipo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $os_tipo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $os_tipo->saveDesdeListado($campo, $valor);
            }
