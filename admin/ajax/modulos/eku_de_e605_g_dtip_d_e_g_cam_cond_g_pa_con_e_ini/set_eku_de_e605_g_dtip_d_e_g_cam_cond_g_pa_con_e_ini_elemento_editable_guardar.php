...
if(UsCredencial::getEsAcreditado('EKU_DE_E605_G_DTIP_D_E_G_CAM_COND_G_PA_CON_E_INI_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE605GDtipDEGCamCondGPaConEIni::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e605_g_dtip_d_e_g_cam_cond_g_pa_con_e_ini){
        if($eku_de_e605_g_dtip_d_e_g_cam_cond_g_pa_con_e_ini->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e605_g_dtip_d_e_g_cam_cond_g_pa_con_e_ini->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e605_g_dtip_d_e_g_cam_cond_g_pa_con_e_ini->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e605_g_dtip_d_e_g_cam_cond_g_pa_con_e_ini->saveDesdeListado($campo, $valor);
            }
