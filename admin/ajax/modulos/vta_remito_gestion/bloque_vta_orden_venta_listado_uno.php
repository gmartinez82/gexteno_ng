...
<td align='center' class='adm_tbl_lineas <?php echo $estado ?> <?php echo $publicado ?>'>
    ...
    <div class="cantidad">
        <input step = "<?php echo $step ?>" min="<?php echo $cantidad_minima ?>" max="<?php Gral::_echo($vta_orden_venta->getCantidadDisponibleEnRemito()) ?>" type="text" id="txt_vta_orden_venta_cantidad_<?php echo $vta_orden_venta->getId() ?>" name="txt_vta_orden_venta_cantidad[<?php echo $vta_orden_venta->getId() ?>]" value="<?php Gral::_echo($vta_orden_venta->getCantidadDisponibleEnRemito()) ?>" size="2" class="textbox txt_vta_orden_venta_cantidad" />
    </div>
    <div class="disponible">
        <?php Gral::_echo($vta_orden_venta->getCantidadEnRemito()) ?> / <?php Gral::_echo($vta_orden_venta->getCantidad()) ?>
    </div>
    <div class="unidad">
