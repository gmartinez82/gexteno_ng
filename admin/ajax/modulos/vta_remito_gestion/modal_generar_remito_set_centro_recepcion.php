...
if($vta_presupuesto){
    ...
    $vta_remito_tipo_despacho = VtaRemitoTipoDespacho::getOxCodigo($vta_presupuesto_tipo_despacho->getCodigo());
    $vta_remito_tipo_despacho_id = $vta_remito_tipo_despacho->getId();
...
?>
<form id='form_generar_remito_centro_recepcion' name='form_generar_remito_centro_recepcion' method='POST' action=''>
    <div class="datos generar-remito-set-centro-recepcion">
        <div class="label-error" id="error_general"></div>

        <div class="par">
            <div class="label"><?php Gral::_echo("Tipo Despacho") ?>: </div>
            <div class="dato gen-refresh-action" gen-refresh-file="ajax/modulos/vta_remito_gestion/refresh_bloque_vta_remito_gestion_datos_despacho.php?cli_cliente_id=<?php echo $cli_cliente->getId() ?>" gen-refresh-content="#bloque_vta_remito_gestion_datos_despacho" gen-refresh-callback="setInit()">
                <?php
                Html::html_dib_select(1, 'cmb_vta_remito_tipo_despacho_id', Gral::getCmbFiltro(VtaRemitoTipoDespacho::getVtaRemitoTipoDespachosCmb(true), '...'), $vta_remito_tipo_despacho_id, 'textbox ' . $error_input_css);
                ?>
                <div class="label-error" id="cmb_vta_remito_tipo_despacho_id_error"></div>
            </div>
        </div>

        <div id="bloque_vta_remito_gestion_datos_despacho">
            <?php include 'bloque_vta_remito_gestion_datos_despacho.php'; ?>  
        </div>
        ...
        <div class="par">
            ...
            <div class="dato">
                <?php
                Html::html_dib_select(1, 'cmb_vta_tipo_remito_id', Gral::getCmbFiltro(VtaTipoRemito::getVtaTipoRemitosCmb(true), '...'), $cmb_vta_tipo_remito_id, 'textbox ' . $error_input_css);
                ?>
                <div class="label-error" id="cmb_vta_tipo_remito_id_error"></div>
            </div>
        </div>
        ...
        <div class="botonera">
            <button class="boton" id='btn_generar_remito_centro_recepcion' name='btn_generar_remito_centro_recepcion' type='button' class='btn_generar_remito'><?php Lang::_lang('Generar Remito') ?></button>
        </div>
    </div>
</form>