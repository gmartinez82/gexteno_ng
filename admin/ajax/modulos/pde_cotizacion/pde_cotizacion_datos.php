...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/pde_cotizacion.php' ?>

<?php if(count($pde_cotizacions) > 0){ ?>
...
<table border='0' align='center' class='listado' id='listado_adm_pde_cotizacion'>
  ...
  <tr>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=importe_total&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('Importe Total') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='importe_total'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_pde_cotizacion_uno_<?php Gral::_echo($pde_cotizacion->getId()) ?>" class="uno" identificador="<?php Gral::_echo($pde_cotizacion->getId()) ?>">
  	<?php include "pde_cotizacion_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($pde_cotizacion->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($pde_cotizacion->getId()) ?>" >
	<td colspan='12' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($pde_cotizacion->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($pde_cotizacion->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $pde_cotizacion->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($pde_cotizacion->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $pde_cotizacion->getId()){ 
			include "pde_cotizacion_adm_masinfo.php";
		}
		?>
	</div>
  ...
  <tr>
	...
	<td align='center' class="adm_tbl_pie" >&nbsp;</td>
	<td align='center' class="adm_tbl_pie" >&nbsp;</td>
	<td align='center' class="adm_tbl_pie" ><?php Gral::_echoImp(PdeCotizacion::getImporteUnidadAdmTotal()) ?></td>
	<td align='center' class="adm_tbl_pie" ><?php Gral::_echoImp(PdeCotizacion::getImporteTotalAdmTotal()) ?></td>
	<td align='center' class="adm_tbl_pie" >&nbsp;</td>
  </tr>
