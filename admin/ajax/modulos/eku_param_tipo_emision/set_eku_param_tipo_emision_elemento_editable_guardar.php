...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_EMISION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoEmision::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_emision = EkuParamTipoEmision::getOxId($identificador);
    if($eku_param_tipo_emision){
        if($eku_param_tipo_emision->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_emision->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_emision->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_emision->saveDesdeListado($campo, $valor);
            }
