...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/afip_citi_ventas_alicuotas.php' ?>

<?php if(count($afip_citi_ventas_alicuotass) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_afip_citi_ventas_alicuotas'>
  ...
  foreach($afip_citi_ventas_alicuotass as $afip_citi_ventas_alicuotas){ 
  	$estado = ($afip_citi_ventas_alicuotas->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

  <tr id="tr_afip_citi_ventas_alicuotas_uno_<?php Gral::_echo($afip_citi_ventas_alicuotas->getId()) ?>" class="uno" identificador="<?php Gral::_echo($afip_citi_ventas_alicuotas->getId()) ?>">
  	<?php include "afip_citi_ventas_alicuotas_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($afip_citi_ventas_alicuotas->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($afip_citi_ventas_alicuotas->getId()) ?>" >
	<td colspan='16' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($afip_citi_ventas_alicuotas->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($afip_citi_ventas_alicuotas->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $afip_citi_ventas_alicuotas->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($afip_citi_ventas_alicuotas->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $afip_citi_ventas_alicuotas->getId()){ 
			include "afip_citi_ventas_alicuotas_adm_masinfo.php";
		}
		?>
	</div>
