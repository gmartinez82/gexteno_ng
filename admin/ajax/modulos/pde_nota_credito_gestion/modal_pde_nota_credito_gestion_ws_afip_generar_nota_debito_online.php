...
$gral_empresa = $pde_nota_credito->getGralEmpresa();
$pde_centro_pedido = $pde_nota_credito->getPdeCentroPedido();

$importe_total_neto = $pde_nota_credito->getPdeNotaCreditoSubtotal();
$importe_iva = $pde_nota_credito->getPdeNotaCreditoIva();
...
<div class="div_modal">
    <div class="datos">
        <form id='form_generar_nota_debito_ws_fe_afip' name='form_generar_nota_debito_ws_fe_afip' method='POST' action='' >
            <div class="col col1">    
                <div class="par">
            ...
            <div class="col col2">
                <table border='0' align='center' class='detalle_pde_comprobante_total' id='detalle_pde_nota_credito_total'>
                    <tr class="subtotal">
                    ...
                    <?php if (count($pde_nota_credito_pde_tributos) > 0) { ?>
                        ...
                        <?php foreach ($pde_nota_credito_pde_tributos as $pde_nota_credito_pde_tributo) { ?>
                            <tr class="otros-tributos">
                                <td>&nbsp;</td>
                                <td>
                                    <?php Lang::_lang($pde_nota_credito_pde_tributo->getPdeTributo()->getDescripcion()) ?>
                                </td>
                                <td align='right'>
                                    <?php Gral::_echoImp($pde_nota_credito_pde_tributo->getImporteTributo()) ?>              
                                </td>
                            </tr>
                            <?php $importe_total += $pde_nota_credito_pde_tributo->getImporteTributo(); ?>
                        <?php } ?>
                    <?php } ?>
    ...
    <div class="botonera">
        <button pde_nota_credito_id="<?php echo $pde_nota_credito_id ?>" class="boton" id='btn_generar_nota_debito' name='btn_generar_nota_debito' type='button' class='btn_generar_nota_debito'><?php Lang::_lang('Generar Nota de Debito Online AFIP') ?></button>
    </div>
</div>
<script>
    setInit();
