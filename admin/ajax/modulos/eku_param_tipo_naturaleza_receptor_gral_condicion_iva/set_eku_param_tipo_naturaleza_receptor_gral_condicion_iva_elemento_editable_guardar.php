...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_NATURALEZA_RECEPTOR_GRAL_CONDICION_IVA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoNaturalezaReceptorGralCondicionIva::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_naturaleza_receptor_gral_condicion_iva = EkuParamTipoNaturalezaReceptorGralCondicionIva::getOxId($identificador);
    if($eku_param_tipo_naturaleza_receptor_gral_condicion_iva){
        if($eku_param_tipo_naturaleza_receptor_gral_condicion_iva->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_naturaleza_receptor_gral_condicion_iva->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_naturaleza_receptor_gral_condicion_iva->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_naturaleza_receptor_gral_condicion_iva->saveDesdeListado($campo, $valor);
            }
