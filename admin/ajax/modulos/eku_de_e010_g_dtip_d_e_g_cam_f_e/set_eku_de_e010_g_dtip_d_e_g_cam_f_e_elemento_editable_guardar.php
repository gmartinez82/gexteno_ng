...
if(UsCredencial::getEsAcreditado('EKU_DE_E010_G_DTIP_D_E_G_CAM_F_E_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE010GDtipDEGCamFE::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_e010_g_dtip_d_e_g_cam_f_e = EkuDeE010GDtipDEGCamFE::getOxId($identificador);
    if($eku_de_e010_g_dtip_d_e_g_cam_f_e){
        if($eku_de_e010_g_dtip_d_e_g_cam_f_e->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e010_g_dtip_d_e_g_cam_f_e->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e010_g_dtip_d_e_g_cam_f_e->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e010_g_dtip_d_e_g_cam_f_e->saveDesdeListado($campo, $valor);
            }
