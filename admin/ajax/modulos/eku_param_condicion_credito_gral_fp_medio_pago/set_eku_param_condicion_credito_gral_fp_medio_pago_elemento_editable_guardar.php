...
if(UsCredencial::getEsAcreditado('EKU_PARAM_CONDICION_CREDITO_GRAL_FP_MEDIO_PAGO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamCondicionCreditoGralFpMedioPago::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_condicion_credito_gral_fp_medio_pago){
        if($eku_param_condicion_credito_gral_fp_medio_pago->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_condicion_credito_gral_fp_medio_pago->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_condicion_credito_gral_fp_medio_pago->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_condicion_credito_gral_fp_medio_pago->saveDesdeListado($campo, $valor);
            }
