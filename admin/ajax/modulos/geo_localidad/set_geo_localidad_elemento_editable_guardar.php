...
if(UsCredencial::getEsAcreditado('GEO_LOCALIDAD_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GeoLocalidad::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $geo_localidad = GeoLocalidad::getOxId($identificador);
    if($geo_localidad){
        if($geo_localidad->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$geo_localidad->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $geo_localidad->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $geo_localidad->saveDesdeListado($campo, $valor);
            }
