...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TRANSPORTE_TIPO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTransporteTipo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_transporte_tipo = EkuParamTransporteTipo::getOxId($identificador);
    if($eku_param_transporte_tipo){
        if($eku_param_transporte_tipo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_transporte_tipo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_transporte_tipo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_transporte_tipo->saveDesdeListado($campo, $valor);
            }
