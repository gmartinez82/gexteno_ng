...
if(UsCredencial::getEsAcreditado('EKU_DE_E630_G_DTIP_D_E_G_CAM_COND_G_PAG_CHEQ_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE630GDtipDEGCamCondGPagCheq::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_e630_g_dtip_d_e_g_cam_cond_g_pag_cheq = EkuDeE630GDtipDEGCamCondGPagCheq::getOxId($identificador);
    if($eku_de_e630_g_dtip_d_e_g_cam_cond_g_pag_cheq){
        if($eku_de_e630_g_dtip_d_e_g_cam_cond_g_pag_cheq->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e630_g_dtip_d_e_g_cam_cond_g_pag_cheq->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e630_g_dtip_d_e_g_cam_cond_g_pag_cheq->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e630_g_dtip_d_e_g_cam_cond_g_pag_cheq->saveDesdeListado($campo, $valor);
            }
