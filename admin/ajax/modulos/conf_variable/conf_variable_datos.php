...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/conf_variable.php' ?>

<?php if(count($conf_variables) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_conf_variable'>
  ...
  <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=conf_categoria_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('Categoria') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='conf_categoria_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_conf_variable_uno_<?php Gral::_echo($conf_variable->getId()) ?>" class="uno" identificador="<?php Gral::_echo($conf_variable->getId()) ?>">
  	<?php include "conf_variable_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($conf_variable->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($conf_variable->getId()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($conf_variable->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($conf_variable->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $conf_variable->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($conf_variable->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $conf_variable->getId()){ 
			include "conf_variable_adm_masinfo.php";
		}
		?>
	</div>
