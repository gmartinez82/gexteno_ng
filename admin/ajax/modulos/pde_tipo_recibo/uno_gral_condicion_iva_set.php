...
if(in_array($gral_condicion_iva_relacionado->getId(), $o_ids)){
    ...
    $array = array(
        array('campo' => 'pde_tipo_recibo_id', 'valor' => $o_padre->getId()),
...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('PDE_TIPO_RECIBO_ALTA_RELACION_GRAL_CONDICION_IVA_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_gral_condicion_iva_id_<?php echo $gral_condicion_iva_relacionado->getId() ?>' name='chk_gral_condicion_iva[<?php echo $gral_condicion_iva_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $gral_condicion_iva_relacionado->getId() ?>' relacion_id='<?php echo $gral_condicion_iva_pde_tipo_recibo_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('PDE_TIPO_RECIBO_ALTA_RELACION_GRAL_CONDICION_IVA_ACCIONES_EDITAR')){ ?>	
        <?php if($gral_condicion_iva_pde_tipo_recibo_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/gral_condicion_iva_pde_tipo_recibo/gral_condicion_iva_pde_tipo_recibo_alta.php?id=<?php Gral::_echo($gral_condicion_iva_pde_tipo_recibo_id) ?>" contenedor="div_modal" ancho="750" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($gral_condicion_iva_relacionado->getId()) ?>, 'gral_condicion_iva', 'pde_tipo_recibo', <?php Gral::_echo($o_padre->getId()) ?>, 'PdeTipoRecibo', 'GralCondicionIvaPdeTipoRecibo')" title="Editar GralCondicionIvaPdeTipoRecibo"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
