...
if(UsCredencial::getEsAcreditado('EKU_DE_I001_SIGNATURE_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeI001Signature::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_i001_signature = EkuDeI001Signature::getOxId($identificador);
    if($eku_de_i001_signature){
        if($eku_de_i001_signature->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_i001_signature->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_i001_signature->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_i001_signature->saveDesdeListado($campo, $valor);
            }
