...
if(UsCredencial::getEsAcreditado('EKU_PARAM_SISTEMA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamSistema::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_sistema = EkuParamSistema::getOxId($identificador);
    if($eku_param_sistema){
        if($eku_param_sistema->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_sistema->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_sistema->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_sistema->saveDesdeListado($campo, $valor);
            }
