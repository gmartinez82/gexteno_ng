...
if(UsCredencial::getEsAcreditado('EKU_DE_E920_G_DTIP_D_E_G_TRANSP_G_CAM_SAL_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE920GDtipDEGTranspGCamSal::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_e920_g_dtip_d_e_g_transp_g_cam_sal = EkuDeE920GDtipDEGTranspGCamSal::getOxId($identificador);
    if($eku_de_e920_g_dtip_d_e_g_transp_g_cam_sal){
        if($eku_de_e920_g_dtip_d_e_g_transp_g_cam_sal->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e920_g_dtip_d_e_g_transp_g_cam_sal->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e920_g_dtip_d_e_g_transp_g_cam_sal->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e920_g_dtip_d_e_g_transp_g_cam_sal->saveDesdeListado($campo, $valor);
            }
