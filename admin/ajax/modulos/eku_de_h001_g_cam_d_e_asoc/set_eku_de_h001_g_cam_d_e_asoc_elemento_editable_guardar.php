...
if(UsCredencial::getEsAcreditado('EKU_DE_H001_G_CAM_D_E_ASOC_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeH001GCamDEAsoc::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_h001_g_cam_d_e_asoc){
        if($eku_de_h001_g_cam_d_e_asoc->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_h001_g_cam_d_e_asoc->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_h001_g_cam_d_e_asoc->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_h001_g_cam_d_e_asoc->saveDesdeListado($campo, $valor);
            }
