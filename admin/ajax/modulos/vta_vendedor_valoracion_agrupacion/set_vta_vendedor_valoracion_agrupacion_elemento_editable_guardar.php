...
if(UsCredencial::getEsAcreditado('VTA_VENDEDOR_VALORACION_AGRUPACION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = VtaVendedorValoracionAgrupacion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($vta_vendedor_valoracion_agrupacion){
        if($vta_vendedor_valoracion_agrupacion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$vta_vendedor_valoracion_agrupacion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $vta_vendedor_valoracion_agrupacion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $vta_vendedor_valoracion_agrupacion->saveDesdeListado($campo, $valor);
            }
