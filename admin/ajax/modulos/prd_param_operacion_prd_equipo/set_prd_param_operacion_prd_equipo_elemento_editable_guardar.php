...
if(UsCredencial::getEsAcreditado('PRD_PARAM_OPERACION_PRD_EQUIPO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdParamOperacionPrdEquipo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($prd_param_operacion_prd_equipo){
        if($prd_param_operacion_prd_equipo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_param_operacion_prd_equipo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_param_operacion_prd_equipo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_param_operacion_prd_equipo->saveDesdeListado($campo, $valor);
            }
