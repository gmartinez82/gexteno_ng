...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/pde_factura_pde_recepcion.php' ?>

<?php if(count($pde_factura_pde_recepcions) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_pde_factura_pde_recepcion'>
  ...
  <tr id="tr_pde_factura_pde_recepcion_uno_<?php Gral::_echo($pde_factura_pde_recepcion->getId()) ?>" class="uno" identificador="<?php Gral::_echo($pde_factura_pde_recepcion->getId()) ?>">
  	<?php include "pde_factura_pde_recepcion_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($pde_factura_pde_recepcion->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($pde_factura_pde_recepcion->getId()) ?>" >
	<td colspan='13' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($pde_factura_pde_recepcion->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($pde_factura_pde_recepcion->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $pde_factura_pde_recepcion->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($pde_factura_pde_recepcion->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $pde_factura_pde_recepcion->getId()){ 
			include "pde_factura_pde_recepcion_adm_masinfo.php";
		}
		?>
	</div>
