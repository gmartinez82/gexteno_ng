...
if (Gral::esPost()) {
    ...
    if ($estado) {
        ...
        $pdi_estado_nuevo = $pdi_pedido->setPdiPedidoEstado(PdiTipoEstado::TIPO_ESTADO_RECHAZADO_ERRONEO, $pdi_estado->getCantidad(), $pdi_pedido_txa_observacion);
        ...
        // se envia aviso ---------------------------------------------------------------------
        $asunto = 'PDI Pedido Interno Rechazado #' . $pdi_pedido->getCodigo() . ' ' . date('d/m/Y H:m');
...
?>
<form id='form_pedido' name='form_pedido' method='post' action='ajax/modulos/pdi_pedido_gestion/modal_pedido_rechazar_por_erroneo.php' >
    <div class="datos">
        <?php include "pdi_pedido_gestion_modal_top.php"; ?>   
        <div class="par">
            <div class="label"><?php Lang::_lang('Motivo del Rechazo') ?></div>
            <div class="dato">
                <textarea name='pdi_pedido_txa_observacion' rows='10' cols='60' id='pdi_pedido_txa_observacion' class='textbox'><?php Gral::_echoTxt($pdi_pedido->getObservacion(), true) ?></textarea>
                <div class="error"><?php Gral::_echo($pdi_pedido_txa_observacion_error) ?></div>
            </div>
        </div>
        <div class="botonera">
            <input id="btn_guardar" name='btn_guardar' type='submit' class='boton' value='<?php Lang::_lang('Rechazar Pedido') ?>' />
            ...
            <input id="hdn_mensaje" name='hdn_mensaje' type='hidden' value='<?php Lang::_lang('Se registro el Rechazo del Pedido Correctamente') ?>' />
        </div>
    </div>
</form>