...
if(UsCredencial::getEsAcreditado('GRAL_SUCURSAL_VALORACION_TIPO_ITEM_GRAL_SUCURSAL_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralSucursalValoracionTipoItemGralSucursal::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($gral_sucursal_valoracion_tipo_item_gral_sucursal){
        if($gral_sucursal_valoracion_tipo_item_gral_sucursal->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_sucursal_valoracion_tipo_item_gral_sucursal->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_sucursal_valoracion_tipo_item_gral_sucursal->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_sucursal_valoracion_tipo_item_gral_sucursal->saveDesdeListado($campo, $valor);
            }
