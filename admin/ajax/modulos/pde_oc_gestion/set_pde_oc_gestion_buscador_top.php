...
$cmb_filtro_pde_oc_estado_id = Gral::getVars(1, 'cmb_filtro_pde_oc_estado_id', 0);
$cmb_filtro_pde_oc_tipo_estado_recepcion_id = Gral::getVars(1, 'cmb_filtro_pde_oc_tipo_estado_recepcion_id', 0);
$cmb_filtro_pde_oc_tipo_estado_facturacion_id = Gral::getVars(1, 'cmb_filtro_pde_oc_tipo_estado_facturacion_id', 0);
...
if($cmb_filtro_ins_insumo_id != 0){
    $criterio->add('pde_oc.ins_insumo_id', $cmb_filtro_ins_insumo_id, Criterio::IGUAL);
}
if($cmb_filtro_pde_oc_estado_id != 0){
    $criterio->add('pde_oc_tipo_estado.id', $cmb_filtro_pde_oc_estado_id, Criterio::IGUAL);
}
if($cmb_filtro_pde_oc_tipo_estado_recepcion_id != 0){
    $criterio->add('pde_oc_tipo_estado_recepcion.id', $cmb_filtro_pde_oc_tipo_estado_recepcion_id, Criterio::IGUAL);
}
if($cmb_filtro_pde_oc_tipo_estado_facturacion_id != 0){
    $criterio->add('pde_oc_tipo_estado_facturacion.id', $cmb_filtro_pde_oc_tipo_estado_facturacion_id, Criterio::IGUAL);
}
if($cmb_filtro_destacado != -1){
    $criterio->add('pde_oc_destinatario.destacado', $cmb_filtro_destacado, Criterio::IGUAL);
}
$criterio->addFinSubconsulta();

$criterio->addInicioSubconsulta();
$criterio->add('pde_oc.codigo', $txt_buscador, Criterio::LIKE);
