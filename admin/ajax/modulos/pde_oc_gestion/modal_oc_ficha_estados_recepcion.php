<table border="0" class="tabla-modal">
  ...
  <tr>
    <td class="adm_tbl_lineas <?php echo $strong ?>" align="center">
        <?php if($pde_oc_estado_recepcion->getActual()){ ?> 
        <img src="<?php echo Gral::getPath('path_http') ?>admin/imgs/btn_estado_1.gif" width="16" alt="actual">
        <?php } ?>
    </td>
    <td class="adm_tbl_lineas <?php echo $strong ?>" align="center">
        <?php Gral::_echo(Gral::getFechaHoraToWeb($pde_oc_estado_recepcion->getCreado())) ?> hs.
    </td>
    <td class="adm_tbl_lineas <?php echo $strong ?>" align="left">
        &nbsp;&nbsp;
        <img src='imgs/pde_oc_tipo_estado_recepcion/<?php Gral::_echo($pde_oc_estado_recepcion->getPdeOcTipoEstadoRecepcion()->getCodigo()) ?>.png' width="10" align='absmiddle' />
        &nbsp;
        <?php Gral::_echo($pde_oc_estado_recepcion->getPdeOcTipoEstadoRecepcion()->getDescripcion()) ?>
    </td>
    <td class="adm_tbl_lineas <?php echo $strong ?>">
        <span title="<?php Gral::_echo($pde_oc_estado_recepcion->getObservacion()) ?>">
            <?php Gral::_echo(substr($pde_oc_estado_recepcion->getObservacion(), 0, 150)) ?> ...
        </span>
    </td>
    <td class="adm_tbl_lineas <?php echo $strong ?>" align="center">
        <?php if($pde_oc_estado_recepcion->getCreadoPor() != 'null'){ ?>
        <img src='imgs/icn_usuario.gif' width="15" align='absmiddle' alt="usuario" title="<?php Gral::_echo($pde_oc_estado_recepcion->getCreadoPorDescripcion()) ?>" /> <?php Gral::_echo($pde_oc_estado_recepcion->getCreadoPorDescripcion()) ?>
        <?php } ?>
    </td>
  </tr>
  <?php } ?>
