...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/eku_de_vta_nota_debito.php' ?>

<?php if(count($eku_de_vta_nota_debitos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_eku_de_vta_nota_debito'>
    <thead>
    <tr>
  ...
  foreach($eku_de_vta_nota_debitos as $eku_de_vta_nota_debito){ 
  	$estado = ($eku_de_vta_nota_debito->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

    <tr id="tr_eku_de_vta_nota_debito_uno_<?php Gral::_echo($eku_de_vta_nota_debito->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($eku_de_vta_nota_debito->getId()) ?>" hash="<?php Gral::_echo($eku_de_vta_nota_debito->getHash()) ?>" >
  	<?php include "eku_de_vta_nota_debito_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($eku_de_vta_nota_debito->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($eku_de_vta_nota_debito->getId()) ?>" hash="<?php Gral::_echo($eku_de_vta_nota_debito->getHash()) ?>" >
	<td colspan='9' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($eku_de_vta_nota_debito->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($eku_de_vta_nota_debito->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($eku_de_vta_nota_debito->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $eku_de_vta_nota_debito->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($eku_de_vta_nota_debito->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='9' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $eku_de_vta_nota_debito->getId()){ 
			include "eku_de_vta_nota_debito_adm_masinfo.php";
		}
		?>
	</div>
