...
if(UsCredencial::getEsAcreditado('EKU_DE_VTA_NOTA_DEBITO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeVtaNotaDebito::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_vta_nota_debito = EkuDeVtaNotaDebito::getOxId($identificador);
    if($eku_de_vta_nota_debito){
        if($eku_de_vta_nota_debito->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_vta_nota_debito->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_vta_nota_debito->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_vta_nota_debito->saveDesdeListado($campo, $valor);
            }
