...
if(UsCredencial::getEsAcreditado('EKU_DE_VTA_REMITO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeVtaRemito::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_vta_remito = EkuDeVtaRemito::getOxId($identificador);
    if($eku_de_vta_remito){
        if($eku_de_vta_remito->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_vta_remito->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_vta_remito->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_vta_remito->saveDesdeListado($campo, $valor);
            }
