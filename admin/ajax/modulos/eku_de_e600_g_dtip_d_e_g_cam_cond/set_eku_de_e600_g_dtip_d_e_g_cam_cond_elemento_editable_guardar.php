...
if(UsCredencial::getEsAcreditado('EKU_DE_E600_G_DTIP_D_E_G_CAM_COND_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE600GDtipDEGCamCond::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e600_g_dtip_d_e_g_cam_cond){
        if($eku_de_e600_g_dtip_d_e_g_cam_cond->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e600_g_dtip_d_e_g_cam_cond->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e600_g_dtip_d_e_g_cam_cond->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e600_g_dtip_d_e_g_cam_cond->saveDesdeListado($campo, $valor);
            }
