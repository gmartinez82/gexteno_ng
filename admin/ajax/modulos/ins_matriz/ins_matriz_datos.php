...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/ins_matriz.php' ?>

<?php if(count($ins_matrizs) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_ins_matriz'>
  ...
  <tr>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=codigo_pieza&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('CodigoPieza') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='codigo_pieza'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_ins_matriz_uno_<?php Gral::_echo($ins_matriz->getId()) ?>" class="uno" identificador="<?php Gral::_echo($ins_matriz->getId()) ?>">
  	<?php include "ins_matriz_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($ins_matriz->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($ins_matriz->getId()) ?>" >
	<td colspan='6' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($ins_matriz->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($ins_matriz->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $ins_matriz->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($ins_matriz->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $ins_matriz->getId()){ 
			include "ins_matriz_adm_masinfo.php";
		}
		?>
	</div>
