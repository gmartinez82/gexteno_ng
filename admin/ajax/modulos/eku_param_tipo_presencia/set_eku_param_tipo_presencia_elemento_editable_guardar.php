...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_PRESENCIA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoPresencia::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_tipo_presencia){
        if($eku_param_tipo_presencia->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_presencia->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_presencia->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_presencia->saveDesdeListado($campo, $valor);
            }
