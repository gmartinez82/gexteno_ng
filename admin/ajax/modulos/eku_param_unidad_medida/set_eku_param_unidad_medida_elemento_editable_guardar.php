...
if(UsCredencial::getEsAcreditado('EKU_PARAM_UNIDAD_MEDIDA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamUnidadMedida::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_unidad_medida = EkuParamUnidadMedida::getOxId($identificador);
    if($eku_param_unidad_medida){
        if($eku_param_unidad_medida->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_unidad_medida->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_unidad_medida->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_unidad_medida->saveDesdeListado($campo, $valor);
            }
