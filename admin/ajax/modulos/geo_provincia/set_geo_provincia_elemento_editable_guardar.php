...
if(UsCredencial::getEsAcreditado('GEO_PROVINCIA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GeoProvincia::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($geo_provincia){
        if($geo_provincia->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$geo_provincia->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $geo_provincia->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $geo_provincia->saveDesdeListado($campo, $valor);
            }
