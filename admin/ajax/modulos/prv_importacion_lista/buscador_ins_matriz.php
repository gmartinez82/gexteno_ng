...
function buscarInsumo($txt_buscador) {
    ...
    $criterio_insumo->add(InsInsumo::GEN_ATTR_CODIGO_INTERNO, $txt_buscador, Criterio::LIKE, false, Criterio::_OR);
...
if($buscar_insumo != ''){
    if ($ins_insumos) {
        ...
        ?>
        <h4>No se encontraron insumos para su busqueda</h4>
        <?php
    }
}
?> 
...
if($buscar_matriz != ''){
    if ($ins_matrizs) {
        ...
        ?>
        <h4>No se encontraron matrices para su busqueda</h4>
        <?php
    }
}
?>