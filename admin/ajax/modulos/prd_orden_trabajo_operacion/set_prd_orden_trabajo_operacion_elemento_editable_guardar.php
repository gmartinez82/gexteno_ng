...
if(UsCredencial::getEsAcreditado('PRD_ORDEN_TRABAJO_OPERACION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdOrdenTrabajoOperacion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($prd_orden_trabajo_operacion){
        if($prd_orden_trabajo_operacion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_orden_trabajo_operacion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_orden_trabajo_operacion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_orden_trabajo_operacion->saveDesdeListado($campo, $valor);
            }
