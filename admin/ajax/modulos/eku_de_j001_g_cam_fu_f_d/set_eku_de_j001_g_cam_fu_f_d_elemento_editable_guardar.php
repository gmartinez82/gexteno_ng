...
if(UsCredencial::getEsAcreditado('EKU_DE_J001_G_CAM_FU_F_D_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeJ001GCamFuFD::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_j001_g_cam_fu_f_d = EkuDeJ001GCamFuFD::getOxId($identificador);
    if($eku_de_j001_g_cam_fu_f_d){
        if($eku_de_j001_g_cam_fu_f_d->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_j001_g_cam_fu_f_d->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_j001_g_cam_fu_f_d->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_j001_g_cam_fu_f_d->saveDesdeListado($campo, $valor);
            }
