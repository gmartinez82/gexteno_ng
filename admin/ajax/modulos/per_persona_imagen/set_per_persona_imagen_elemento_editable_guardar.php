...
if(UsCredencial::getEsAcreditado('PER_PERSONA_IMAGEN_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerPersonaImagen::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_persona_imagen = PerPersonaImagen::getOxId($identificador);
    if($per_persona_imagen){
        if($per_persona_imagen->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_persona_imagen->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_persona_imagen->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_persona_imagen->saveDesdeListado($campo, $valor);
            }
