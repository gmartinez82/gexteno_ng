...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/per_persona_imagen.php' ?>

<?php if(count($per_persona_imagens) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_per_persona_imagen'>
    <thead>
    <tr>
  ...
  foreach($per_persona_imagens as $per_persona_imagen){ 
  	$estado = ($per_persona_imagen->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

    <tr id="tr_per_persona_imagen_uno_<?php Gral::_echo($per_persona_imagen->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($per_persona_imagen->getId()) ?>" hash="<?php Gral::_echo($per_persona_imagen->getHash()) ?>" >
  	<?php include "per_persona_imagen_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($per_persona_imagen->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($per_persona_imagen->getId()) ?>" hash="<?php Gral::_echo($per_persona_imagen->getHash()) ?>" >
	<td colspan='11' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($per_persona_imagen->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($per_persona_imagen->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($per_persona_imagen->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $per_persona_imagen->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($per_persona_imagen->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='11' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $per_persona_imagen->getId()){ 
			include "per_persona_imagen_adm_masinfo.php";
		}
		?>
	</div>
