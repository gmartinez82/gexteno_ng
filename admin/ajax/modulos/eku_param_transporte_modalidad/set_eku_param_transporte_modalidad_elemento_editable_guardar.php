...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TRANSPORTE_MODALIDAD_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTransporteModalidad::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_transporte_modalidad = EkuParamTransporteModalidad::getOxId($identificador);
    if($eku_param_transporte_modalidad){
        if($eku_param_transporte_modalidad->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_transporte_modalidad->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_transporte_modalidad->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_transporte_modalidad->saveDesdeListado($campo, $valor);
            }
