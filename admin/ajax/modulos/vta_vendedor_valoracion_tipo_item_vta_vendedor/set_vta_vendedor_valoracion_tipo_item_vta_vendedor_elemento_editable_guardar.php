...
if(UsCredencial::getEsAcreditado('VTA_VENDEDOR_VALORACION_TIPO_ITEM_VTA_VENDEDOR_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = VtaVendedorValoracionTipoItemVtaVendedor::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $vta_vendedor_valoracion_tipo_item_vta_vendedor = VtaVendedorValoracionTipoItemVtaVendedor::getOxId($identificador);
    if($vta_vendedor_valoracion_tipo_item_vta_vendedor){
        if($vta_vendedor_valoracion_tipo_item_vta_vendedor->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$vta_vendedor_valoracion_tipo_item_vta_vendedor->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $vta_vendedor_valoracion_tipo_item_vta_vendedor->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $vta_vendedor_valoracion_tipo_item_vta_vendedor->saveDesdeListado($campo, $valor);
            }
