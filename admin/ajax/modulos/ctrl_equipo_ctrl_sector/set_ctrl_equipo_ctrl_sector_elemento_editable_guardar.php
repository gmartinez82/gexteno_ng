...
if(UsCredencial::getEsAcreditado('CTRL_EQUIPO_CTRL_SECTOR_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = CtrlEquipoCtrlSector::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($ctrl_equipo_ctrl_sector){
        if($ctrl_equipo_ctrl_sector->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$ctrl_equipo_ctrl_sector->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $ctrl_equipo_ctrl_sector->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $ctrl_equipo_ctrl_sector->saveDesdeListado($campo, $valor);
            }
