...
if(UsCredencial::getEsAcreditado('EKU_DE_E700_G_DTIP_D_E_G_CAM_ITEM_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE700GDtipDEGCamItem::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e700_g_dtip_d_e_g_cam_item){
        if($eku_de_e700_g_dtip_d_e_g_cam_item->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e700_g_dtip_d_e_g_cam_item->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e700_g_dtip_d_e_g_cam_item->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e700_g_dtip_d_e_g_cam_item->saveDesdeListado($campo, $valor);
            }
