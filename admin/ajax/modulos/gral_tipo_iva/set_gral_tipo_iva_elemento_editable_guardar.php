...
if(UsCredencial::getEsAcreditado('GRAL_TIPO_IVA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralTipoIva::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $gral_tipo_iva = GralTipoIva::getOxId($identificador);
    if($gral_tipo_iva){
        if($gral_tipo_iva->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_tipo_iva->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_tipo_iva->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_tipo_iva->saveDesdeListado($campo, $valor);
            }
