...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_PAGO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoPago::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_pago = EkuParamTipoPago::getOxId($identificador);
    if($eku_param_tipo_pago){
        if($eku_param_tipo_pago->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_pago->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_pago->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_pago->saveDesdeListado($campo, $valor);
            }
