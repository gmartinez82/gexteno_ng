...
<?php if(count($prv_insumos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_prv_insumo'>
  ...
  <tr>
	...
	<td width='150' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=ins_marca_pieza&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('InsMarcaPieza') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='ins_marca_pieza'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_prv_insumo_uno_<?php Gral::_echo($prv_insumo->getId()) ?>" class="uno" identificador="<?php Gral::_echo($prv_insumo->getId()) ?>">
  ...
  <tr id='tr_eliminar_<?php Gral::_echo($prv_insumo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($prv_insumo->getId()) ?>" >
	<td colspan='9' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($prv_insumo->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
...
if(trim($id) == $prv_insumo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($prv_insumo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $prv_insumo->getId()){ 
			include "prv_insumo_adm_masinfo.php";
		}
		?>
	</div>
