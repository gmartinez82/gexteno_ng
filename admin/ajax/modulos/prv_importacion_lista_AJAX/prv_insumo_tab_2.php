...
if (!isset($_POST['pos_worksheet'])) {
    ...
    $prv_importacion->setCantidadTab1(0);
    $prv_importacion->setCantidadTab2(0);
    $prv_importacion->setCantidadTab3(0);
    $prv_importacion->setCantidadTab4(0);
    $prv_importacion->setSeleccionarTodos(0);
    $prv_importacion->setRegistrarImportacion();
    //Gral::prr($prv_importacion);

    $prv_proveedor = $prv_importacion->getPrvProveedor();
    ...
    if ($_FILES['file']['size'] > 10000000) {
        $arr['error'] = 1;
        $arr['error_code'] = 100;
        $arr['error_msg'] = "El archivo es mayor que 10 MB, debes reduzcirlo antes de subirlo.";

        //$msg = $msg . "El archivo es mayor que 10 MB, debes reduzcirlo antes de subirlo.";
    ...
    if ($_FILES['file']['type'] != "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" && $_FILES['file']['type'] != "application/vnd.ms-excel") {
        ...
        $arr['error'] = 1;
        $arr['error_code'] = 105;
        $arr['error_msg'] = "El archivo tiene que ser formato XLS o XLSX. Otros formatos de archivos no son permitidos. Tipo: " . $_FILES['file']['type'];
    ...
    $nombre_archivo = $prv_importacion->getId() . '.' . $extension;
    ...
    // -----------------------------------------------------
    // se registra el nombre del archivo en el campo codigo
    $prv_importacion->setCodigo($nombre_archivo);
    ...
    if ($uploadedfileload) {
        if (!move_uploaded_file($path, $path_destino_tab_1)) {
            $arr['error'] = 1;
            $arr['error_code'] = 108;
            $arr['error_msg'] = "Error al subir el archivo. ";
    ...
    // si esta cargado el arr es porque existe algun error y no continua con la ejecucion
    if ($arr && is_array($arr)) {
    ...
    if ($prv_importacion) {
        $nombre_archivo = $prv_importacion->getCodigo();
        $path_destino_tab_1 = Gral::getPathAbs() . 'excel/tab_1/' . $nombre_archivo;
        $path_destino_tab_2 = Gral::getPathAbs() . 'excel/tab_2/' . $nombre_archivo;
    } else {
    ...
    echo $extension = trim(end(explode(".", $nombre_archivo)));
...
//            if ($cell->getValue()!="") {
        ...
        if (!$vacia) {
            ...
            foreach ($cellIterator as $cell) {
                ...
                /*
                  $prefijo_proveedor = $prv_proveedor->getCodigoMin();

                  if(trim($prefijo_proveedor) != ''){
                  $valor_saneado = str_replace($prefijo_proveedor, '', $valor_saneado);
                  $valor_saneado_con_prefijo = $prefijo_proveedor.'-'.$valor_saneado;
                  }else{
                  $valor_saneado_con_prefijo = $valor_saneado;
                  }
                 * 
                 */

                $type = PHPExcel_Cell_DataType::TYPE_STRING;
                $objWorksheet->getCellByColumnAndRow($columna, $fila)->setValueExplicit(' ' . $valor_saneado, $type);

                //$objPHPExcel->getActiveSheet()->setCellValueExplicit('A1', '0029', PHPExcel_Cell_DataType::TYPE_STRING);
                //$objPHPExcel->getActiveSheet()->getStyle('A1')->getNumberFormat()->setFormatCode('000000');
                //$objWorksheet->setCellValueExplicitByColumnAndRow($columna, $fila, '0078956', $type);
    ...
    $prv_importacion->setCantidadTab1($cantidad_tab1);
    $prv_importacion->setCantidadTab2($cantidad_tab2);
    $prv_importacion->save();
...
<form action="" method="POST" name="formulario_configurar_columnas" id="formulario_configurar_columnas">
    ...
    <table class="display" id="tabla_tabs_2">
        ...
        <?php //include "prv_insumo_tab_2_html.php" ?>
        <?php //include "prv_insumo_tab_2_data.php" ?>
        <?php include "prv_insumo_tab_2_data_php.php" ?>
