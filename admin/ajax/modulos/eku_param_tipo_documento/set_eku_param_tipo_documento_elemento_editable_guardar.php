...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_DOCUMENTO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoDocumento::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_documento = EkuParamTipoDocumento::getOxId($identificador);
    if($eku_param_tipo_documento){
        if($eku_param_tipo_documento->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_documento->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_documento->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_documento->saveDesdeListado($campo, $valor);
            }
