...
if(UsCredencial::getEsAcreditado('EKU_DE_E020_G_DTIP_D_E_G_COMP_PUB_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE020GDtipDEGCompPub::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_e020_g_dtip_d_e_g_comp_pub = EkuDeE020GDtipDEGCompPub::getOxId($identificador);
    if($eku_de_e020_g_dtip_d_e_g_comp_pub){
        if($eku_de_e020_g_dtip_d_e_g_comp_pub->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e020_g_dtip_d_e_g_comp_pub->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e020_g_dtip_d_e_g_comp_pub->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e020_g_dtip_d_e_g_comp_pub->saveDesdeListado($campo, $valor);
            }
