...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/gral_ruta.php' ?>

<?php if(count($gral_rutas) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_gral_ruta'>
  ...
  <tr id="tr_gral_ruta_uno_<?php Gral::_echo($gral_ruta->getId()) ?>" class="uno" identificador="<?php Gral::_echo($gral_ruta->getId()) ?>">
  	<?php include "gral_ruta_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($gral_ruta->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($gral_ruta->getId()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'>
            <div id='div_eliminar_<?php Gral::_echo($gral_ruta->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
                <br />
                <div>
                      <input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($gral_ruta->getId()) ?>,0)'/>
                </div>
            </div>		
        </td>
	<td align='center'></td>
...
if(trim($id) == $gral_ruta->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($gral_ruta->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='5' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $gral_ruta->getId()){ 
			include "gral_ruta_adm_masinfo.php";
		}
		?>
	</div>
