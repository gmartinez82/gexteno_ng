...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/gral_escenario.php' ?>

<?php if(count($gral_escenarios) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_gral_escenario'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_gral_escenario_uno_<?php Gral::_echo($gral_escenario->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($gral_escenario->getId()) ?>" hash="<?php Gral::_echo($gral_escenario->getHash()) ?>" >
  	<?php include "gral_escenario_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($gral_escenario->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($gral_escenario->getId()) ?>" hash="<?php Gral::_echo($gral_escenario->getHash()) ?>" >
	<td colspan='6' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($gral_escenario->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($gral_escenario->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($gral_escenario->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $gral_escenario->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($gral_escenario->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='6' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $gral_escenario->getId()){ 
			include "gral_escenario_adm_masinfo.php";
		}
		?>
	</div>
