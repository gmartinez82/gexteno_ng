...
if(UsCredencial::getEsAcreditado('GRAL_ESCENARIO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralEscenario::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $gral_escenario = GralEscenario::getOxId($identificador);
    if($gral_escenario){
        if($gral_escenario->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_escenario->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_escenario->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_escenario->saveDesdeListado($campo, $valor);
            }
