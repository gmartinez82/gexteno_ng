...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_NATURALEZA_TRANSPORTISTA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoNaturalezaTransportista::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_tipo_naturaleza_transportista){
        if($eku_param_tipo_naturaleza_transportista->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_naturaleza_transportista->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_naturaleza_transportista->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_naturaleza_transportista->saveDesdeListado($campo, $valor);
            }
