...
if(UsCredencial::getEsAcreditado('CLI_CLIENTE_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = CliCliente::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $cli_cliente = CliCliente::getOxId($identificador);
    if($cli_cliente){
        if($cli_cliente->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$cli_cliente->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $cli_cliente->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $cli_cliente->saveDesdeListado($campo, $valor);
            }
