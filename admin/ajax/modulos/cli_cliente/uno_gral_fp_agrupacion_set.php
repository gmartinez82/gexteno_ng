...
<div class='list <?php echo $css_sel ?>'>
    ...
    <label class='link'>
        ...
        <div class="trigger wopenModal boton" archivo="ajax/modulos/cli_cliente_gral_fp_agrupacion/cli_cliente_gral_fp_agrupacion_alta.php?id=<?php Gral::_echo($cli_cliente_gral_fp_agrupacion_id) ?>" contenedor="div_modal" ancho="950" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($gral_fp_agrupacion_relacionado->getId()) ?>, 'gral_fp_agrupacion', 'cli_cliente', <?php Gral::_echo($o_padre->getId()) ?>, 'CliCliente', 'CliClienteGralFpAgrupacion')" title="Editar CliClienteGralFpAgrupacion"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>

        <?php if(UsCredencial::getEsAcreditado('GRAL_FP_AGRUPACION_ALTA')){ ?>	
        <a class='boton' href='gral_fp_agrupacion_alta.php?id=<?php echo $gral_fp_agrupacion_relacionado->getId() ?>&hash=<?php echo $gral_fp_agrupacion_relacionado->getHash() ?>' title="<?php Lang::_lang('Ver/Editar') ?>: '<?php Gral::_echo($gral_fp_agrupacion_relacionado->getDescripcion()) ?>' "><?php Lang::_lang('Ver') ?></a>
        <?php } ?>
    </label>
