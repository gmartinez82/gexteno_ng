...
if(UsCredencial::getEsAcreditado('PRD_ORDEN_TRABAJO_OPERACION_ESTADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdOrdenTrabajoOperacionEstado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_orden_trabajo_operacion_estado = PrdOrdenTrabajoOperacionEstado::getOxId($identificador);
    if($prd_orden_trabajo_operacion_estado){
        if($prd_orden_trabajo_operacion_estado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_orden_trabajo_operacion_estado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_orden_trabajo_operacion_estado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_orden_trabajo_operacion_estado->saveDesdeListado($campo, $valor);
            }
