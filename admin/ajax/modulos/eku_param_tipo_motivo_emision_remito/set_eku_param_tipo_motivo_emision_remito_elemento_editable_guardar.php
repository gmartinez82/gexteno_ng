...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_MOTIVO_EMISION_REMITO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoMotivoEmisionRemito::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_tipo_motivo_emision_remito){
        if($eku_param_tipo_motivo_emision_remito->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_motivo_emision_remito->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_motivo_emision_remito->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_motivo_emision_remito->saveDesdeListado($campo, $valor);
            }
