...
if(UsCredencial::getEsAcreditado('GRAL_SUCURSAL_CLI_CLIENTE_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralSucursalCliCliente::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $gral_sucursal_cli_cliente = GralSucursalCliCliente::getOxId($identificador);
    if($gral_sucursal_cli_cliente){
        if($gral_sucursal_cli_cliente->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_sucursal_cli_cliente->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_sucursal_cli_cliente->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_sucursal_cli_cliente->saveDesdeListado($campo, $valor);
            }
