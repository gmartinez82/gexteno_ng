...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_NATURALEZA_VENDEDOR_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoNaturalezaVendedor::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_naturaleza_vendedor = EkuParamTipoNaturalezaVendedor::getOxId($identificador);
    if($eku_param_tipo_naturaleza_vendedor){
        if($eku_param_tipo_naturaleza_vendedor->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_naturaleza_vendedor->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_naturaleza_vendedor->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_naturaleza_vendedor->saveDesdeListado($campo, $valor);
            }
