...
if(UsCredencial::getEsAcreditado('EKU_DE_E620_G_DTIP_D_E_G_CAM_COND_G_PAG_TAR_C_D_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE620GDtipDEGCamCondGPagTarCD::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e620_g_dtip_d_e_g_cam_cond_g_pag_tar_c_d){
        if($eku_de_e620_g_dtip_d_e_g_cam_cond_g_pag_tar_c_d->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e620_g_dtip_d_e_g_cam_cond_g_pag_tar_c_d->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e620_g_dtip_d_e_g_cam_cond_g_pag_tar_c_d->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e620_g_dtip_d_e_g_cam_cond_g_pag_tar_c_d->saveDesdeListado($campo, $valor);
            }
