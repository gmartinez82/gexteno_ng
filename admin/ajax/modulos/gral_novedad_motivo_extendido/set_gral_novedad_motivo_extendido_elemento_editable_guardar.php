...
if(UsCredencial::getEsAcreditado('GRAL_NOVEDAD_MOTIVO_EXTENDIDO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralNovedadMotivoExtendido::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($gral_novedad_motivo_extendido){
        if($gral_novedad_motivo_extendido->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_novedad_motivo_extendido->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_novedad_motivo_extendido->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_novedad_motivo_extendido->saveDesdeListado($campo, $valor);
            }
