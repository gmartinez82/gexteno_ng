...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_DOCUMENTO_ASOCIADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoDocumentoAsociado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_documento_asociado = EkuParamTipoDocumentoAsociado::getOxId($identificador);
    if($eku_param_tipo_documento_asociado){
        if($eku_param_tipo_documento_asociado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_documento_asociado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_documento_asociado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_documento_asociado->saveDesdeListado($campo, $valor);
            }
