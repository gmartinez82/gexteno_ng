...
if(UsCredencial::getEsAcreditado('EKU_DE_D140_G_DAT_GRAL_OPE_G_RESP_D_E_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeD140GDatGralOpeGRespDE::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_d140_g_dat_gral_ope_g_resp_d_e = EkuDeD140GDatGralOpeGRespDE::getOxId($identificador);
    if($eku_de_d140_g_dat_gral_ope_g_resp_d_e){
        if($eku_de_d140_g_dat_gral_ope_g_resp_d_e->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_d140_g_dat_gral_ope_g_resp_d_e->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_d140_g_dat_gral_ope_g_resp_d_e->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_d140_g_dat_gral_ope_g_resp_d_e->saveDesdeListado($campo, $valor);
            }
