...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/eku_de_d001_g_dat_gral_ope.php' ?>

<?php if(count($eku_de_d001_g_dat_gral_opes) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_eku_de_d001_g_dat_gral_ope'>
    <thead>
    <tr>
	...
	<td width='200' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=eku_d002_dfeemide&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('eku_d002_dFeEmiDE') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='eku_d002_dfeemide'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
    ...
    <tbody>
  <?php 
  foreach($eku_de_d001_g_dat_gral_opes as $eku_de_d001_g_dat_gral_ope){ 
  	$estado = ($eku_de_d001_g_dat_gral_ope->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

    <tr id="tr_eku_de_d001_g_dat_gral_ope_uno_<?php Gral::_echo($eku_de_d001_g_dat_gral_ope->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($eku_de_d001_g_dat_gral_ope->getId()) ?>" hash="<?php Gral::_echo($eku_de_d001_g_dat_gral_ope->getHash()) ?>" >
  	<?php include "eku_de_d001_g_dat_gral_ope_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($eku_de_d001_g_dat_gral_ope->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($eku_de_d001_g_dat_gral_ope->getId()) ?>" hash="<?php Gral::_echo($eku_de_d001_g_dat_gral_ope->getHash()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($eku_de_d001_g_dat_gral_ope->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($eku_de_d001_g_dat_gral_ope->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($eku_de_d001_g_dat_gral_ope->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $eku_de_d001_g_dat_gral_ope->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($eku_de_d001_g_dat_gral_ope->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='7' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $eku_de_d001_g_dat_gral_ope->getId()){ 
			include "eku_de_d001_g_dat_gral_ope_adm_masinfo.php";
		}
		?>
	</div>
