...
if(UsCredencial::getEsAcreditado('GRAL_SUCURSAL_VALORACION_AGRUPACION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralSucursalValoracionAgrupacion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($gral_sucursal_valoracion_agrupacion){
        if($gral_sucursal_valoracion_agrupacion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_sucursal_valoracion_agrupacion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_sucursal_valoracion_agrupacion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_sucursal_valoracion_agrupacion->saveDesdeListado($campo, $valor);
            }
