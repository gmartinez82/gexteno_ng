...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/per_estado.php' ?>

<?php if(count($per_estados) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_per_estado'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_per_estado_uno_<?php Gral::_echo($per_estado->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($per_estado->getId()) ?>" hash="<?php Gral::_echo($per_estado->getHash()) ?>" >
  	<?php include "per_estado_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($per_estado->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($per_estado->getId()) ?>" hash="<?php Gral::_echo($per_estado->getHash()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($per_estado->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($per_estado->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($per_estado->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $per_estado->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($per_estado->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='7' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $per_estado->getId()){ 
			include "per_estado_adm_masinfo.php";
		}
		?>
	</div>
