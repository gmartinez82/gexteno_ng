...
if(UsCredencial::getEsAcreditado('PER_ESTADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerEstado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_estado = PerEstado::getOxId($identificador);
    if($per_estado){
        if($per_estado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_estado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_estado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_estado->saveDesdeListado($campo, $valor);
            }
