...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_TRANSACCION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoTransaccion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_transaccion = EkuParamTipoTransaccion::getOxId($identificador);
    if($eku_param_tipo_transaccion){
        if($eku_param_tipo_transaccion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_transaccion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_transaccion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_transaccion->saveDesdeListado($campo, $valor);
            }
