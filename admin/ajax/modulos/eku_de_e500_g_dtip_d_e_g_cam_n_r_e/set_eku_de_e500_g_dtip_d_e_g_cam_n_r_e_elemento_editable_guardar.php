...
if(UsCredencial::getEsAcreditado('EKU_DE_E500_G_DTIP_D_E_G_CAM_N_R_E_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE500GDtipDEGCamNRE::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e500_g_dtip_d_e_g_cam_n_r_e){
        if($eku_de_e500_g_dtip_d_e_g_cam_n_r_e->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e500_g_dtip_d_e_g_cam_n_r_e->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e500_g_dtip_d_e_g_cam_n_r_e->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e500_g_dtip_d_e_g_cam_n_r_e->saveDesdeListado($campo, $valor);
            }
