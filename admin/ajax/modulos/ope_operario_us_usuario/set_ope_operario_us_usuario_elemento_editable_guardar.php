...
if(UsCredencial::getEsAcreditado('OPE_OPERARIO_US_USUARIO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = OpeOperarioUsUsuario::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $ope_operario_us_usuario = OpeOperarioUsUsuario::getOxId($identificador);
    if($ope_operario_us_usuario){
        if($ope_operario_us_usuario->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$ope_operario_us_usuario->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $ope_operario_us_usuario->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $ope_operario_us_usuario->saveDesdeListado($campo, $valor);
            }
