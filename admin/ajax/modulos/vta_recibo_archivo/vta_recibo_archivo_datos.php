...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/vta_recibo_archivo.php' ?>

<?php if(count($vta_recibo_archivos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_vta_recibo_archivo'>
  ...
  <?php 
  foreach($vta_recibo_archivos as $vta_recibo_archivo){ 
  	$estado = ($vta_recibo_archivo->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

  <tr id="tr_vta_recibo_archivo_uno_<?php Gral::_echo($vta_recibo_archivo->getId()) ?>" class="uno" identificador="<?php Gral::_echo($vta_recibo_archivo->getId()) ?>">
  	<?php include "vta_recibo_archivo_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($vta_recibo_archivo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($vta_recibo_archivo->getId()) ?>" >
	<td colspan='9' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($vta_recibo_archivo->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($vta_recibo_archivo->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $vta_recibo_archivo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($vta_recibo_archivo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $vta_recibo_archivo->getId()){ 
			include "vta_recibo_archivo_adm_masinfo.php";
		}
		?>
	</div>
