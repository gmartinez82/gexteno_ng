...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/conf_categoria.php' ?>

<?php if(count($conf_categorias) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_conf_categoria'>
  ...
  <tr id="tr_conf_categoria_uno_<?php Gral::_echo($conf_categoria->getId()) ?>" class="uno" identificador="<?php Gral::_echo($conf_categoria->getId()) ?>">
  	<?php include "conf_categoria_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($conf_categoria->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($conf_categoria->getId()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($conf_categoria->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($conf_categoria->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $conf_categoria->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($conf_categoria->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $conf_categoria->getId()){ 
			include "conf_categoria_adm_masinfo.php";
		}
		?>
	</div>
