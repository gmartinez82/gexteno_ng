...
$cmb_vta_remito_ajuste_tipo_despacho_id = Gral::getVars(Gral::VARS_POST, "cmb_vta_remito_ajuste_tipo_despacho_id", 0);
...
$cmb_vta_tipo_remito_ajuste_id = Gral::getVars(Gral::VARS_POST, "cmb_vta_tipo_remito_ajuste_id", 0);
...
if ($cmb_vta_remito_ajuste_tipo_despacho_id == 0) {
    $arr["cmb_vta_remito_ajuste_tipo_despacho_id"] = Lang::_lang("Debe seleccionar un tipo de despacho", true);
    $arr["error"] = true;
}else{
    $cmb_vta_remito_ajuste_tipo_despacho = VtaRemitoAjusteTipoDespacho::getOxId($cmb_vta_remito_ajuste_tipo_despacho_id);
    if($cmb_vta_remito_ajuste_tipo_despacho){
        if($cmb_vta_remito_ajuste_tipo_despacho->getCodigo() == VtaRemitoAjusteTipoDespacho::TIPO_ENVIO_DOMICILIO){
        ...
        }elseif($cmb_vta_remito_ajuste_tipo_despacho->getCodigo() == VtaRemitoAjusteTipoDespacho::TIPO_RETIRO_SUCURSAL){
...
if ($cmb_vta_tipo_remito_ajuste_id == 0) {
    $arr["cmb_vta_tipo_remito_ajuste_id"] = Lang::_lang("Debe seleccionar un tipo de remito ajuste", true);
    $arr["error"] = true;
}
...
if (!$arr["error"]) {
    ...
    VtaRemitoAjuste::setInicializarVtaRemitoAjuste(false, $vta_orden_venta_ids, $vta_orden_venta_cantidads, $cli_cliente_id, $cmb_vta_remito_ajuste_tipo_despacho_id, $cmb_cli_centro_recepcion_id, $cmb_gral_sucursal_retiro, $txa_nota_interna, $txa_nota_publica, $cmb_registrar_movimiento_stock, $cmb_pan_panol_id, $cmb_finalizar_remito, $cmb_vta_tipo_remito_ajuste_id, $cmb_gral_empresa_id, $cmb_vta_punto_venta_id);
}
