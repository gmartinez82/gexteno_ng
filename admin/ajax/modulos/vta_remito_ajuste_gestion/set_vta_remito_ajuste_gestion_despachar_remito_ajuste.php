...
if (!$arr["error"]) {
    ...
    if ($vta_remito_ajuste) {
        ...
        $vta_remito_ajuste_estado->setCantidadBultos($txt_cantidad_bultos);
        $vta_remito_ajuste_estado->setPeso($txt_peso);
        ...
        foreach ($vta_remito_ajuste_vta_orden_ventas as $vta_remito_ajuste_vta_orden_venta) {
            ...
            if ($vta_orden_venta->getCantidadDisponibleEnRemito() > 0) {
                $vta_orden_venta->setVtaOrdenVentaEstadoRemision(VtaOrdenVentaTipoEstadoRemision::TIPO_AJUSTE_PARCIAL, $txa_nota_interna);
            } else {
                $vta_orden_venta->setVtaOrdenVentaEstadoRemision(VtaOrdenVentaTipoEstadoRemision::TIPO_AJUSTE_TOTAL, $txa_nota_interna);
            }
        }
