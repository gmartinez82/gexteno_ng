...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/fnd_chq_estado.php' ?>

<?php if(count($fnd_chq_estados) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_fnd_chq_estado'>
  ...
  <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=fnd_chq_cheque_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('FndChqCheque') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='fnd_chq_cheque_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_fnd_chq_estado_uno_<?php Gral::_echo($fnd_chq_estado->getId()) ?>" class="uno" identificador="<?php Gral::_echo($fnd_chq_estado->getId()) ?>">
  	<?php include "fnd_chq_estado_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($fnd_chq_estado->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($fnd_chq_estado->getId()) ?>" >
	<td colspan='9' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($fnd_chq_estado->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($fnd_chq_estado->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $fnd_chq_estado->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($fnd_chq_estado->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $fnd_chq_estado->getId()){ 
			include "fnd_chq_estado_adm_masinfo.php";
		}
		?>
	</div>
