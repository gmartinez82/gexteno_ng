...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/cli_cliente_vta_comprador.php' ?>

<?php if(count($cli_cliente_vta_compradors) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_cli_cliente_vta_comprador'>
  ...
  <tr id="tr_cli_cliente_vta_comprador_uno_<?php Gral::_echo($cli_cliente_vta_comprador->getId()) ?>" class="uno" identificador="<?php Gral::_echo($cli_cliente_vta_comprador->getId()) ?>">
  	<?php include "cli_cliente_vta_comprador_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($cli_cliente_vta_comprador->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($cli_cliente_vta_comprador->getId()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($cli_cliente_vta_comprador->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($cli_cliente_vta_comprador->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $cli_cliente_vta_comprador->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($cli_cliente_vta_comprador->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $cli_cliente_vta_comprador->getId()){ 
			include "cli_cliente_vta_comprador_adm_masinfo.php";
		}
		?>
	</div>
