...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_MOTIVO_EMISION_CREDITO_DEBITO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoMotivoEmisionCreditoDebito::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_tipo_motivo_emision_credito_debito){
        if($eku_param_tipo_motivo_emision_credito_debito->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_motivo_emision_credito_debito->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_motivo_emision_credito_debito->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_motivo_emision_credito_debito->saveDesdeListado($campo, $valor);
            }
