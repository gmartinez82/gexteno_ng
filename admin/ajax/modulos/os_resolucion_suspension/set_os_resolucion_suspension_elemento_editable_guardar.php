...
if(UsCredencial::getEsAcreditado('OS_RESOLUCION_SUSPENSION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = OsResolucionSuspension::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $os_resolucion_suspension = OsResolucionSuspension::getOxId($identificador);
    if($os_resolucion_suspension){
        if($os_resolucion_suspension->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$os_resolucion_suspension->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $os_resolucion_suspension->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $os_resolucion_suspension->saveDesdeListado($campo, $valor);
            }
