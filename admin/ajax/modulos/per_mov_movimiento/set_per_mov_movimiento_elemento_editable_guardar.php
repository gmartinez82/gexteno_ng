...
if(UsCredencial::getEsAcreditado('PER_MOV_MOVIMIENTO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerMovMovimiento::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_mov_movimiento = PerMovMovimiento::getOxId($identificador);
    if($per_mov_movimiento){
        if($per_mov_movimiento->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_mov_movimiento->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_mov_movimiento->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_mov_movimiento->saveDesdeListado($campo, $valor);
            }
