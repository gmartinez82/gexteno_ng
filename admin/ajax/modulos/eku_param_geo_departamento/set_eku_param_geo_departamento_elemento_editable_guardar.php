...
if(UsCredencial::getEsAcreditado('EKU_PARAM_GEO_DEPARTAMENTO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamGeoDepartamento::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_geo_departamento = EkuParamGeoDepartamento::getOxId($identificador);
    if($eku_param_geo_departamento){
        if($eku_param_geo_departamento->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_geo_departamento->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_geo_departamento->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_geo_departamento->saveDesdeListado($campo, $valor);
            }
