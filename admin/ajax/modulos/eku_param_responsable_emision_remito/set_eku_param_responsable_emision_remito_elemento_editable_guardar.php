...
if(UsCredencial::getEsAcreditado('EKU_PARAM_RESPONSABLE_EMISION_REMITO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamResponsableEmisionRemito::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_responsable_emision_remito = EkuParamResponsableEmisionRemito::getOxId($identificador);
    if($eku_param_responsable_emision_remito){
        if($eku_param_responsable_emision_remito->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_responsable_emision_remito->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_responsable_emision_remito->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_responsable_emision_remito->saveDesdeListado($campo, $valor);
            }
