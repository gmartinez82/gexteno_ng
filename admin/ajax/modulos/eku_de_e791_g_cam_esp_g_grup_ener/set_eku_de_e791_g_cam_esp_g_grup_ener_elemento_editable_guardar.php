...
if(UsCredencial::getEsAcreditado('EKU_DE_E791_G_CAM_ESP_G_GRUP_ENER_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE791GCamEspGGrupEner::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_e791_g_cam_esp_g_grup_ener = EkuDeE791GCamEspGGrupEner::getOxId($identificador);
    if($eku_de_e791_g_cam_esp_g_grup_ener){
        if($eku_de_e791_g_cam_esp_g_grup_ener->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e791_g_cam_esp_g_grup_ener->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e791_g_cam_esp_g_grup_ener->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e791_g_cam_esp_g_grup_ener->saveDesdeListado($campo, $valor);
            }
