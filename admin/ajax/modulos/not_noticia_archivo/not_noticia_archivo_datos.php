...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/not_noticia_archivo.php' ?>

<?php if(count($not_noticia_archivos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_not_noticia_archivo'>
  ...
  <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=not_tipo_archivo_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('NotTipoArchivo') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='not_tipo_archivo_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  <tr id="tr_not_noticia_archivo_uno_<?php Gral::_echo($not_noticia_archivo->getId()) ?>" class="uno" identificador="<?php Gral::_echo($not_noticia_archivo->getId()) ?>">
  	<?php include "not_noticia_archivo_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($not_noticia_archivo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($not_noticia_archivo->getId()) ?>" >
	<td colspan='10' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($not_noticia_archivo->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($not_noticia_archivo->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $not_noticia_archivo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($not_noticia_archivo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $not_noticia_archivo->getId()){ 
			include "not_noticia_archivo_adm_masinfo.php";
		}
		?>
	</div>
