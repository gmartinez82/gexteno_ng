...
if(UsCredencial::getEsAcreditado('EKU_PARAM_DENOMINACION_TARJETA_GRAL_FP_MEDIO_PAGO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamDenominacionTarjetaGralFpMedioPago::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_denominacion_tarjeta_gral_fp_medio_pago){
        if($eku_param_denominacion_tarjeta_gral_fp_medio_pago->getHash() == $hash){
            
            // -----------------------------------------------------------------
            // Se ejecutan controles
            // -----------------------------------------------------------------
            eval("\$eku_param_denominacion_tarjeta_gral_fp_medio_pago->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_denominacion_tarjeta_gral_fp_medio_pago->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_denominacion_tarjeta_gral_fp_medio_pago->saveDesdeListado($campo, $valor);
            }
