...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/prd_proceso_productivo.php' ?>

<?php if(count($prd_proceso_productivos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_prd_proceso_productivo'>
    <thead>
    <tr>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=configurado&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('Configurado') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='configurado'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  ?>
    ...
    <tr id="tr_prd_proceso_productivo_uno_<?php Gral::_echo($prd_proceso_productivo->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($prd_proceso_productivo->getId()) ?>" hash="<?php Gral::_echo($prd_proceso_productivo->getHash()) ?>" >
  	<?php include "prd_proceso_productivo_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($prd_proceso_productivo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($prd_proceso_productivo->getId()) ?>" hash="<?php Gral::_echo($prd_proceso_productivo->getHash()) ?>" >
	<td colspan='9' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($prd_proceso_productivo->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($prd_proceso_productivo->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($prd_proceso_productivo->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $prd_proceso_productivo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($prd_proceso_productivo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='9' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $prd_proceso_productivo->getId()){ 
			include "prd_proceso_productivo_adm_masinfo.php";
		}
		?>
	</div>
