...
if(UsCredencial::getEsAcreditado('PRD_PROCESO_PRODUCTIVO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdProcesoProductivo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_proceso_productivo = PrdProcesoProductivo::getOxId($identificador);
    if($prd_proceso_productivo){
        if($prd_proceso_productivo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_proceso_productivo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_proceso_productivo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_proceso_productivo->saveDesdeListado($campo, $valor);
            }
