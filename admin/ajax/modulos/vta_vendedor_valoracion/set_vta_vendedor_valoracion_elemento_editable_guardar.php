...
if(UsCredencial::getEsAcreditado('VTA_VENDEDOR_VALORACION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = VtaVendedorValoracion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($vta_vendedor_valoracion){
        if($vta_vendedor_valoracion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$vta_vendedor_valoracion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $vta_vendedor_valoracion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $vta_vendedor_valoracion->saveDesdeListado($campo, $valor);
            }
