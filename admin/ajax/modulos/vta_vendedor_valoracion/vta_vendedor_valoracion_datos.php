...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/vta_vendedor_valoracion.php' ?>

<?php if(count($vta_vendedor_valoracions) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_vta_vendedor_valoracion'>
    <thead>
    <tr>
  ...
  foreach($vta_vendedor_valoracions as $vta_vendedor_valoracion){ 
  	$estado = ($vta_vendedor_valoracion->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

    <tr id="tr_vta_vendedor_valoracion_uno_<?php Gral::_echo($vta_vendedor_valoracion->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($vta_vendedor_valoracion->getId()) ?>" hash="<?php Gral::_echo($vta_vendedor_valoracion->getHash()) ?>" >
  	<?php include "vta_vendedor_valoracion_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($vta_vendedor_valoracion->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($vta_vendedor_valoracion->getId()) ?>" hash="<?php Gral::_echo($vta_vendedor_valoracion->getHash()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($vta_vendedor_valoracion->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($vta_vendedor_valoracion->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($vta_vendedor_valoracion->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $vta_vendedor_valoracion->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($vta_vendedor_valoracion->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='7' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $vta_vendedor_valoracion->getId()){ 
			include "vta_vendedor_valoracion_adm_masinfo.php";
		}
		?>
	</div>
