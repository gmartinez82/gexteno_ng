...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/prv_descuento_comercial.php' ?>

<?php if(count($prv_descuento_comercials) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_prv_descuento_comercial'>
  ...
  <tr id="tr_prv_descuento_comercial_uno_<?php Gral::_echo($prv_descuento_comercial->getId()) ?>" class="uno" identificador="<?php Gral::_echo($prv_descuento_comercial->getId()) ?>">
  	<?php include "prv_descuento_comercial_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($prv_descuento_comercial->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($prv_descuento_comercial->getId()) ?>" >
	<td colspan='6' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($prv_descuento_comercial->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($prv_descuento_comercial->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $prv_descuento_comercial->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($prv_descuento_comercial->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $prv_descuento_comercial->getId()){ 
			include "prv_descuento_comercial_adm_masinfo.php";
		}
		?>
	</div>
