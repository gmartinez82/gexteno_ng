...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/afip_citi_compras_importaciones.php' ?>

<?php if(count($afip_citi_compras_importacioness) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_afip_citi_compras_importaciones'>
  ...
  <tr>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=afip_citi_importe_neto_gravado&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('afip_citi_importe_neto_gravado') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='afip_citi_importe_neto_gravado'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=afip_citi_alicuota_iva&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('afip_citi_alicuota_iva') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='afip_citi_alicuota_iva'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=afip_citi_importe_impuesto_liquidado&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('afip_citi_importe_impuesto_liquidado') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='afip_citi_importe_impuesto_liquidado'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  foreach($afip_citi_compras_importacioness as $afip_citi_compras_importaciones){ 
  	$estado = ($afip_citi_compras_importaciones->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

  <tr id="tr_afip_citi_compras_importaciones_uno_<?php Gral::_echo($afip_citi_compras_importaciones->getId()) ?>" class="uno" identificador="<?php Gral::_echo($afip_citi_compras_importaciones->getId()) ?>">
  	<?php include "afip_citi_compras_importaciones_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($afip_citi_compras_importaciones->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($afip_citi_compras_importaciones->getId()) ?>" >
	<td colspan='14' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($afip_citi_compras_importaciones->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($afip_citi_compras_importaciones->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $afip_citi_compras_importaciones->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($afip_citi_compras_importaciones->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $afip_citi_compras_importaciones->getId()){ 
			include "afip_citi_compras_importaciones_adm_masinfo.php";
		}
		?>
	</div>
