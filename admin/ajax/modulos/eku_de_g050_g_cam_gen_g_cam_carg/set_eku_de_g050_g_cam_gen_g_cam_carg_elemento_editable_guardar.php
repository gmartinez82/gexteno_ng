...
if(UsCredencial::getEsAcreditado('EKU_DE_G050_G_CAM_GEN_G_CAM_CARG_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeG050GCamGenGCamCarg::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_g050_g_cam_gen_g_cam_carg){
        if($eku_de_g050_g_cam_gen_g_cam_carg->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_g050_g_cam_gen_g_cam_carg->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_g050_g_cam_gen_g_cam_carg->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_g050_g_cam_gen_g_cam_carg->saveDesdeListado($campo, $valor);
            }
