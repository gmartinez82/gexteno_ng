...
if(UsCredencial::getEsAcreditado('GRAL_CONDICION_IVA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralCondicionIva::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $gral_condicion_iva = GralCondicionIva::getOxId($identificador);
    if($gral_condicion_iva){
        if($gral_condicion_iva->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_condicion_iva->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_condicion_iva->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_condicion_iva->saveDesdeListado($campo, $valor);
            }
