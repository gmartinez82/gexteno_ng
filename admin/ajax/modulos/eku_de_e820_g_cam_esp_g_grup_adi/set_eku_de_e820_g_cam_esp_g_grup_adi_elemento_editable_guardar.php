...
if(UsCredencial::getEsAcreditado('EKU_DE_E820_G_CAM_ESP_G_GRUP_ADI_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE820GCamEspGGrupAdi::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e820_g_cam_esp_g_grup_adi){
        if($eku_de_e820_g_cam_esp_g_grup_adi->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e820_g_cam_esp_g_grup_adi->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e820_g_cam_esp_g_grup_adi->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e820_g_cam_esp_g_grup_adi->saveDesdeListado($campo, $valor);
            }
