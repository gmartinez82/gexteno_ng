...
if(UsCredencial::getEsAcreditado('EKU_DE_ARSCH01_RESP_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeArsch01Resp::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_arsch01_resp = EkuDeArsch01Resp::getOxId($identificador);
    if($eku_de_arsch01_resp){
        if($eku_de_arsch01_resp->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_arsch01_resp->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_arsch01_resp->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_arsch01_resp->saveDesdeListado($campo, $valor);
            }
