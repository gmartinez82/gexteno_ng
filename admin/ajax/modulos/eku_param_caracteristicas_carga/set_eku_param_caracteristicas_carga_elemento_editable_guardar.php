...
if(UsCredencial::getEsAcreditado('EKU_PARAM_CARACTERISTICAS_CARGA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamCaracteristicasCarga::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_caracteristicas_carga = EkuParamCaracteristicasCarga::getOxId($identificador);
    if($eku_param_caracteristicas_carga){
        if($eku_param_caracteristicas_carga->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_caracteristicas_carga->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_caracteristicas_carga->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_caracteristicas_carga->saveDesdeListado($campo, $valor);
            }
