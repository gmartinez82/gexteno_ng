...
if(UsCredencial::getEsAcreditado('EKU_DE_E770_G_DTIP_D_E_G_CAM_ITEM_G_VEH_NUEVO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE770GDtipDEGCamItemGVehNuevo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e770_g_dtip_d_e_g_cam_item_g_veh_nuevo){
        if($eku_de_e770_g_dtip_d_e_g_cam_item_g_veh_nuevo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e770_g_dtip_d_e_g_cam_item_g_veh_nuevo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e770_g_dtip_d_e_g_cam_item_g_veh_nuevo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e770_g_dtip_d_e_g_cam_item_g_veh_nuevo->saveDesdeListado($campo, $valor);
            }
