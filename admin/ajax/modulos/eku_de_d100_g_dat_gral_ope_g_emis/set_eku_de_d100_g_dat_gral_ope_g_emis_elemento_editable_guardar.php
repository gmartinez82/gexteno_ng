...
if(UsCredencial::getEsAcreditado('EKU_DE_D100_G_DAT_GRAL_OPE_G_EMIS_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeD100GDatGralOpeGEmis::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_d100_g_dat_gral_ope_g_emis = EkuDeD100GDatGralOpeGEmis::getOxId($identificador);
    if($eku_de_d100_g_dat_gral_ope_g_emis){
        if($eku_de_d100_g_dat_gral_ope_g_emis->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_d100_g_dat_gral_ope_g_emis->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_d100_g_dat_gral_ope_g_emis->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_d100_g_dat_gral_ope_g_emis->saveDesdeListado($campo, $valor);
            }
