...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_OPERACION_CLI_TIPO_CLIENTE_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoOperacionCliTipoCliente::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_tipo_operacion_cli_tipo_cliente){
        if($eku_param_tipo_operacion_cli_tipo_cliente->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_operacion_cli_tipo_cliente->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_operacion_cli_tipo_cliente->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_operacion_cli_tipo_cliente->saveDesdeListado($campo, $valor);
            }
