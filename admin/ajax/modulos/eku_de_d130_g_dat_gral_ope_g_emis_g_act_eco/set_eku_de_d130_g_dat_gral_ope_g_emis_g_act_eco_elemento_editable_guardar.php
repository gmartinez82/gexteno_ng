...
if(UsCredencial::getEsAcreditado('EKU_DE_D130_G_DAT_GRAL_OPE_G_EMIS_G_ACT_ECO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeD130GDatGralOpeGEmisGActEco::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_d130_g_dat_gral_ope_g_emis_g_act_eco){
        if($eku_de_d130_g_dat_gral_ope_g_emis_g_act_eco->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_d130_g_dat_gral_ope_g_emis_g_act_eco->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_d130_g_dat_gral_ope_g_emis_g_act_eco->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_d130_g_dat_gral_ope_g_emis_g_act_eco->saveDesdeListado($campo, $valor);
            }
