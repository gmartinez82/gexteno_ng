...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TRASLADO_TIPO_IDENTIFICACION_VEHICULO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTrasladoTipoIdentificacionVehiculo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_traslado_tipo_identificacion_vehiculo = EkuParamTrasladoTipoIdentificacionVehiculo::getOxId($identificador);
    if($eku_param_traslado_tipo_identificacion_vehiculo){
        if($eku_param_traslado_tipo_identificacion_vehiculo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_traslado_tipo_identificacion_vehiculo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_traslado_tipo_identificacion_vehiculo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_traslado_tipo_identificacion_vehiculo->saveDesdeListado($campo, $valor);
            }
