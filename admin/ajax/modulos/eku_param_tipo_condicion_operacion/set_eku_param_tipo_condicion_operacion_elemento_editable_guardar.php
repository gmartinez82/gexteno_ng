...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_CONDICION_OPERACION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoCondicionOperacion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_condicion_operacion = EkuParamTipoCondicionOperacion::getOxId($identificador);
    if($eku_param_tipo_condicion_operacion){
        if($eku_param_tipo_condicion_operacion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_condicion_operacion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_condicion_operacion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_condicion_operacion->saveDesdeListado($campo, $valor);
            }
