...
if(UsCredencial::getEsAcreditado('CTRL_EQUIPO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = CtrlEquipo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $ctrl_equipo = CtrlEquipo::getOxId($identificador);
    if($ctrl_equipo){
        if($ctrl_equipo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$ctrl_equipo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $ctrl_equipo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $ctrl_equipo->saveDesdeListado($campo, $valor);
            }
