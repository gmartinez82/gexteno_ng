...
if (count($txt_descripcions) > 0) {
    ?>
    <table border='0' align='center' class='listado_vta_ajuste_debe_items' id='listado_vta_ajuste_debe_items_confirmacion' multiseleccion = "<?php echo $control_presupuesto ?>">
        <tr>
        ...
        foreach ($txt_descripcions as $key => $txt_descripcion) {
            ...
            $vta_ajuste_debe_concepto = VtaAjusteDebeConcepto::getOxId($vta_ajuste_debe_concepto_id);
            ...
            ?>
            <tr id="tr_vta_ajuste_debe_uno_<?php Gral::_echo($key) ?>" class="uno">
                <?php //include "bloque_modal_vta_ajuste_debe_listado_uno.php"  ?>

                <td align="center">
                    <div class="vta_ajuste_debe_concepto" id="cmb_vta_ajuste_debe_concepto_id[<?php echo $key ?>]" name="cmb_vta_ajuste_debe_concepto_id[<?php echo $key ?>]">
                        <?php Gral::_echo($vta_ajuste_debe_concepto->getDescripcion()) ?>
                    </div>
