...
$vta_ajuste_debe_vta_orden_venta_ids = Gral::getVars(Gral::VARS_POST, "chk_vta_ajuste_debe_vta_orden_venta", null);
$vta_ajuste_debe_vta_orden_venta_cantidads = Gral::getVars(Gral::VARS_POST, "txt_vta_ajuste_debe_vta_orden_venta_cantidad", null);
...
if (is_null($vta_ajuste_debe_vta_orden_venta_ids)) {
    $arr["error_general"] = Lang::_lang("Debe seleccionar una Orden de Venta.", true);
    $arr["error"] = true;
}

if (is_null($vta_ajuste_debe_vta_orden_venta_cantidads)) {
    $arr["error_general"] .= Lang::_lang(" La cantidad es incorrecta.", true);
    $arr["error"] = true;
}

foreach ($vta_ajuste_debe_vta_orden_venta_cantidads as $vta_ajuste_debe_vta_orden_venta_cantidad) {
    if ($vta_ajuste_debe_vta_orden_venta_cantidad == 0) {
...
if (!$arr["error"]) {
    ...
    $vta_nota_credito = VtaAjusteHaber::setInicializarVtaAjusteHaberDesdeAjusteDebe($vta_ajuste_debe, $vta_ajuste_debe_vta_orden_venta_ids, $vta_ajuste_debe_vta_orden_venta_cantidads, $txa_observacion, $chk_afectar_stock, $cmb_pan_panol_id);
}
