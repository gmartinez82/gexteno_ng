...
// se obtienen variables si es ajuste_debe de orden-venta
...
// se obtienen variables si es ajuste_debe de item
...
$cmb_porcentaje_iva = Gral::getVars(Gral::VARS_POST, "cmb_porcentaje_iva", 0);
...
if (Ctrl::esVacio($txt_fecha_emision) || $txt_fecha_emision == '1900-01-01') {
    $arr["txt_fecha_emision"] = Lang::_lang("Debe ingresar una fecha de emision", true);
    $arr["error"] = true;
}
else
{
    ...
    elseif(!Date::esRangoValido($txt_fecha_emision, date('Y-m-d'))){
        $arr["txt_fecha_emision"] = Lang::_lang("No debe ser mayor a la fecha actual", true);
        $arr["error"] = true;        
    }
}

if (Ctrl::esVacio($txt_fecha_vencimiento || $txt_fecha_vencimiento == '1900-01-01')) {
...
else
{
    ...
    elseif(!Date::esRangoValido($txt_fecha_emision, $txt_fecha_vencimiento)){
        $arr["txt_fecha_vencimiento"] = Lang::_lang("No debe ser menor a la fecha de emision", true);
        $arr["error"] = true;        
    }
}
...
if (!Gral::getExisteConexionInternet($url = Gral::URL_AFIP_DOMINIO, $port = Gral::URL_AFIP_PUERTO)) {
    //$arr["generar_ajuste_debe_online_afip"] = Lang::_lang("No se pudo establecer una conexion con AFIP para generar CAE del comprobante", true);
    //$arr["error"] = true;
}

// -----------------------------------------------------------------------------
// controles de ajuste_debes por orden venta
// -----------------------------------------------------------------------------
if ($tipo == 'orden-venta') {
...
// -----------------------------------------------------------------------------
// controles de ajuste_debes por item
// -----------------------------------------------------------------------------
if ($tipo == 'item') {
    ...
    if (is_null($cmb_vta_ajuste_debe_concepto_ids)) {
        $arr["error_general"] .= Lang::_lang(" Debe seleccionar un concepto.", true);
        $arr["error"] = true;
    } else {
        foreach ($cmb_vta_ajuste_debe_concepto_ids as $key => $cmb_vta_ajuste_debe_concepto_id) {
            if ($cmb_vta_ajuste_debe_concepto_id == '') {
                $arr["cmb_vta_ajuste_debe_concepto_id_" . $key] = Lang::_lang("Debe seleccionar un tipo de concepto.", true);
                $arr["error"] = true;
            }
        }
    }
...
if (!$arr["error"]) {
    ...
    if ($tipo == 'orden-venta') {
        $vta_ajuste_debe = VtaAjusteDebe::setInicializarVtaAjusteDebeOrdenVenta($vta_presupuesto_seleccionado, $cmb_vta_preventista_id, $cmb_vta_comprador_id, $cmb_vta_vendedor_id, $vta_orden_venta_ids, $vta_orden_venta_cantidads, $cli_cliente_id, $txt_persona_descripcion, $cmb_gral_tipo_documento_id, $txt_persona_documento, $txt_persona_email, $gral_empresa_id, $vta_punto_venta_id, $gral_fp_forma_pago_id, $cmb_gral_fp_cuota_id, $txt_fecha_vencimiento, $txt_fecha_emision, $gral_actividad_id, $gral_escenario_id, $vta_tipo_ajuste_debe_id = 0, $ws_fe_param_tipo_concepto_id = 1, $txa_nota_publica, $txa_observacion, $cmb_porcentaje_iva);
    }

    if ($tipo == 'item') {
        $vta_ajuste_debe = VtaAjusteDebe::setInicializarVtaAjusteDebeItem($cmb_vta_preventista_id, $cmb_vta_comprador_id, $cmb_vta_vendedor_id, $gral_empresa_id, $vta_punto_venta_id, $gral_fp_forma_pago_id, $cmb_gral_fp_cuota_id, $txt_fecha_vencimiento, $txt_fecha_emision, $gral_actividad_id, $gral_escenario_id, $cli_cliente_id, $txt_cantidads, $txt_descripcions, $txt_importe_unitarios, $cmb_gral_tipo_iva_ids, $cmb_vta_ajuste_debe_concepto_ids, $txa_nota_publica, $txa_observacion);
    }
}
