...
if(UsCredencial::getEsAcreditado('VTA_PUNTO_VENTA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = VtaPuntoVenta::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $vta_punto_venta = VtaPuntoVenta::getOxId($identificador);
    if($vta_punto_venta){
        if($vta_punto_venta->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$vta_punto_venta->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $vta_punto_venta->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $vta_punto_venta->saveDesdeListado($campo, $valor);
            }
