...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('VTA_PUNTO_VENTA_ALTA_RELACION_CLI_CLIENTE_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_cli_cliente_id_<?php echo $cli_cliente_relacionado->getId() ?>' name='chk_cli_cliente[<?php echo $cli_cliente_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $cli_cliente_relacionado->getId() ?>' relacion_id='<?php echo $cli_cliente_vta_punto_venta_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('VTA_PUNTO_VENTA_ALTA_RELACION_CLI_CLIENTE_ACCIONES_EDITAR')){ ?>	
        <?php if($cli_cliente_vta_punto_venta_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/cli_cliente_vta_punto_venta/cli_cliente_vta_punto_venta_alta.php?id=<?php Gral::_echo($cli_cliente_vta_punto_venta_id) ?>" contenedor="div_modal" ancho="950" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($cli_cliente_relacionado->getId()) ?>, 'cli_cliente', 'vta_punto_venta', <?php Gral::_echo($o_padre->getId()) ?>, 'VtaPuntoVenta', 'CliClienteVtaPuntoVenta')" title="Editar CliClienteVtaPuntoVenta"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
