...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/cntb_ejercicio.php' ?>

<?php if(count($cntb_ejercicios) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_cntb_ejercicio'>
  ...
  <tr id="tr_cntb_ejercicio_uno_<?php Gral::_echo($cntb_ejercicio->getId()) ?>" class="uno" identificador="<?php Gral::_echo($cntb_ejercicio->getId()) ?>">
  	<?php include "cntb_ejercicio_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($cntb_ejercicio->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($cntb_ejercicio->getId()) ?>" >
	<td colspan='6' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($cntb_ejercicio->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($cntb_ejercicio->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $cntb_ejercicio->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($cntb_ejercicio->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $cntb_ejercicio->getId()){ 
			include "cntb_ejercicio_adm_masinfo.php";
		}
		?>
	</div>
