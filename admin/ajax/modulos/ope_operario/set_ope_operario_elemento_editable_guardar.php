...
if(UsCredencial::getEsAcreditado('OPE_OPERARIO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = OpeOperario::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $ope_operario = OpeOperario::getOxId($identificador);
    if($ope_operario){
        if($ope_operario->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$ope_operario->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $ope_operario->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $ope_operario->saveDesdeListado($campo, $valor);
            }
