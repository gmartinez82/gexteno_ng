...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/ope_operario.php' ?>

<?php if(count($ope_operarios) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_ope_operario'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_ope_operario_uno_<?php Gral::_echo($ope_operario->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($ope_operario->getId()) ?>" hash="<?php Gral::_echo($ope_operario->getHash()) ?>" >
  	<?php include "ope_operario_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($ope_operario->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($ope_operario->getId()) ?>" hash="<?php Gral::_echo($ope_operario->getHash()) ?>" >
	<td colspan='6' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($ope_operario->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($ope_operario->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($ope_operario->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $ope_operario->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($ope_operario->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='6' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $ope_operario->getId()){ 
			include "ope_operario_adm_masinfo.php";
		}
		?>
	</div>
