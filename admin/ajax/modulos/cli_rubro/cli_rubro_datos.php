...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/cli_rubro.php' ?>

<?php if(count($cli_rubros) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_cli_rubro'>
  ...
  <tr id="tr_cli_rubro_uno_<?php Gral::_echo($cli_rubro->getId()) ?>" class="uno" identificador="<?php Gral::_echo($cli_rubro->getId()) ?>">
  	<?php include "cli_rubro_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($cli_rubro->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($cli_rubro->getId()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($cli_rubro->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($cli_rubro->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($cli_rubro->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $cli_rubro->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($cli_rubro->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='5' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $cli_rubro->getId()){ 
			include "cli_rubro_adm_masinfo.php";
		}
		?>
	</div>
