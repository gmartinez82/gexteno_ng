...
<td align='center' class='adm_tbl_lineas <?php echo $estado ?> <?php echo $publicado ?>'>
    <div class="cantidad">
        <input step = "<?php echo $step ?>" min="<?php echo $cantidad_minima ?>" max="<?php Gral::_echo($vta_orden_venta->getCantidadDisponibleEnFactura()) ?>" type="text" id="txt_vta_orden_venta_cantidad_<?php echo $vta_orden_venta->getId() ?>" name="txt_vta_orden_venta_cantidad[<?php echo $vta_orden_venta->getId() ?>]" value="<?php Gral::_echo($vta_orden_venta->getCantidadDisponibleEnFactura()) ?>" size="4" class="textbox txt_vta_orden_venta_cantidad" />
    </div>
    <div class="unidad">
...
<td align='right' class='adm_tbl_lineas <?php echo $estado ?> <?php echo $publicado ?>'>
    ...
    <?php if (UsCredencial::getEsAcreditado('VTA_FACTURA_GESTION_GENERAR_FACTURA_MODIFICAR_PRECIO') && false) { ?>
        <div class="importe-unitario neto real">
            <input type="text" id="txt_vta_orden_venta_importe_unitario_<?php echo $vta_orden_venta->getId() ?>" name="txt_vta_orden_venta_importe_unitario[<?php echo $vta_orden_venta->getId() ?>]" value="<?php Gral::_echo(Gral::getImporteDesdeDbToPriceFormat($vta_orden_venta->getImporteUnitario())) ?>" size="8" class="textbox moneda txt_vta_orden_venta_importe_unitario" />
        </div>     
    <?php } ?>

    <div class="ins_tipo_lista_precio_id" title="<?php Gral::_echo($vta_orden_venta->getInsTipoListaPrecio()->getDescripcion()) ?>">	
