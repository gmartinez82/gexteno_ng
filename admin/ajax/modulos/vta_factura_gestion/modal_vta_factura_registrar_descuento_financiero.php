...
$chk_vta_facturas = Gral::getVars(Gral::VARS_POST, 'chk_vta_factura');

if (is_array($chk_vta_facturas)) {
    foreach ($chk_vta_facturas as $vta_factura_id) {
        $vta_factura = VtaFactura::getOxId($vta_factura_id);
        $vta_facturas[] = $vta_factura;
        ...
        <table border='0' align='center' class='listado' id='listado_adm_vta_factura_gestion'>
            ...
            foreach ($vta_facturas as $vta_factura) {
                ...
                <tr id="tr_vta_factura_gestion_uno_<?php Gral::_echo($vta_factura->getId()) ?>" class="uno" identificador="<?php Gral::_echo($vta_factura->getId()) ?>">
                    ...
                    $cantidad_items = count($vta_factura_vta_orden_ventas);
                    ?>

                    <td align='center' class='adm_tbl_lineas <?php echo $noleido ?> <?php echo $destacado ?>'>
                        <div class="checkbox">
                            <input checked type="checkbox" class="textbox chk_vta_factura" id="chk_vta_factura_<?php echo $vta_factura->getId() ?>" name="chk_vta_factura[<?php echo $vta_factura->getId() ?>]" value="<?php echo $vta_factura->getId() ?>" />
                        </div>
                    </td>
        ...
        <div class='datos registrar-descuento-financiero' >
            ...
            <div class="col c1">
                ...
                <div class="div-bloque-items-datos">
                    <?php include "bloque_vta_factura_descuento_financiero_items_datos_x_cli_cliente.php"; ?>
                </div>
            </div>
