...
$gral_empresa = $vta_factura->getGralEmpresa();
$vta_punto_venta = $vta_factura->getVtaPuntoVenta();

$importe_total_neto = $vta_factura->getVtaFacturaSubtotal();
$importe_iva = $vta_factura->getVtaFacturaIva();
...
if ($vta_tipo_origen_factura->getCodigo() == VtaTipoOrigenFactura::ORIGEN_ORDEN_VENTA) {
    if (count($vta_factura_vta_orden_ventas) > 0) {
        ...
        <form id='form_generar_nota_credito' name='form_generar_nota_credito' method='POST' action='' >
            ...
            <table border='0' align='center' class='listado' id='listado_vta_factura_generar_nota_credito' multiseleccion = "<?php echo $control_presupuesto ?>">
                <tr>
                    <td align='center' class='adm_tbl_encabezado'>
                        <?php if ($control_presupuesto) { ?>
                            <input type="checkbox" name="chk_vta_factura_vta_orden_venta_select_all" id="chk_vta_factura_vta_orden_venta_select_all" >
                        <?php } ?>
                    </td>
                ...
                foreach ($vta_factura_vta_orden_ventas as $vta_factura_vta_orden_venta) {
                    ...
                    $vta_presupuesto_id_agrupacion = $vta_presupuesto->getId();
                    if ($vta_factura_vta_orden_venta->getCantidadDisponibleNotaCredito() > 0) {
                        $existe_orden_venta_disponible_para_nc = true;
                        ?>

                        <tr id="tr_vta_factura_vta_orden_venta_uno_<?php Gral::_echo($vta_factura_vta_orden_venta->getId()) ?>" class="uno <?php echo $par ?>" vta_factura_vta_orden_venta_id="<?php Gral::_echo($vta_factura_vta_orden_venta->getId()) ?>">
            ...
            <div class="botonera">
                <button class="boton" id='btn_generar_nota_credito' name='btn_generar_nota_credito' type='button' vta_factura_id="<?php echo $vta_factura_id ?>"><?php Lang::_lang('Generar Nota de Credito') ?></button>
            </div>
...
<?php
if ($vta_tipo_origen_factura->getCodigo() == VtaTipoOrigenFactura::ORIGEN_ITEM) {
    if (count($vta_factura_items) > 0) {
        ...
        <div class="datos">
            <form id='form_generar_nota_credito_item' name='form_generar_nota_credito_item' method='POST' action='' >
                ...
                <div class="col col2">
                    <table border='0' align='center' class='detalle_vta_comprobante_total' id='detalle_vta_factura_total'>
                        <tr class="subtotal">
                        ...
                        <?php if (count($vta_factura_vta_tributos) > 0) { ?>
                            ...
                            <?php foreach ($vta_factura_vta_tributos as $vta_factura_vta_tributo) { ?>
                                <tr class="otros-tributos">
                                    <td>&nbsp;</td>
                                    <td>
                                        <?php Lang::_lang($vta_factura_vta_tributo->getVtaTributo()->getDescripcion()) ?>
                                    </td>
                                    <td align='right'>
                                        <?php Gral::_echoImp($vta_factura_vta_tributo->getImporteTributo()) ?>              
                                    </td>
                                </tr>
                                <?php $importe_total += $vta_factura_vta_tributo->getImporteTributo(); ?>
                            <?php } ?>
                        <?php } ?>
                ...
                <div class="botonera">
                    <button class="boton" id='btn_generar_nota_credito_item' name='btn_generar_nota_credito_item' type='button' vta_factura_id="<?php echo $vta_factura_id ?>"><?php Lang::_lang('Generar Nota de Credito') ?></button>
                </div>
