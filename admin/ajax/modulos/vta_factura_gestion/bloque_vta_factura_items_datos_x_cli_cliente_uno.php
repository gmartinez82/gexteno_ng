...
<tr class="tr-item" key = "<?php echo $key ?>">
    ...
    <td>
        <div class="vta_factura_concepto">
            <?php
            $vta_factura_concepto_id = $cmb_vta_factura_concepto_ids[$key];
            Html::html_dib_select(1, "cmb_vta_factura_concepto_id[" . $key . "]", Gral::getCmbFiltro(VtaFacturaConcepto::getVtaFacturaConceptosCmb(), '...'), $vta_factura_concepto_id, 'textbox vta_factura_concepto_id' . $error_input_css);
            ?>
        </div>
    ...
    <td>
        <label class="boton quitar_item_factura" title="<?php Lang::_lang('Quitar Item a Nota de Credito') ?>" key='<?php echo $key ?>'><img src="imgs/btn_cancelar.png" width="15"></label>
    </td>
</tr>
