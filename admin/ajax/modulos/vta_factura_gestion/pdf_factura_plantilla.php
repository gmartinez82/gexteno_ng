...
$eje_y = 47;
// fecha emision
$pdf->setXY(CONST_EJE_X_CLIENTE_1ER_COL, 47);
$pdf->Cell(1, 3, 'Fecha de Emision: ' . Gral::getFechaToWEB($vta_factura->getFechaEmision()), 0, 1, 'L', 1);

// razon social
$pdf->setXY(CONST_EJE_X_CLIENTE_2DO_COL, 47);
$pdf->Cell(1, 3, 'Cond Venta: ' . $vta_factura->getGralFpFormaPago()->getDescripcion(), 0, 1, 'L', 1);

// razon social
$pdf->setXY(CONST_EJE_X_CLIENTE_1ER_COL, 52);
$pdf->Cell(1, 3, 'Nombre o Razon Social: ' . $vta_factura->getPersonaDescripcion(), 0, 1, 'L', 1);

// documento
$pdf->setXY(CONST_EJE_X_CLIENTE_2DO_COL, 52);
$pdf->Cell(1, 3, 'C.I. o RUC: ' . $vta_factura->getPersonaDocumento(), 0, 1, 'L', 1);

// domicilio
$pdf->setXY(CONST_EJE_X_CLIENTE_1ER_COL, 57);
$pdf->Cell(1, 3, 'Direccion: ' . $vta_factura->getDomicilioLegal(), 0, 1, 'L', 1);

// numero de nota de remito asociada a la factura
$vta_remitos_vinculados = $vta_factura->getVtaRemitosVinculados();
foreach ($vta_remitos_vinculados as $vta_remitos_vinculado) {
    $remito_codigo = $vta_remitos_vinculado->getCodigo();
}
$pdf->setXY(CONST_EJE_X_CLIENTE_2DO_COL, 57);
...
foreach($vta_factura_vta_orden_ventas as $vta_factura_vta_orden_venta){
    ...
    $importe_total_iva_exento_incluido = $vta_factura_vta_orden_venta->getImporteTotalPorTipoIVA(GralTipoIva::TIPO_EXENTO);
    $importe_total_iva_5_incluido = $vta_factura_vta_orden_venta->getImporteTotalPorTipoIVA(GralTipoIva::TIPO_5);
    $importe_total_iva_10_incluido = $vta_factura_vta_orden_venta->getImporteTotalPorTipoIVA(GralTipoIva::TIPO_10);

    // cantidad
    $pdf->setXY(CONST_EJE_X_PRODUCTO_CANTIDAD, $eje_y);
    //$pdf->Cell(1, 3, 123456, 0, 1, 'C', 1);//FORMATO
    $pdf->Cell(1, 3, $vta_factura_vta_orden_venta->getCantidad(), 0, 1, 'C', 1);
    ...
    $pdf->Cell(1, 3, Gral::_echoImp($vta_factura_vta_orden_venta->getImporteUnitarioParaComprobante(), false, true, ' '), 0, 1, 'R', 1);
...
foreach($vta_factura_items as $vta_factura_item){
    ...
    $importe_total_iva_exento_incluido = $vta_factura_item->getImporteTotalPorTipoIVA(GralTipoIva::TIPO_EXENTO);
    $importe_total_iva_5_incluido = $vta_factura_item->getImporteTotalPorTipoIVA(GralTipoIva::TIPO_5);
    $importe_total_iva_10_incluido = $vta_factura_item->getImporteTotalPorTipoIVA(GralTipoIva::TIPO_10);

    // cantidad
    $pdf->setXY(CONST_EJE_X_PRODUCTO_CANTIDAD, $eje_y);
    //$pdf->Cell(1, 3, 123456, 0, 1, 'C', 1);//FORMATO
    $pdf->Cell(1, 3, $vta_factura_item->getCantidad(), 0, 1, 'C', 1);

    // descripcion
    $pdf->setXY(CONST_EJE_X_PRODUCTO_DESCRIPCION, $eje_y);
    $pdf->MultiCell(80, 5, $vta_factura_item->getDescripcion(), 0, 'L', false, 1);

    // importe unitario
    $pdf->setXY(CONST_EJE_X_PRODUCTO_IMP_UNITARIO, $eje_y);
    //$pdf->Cell(1, 3, Gral::_echoImp(123456789, false, true, ' '), 0, 1, 'R', 1);//FORMATO
    $pdf->Cell(1, 3, Gral::_echoImp($vta_factura_item->getImporteUnitarioParaComprobante(), false, true, ' '), 0, 1, 'R', 1);
...
// Datos Totalizadores
// -----------------------------------------------------------------------------
$importe_iva_exento = $vta_factura->getImporteTotalPorTipoIVA(GralTipoIva::TIPO_EXENTO);
$importe_iva_5 = $vta_factura->getImporteTotalPorTipoIVA(GralTipoIva::TIPO_5);
$importe_iva_10 = $vta_factura->getImporteTotalPorTipoIVA(GralTipoIva::TIPO_10);
...
$pdf->MultiCell(130, 5, Gral::getNumeroEscritoEnLetras(Gral::rnd($total)), 0, 'L', false, 1);
...
$importe_iva = $vta_factura->getImporteIvaImporte(false, false);
$importe_iva_10 = $vta_factura->getImporteIvaImporte(GralTipoIva::TIPO_10, false);
$importe_iva_5 = $vta_factura->getImporteIvaImporte(GralTipoIva::TIPO_5, false);
