...
if (count($txt_descripcions) > 0) {
    ?>
    <table border='0' align='center' class='listado_vta_factura_items' id='listado_vta_factura_items_confirmacion' multiseleccion = "<?php echo $control_presupuesto ?>">
        <tr>
        ...
        foreach ($txt_descripcions as $key => $txt_descripcion) {
            ...
            $vta_factura_concepto_id = $cmb_vta_factura_concepto_ids[$key];
            ...
            ?>
            <tr id="tr_vta_factura_uno_<?php Gral::_echo($key) ?>" class="uno">
                <?php //include "bloque_modal_vta_factura_listado_uno.php"  ?>

                <td align="center">
                    <div class="vta_factura_concepto" id="cmb_vta_factura_concepto_id[<?php echo $key ?>]" name="cmb_vta_factura_concepto_id[<?php echo $key ?>]">
                        <?php Gral::_echo($vta_factura_concepto->getDescripcion()) ?>
                    </div>
                    <div class="label-error" id="cmb_vta_factura_concepto_id_<?php echo $key ?>_error"></div>
                </td>
