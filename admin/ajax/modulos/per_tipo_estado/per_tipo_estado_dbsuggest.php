...
if($buscar != '...'){
    ...
    $c->add('per_tipo_estado.id', $buscar, Criterio::LIKE, false, Criterio::_AND);

	$c->add('per_tipo_estado.descripcion', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('per_tipo_estado.codigo', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('per_tipo_estado.observacion', $buscar, Criterio::LIKE, false, Criterio::_OR);
    $c->addFinSubconsulta();
}

$c->addTabla('per_tipo_estado');
$c->addOrden('per_tipo_estado.orden', 'asc');
$c->setCriterios();

$p = new Paginador(10, $pag);

$os = PerTipoEstado::getPerTipoEstados($p, $c);
