...
if(UsCredencial::getEsAcreditado('PER_TIPO_ESTADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PerTipoEstado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $per_tipo_estado = PerTipoEstado::getOxId($identificador);
    if($per_tipo_estado){
        if($per_tipo_estado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$per_tipo_estado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $per_tipo_estado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $per_tipo_estado->saveDesdeListado($campo, $valor);
            }
