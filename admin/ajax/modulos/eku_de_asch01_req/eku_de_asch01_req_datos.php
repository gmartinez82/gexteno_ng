...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/eku_de_asch01_req.php' ?>

<?php if(count($eku_de_asch01_reqs) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_eku_de_asch01_req'>
    <thead>
    <tr>
	...
	<td width='200' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=eku_asch02_did&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('eku_asch02_did') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='eku_asch02_did'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
	...
	<td width='200' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=eku_asch03_xde&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('eku_asch03_xde') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='eku_asch03_xde'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  ?>
    ...
    <tr id="tr_eku_de_asch01_req_uno_<?php Gral::_echo($eku_de_asch01_req->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($eku_de_asch01_req->getId()) ?>" hash="<?php Gral::_echo($eku_de_asch01_req->getHash()) ?>" >
  	<?php include "eku_de_asch01_req_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($eku_de_asch01_req->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($eku_de_asch01_req->getId()) ?>" hash="<?php Gral::_echo($eku_de_asch01_req->getHash()) ?>" >
	<td colspan='8' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($eku_de_asch01_req->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($eku_de_asch01_req->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($eku_de_asch01_req->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $eku_de_asch01_req->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($eku_de_asch01_req->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='8' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $eku_de_asch01_req->getId()){ 
			include "eku_de_asch01_req_adm_masinfo.php";
		}
		?>
	</div>
