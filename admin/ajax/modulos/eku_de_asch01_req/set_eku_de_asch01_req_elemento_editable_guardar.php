...
if(UsCredencial::getEsAcreditado('EKU_DE_ASCH01_REQ_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeAsch01Req::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_asch01_req){
        if($eku_de_asch01_req->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_asch01_req->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_asch01_req->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_asch01_req->saveDesdeListado($campo, $valor);
            }
