...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/gen_prca_ejecucion.php' ?>

<?php if(count($gen_prca_ejecucions) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_gen_prca_ejecucion'>
  ...
  <tr id="tr_gen_prca_ejecucion_uno_<?php Gral::_echo($gen_prca_ejecucion->getId()) ?>" class="uno" identificador="<?php Gral::_echo($gen_prca_ejecucion->getId()) ?>">
  	<?php include "gen_prca_ejecucion_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($gen_prca_ejecucion->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($gen_prca_ejecucion->getId()) ?>" >
	<td colspan='11' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($gen_prca_ejecucion->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($gen_prca_ejecucion->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $gen_prca_ejecucion->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($gen_prca_ejecucion->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $gen_prca_ejecucion->getId()){ 
			include "gen_prca_ejecucion_adm_masinfo.php";
		}
		?>
	</div>
