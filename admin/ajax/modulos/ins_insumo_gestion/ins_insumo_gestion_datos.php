
<?php //include Gral::getPathAbs() . 'admin/parciales/buscadores/activos/ins_insumo.php' ?>

<?php if (count($ins_insumos) > 0) { ?>

    <table border='0' align='center' class='listado' id='listado_adm_ins_insumo_gestion'>

        <tr>
            <td width='15' align='center' class='adm_tbl_encabezado'>&nbsp;</td>

            <td width='80' align='center' class='adm_tbl_encabezado'>
                <?php Lang::_lang('Cod Int') ?>
            </td>

            <td width='100' align='center' class='adm_tbl_encabezado'>
                <?php Lang::_lang('Foto') ?>
            </td>

            <td width='380' align='center' class='adm_tbl_encabezado'>
                <?php Lang::_lang('Datos del Producto') ?>
            </td>

            <td width='300' align='center' class='adm_tbl_encabezado'>
                <?php Lang::_lang('Stock') ?>
            </td>
            
            <td width='180' align='center' class='adm_tbl_encabezado'>
                <?php Lang::_lang('Importe') ?>
            </td>
            
            <td width='120' align='center' class='adm_tbl_encabezado'>&nbsp;</td>
        </tr>


        <?php
        foreach ($ins_insumos as $ins_insumo) {
            $estado = ($ins_insumo->getEstado()) ? 'habilitado' : 'deshabilitado';
            ?>

            <tr id="tr_ins_insumo_gestion_uno_<?php Gral::_echo($ins_insumo->getId()) ?>" class="uno" identificador="<?php Gral::_echo($ins_insumo->getId()) ?>">
                <?php include "ins_insumo_gestion_uno.php" ?>
            </tr>


            <tr id='tr_eliminar_<?php Gral::_echo($ins_insumo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($ins_insumo->getId()) ?>" >
                <td colspan='7' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($ins_insumo->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
                        <br />
                        <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
                            <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($ins_insumo->getId()) ?>, 0)'/>
                        </div>
                    </div>		
                </td>
                <td align='center'></td>
            </tr>


            <?php
            // auto hash de mas info de acuerdo al id recibido por url
            $id = Gral::getVars(2, 'id');
            $mi_display = "style='display:none;'";
            if (trim($id) == $ins_insumo->getId()) {
                $mi_display = '';
                $mi_hash = "location.hash = 'mi_" . $id . "';";
            }
            ?>
            <tr id='tr_mi_<?php Gral::_echo($ins_insumo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
                <td colspan='7' align='center' class='adm_tbl_lineas'>


                    <div class="masinfo">
                        <?php
                        if (trim($id) == $ins_insumo->getId()) {
                            include "ins_insumo_adm_masinfo.php";
                        }
                        ?>
                    </div>

                </td>
                <td align='center'></td>
            </tr>


        <?php } ?>

        <tr>
            <td align='center' class="adm_tbl_pie" >&nbsp;</td>
            <td align='center' class="adm_tbl_pie" >&nbsp;</td>
            <td align='center' class="adm_tbl_pie" >&nbsp;</td>
            <td align='center' class="adm_tbl_pie" >&nbsp;</td>
            <td align='center' class="adm_tbl_pie" >&nbsp;</td>
            <td align='center' class="adm_tbl_pie" >&nbsp;</td>
            <td align='center' class="adm_tbl_pie" >&nbsp;</td>
        </tr>

        <tr>
            <td colspan='14' align='center'><?php include Gral::getPathAbs() . 'admin/parciales/paginador_adm.php'; ?></td>
        </tr>
    </table>

<?php } else { ?>

    <div class="mensaje-sin-resultado">
        <div class="mensaje"><?php Lang::_lang('No se encontraron datos para los criterios elegidos') ?></div>
        <div class="paginador-oculto"><?php include Gral::getPathAbs() . 'admin/parciales/paginador_adm.php' ?></div>
    </div>

<?php } ?>


