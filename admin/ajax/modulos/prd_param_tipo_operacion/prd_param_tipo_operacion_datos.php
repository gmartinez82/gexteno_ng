...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/prd_param_tipo_operacion.php' ?>

<?php if(count($prd_param_tipo_operacions) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_prd_param_tipo_operacion'>
    <thead>
    <tr>
    ...
    <tbody>
  <?php 
  foreach($prd_param_tipo_operacions as $prd_param_tipo_operacion){ 
  	$estado = ($prd_param_tipo_operacion->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

    <tr id="tr_prd_param_tipo_operacion_uno_<?php Gral::_echo($prd_param_tipo_operacion->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($prd_param_tipo_operacion->getId()) ?>" hash="<?php Gral::_echo($prd_param_tipo_operacion->getHash()) ?>" >
  	<?php include "prd_param_tipo_operacion_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($prd_param_tipo_operacion->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($prd_param_tipo_operacion->getId()) ?>" hash="<?php Gral::_echo($prd_param_tipo_operacion->getHash()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($prd_param_tipo_operacion->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($prd_param_tipo_operacion->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($prd_param_tipo_operacion->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $prd_param_tipo_operacion->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($prd_param_tipo_operacion->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='7' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $prd_param_tipo_operacion->getId()){ 
			include "prd_param_tipo_operacion_adm_masinfo.php";
		}
		?>
	</div>
