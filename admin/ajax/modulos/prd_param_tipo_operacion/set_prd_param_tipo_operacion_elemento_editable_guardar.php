...
if(UsCredencial::getEsAcreditado('PRD_PARAM_TIPO_OPERACION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdParamTipoOperacion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_param_tipo_operacion = PrdParamTipoOperacion::getOxId($identificador);
    if($prd_param_tipo_operacion){
        if($prd_param_tipo_operacion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_param_tipo_operacion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_param_tipo_operacion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_param_tipo_operacion->saveDesdeListado($campo, $valor);
            }
