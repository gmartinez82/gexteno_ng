...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_CONSTANCIA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoConstancia::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_constancia = EkuParamTipoConstancia::getOxId($identificador);
    if($eku_param_tipo_constancia){
        if($eku_param_tipo_constancia->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_constancia->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_constancia->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_constancia->saveDesdeListado($campo, $valor);
            }
