<table border="0" class="tabla-modal-ficha-emails">
    ...
    <?php
    $pde_recibo_enviados = $pde_recibo->getPdeReciboEnviados();
    $cont = 0;
    foreach ($pde_recibo_enviados as $pde_recibo_enviado) {
        $cont++;
        ...
        <tr>
            <td class="adm_tbl_lineas <?php echo $strong ?>" align="center">
                <div class="fecha">
                    <?php Gral::_echo(Gral::getFechaHoraToWEB($pde_recibo_enviado->getCreado())); ?>
                </div>
            </td>	

            <td class="adm_tbl_lineas <?php echo $strong ?>" align="left">
                <div class="asunto">
                    <?php Gral::_echo($pde_recibo_enviado->getAsunto()); ?>
                </div>
                <div class="destinatario">
                    <?php Gral::_echo($pde_recibo_enviado->getDestinatario()); ?>
                </div>
                <div class="observacion">
                    <?php Gral::_echo($pde_recibo_enviado->getObservacion()); ?>
                </div>                 
            </td>

            <td class="adm_tbl_lineas <?php echo $strong ?>" align="center">
                <div class="estado">
                    <img src="imgs/btn_estado_<?php echo $pde_recibo_enviado->getEstado(); ?>.gif" width="15" alt="hab" />
                </div>
            </td>

            <td class="adm_tbl_lineas <?php echo $strong ?>" align="left">
                <div class="codigo_envio">
                    <?php Gral::_echo($pde_recibo_enviado->getCodigoEnvio()); ?>
                </div>
            </td>

            <td class="adm_tbl_lineas <?php echo $strong ?>" align="center">
                <div class="creado-por-descripcion">
                    <?php Gral::_echo($pde_recibo_enviado->getCreadoPorDescripcion()); ?>
                </div>
            </td>
            ...
            <td align='center' class='adm_tbl_lineas <?php echo $strong ?>' align="center">
                <ul class="adm_botones_acciones_modal_ficha">
                    ...
                    <?php if (UsCredencial::getEsAcreditado('VTA_RECIBO_GESTION_ACCION_COMPROBANTE')) { ?>
                        <li class='adm_botones_accion pde-recibo-comprobante' pde_recibo_enviado_id='<?php echo $pde_recibo_enviado->getId() ?>'>
                    ...
                    <?php if (UsCredencial::getEsAcreditado('VTA_RECIBO_GESTION_ACCION_ENVIAR_POR_CORREO')) { ?>
                        <li class='adm_botones_accion pde-recibo-ver-cuerpo-correo' pde_recibo_enviado_id='<?php echo $pde_recibo_enviado->getId() ?>'>
