...
//$arr_pde_recibo_cheque_infos = Gral::getSes("pde_recibo_cheque_info");
...
$arr_pde_recibo_cheque_infos = Gral::getSes($var_sesion_modulo);

// se realizan los controles de datos
$arr["error"] = false;

if ($prv_proveedor_id == 0) {
    $arr["cmb_prv_proveedor_id"] = Lang::_lang("Seleccionar un cliente", true);
    $arr["error"] = true;
}
...
if (is_null($cmb_pde_recibo_concepto_ids)) {
    ...
    foreach ($cmb_pde_recibo_concepto_ids as $key => $cmb_pde_recibo_concepto_id) {
        if ($cmb_pde_recibo_concepto_id == '') {
            $arr["cmb_pde_recibo_concepto_id_" . $key] = Lang::_lang("Seleccionar un concepto", true);
            $arr["error"] = true;
        }
    }
}
