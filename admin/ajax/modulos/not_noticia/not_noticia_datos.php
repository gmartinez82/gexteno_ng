...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/not_noticia.php' ?>

<?php if(count($not_noticias) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_not_noticia'>
  ...
  <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=not_categoria_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('NotCategoria') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='not_categoria_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
			
	<td width='10' align='center' class='adm_tbl_encabezado'>
  ...
  <tr id="tr_not_noticia_uno_<?php Gral::_echo($not_noticia->getId()) ?>" class="uno" identificador="<?php Gral::_echo($not_noticia->getId()) ?>">
  	<?php include "not_noticia_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($not_noticia->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($not_noticia->getId()) ?>" >
	<td colspan='8' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($not_noticia->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($not_noticia->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $not_noticia->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($not_noticia->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $not_noticia->getId()){ 
			include "not_noticia_adm_masinfo.php";
		}
		?>
	</div>
