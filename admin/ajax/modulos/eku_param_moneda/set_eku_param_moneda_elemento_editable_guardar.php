...
if(UsCredencial::getEsAcreditado('EKU_PARAM_MONEDA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamMoneda::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_moneda = EkuParamMoneda::getOxId($identificador);
    if($eku_param_moneda){
        if($eku_param_moneda->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_moneda->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_moneda->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_moneda->saveDesdeListado($campo, $valor);
            }
