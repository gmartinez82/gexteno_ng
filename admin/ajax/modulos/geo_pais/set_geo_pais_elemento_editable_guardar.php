...
if(UsCredencial::getEsAcreditado('GEO_PAIS_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GeoPais::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $geo_pais = GeoPais::getOxId($identificador);
    if($geo_pais){
        if($geo_pais->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$geo_pais->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $geo_pais->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $geo_pais->saveDesdeListado($campo, $valor);
            }
