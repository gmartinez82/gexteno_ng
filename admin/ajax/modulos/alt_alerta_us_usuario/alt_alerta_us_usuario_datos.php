...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/alt_alerta_us_usuario.php' ?>

<?php if(count($alt_alerta_us_usuarios) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_alt_alerta_us_usuario'>
  ...
  <?php 
  foreach($alt_alerta_us_usuarios as $alt_alerta_us_usuario){ 
  ?>

  <tr id="tr_alt_alerta_us_usuario_uno_<?php Gral::_echo($alt_alerta_us_usuario->getId()) ?>" class="uno" identificador="<?php Gral::_echo($alt_alerta_us_usuario->getId()) ?>">
  	<?php include "alt_alerta_us_usuario_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($alt_alerta_us_usuario->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($alt_alerta_us_usuario->getId()) ?>" >
	<td colspan='10' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($alt_alerta_us_usuario->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($alt_alerta_us_usuario->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $alt_alerta_us_usuario->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($alt_alerta_us_usuario->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $alt_alerta_us_usuario->getId()){ 
			include "alt_alerta_us_usuario_adm_masinfo.php";
		}
		?>
	</div>
