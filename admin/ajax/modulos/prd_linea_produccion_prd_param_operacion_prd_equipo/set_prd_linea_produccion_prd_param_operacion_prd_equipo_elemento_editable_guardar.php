...
if(UsCredencial::getEsAcreditado('PRD_LINEA_PRODUCCION_PRD_PARAM_OPERACION_PRD_EQUIPO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdLineaProduccionPrdParamOperacionPrdEquipo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_linea_produccion_prd_param_operacion_prd_equipo = PrdLineaProduccionPrdParamOperacionPrdEquipo::getOxId($identificador);
    if($prd_linea_produccion_prd_param_operacion_prd_equipo){
        if($prd_linea_produccion_prd_param_operacion_prd_equipo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_linea_produccion_prd_param_operacion_prd_equipo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_linea_produccion_prd_param_operacion_prd_equipo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_linea_produccion_prd_param_operacion_prd_equipo->saveDesdeListado($campo, $valor);
            }
