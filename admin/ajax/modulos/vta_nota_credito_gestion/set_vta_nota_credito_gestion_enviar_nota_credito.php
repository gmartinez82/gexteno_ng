...
if (!$arr["error"]) {
    ...
    if ($vta_nota_credito) {
        ...
        $strContenidoPdf = $pdf->Output($vta_nota_credito->getNombreArchivoAdjuntoNotaCredito(), 'S');

        $enviar = true;
        //$enviar = false;

        $destinatarios = $txt_direccion_mail;
        $mail_asunto = Gral::getConfig('gral_cliente').' - Nota de Credito #' . $vta_nota_credito->getNumeroComprobanteCompleto() . ' ' . date('d/m/Y H:i');
        $mail_contendio = Gral::getPathAbs() . "mailing/plantillas/GENERAL/index_vta_nota_credito.php";
        $from = Mail::MAIL_CASILLA_REMITENTE;
        $from_name = Mail::MAIL_NOMBRE_REMITENTE;

        $strFilePDF = $vta_nota_credito->getNombreArchivoAdjuntoNotaCredito();

        $archivo_adjunto_urls = array($strFilePDF => $strContenidoPdf);

        $envio_ok = $vta_nota_credito->enviarNotaCreditoEmail($enviar, $destinatarios, $txa_observacion, $archivo_adjunto_urls);
