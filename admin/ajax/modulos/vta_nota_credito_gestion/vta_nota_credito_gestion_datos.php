...
<?php include Gral::getPathAbs() . 'admin/parciales/buscadores/activos/vta_nota_credito_gestion.php' ?>

<?php if (count($vta_nota_creditos) > 0) { ?>

    <table border='0' align='center' class='listado' id='listado_adm_vta_nota_credito_gestion'>
        ...
        <tr>
            ...
            <td width='170' align='center' class='adm_tbl_encabezado'>
                <a class='ordenar' href="?ord=1&c=vta_nota_credito_tipo_estado_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                    <?php //Lang::_lang('VtaNotaCreditoTipoEstado') ?>
                    <?php Lang::_lang('Estado') ?>

                    <?php if ($criterio->getOrdenDato('campo') == 'vta_nota_credito_tipo_estado_id') { ?>
                        <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                    <?php } ?>
                </a>
            </td>
        ...
        foreach ($vta_nota_creditos as $vta_nota_credito) {
            ...
            <tr id="tr_vta_nota_credito_gestion_uno_<?php Gral::_echo($vta_nota_credito->getId()) ?>" class="uno" identificador="<?php Gral::_echo($vta_nota_credito->getId()) ?>">
                <?php include "vta_nota_credito_gestion_uno.php" ?>
            </tr>


            <tr id='tr_eliminar_<?php Gral::_echo($vta_nota_credito->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($vta_nota_credito->getId()) ?>" >
                <td colspan='14' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($vta_nota_credito->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
                        <br />
                        <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
                            <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($vta_nota_credito->getId()) ?>, 0)'/>
                        </div>
                    </div>		</td>
                <td align='center'></td>
            </tr>
...
// auto hash de mas info de acuerdo al id recibido por url
            ...
            if (trim($id) == $vta_nota_credito->getId()) {
                $mi_display = '';
                $mi_hash = "location.hash = 'mi_" . $id . "';";
            }
            ?>
            <tr id='tr_mi_<?php Gral::_echo($vta_nota_credito->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
                <td colspan='14' align='center' class='adm_tbl_lineas'>
                    ...
                    <div class="masinfo">
                        <?php
                        if (trim($id) == $vta_nota_credito->getId()) {
                            include "vta_nota_credito_gestion_adm_masinfo.php";
                        }
                        ?>
                    </div>
