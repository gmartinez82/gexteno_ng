...
if(UsCredencial::getEsAcreditado('EKU_DE_E650_G_DTIP_D_E_G_CAM_COND_G_PAG_CRED_G_CUOTAS_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeE650GDtipDEGCamCondGPagCredGCuotas::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_e650_g_dtip_d_e_g_cam_cond_g_pag_cred_g_cuotas){
        if($eku_de_e650_g_dtip_d_e_g_cam_cond_g_pag_cred_g_cuotas->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_e650_g_dtip_d_e_g_cam_cond_g_pag_cred_g_cuotas->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_e650_g_dtip_d_e_g_cam_cond_g_pag_cred_g_cuotas->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_e650_g_dtip_d_e_g_cam_cond_g_pag_cred_g_cuotas->saveDesdeListado($campo, $valor);
            }
