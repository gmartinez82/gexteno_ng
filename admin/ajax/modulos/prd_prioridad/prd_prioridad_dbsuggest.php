...
if($buscar != '...'){
    ...
    $c->add('prd_prioridad.id', $buscar, Criterio::LIKE, false, Criterio::_AND);

	$c->add('prd_prioridad.descripcion', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('prd_prioridad.codigo', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('prd_prioridad.observacion', $buscar, Criterio::LIKE, false, Criterio::_OR);
    $c->addFinSubconsulta();
}

$c->addTabla('prd_prioridad');
$c->addOrden('prd_prioridad.orden', 'asc');
$c->setCriterios();

$p = new Paginador(10, $pag);

$os = PrdPrioridad::getPrdPrioridads($p, $c);
