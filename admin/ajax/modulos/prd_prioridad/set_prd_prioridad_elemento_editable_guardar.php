...
if(UsCredencial::getEsAcreditado('PRD_PRIORIDAD_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdPrioridad::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_prioridad = PrdPrioridad::getOxId($identificador);
    if($prd_prioridad){
        if($prd_prioridad->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_prioridad->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_prioridad->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_prioridad->saveDesdeListado($campo, $valor);
            }
