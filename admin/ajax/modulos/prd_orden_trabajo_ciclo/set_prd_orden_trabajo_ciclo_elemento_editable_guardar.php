...
if(UsCredencial::getEsAcreditado('PRD_ORDEN_TRABAJO_CICLO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdOrdenTrabajoCiclo::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_orden_trabajo_ciclo = PrdOrdenTrabajoCiclo::getOxId($identificador);
    if($prd_orden_trabajo_ciclo){
        if($prd_orden_trabajo_ciclo->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_orden_trabajo_ciclo->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_orden_trabajo_ciclo->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_orden_trabajo_ciclo->saveDesdeListado($campo, $valor);
            }
