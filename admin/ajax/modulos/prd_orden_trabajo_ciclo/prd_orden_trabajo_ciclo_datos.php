...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/prd_orden_trabajo_ciclo.php' ?>

<?php if(count($prd_orden_trabajo_ciclos) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_prd_orden_trabajo_ciclo'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_prd_orden_trabajo_ciclo_uno_<?php Gral::_echo($prd_orden_trabajo_ciclo->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($prd_orden_trabajo_ciclo->getId()) ?>" hash="<?php Gral::_echo($prd_orden_trabajo_ciclo->getHash()) ?>" >
  	<?php include "prd_orden_trabajo_ciclo_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($prd_orden_trabajo_ciclo->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($prd_orden_trabajo_ciclo->getId()) ?>" hash="<?php Gral::_echo($prd_orden_trabajo_ciclo->getHash()) ?>" >
	<td colspan='8' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($prd_orden_trabajo_ciclo->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($prd_orden_trabajo_ciclo->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($prd_orden_trabajo_ciclo->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $prd_orden_trabajo_ciclo->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($prd_orden_trabajo_ciclo->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='8' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $prd_orden_trabajo_ciclo->getId()){ 
			include "prd_orden_trabajo_ciclo_adm_masinfo.php";
		}
		?>
	</div>
