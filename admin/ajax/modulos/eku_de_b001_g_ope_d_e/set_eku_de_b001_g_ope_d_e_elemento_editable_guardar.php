...
if(UsCredencial::getEsAcreditado('EKU_DE_B001_G_OPE_D_E_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeB001GOpeDE::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_b001_g_ope_d_e){
        if($eku_de_b001_g_ope_d_e->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_b001_g_ope_d_e->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_b001_g_ope_d_e->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_b001_g_ope_d_e->saveDesdeListado($campo, $valor);
            }
