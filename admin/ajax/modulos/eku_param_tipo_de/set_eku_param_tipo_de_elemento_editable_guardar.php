...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_DE_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoDe::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_de = EkuParamTipoDe::getOxId($identificador);
    if($eku_param_tipo_de){
        if($eku_param_tipo_de->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_de->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_de->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_de->saveDesdeListado($campo, $valor);
            }
