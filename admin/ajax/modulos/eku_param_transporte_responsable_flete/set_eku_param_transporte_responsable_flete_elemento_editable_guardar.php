...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TRANSPORTE_RESPONSABLE_FLETE_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTransporteResponsableFlete::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_transporte_responsable_flete = EkuParamTransporteResponsableFlete::getOxId($identificador);
    if($eku_param_transporte_responsable_flete){
        if($eku_param_transporte_responsable_flete->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_transporte_responsable_flete->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_transporte_responsable_flete->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_transporte_responsable_flete->saveDesdeListado($campo, $valor);
            }
