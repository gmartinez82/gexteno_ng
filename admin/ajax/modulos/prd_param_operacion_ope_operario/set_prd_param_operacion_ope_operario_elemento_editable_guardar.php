...
if(UsCredencial::getEsAcreditado('PRD_PARAM_OPERACION_OPE_OPERARIO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdParamOperacionOpeOperario::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_param_operacion_ope_operario = PrdParamOperacionOpeOperario::getOxId($identificador);
    if($prd_param_operacion_ope_operario){
        if($prd_param_operacion_ope_operario->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_param_operacion_ope_operario->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_param_operacion_ope_operario->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_param_operacion_ope_operario->saveDesdeListado($campo, $valor);
            }
