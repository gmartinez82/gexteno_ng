...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/cli_subcategoria.php' ?>

<?php if(count($cli_subcategorias) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_cli_subcategoria'>
  ...
  <tr id="tr_cli_subcategoria_uno_<?php Gral::_echo($cli_subcategoria->getId()) ?>" class="uno" identificador="<?php Gral::_echo($cli_subcategoria->getId()) ?>">
  	<?php include "cli_subcategoria_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($cli_subcategoria->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($cli_subcategoria->getId()) ?>" >
	<td colspan='6' align='center' class='adm_tbl_lineas'>
            <div id='div_eliminar_<?php Gral::_echo($cli_subcategoria->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
                <br />
                <div>
                      <input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($cli_subcategoria->getId()) ?>,0)'/>
                </div>
            </div>		
        </td>
	<td align='center'></td>
...
if(trim($id) == $cli_subcategoria->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($cli_subcategoria->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='6' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $cli_subcategoria->getId()){ 
			include "cli_subcategoria_adm_masinfo.php";
		}
		?>
	</div>
