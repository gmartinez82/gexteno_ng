...
if(UsCredencial::getEsAcreditado('GRAL_TIPO_DOCUMENTO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralTipoDocumento::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $gral_tipo_documento = GralTipoDocumento::getOxId($identificador);
    if($gral_tipo_documento){
        if($gral_tipo_documento->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_tipo_documento->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_tipo_documento->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_tipo_documento->saveDesdeListado($campo, $valor);
            }
