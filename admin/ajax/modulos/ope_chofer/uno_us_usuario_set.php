...
if(in_array($us_usuario_relacionado->getId(), $o_ids)){
    ...
    $array = array(
        array('campo' => 'ope_chofer_id', 'valor' => $o_padre->getId()),
...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('OPE_CHOFER_ALTA_RELACION_US_USUARIO_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_us_usuario_id_<?php echo $us_usuario_relacionado->getId() ?>' name='chk_us_usuario[<?php echo $us_usuario_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $us_usuario_relacionado->getId() ?>' relacion_id='<?php echo $ope_chofer_us_usuario_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('OPE_CHOFER_ALTA_RELACION_US_USUARIO_ACCIONES_EDITAR')){ ?>	
        <?php if($ope_chofer_us_usuario_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/ope_chofer_us_usuario/ope_chofer_us_usuario_alta.php?id=<?php Gral::_echo($ope_chofer_us_usuario_id) ?>" contenedor="div_modal" ancho="750" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($us_usuario_relacionado->getId()) ?>, 'us_usuario', 'ope_chofer', <?php Gral::_echo($o_padre->getId()) ?>, 'OpeChofer', 'OpeChoferUsUsuario')" title="Editar OpeChoferUsUsuario"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
