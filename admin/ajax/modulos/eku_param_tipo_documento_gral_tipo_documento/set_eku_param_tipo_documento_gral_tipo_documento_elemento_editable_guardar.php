...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_DOCUMENTO_GRAL_TIPO_DOCUMENTO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoDocumentoGralTipoDocumento::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_param_tipo_documento_gral_tipo_documento){
        if($eku_param_tipo_documento_gral_tipo_documento->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_documento_gral_tipo_documento->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_documento_gral_tipo_documento->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_documento_gral_tipo_documento->saveDesdeListado($campo, $valor);
            }
