...
if($buscar != '...'){
    ...
    $c->add('ctrl_equipo_estado.id', $buscar, Criterio::LIKE, false, Criterio::_AND);

	$c->add('ctrl_equipo_estado.descripcion', $buscar, Criterio::LIKE, false, Criterio::_OR);
	$c->add('ctrl_equipo_estado.codigo', $buscar, Criterio::LIKE, false, Criterio::_OR);
    $c->addFinSubconsulta();
}

$c->addTabla('ctrl_equipo_estado');
$c->addOrden('ctrl_equipo_estado.orden', 'asc');
$c->setCriterios();

$p = new Paginador(10, $pag);

$os = CtrlEquipoEstado::getCtrlEquipoEstados($p, $c);
