...
if(UsCredencial::getEsAcreditado('CTRL_EQUIPO_ESTADO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = CtrlEquipoEstado::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $ctrl_equipo_estado = CtrlEquipoEstado::getOxId($identificador);
    if($ctrl_equipo_estado){
        if($ctrl_equipo_estado->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$ctrl_equipo_estado->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $ctrl_equipo_estado->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $ctrl_equipo_estado->saveDesdeListado($campo, $valor);
            }
