...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/ctrl_equipo_estado.php' ?>

<?php if(count($ctrl_equipo_estados) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_ctrl_equipo_estado'>
    <thead>
    <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=ctrl_equipo_tipo_estado_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('CtrlEquipoTipoEstado') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='ctrl_equipo_tipo_estado_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  ?>
    ...
    <tr id="tr_ctrl_equipo_estado_uno_<?php Gral::_echo($ctrl_equipo_estado->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($ctrl_equipo_estado->getId()) ?>" hash="<?php Gral::_echo($ctrl_equipo_estado->getHash()) ?>" >
  	<?php include "ctrl_equipo_estado_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($ctrl_equipo_estado->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($ctrl_equipo_estado->getId()) ?>" hash="<?php Gral::_echo($ctrl_equipo_estado->getHash()) ?>" >
	<td colspan='9' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($ctrl_equipo_estado->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($ctrl_equipo_estado->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($ctrl_equipo_estado->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $ctrl_equipo_estado->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($ctrl_equipo_estado->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='9' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $ctrl_equipo_estado->getId()){ 
			include "ctrl_equipo_estado_adm_masinfo.php";
		}
		?>
	</div>
