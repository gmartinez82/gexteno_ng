...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/pln_turno_novedad.php' ?>

<?php if(count($pln_turno_novedads) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_pln_turno_novedad'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_pln_turno_novedad_uno_<?php Gral::_echo($pln_turno_novedad->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($pln_turno_novedad->getId()) ?>" hash="<?php Gral::_echo($pln_turno_novedad->getHash()) ?>" >
  	<?php include "pln_turno_novedad_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($pln_turno_novedad->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($pln_turno_novedad->getId()) ?>" hash="<?php Gral::_echo($pln_turno_novedad->getHash()) ?>" >
	<td colspan='6' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($pln_turno_novedad->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($pln_turno_novedad->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($pln_turno_novedad->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $pln_turno_novedad->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($pln_turno_novedad->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='6' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $pln_turno_novedad->getId()){ 
			include "pln_turno_novedad_adm_masinfo.php";
		}
		?>
	</div>
