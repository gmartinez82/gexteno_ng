...
if(UsCredencial::getEsAcreditado('GRAL_TIPO_PERSONERIA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralTipoPersoneria::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($gral_tipo_personeria){
        if($gral_tipo_personeria->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_tipo_personeria->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_tipo_personeria->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_tipo_personeria->saveDesdeListado($campo, $valor);
            }
