...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/gral_tipo_personeria.php' ?>

<?php if(count($gral_tipo_personerias) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_gral_tipo_personeria'>
    <thead>
    <tr>
    ...
    <tbody>
  <?php 
  foreach($gral_tipo_personerias as $gral_tipo_personeria){ 
  	$estado = ($gral_tipo_personeria->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

    <tr id="tr_gral_tipo_personeria_uno_<?php Gral::_echo($gral_tipo_personeria->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($gral_tipo_personeria->getId()) ?>" hash="<?php Gral::_echo($gral_tipo_personeria->getHash()) ?>" >
  	<?php include "gral_tipo_personeria_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($gral_tipo_personeria->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($gral_tipo_personeria->getId()) ?>" hash="<?php Gral::_echo($gral_tipo_personeria->getHash()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($gral_tipo_personeria->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($gral_tipo_personeria->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($gral_tipo_personeria->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $gral_tipo_personeria->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($gral_tipo_personeria->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='5' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $gral_tipo_personeria->getId()){ 
			include "gral_tipo_personeria_adm_masinfo.php";
		}
		?>
	</div>
