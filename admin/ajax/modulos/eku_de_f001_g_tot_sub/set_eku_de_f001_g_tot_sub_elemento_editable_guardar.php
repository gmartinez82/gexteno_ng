...
if(UsCredencial::getEsAcreditado('EKU_DE_F001_G_TOT_SUB_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeF001GTotSub::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_f001_g_tot_sub){
        if($eku_de_f001_g_tot_sub->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_f001_g_tot_sub->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_f001_g_tot_sub->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_f001_g_tot_sub->saveDesdeListado($campo, $valor);
            }
