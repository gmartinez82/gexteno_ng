...
if(UsCredencial::getEsAcreditado('PLN_JORNADA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PlnJornada::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $pln_jornada = PlnJornada::getOxId($identificador);
    if($pln_jornada){
        if($pln_jornada->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$pln_jornada->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $pln_jornada->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $pln_jornada->saveDesdeListado($campo, $valor);
            }
