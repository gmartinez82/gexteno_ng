...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/pln_jornada.php' ?>

<?php if(count($pln_jornadas) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_pln_jornada'>
    <thead>
    <tr>
	...
	<td width='70' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=jornada_completa&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('Jornada Completa') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='jornada_completa'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  ?>
    ...
    <tr id="tr_pln_jornada_uno_<?php Gral::_echo($pln_jornada->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($pln_jornada->getId()) ?>" hash="<?php Gral::_echo($pln_jornada->getHash()) ?>" >
  	<?php include "pln_jornada_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($pln_jornada->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($pln_jornada->getId()) ?>" hash="<?php Gral::_echo($pln_jornada->getHash()) ?>" >
	<td colspan='7' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($pln_jornada->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($pln_jornada->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($pln_jornada->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $pln_jornada->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($pln_jornada->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='7' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $pln_jornada->getId()){ 
			include "pln_jornada_adm_masinfo.php";
		}
		?>
	</div>
