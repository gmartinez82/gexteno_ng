...
if(UsCredencial::getEsAcreditado('EKU_DE_ARSCH01_RESP_MENSAJE_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeArsch01RespMensaje::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($eku_de_arsch01_resp_mensaje){
        if($eku_de_arsch01_resp_mensaje->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_arsch01_resp_mensaje->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_arsch01_resp_mensaje->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_arsch01_resp_mensaje->saveDesdeListado($campo, $valor);
            }
