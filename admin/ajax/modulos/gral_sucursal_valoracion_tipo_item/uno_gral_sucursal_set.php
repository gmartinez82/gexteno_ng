...
<div class='list <?php echo $css_sel ?>'>
    ...
    <?php if(UsCredencial::getEsAcreditado('GRAL_SUCURSAL_VALORACION_TIPO_ITEM_ALTA_RELACION_GRAL_SUCURSAL_ACCIONES_SELECCION')){ ?>	
    <input type='checkbox' id='chk_gral_sucursal_id_<?php echo $gral_sucursal_relacionado->getId() ?>' name='chk_gral_sucursal[<?php echo $gral_sucursal_relacionado->getId() ?>]' <?php echo $sel ?> value='1' identificador='<?php echo $gral_sucursal_relacionado->getId() ?>' relacion_id='<?php echo $gral_sucursal_valoracion_tipo_item_gral_sucursal_id ?>' />
    <?php } ?>

    <label class='link'>
        <?php if(UsCredencial::getEsAcreditado('GRAL_SUCURSAL_VALORACION_TIPO_ITEM_ALTA_RELACION_GRAL_SUCURSAL_ACCIONES_EDITAR')){ ?>	
        <?php if($gral_sucursal_valoracion_tipo_item_gral_sucursal_id != ''){ ?>
        <div class="trigger wopenModal boton" archivo="ajax/modulos/gral_sucursal_valoracion_tipo_item_gral_sucursal/gral_sucursal_valoracion_tipo_item_gral_sucursal_alta.php?id=<?php Gral::_echo($gral_sucursal_valoracion_tipo_item_gral_sucursal_id) ?>" contenedor="div_modal" ancho="950" alto="600" tipo="formulario" post="refreshSetUno(<?php Gral::_echo($gral_sucursal_relacionado->getId()) ?>, 'gral_sucursal', 'gral_sucursal_valoracion_tipo_item', <?php Gral::_echo($o_padre->getId()) ?>, 'GralSucursalValoracionTipoItem', 'GralSucursalValoracionTipoItemGralSucursal')" title="Editar GralSucursalValoracionTipoItemGralSucursal"><?php Lang::_lang('Editar') ?></div>
        <?php } ?>
        <?php } ?>
