...
if(UsCredencial::getEsAcreditado('GRAL_SUCURSAL_VALORACION_TIPO_ITEM_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = GralSucursalValoracionTipoItem::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($gral_sucursal_valoracion_tipo_item){
        if($gral_sucursal_valoracion_tipo_item->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$gral_sucursal_valoracion_tipo_item->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $gral_sucursal_valoracion_tipo_item->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $gral_sucursal_valoracion_tipo_item->saveDesdeListado($campo, $valor);
            }
