...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/geo_zona.php' ?>

<?php if(count($geo_zonas) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_geo_zona'>
  ...
  <tr id="tr_geo_zona_uno_<?php Gral::_echo($geo_zona->getId()) ?>" class="uno" identificador="<?php Gral::_echo($geo_zona->getId()) ?>">
  	<?php include "geo_zona_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($geo_zona->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($geo_zona->getId()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($geo_zona->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($geo_zona->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $geo_zona->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($geo_zona->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $geo_zona->getId()){ 
			include "geo_zona_adm_masinfo.php";
		}
		?>
	</div>
