...
if(UsCredencial::getEsAcreditado('EKU_PARAM_GEO_PAIS_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamGeoPais::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_geo_pais = EkuParamGeoPais::getOxId($identificador);
    if($eku_param_geo_pais){
        if($eku_param_geo_pais->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_geo_pais->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_geo_pais->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_geo_pais->saveDesdeListado($campo, $valor);
            }
