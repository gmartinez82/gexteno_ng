...
if(UsCredencial::getEsAcreditado('PRD_PARAM_OPERACION_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdParamOperacion::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_param_operacion = PrdParamOperacion::getOxId($identificador);
    if($prd_param_operacion){
        if($prd_param_operacion->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_param_operacion->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_param_operacion->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_param_operacion->saveDesdeListado($campo, $valor);
            }
