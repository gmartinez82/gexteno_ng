...
if(UsCredencial::getEsAcreditado('EKU_DE_VTA_NOTA_CREDITO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuDeVtaNotaCredito::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_de_vta_nota_credito = EkuDeVtaNotaCredito::getOxId($identificador);
    if($eku_de_vta_nota_credito){
        if($eku_de_vta_nota_credito->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_de_vta_nota_credito->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_de_vta_nota_credito->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_de_vta_nota_credito->saveDesdeListado($campo, $valor);
            }
