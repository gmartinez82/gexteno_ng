...
if(UsCredencial::getEsAcreditado('VTA_TIPO_PUNTO_VENTA_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = VtaTipoPuntoVenta::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    if($vta_tipo_punto_venta){
        if($vta_tipo_punto_venta->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$vta_tipo_punto_venta->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $vta_tipo_punto_venta->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $vta_tipo_punto_venta->saveDesdeListado($campo, $valor);
            }
