...
<?php //include Gral::getPathAbs().'admin/parciales/buscadores/activos/vta_tipo_punto_venta.php' ?>

<?php if(count($vta_tipo_punto_ventas) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_vta_tipo_punto_venta'>
    <thead>
    <tr>
  ...
  ?>
    ...
    <tr id="tr_vta_tipo_punto_venta_uno_<?php Gral::_echo($vta_tipo_punto_venta->getId()) ?>" class="uno tr_datos" identificador="<?php Gral::_echo($vta_tipo_punto_venta->getId()) ?>" hash="<?php Gral::_echo($vta_tipo_punto_venta->getHash()) ?>" >
  	<?php include "vta_tipo_punto_venta_uno.php" ?>
    </tr>
  
  
    <tr id='tr_eliminar_<?php Gral::_echo($vta_tipo_punto_venta->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($vta_tipo_punto_venta->getId()) ?>" hash="<?php Gral::_echo($vta_tipo_punto_venta->getHash()) ?>" >
	<td colspan='5' align='center' class='adm_tbl_lineas'>
            
            <div id='div_eliminar_<?php Gral::_echo($vta_tipo_punto_venta->getId()) ?>'  class='div_eliminar warning'>
                ...
                <div class="eliminar-mensaje">
                    <?php Lang::_lang('Esta a punto de eliminar') ?> "<strong><?php Gral::_echo($vta_tipo_punto_venta->getDescripcion()) ?></strong>".<br />
                ...
                <div class="eliminar-botonera">
                      ...
                      <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($vta_tipo_punto_venta->getId()) ?>,0)'/>
                </div>
            </div>
...
if(trim($id) == $vta_tipo_punto_venta->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
    <tr id='tr_mi_<?php Gral::_echo($vta_tipo_punto_venta->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	<td colspan='5' align='center' class='adm_tbl_lineas'>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $vta_tipo_punto_venta->getId()){ 
			include "vta_tipo_punto_venta_adm_masinfo.php";
		}
		?>
	</div>
