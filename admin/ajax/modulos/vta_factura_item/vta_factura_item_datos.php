...
<?php include Gral::getPathAbs().'admin/parciales/buscadores/activos/vta_factura_item.php' ?>

<?php if(count($vta_factura_items) > 0){ ?>

<table border='0' align='center' class='listado' id='listado_adm_vta_factura_item'>
  ...
  <tr>
	...
	<td width='120' align='center' class='adm_tbl_encabezado'>
            <a class='ordenar' href="?ord=1&c=vta_factura_concepto_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                <?php Lang::_lang('VtaFacturaConcepto') ?>
                    
                <?php if($criterio->getOrdenDato('campo')=='vta_factura_concepto_id'){ ?>
                <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                <?php } ?>
            </a>
	</td>
  ...
  foreach($vta_factura_items as $vta_factura_item){ 
  	$estado = ($vta_factura_item->getEstado()) ? 'habilitado' : 'deshabilitado';
  ?>

  <tr id="tr_vta_factura_item_uno_<?php Gral::_echo($vta_factura_item->getId()) ?>" class="uno" identificador="<?php Gral::_echo($vta_factura_item->getId()) ?>">
  	<?php include "vta_factura_item_uno.php" ?>
  </tr>
  
  
  <tr id='tr_eliminar_<?php Gral::_echo($vta_factura_item->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($vta_factura_item->getId()) ?>" >
	<td colspan='9' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($vta_factura_item->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
	  <br />
	  <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
		<input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($vta_factura_item->getId()) ?>,0)'/>
	  </div>
	</div>		</td>
	<td align='center'></td>
  </tr>
...
if(trim($id) == $vta_factura_item->getId()){ 
	$mi_display = '';
	$mi_hash = "location.hash = 'mi_".$id."';"; 
}
?>
  <tr id='tr_mi_<?php Gral::_echo($vta_factura_item->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
	...
	<div class="masinfo">
		<?php 
		if(trim($id) == $vta_factura_item->getId()){ 
			include "vta_factura_item_adm_masinfo.php";
		}
		?>
	</div>
