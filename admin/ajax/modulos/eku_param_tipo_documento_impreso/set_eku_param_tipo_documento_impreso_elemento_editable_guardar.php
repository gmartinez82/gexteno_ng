...
if(UsCredencial::getEsAcreditado('EKU_PARAM_TIPO_DOCUMENTO_IMPRESO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = EkuParamTipoDocumentoImpreso::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $eku_param_tipo_documento_impreso = EkuParamTipoDocumentoImpreso::getOxId($identificador);
    if($eku_param_tipo_documento_impreso){
        if($eku_param_tipo_documento_impreso->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$eku_param_tipo_documento_impreso->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $eku_param_tipo_documento_impreso->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $eku_param_tipo_documento_impreso->saveDesdeListado($campo, $valor);
            }
