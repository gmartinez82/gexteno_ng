...
if(UsCredencial::getEsAcreditado('PRD_ORDEN_TRABAJO_OPERACION_OPE_OPERARIO_ALTA')){
    ...
    // -------------------------------------------------------------------------
    $valor = PrdOrdenTrabajoOperacionOpeOperario::getSaneamientoParaRegistroSimple($campo, $valor, $type, $mascara);
    ...
    $prd_orden_trabajo_operacion_ope_operario = PrdOrdenTrabajoOperacionOpeOperario::getOxId($identificador);
    if($prd_orden_trabajo_operacion_ope_operario){
        if($prd_orden_trabajo_operacion_ope_operario->getHash() == $hash){
            ...
            // -----------------------------------------------------------------
            eval("\$prd_orden_trabajo_operacion_ope_operario->set".Gral::getCamelCase($campo).'($valor);');
            
            $db_error = $prd_orden_trabajo_operacion_ope_operario->control();
            ...
            if(!$arr['error']){
                ...
                // -------------------------------------------------------------
                $prd_orden_trabajo_operacion_ope_operario->saveDesdeListado($campo, $valor);
            }
