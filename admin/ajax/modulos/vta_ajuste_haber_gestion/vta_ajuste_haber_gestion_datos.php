...
<?php //include Gral::getPathAbs() . 'admin/parciales/buscadores/activos/vta_ajuste_haber.php' ?>
...
<?php if (count($vta_ajuste_habers) > 0) { ?>

    <table border='0' align='center' class='listado' id='listado_adm_vta_ajuste_haber_gestion'>
        ...
        <tr>
            ...
            <td width='210' align='center' class='adm_tbl_encabezado'>
                <a class='ordenar' href="?ord=1&c=vta_ajuste_haber_tipo_estado_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                    <?php //Lang::_lang('VtaAjusteHaberTipoEstado') ?>
                    <?php Lang::_lang('Estado') ?>

                    <?php if ($criterio->getOrdenDato('campo') == 'vta_ajuste_haber_tipo_estado_id') { ?>
                        <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                    <?php } ?>
                </a>
            </td>
            ...
            <td width='40' align='center' class='adm_tbl_encabezado'>
                <a class='ordenar' href="?ord=1&c=vta_tipo_ajuste_haber_id&t=<?php Gral::_echo(($criterio->getOrdenDato('tipo') == 'asc') ? 'desc' : 'asc'); ?>">
                    <?php Lang::_lang('Tipo') ?>

                    <?php if ($criterio->getOrdenDato('campo') == 'vta_tipo_ajuste_haber_id') { ?>
                        <img src="imgs/ord_<?php Gral::_echo($criterio->getOrdenDato('tipo')) ?>.png" border='0'>
                    <?php } ?>
                </a>
            </td>
        ...
        foreach ($vta_ajuste_habers as $vta_ajuste_haber) {
            ...
            <tr id="tr_vta_ajuste_haber_gestion_uno_<?php Gral::_echo($vta_ajuste_haber->getId()) ?>" class="uno" identificador="<?php Gral::_echo($vta_ajuste_haber->getId()) ?>">
                <?php include "vta_ajuste_haber_gestion_uno.php" ?>
            </tr>


            <tr id='tr_eliminar_<?php Gral::_echo($vta_ajuste_haber->getId()) ?>' class='uno tr_eliminar' identificador="<?php Gral::_echo($vta_ajuste_haber->getId()) ?>" >
                <td colspan='14' align='center' class='adm_tbl_lineas'><div id='div_eliminar_<?php Gral::_echo($vta_ajuste_haber->getId()) ?>'  class='div_eliminar warning'><?php Lang::_lang('Confirma la Eliminacion') ?> <br />
                        <br />
                        <div><input name='btn_elim_aceptar' type='button' id='btn_elim_aceptar' value='<?php Lang::_lang('Aceptar') ?>'  class='btn_mensaje_aceptar' />
                            <input name='btn_elim_cancelar' type='button' id='btn_elim_cancelar' value='<?php Lang::_lang('Cancelar') ?>'  class='btn_mensaje_cancelar' onclick='eliminar_conf(<?php Gral::_echo($vta_ajuste_haber->getId()) ?>, 0)'/>
                        </div>
                    </div>		</td>
                <td align='center'></td>
            </tr>
            ...
            if (trim($id) == $vta_ajuste_haber->getId()) {
                $mi_display = '';
                $mi_hash = "location.hash = 'mi_" . $id . "';";
            }
            ?>
            <tr id='tr_mi_<?php Gral::_echo($vta_ajuste_haber->getId()) ?>' <?php Gral::_echo($mi_display) ?>>
                <td colspan='14' align='center' class='adm_tbl_lineas'>
                    ...
                    <div class="masinfo">
                        <?php
                        if (trim($id) == $vta_ajuste_haber->getId()) {
                            include "vta_ajuste_haber_adm_masinfo.php";
                        }
                        ?>
                    </div>
