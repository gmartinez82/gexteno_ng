...
$txt_numero_comprobante     = Gral::getVars(Gral::VARS_POST, "txt_numero_comprobante", '');
$txt_fecha_emision          = Gral::getVars(Gral::VARS_POST, "txt_fecha_emision", '');
$txt_importe_base_imponible = Gral::getVars(Gral::VARS_POST, "txt_importe_base_imponible", '');
...
if (Ctrl::esVacio($txt_descripcion)) {
    $arr["txt_descripcion"] = Lang::_lang("Debe agregar una descripcion", true);
    $arr["error"] = true;
}
if (Ctrl::esVacio($txt_numero_comprobante)) {
    $arr["txt_numero_comprobante"] = Lang::_lang("Debe agregar un numero de comprobante", true);
    $arr["error"] = true;
}
if (Ctrl::esVacio($txt_fecha_emision)) {
    $arr["txt_fecha_emision"] = Lang::_lang("Debe agregar una fecha", true);
    $arr["error"] = true;
} else {
    $txt_fecha_emision = Gral::getFechaToDB($txt_fecha_emision);
    if (!Ctrl::esFechaValida($txt_fecha_emision)) {
        $arr["txt_fecha_emision"] = Lang::_lang("Debe agregar una fecha valida", true);
        $arr["error"] = true;
    }
}
if ($txt_importe_base_imponible == 0) {
    $arr["txt_importe_base_imponible"] = Lang::_lang("Importe invalido", true);
    $arr["error"] = true;
...
if (!$arr["error"]) {
    ...
    //Gral::prr(Gral::getSes('vta_ajuste_haber_retencion_info'));
}
