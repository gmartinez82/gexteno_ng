<?php

include_once '_autoload.php';

$vta_ajuste_haber_id = Gral::getVars(Gral::VARS_POST, 'vta_ajuste_haber_id', 0);
$vta_ajuste_haber = VtaAjusteHaber::getOxId($vta_ajuste_haber_id);

$txa_observacion = Gral::getVars(Gral::VARS_POST, "txa_observacion");

// control de datos
$arr["error"] = false;

if (Ctrl::esVacio($txa_observacion)) {
    $arr["error"] = true;
    $arr["txa_observacion"] = Lang::_lang("Debe ingresar un motivo de anulacion", true);
}

if (!$arr["error"]) {

    // se registra nuevo estado
    //$vta_ajuste_haber_estado = $vta_ajuste_haber->setVtaAjusteHaberEstado(VtaAjusteHaberTipoEstado::TIPO_ANULADO, $txa_observacion);
    $vta_ajuste_haber_estado = $vta_ajuste_haber->setVtaAjusteHaberAnular($txa_observacion);
}

// se retornan datos
$arr_json = json_encode($arr);
echo $arr_json;
?>