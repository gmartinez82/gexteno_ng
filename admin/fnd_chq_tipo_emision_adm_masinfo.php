...
$fnd_chq_tipo_emision_id = Gral::getVars(2, 'id');
$fnd_chq_tipo_emision = FndChqTipoEmision::getOxId($fnd_chq_tipo_emision_id);
?>
<div class="masinfo-datos">

	<div class="par ">
            <div class="label"><?php Lang::_lang('Observaciones') ?></div>
            <div class="dato"><?php Gral::_echo(nl2br($fnd_chq_tipo_emision->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'FND_CHQ_TIPO_EMISION_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($fnd_chq_tipo_emision->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($fnd_chq_tipo_emision->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'FND_CHQ_TIPO_EMISION_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($fnd_chq_tipo_emision->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($fnd_chq_tipo_emision->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('FND_CHQ_TIPO_EMISION_MASINFO_FND_CHQ_CHEQUE')){ ?>
	...
	<div id="tab_fnd_chq_cheque" class="bloque-relacion fnd_chq_cheque">
            ...
            <h4><?php Lang::_lang('FndChqCheque') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('FndChqTipoEmision') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($fnd_chq_tipo_emision->getFndChqChequesParaBloqueMasInfo() as $fnd_chq_cheque){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('FND_CHQ_TIPO_EMISION_MASINFO_FND_CHQ_CHEQUE_BOTONERA')){ ?>
                    <div class="botonera">
                        ...
                        <?php if($fnd_chq_cheque){ ?>
                            <div class="boton">
                                ...
                                <?php Lang::_lang('Ver FndChqCheques de ') ?> <strong><?php echo($fnd_chq_tipo_emision->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
