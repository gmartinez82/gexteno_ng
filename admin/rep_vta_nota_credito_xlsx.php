...
if($txt_filtro_desde != ''){
    $criterio->add(VtaNotaCredito::GEN_ATTR_CREADO, Gral::getFechaToDb($txt_filtro_desde) . ' 00:00:00', Criterio::MAYORIGUAL);
}

if($txt_filtro_hasta != ''){
    $criterio->add(VtaNotaCredito::GEN_ATTR_CREADO, Gral::getFechaToDb($txt_filtro_hasta) . ' 23:59:59', Criterio::MENORIGUAL);
}

if($cmb_filtro_cli_cliente_id != 0){
    $criterio->add(VtaNotaCredito::GEN_ATTR_CLI_CLIENTE_ID, $cmb_filtro_cli_cliente_id, Criterio::IGUAL);
}

if($cmb_filtro_vta_tipo_nota_credito_id != 0){
    $criterio->add(VtaNotaCredito::GEN_ATTR_VTA_TIPO_NOTA_CREDITO_ID, $cmb_filtro_vta_tipo_nota_credito_id, Criterio::IGUAL);
}

if($cmb_filtro_gral_condicion_iva_id != 0){
    $criterio->add(VtaNotaCredito::GEN_ATTR_GRAL_CONDICION_IVA_ID, $cmb_filtro_gral_condicion_iva_id, Criterio::IGUAL);
}

if($cmb_filtro_gral_empresa_id != 0){
    $criterio->add(VtaNotaCredito::GEN_ATTR_GRAL_EMPRESA_ID, $cmb_filtro_gral_empresa_id, Criterio::IGUAL);
}
...
foreach ($vta_nota_creditos as $vta_nota_credito)
{
    ...
    $cliente_cuit      = $vta_nota_credito->getCuit();
    $estado            = $vta_nota_credito->getVtaNotaCreditoTipoEstado()->getDescripcion();
    $tipo_nc           = $vta_nota_credito->getVtaTipoNotaCredito()->getCodigoMin();
    $subtotal          = $vta_nota_credito->getVtaNotaCreditoSubtotal();
    $subtotal_iva      = $vta_nota_credito->getVtaNotaCreditoIva();
    $subtotal_trib     = $vta_nota_credito->getVtaNotaCreditoTributo();
    $total             = $vta_nota_credito->getVtaNotaCreditoTotal();
    $nota_credito_afip = $vta_nota_credito->getNumeroComprobanteCompleto();
    $cae               = $vta_nota_credito->getCae();
    $cae_vencimiento   = Gral::getFechaToWeb($vta_nota_credito->getCaeVencimiento());
