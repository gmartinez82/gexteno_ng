...
$criterio->add(PdiTipoOrigen::GEN_ATTR_CODIGO, PdiTipoOrigen::TIPO_ORIGEN_AJUSTE_PANOL, Criterio::IGUAL);
...
$arr_cabeceras = array(
    ...
    $columna++ => array('ancho' => 22, 'titulo' => 'Costo en Fecha', 'formato' => DbExcel::FORMATO_IMPORTE),
    $columna++ => array('ancho' => 22, 'titulo' => 'Costo Actual', 'formato' => DbExcel::FORMATO_IMPORTE),
);
...
foreach ($pdi_pedidos as $pdi_pedido) {
    ...
    $ins_insumo_costo_actual = $ins_insumo->getInsInsumoCostoActual($pan_panol->getPdeCentroPedido());
    ...
    if ($ins_insumo_costo_en_fecha) {
        $valor = $ins_insumo_costo_en_fecha->getCosto();
        DbExcel::setCelda($xls, $columna, $fila, $valor, $type = PHPExcel_Cell_DataType::TYPE_NUMERIC);
        $columna++;
    } else {
        $columna++;
    ...
    if ($ins_insumo_costo_actual) {
        $valor = $ins_insumo_costo_actual->getCosto();
        DbExcel::setCelda($xls, $columna, $fila, $valor, $type = PHPExcel_Cell_DataType::TYPE_NUMERIC);
        $columna++;
    } else {
        $columna++;
