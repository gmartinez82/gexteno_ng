...
if ($filtrar) {
    ...
    $widget_facturas_emitidas_cmb_vta_preventista_id = Gral::getVars(2, 'widget_facturas_emitidas_cmb_vta_preventista_id', 0);
    $widget_facturas_emitidas_cmb_vta_vendedor_id = Gral::getVars(2, 'widget_facturas_emitidas_cmb_vta_vendedor_id', 0);
    ...
    $widget_facturas_emitidas_cmb_vta_vendedor_id = 0;
}
...
$sql = "
            SELECT 
                view_vta_factura.fecha_emision, 
                count(view_vta_factura.id) as cantidad_emitido,
                ...
                sum(view_vta_factura.importe_total) as importe_emitido,
                sum(CASE WHEN vta_factura_tipo_estado.aprobado_afip = 1 THEN view_vta_factura.importe_total ELSE null END) as importe_aprobado_afip,
                sum(CASE WHEN vta_factura_tipo_estado.codigo = 'SALDADO' THEN view_vta_factura.importe_total ELSE null END) as importe_saldado
            FROM
                view_vta_factura
            LEFT JOIN vta_factura_tipo_estado on vta_factura_tipo_estado.id = view_vta_factura.vta_factura_tipo_estado_id
            WHERE TRUE
                AND view_vta_factura.fecha_emision BETWEEN '" . $desde . "' AND '" . $hasta . "'
                    ";

if ($widget_facturas_emitidas_cmb_vta_preventista_id != 0) {
    $sql .= "
            AND view_vta_factura.vta_preventista_id = " . $widget_facturas_emitidas_cmb_vta_preventista_id;
}
if ($widget_facturas_emitidas_cmb_vta_vendedor_id != 0) {
    $sql .= "
            AND view_vta_factura.vta_vendedor_id = " . $widget_facturas_emitidas_cmb_vta_vendedor_id;
}
...
$sql .= "
            GROUP BY 
                view_vta_factura.fecha_emision
            ORDER BY
                view_vta_factura.fecha_emision ASC
            ;
        ";
//echo $sql;
