...
$pde_tipo_origen_factura_id = Gral::getVars(2, 'id');
$pde_tipo_origen_factura = PdeTipoOrigenFactura::getOxId($pde_tipo_origen_factura_id);
?>
<div class="masinfo-datos">

	<div class="par ">
            <div class="label"><?php Lang::_lang('Observaciones') ?></div>
            <div class="dato"><?php Gral::_echo(nl2br($pde_tipo_origen_factura->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PDE_TIPO_ORIGEN_FACTURA_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($pde_tipo_origen_factura->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($pde_tipo_origen_factura->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PDE_TIPO_ORIGEN_FACTURA_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($pde_tipo_origen_factura->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($pde_tipo_origen_factura->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('PDE_TIPO_ORIGEN_FACTURA_MASINFO_PDE_FACTURA')){ ?>
	...
	<div id="tab_pde_factura" class="bloque-relacion pde_factura">
            ...
            <h4><?php Lang::_lang('PdeFactura') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('PdeTipoOrigenFactura') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($pde_tipo_origen_factura->getPdeFacturasParaBloqueMasInfo() as $pde_factura){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('PDE_TIPO_ORIGEN_FACTURA_MASINFO_PDE_FACTURA_BOTONERA')){ ?>
                    <div class="botonera">
                        ...
                        <?php if($pde_factura){ ?>
                            <div class="boton">
                                ...
                                <?php Lang::_lang('Ver PdeFacturas de ') ?> <strong><?php echo($pde_tipo_origen_factura->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
