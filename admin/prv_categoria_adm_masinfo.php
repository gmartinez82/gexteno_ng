...
$prv_categoria_id = Gral::getVars(2, 'id');
$prv_categoria = PrvCategoria::getOxId($prv_categoria_id);
?>
<div class="masinfo-datos">

	<div class="par ">
            <div class="label"><?php Lang::_lang('Observaciones') ?></div>
            <div class="dato"><?php Gral::_echo(nl2br($prv_categoria->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PRV_CATEGORIA_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($prv_categoria->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($prv_categoria->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'PRV_CATEGORIA_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($prv_categoria->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($prv_categoria->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('PRV_CATEGORIA_MASINFO_PRV_PROVEEDOR')){ ?>
	...
	<div id="tab_prv_proveedor" class="bloque-relacion prv_proveedor">
            ...
            <h4><?php Lang::_lang('PrvProveedor') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('PrvCategoria') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($prv_categoria->getPrvProveedorsParaBloqueMasInfo() as $prv_proveedor){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('PRV_CATEGORIA_MASINFO_PRV_PROVEEDOR_BOTONERA')){ ?>
                    <div class="botonera">
                            
                        <?php if($prv_proveedor){ ?>
                            <div class="boton">
                                <a href="_init.php?arr_gral=<?php echo PrvProveedor::getFiltrosArrGral() ?>&arr=<?php echo $prv_proveedor->getFiltrosArrXCampo('PrvCategoria', 'prv_categoria_id') ?>" >
                                <?php Lang::_lang('Ver PrvProveedors de ') ?> <strong><?php echo($prv_categoria->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
