...
if($txt_filtro_desde != ''){
    $criterio->add(PdeNotaCredito::GEN_ATTR_CREADO, Gral::getFechaToDb($txt_filtro_desde) . ' 00:00:00', Criterio::MAYORIGUAL);
}

if($txt_filtro_hasta != ''){
    $criterio->add(PdeNotaCredito::GEN_ATTR_CREADO, Gral::getFechaToDb($txt_filtro_hasta) . ' 23:59:59', Criterio::MENORIGUAL);
}

if($cmb_filtro_prv_proveedor_id != 0){
    $criterio->add(PdeNotaCredito::GEN_ATTR_PRV_PROVEEDOR_ID, $cmb_filtro_prv_proveedor_id, Criterio::IGUAL);
}

if($cmb_filtro_pde_tipo_nota_credito_id != 0){
    $criterio->add(PdeNotaCredito::GEN_ATTR_PDE_TIPO_NOTA_CREDITO_ID, $cmb_filtro_pde_tipo_nota_credito_id, Criterio::IGUAL);
}

if($cmb_filtro_gral_condicion_iva_id != 0){
    $criterio->add(PdeNotaCredito::GEN_ATTR_GRAL_CONDICION_IVA_ID, $cmb_filtro_gral_condicion_iva_id, Criterio::IGUAL);
}

if($cmb_filtro_gral_empresa_id != 0){
    $criterio->add(PdeNotaCredito::GEN_ATTR_GRAL_EMPRESA_ID, $cmb_filtro_gral_empresa_id, Criterio::IGUAL);
}
...
$cols = array(
    ...
    $columna++ => array("ancho" => 20, "titulo" => "NC AFIP"        , "indice" => "nota_credito_afip"),
...
foreach ($pde_nota_creditos as $pde_nota_credito)
{
    ...
    $subtotal_trib     = "";
    $total             = "";
    $fecha             = Gral::getFechaToWeb($pde_nota_credito->getFechaEmision());
    $codigo            = $pde_nota_credito->getCodigo();
    $proveedor         = $pde_nota_credito->getPrvProveedor()->getDescripcion();
    $estado            = $pde_nota_credito->getPdeNotaCreditoTipoEstado()->getDescripcion();
    $tipo_nc           = $pde_nota_credito->getPdeTipoNotaCredito()->getCodigoMin();
    $subtotal          = $pde_nota_credito->getPdeNotaCreditoSubtotal();
    $subtotal_iva      = $pde_nota_credito->getPdeNotaCreditoIva();
    $subtotal_trib     = $pde_nota_credito->getPdeNotaCreditoTributo();
    $total             = $pde_nota_credito->getPdeNotaCreditoTotal();
    $nota_credito_afip = $pde_nota_credito->getNumeroComprobanteCompleto();
    $cae               = $pde_nota_credito->getCae();
    $cae_vencimiento   = Gral::getFechaToWeb($pde_nota_credito->getCaeVencimiento());
    ...
    $xls->getActiveSheet()->getStyle($columna.$linea)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
    $columna++;
    $xls->getActiveSheet()->getCell($columna.$linea)->setValueExplicit($nota_credito_afip, PHPExcel_Cell_DataType::TYPE_STRING);
    $columna++;
