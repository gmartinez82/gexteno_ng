...
$alt_nivel_id = Gral::getVars(2, 'id');
$alt_nivel = AltNivel::getOxId($alt_nivel_id);
?>
<div class="masinfo-datos">

	<div class="par ">
            <div class="label"><?php Lang::_lang('Observaciones') ?></div>
            <div class="dato"><?php Gral::_echo(nl2br($alt_nivel->getObservacion())) ?></div>
	</div>

<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'ALT_NIVEL_AUDITOR')){ ?>
    <div class="par inline">
        <div class="label"><?php Lang::_lang('Creado') ?></div>
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($alt_nivel->getCreado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($alt_nivel->getCreadoPorDescripcion()) ?></strong></i></div>
    </div>
<?php } ?>
...
<?php if(Login::esAutorizado($user, 'GRAL_ADMIN_AUDITOR') || Login::esAutorizado($user, 'ALT_NIVEL_AUDITOR')){ ?>
    <div class="par inline">
        ...
        <div class="dato"><?php Gral::_echo(Gral::getFechaHoraToWeb($alt_nivel->getModificado())) ?> hs<br /> <i>Por: <strong><?php Gral::_echo($alt_nivel->getModificadoPorDescripcion()) ?></strong></i></div>
    </div>		
<?php } ?>
...
<div class="tabs">
	<ul>
            <?php if(UsCredencial::getEsAcreditado('ALT_NIVEL_MASINFO_ALT_ALERTA')){ ?>
	...
	<div id="tab_alt_alerta" class="bloque-relacion alt_alerta">
            ...
            <h4><?php Lang::_lang('AltAlerta') ?> <?php Lang::_lang('vinculados al') ?> <?php Lang::_lang('AltNivel') ?></h4>
            <div class="bloque-relacion-datos">
                    ...
                    <?php foreach($alt_nivel->getAltAlertasParaBloqueMasInfo() as $alt_alerta){ ?>
                    <div class="uno">
                    ...
                    <?php if(UsCredencial::getEsAcreditado('ALT_NIVEL_MASINFO_ALT_ALERTA_BOTONERA')){ ?>
                    <div class="botonera">
                            
                        <?php if($alt_alerta){ ?>
                            <div class="boton">
                                <a href="_init.php?arr_gral=<?php echo AltAlerta::getFiltrosArrGral() ?>&arr=<?php echo $alt_alerta->getFiltrosArrXCampo('AltNivel', 'alt_nivel_id') ?>" >
                                <?php Lang::_lang('Ver AltAlertas de ') ?> <strong><?php echo($alt_nivel->getDescripcion()) ?></strong>
                                </a>				
                            </div>
                        <?php } ?>
                        <div class="boton">
